# Graph Report - go-bt-evolve  (2026-09-06)

## Corpus Check
- 8619 files · ~4,864,039 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 30991 nodes · 38192 edges · 7317 communities (6094 shown, 1223 thin omitted)
- Extraction: 76% EXTRACTED · 24% INFERRED · 0% AMBIGUOUS · INFERRED: 9135 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1d25ff6f`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 329|Community 329]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 359|Community 359]]
- [[_COMMUNITY_Community 360|Community 360]]
- [[_COMMUNITY_Community 361|Community 361]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 363|Community 363]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 371|Community 371]]
- [[_COMMUNITY_Community 372|Community 372]]
- [[_COMMUNITY_Community 373|Community 373]]
- [[_COMMUNITY_Community 374|Community 374]]
- [[_COMMUNITY_Community 375|Community 375]]
- [[_COMMUNITY_Community 376|Community 376]]
- [[_COMMUNITY_Community 377|Community 377]]
- [[_COMMUNITY_Community 378|Community 378]]
- [[_COMMUNITY_Community 379|Community 379]]
- [[_COMMUNITY_Community 380|Community 380]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]
- [[_COMMUNITY_Community 383|Community 383]]
- [[_COMMUNITY_Community 384|Community 384]]
- [[_COMMUNITY_Community 385|Community 385]]
- [[_COMMUNITY_Community 386|Community 386]]
- [[_COMMUNITY_Community 387|Community 387]]
- [[_COMMUNITY_Community 388|Community 388]]
- [[_COMMUNITY_Community 389|Community 389]]
- [[_COMMUNITY_Community 390|Community 390]]
- [[_COMMUNITY_Community 391|Community 391]]
- [[_COMMUNITY_Community 392|Community 392]]
- [[_COMMUNITY_Community 393|Community 393]]
- [[_COMMUNITY_Community 394|Community 394]]
- [[_COMMUNITY_Community 395|Community 395]]
- [[_COMMUNITY_Community 396|Community 396]]
- [[_COMMUNITY_Community 397|Community 397]]
- [[_COMMUNITY_Community 398|Community 398]]
- [[_COMMUNITY_Community 399|Community 399]]
- [[_COMMUNITY_Community 401|Community 401]]
- [[_COMMUNITY_Community 402|Community 402]]
- [[_COMMUNITY_Community 403|Community 403]]
- [[_COMMUNITY_Community 404|Community 404]]
- [[_COMMUNITY_Community 405|Community 405]]
- [[_COMMUNITY_Community 406|Community 406]]
- [[_COMMUNITY_Community 407|Community 407]]
- [[_COMMUNITY_Community 408|Community 408]]
- [[_COMMUNITY_Community 409|Community 409]]
- [[_COMMUNITY_Community 410|Community 410]]
- [[_COMMUNITY_Community 411|Community 411]]
- [[_COMMUNITY_Community 412|Community 412]]
- [[_COMMUNITY_Community 413|Community 413]]
- [[_COMMUNITY_Community 414|Community 414]]
- [[_COMMUNITY_Community 415|Community 415]]
- [[_COMMUNITY_Community 416|Community 416]]
- [[_COMMUNITY_Community 417|Community 417]]
- [[_COMMUNITY_Community 418|Community 418]]
- [[_COMMUNITY_Community 419|Community 419]]
- [[_COMMUNITY_Community 420|Community 420]]
- [[_COMMUNITY_Community 421|Community 421]]
- [[_COMMUNITY_Community 422|Community 422]]
- [[_COMMUNITY_Community 423|Community 423]]
- [[_COMMUNITY_Community 424|Community 424]]
- [[_COMMUNITY_Community 425|Community 425]]
- [[_COMMUNITY_Community 426|Community 426]]
- [[_COMMUNITY_Community 427|Community 427]]
- [[_COMMUNITY_Community 428|Community 428]]
- [[_COMMUNITY_Community 429|Community 429]]
- [[_COMMUNITY_Community 430|Community 430]]
- [[_COMMUNITY_Community 431|Community 431]]
- [[_COMMUNITY_Community 432|Community 432]]
- [[_COMMUNITY_Community 433|Community 433]]
- [[_COMMUNITY_Community 434|Community 434]]
- [[_COMMUNITY_Community 435|Community 435]]
- [[_COMMUNITY_Community 436|Community 436]]
- [[_COMMUNITY_Community 437|Community 437]]
- [[_COMMUNITY_Community 438|Community 438]]
- [[_COMMUNITY_Community 439|Community 439]]
- [[_COMMUNITY_Community 440|Community 440]]
- [[_COMMUNITY_Community 441|Community 441]]
- [[_COMMUNITY_Community 442|Community 442]]
- [[_COMMUNITY_Community 443|Community 443]]
- [[_COMMUNITY_Community 444|Community 444]]
- [[_COMMUNITY_Community 445|Community 445]]
- [[_COMMUNITY_Community 446|Community 446]]
- [[_COMMUNITY_Community 447|Community 447]]
- [[_COMMUNITY_Community 448|Community 448]]
- [[_COMMUNITY_Community 449|Community 449]]
- [[_COMMUNITY_Community 450|Community 450]]
- [[_COMMUNITY_Community 451|Community 451]]
- [[_COMMUNITY_Community 452|Community 452]]
- [[_COMMUNITY_Community 453|Community 453]]
- [[_COMMUNITY_Community 454|Community 454]]
- [[_COMMUNITY_Community 455|Community 455]]
- [[_COMMUNITY_Community 456|Community 456]]
- [[_COMMUNITY_Community 457|Community 457]]
- [[_COMMUNITY_Community 458|Community 458]]
- [[_COMMUNITY_Community 459|Community 459]]
- [[_COMMUNITY_Community 460|Community 460]]
- [[_COMMUNITY_Community 461|Community 461]]
- [[_COMMUNITY_Community 462|Community 462]]
- [[_COMMUNITY_Community 463|Community 463]]
- [[_COMMUNITY_Community 464|Community 464]]
- [[_COMMUNITY_Community 465|Community 465]]
- [[_COMMUNITY_Community 466|Community 466]]
- [[_COMMUNITY_Community 467|Community 467]]
- [[_COMMUNITY_Community 468|Community 468]]
- [[_COMMUNITY_Community 469|Community 469]]
- [[_COMMUNITY_Community 470|Community 470]]
- [[_COMMUNITY_Community 471|Community 471]]
- [[_COMMUNITY_Community 472|Community 472]]
- [[_COMMUNITY_Community 473|Community 473]]
- [[_COMMUNITY_Community 474|Community 474]]
- [[_COMMUNITY_Community 475|Community 475]]
- [[_COMMUNITY_Community 476|Community 476]]
- [[_COMMUNITY_Community 477|Community 477]]
- [[_COMMUNITY_Community 478|Community 478]]
- [[_COMMUNITY_Community 479|Community 479]]
- [[_COMMUNITY_Community 480|Community 480]]
- [[_COMMUNITY_Community 481|Community 481]]
- [[_COMMUNITY_Community 482|Community 482]]
- [[_COMMUNITY_Community 483|Community 483]]
- [[_COMMUNITY_Community 484|Community 484]]
- [[_COMMUNITY_Community 485|Community 485]]
- [[_COMMUNITY_Community 486|Community 486]]
- [[_COMMUNITY_Community 487|Community 487]]
- [[_COMMUNITY_Community 488|Community 488]]
- [[_COMMUNITY_Community 489|Community 489]]
- [[_COMMUNITY_Community 490|Community 490]]
- [[_COMMUNITY_Community 491|Community 491]]
- [[_COMMUNITY_Community 493|Community 493]]
- [[_COMMUNITY_Community 494|Community 494]]
- [[_COMMUNITY_Community 495|Community 495]]
- [[_COMMUNITY_Community 496|Community 496]]
- [[_COMMUNITY_Community 497|Community 497]]
- [[_COMMUNITY_Community 498|Community 498]]
- [[_COMMUNITY_Community 499|Community 499]]
- [[_COMMUNITY_Community 500|Community 500]]
- [[_COMMUNITY_Community 501|Community 501]]
- [[_COMMUNITY_Community 502|Community 502]]
- [[_COMMUNITY_Community 503|Community 503]]
- [[_COMMUNITY_Community 504|Community 504]]
- [[_COMMUNITY_Community 505|Community 505]]
- [[_COMMUNITY_Community 506|Community 506]]
- [[_COMMUNITY_Community 507|Community 507]]
- [[_COMMUNITY_Community 508|Community 508]]
- [[_COMMUNITY_Community 509|Community 509]]
- [[_COMMUNITY_Community 510|Community 510]]
- [[_COMMUNITY_Community 511|Community 511]]
- [[_COMMUNITY_Community 512|Community 512]]
- [[_COMMUNITY_Community 513|Community 513]]
- [[_COMMUNITY_Community 514|Community 514]]
- [[_COMMUNITY_Community 515|Community 515]]
- [[_COMMUNITY_Community 516|Community 516]]
- [[_COMMUNITY_Community 517|Community 517]]
- [[_COMMUNITY_Community 518|Community 518]]
- [[_COMMUNITY_Community 519|Community 519]]
- [[_COMMUNITY_Community 520|Community 520]]
- [[_COMMUNITY_Community 521|Community 521]]
- [[_COMMUNITY_Community 522|Community 522]]
- [[_COMMUNITY_Community 523|Community 523]]
- [[_COMMUNITY_Community 525|Community 525]]
- [[_COMMUNITY_Community 526|Community 526]]
- [[_COMMUNITY_Community 527|Community 527]]
- [[_COMMUNITY_Community 528|Community 528]]
- [[_COMMUNITY_Community 529|Community 529]]
- [[_COMMUNITY_Community 530|Community 530]]
- [[_COMMUNITY_Community 531|Community 531]]
- [[_COMMUNITY_Community 532|Community 532]]
- [[_COMMUNITY_Community 533|Community 533]]
- [[_COMMUNITY_Community 534|Community 534]]
- [[_COMMUNITY_Community 535|Community 535]]
- [[_COMMUNITY_Community 536|Community 536]]
- [[_COMMUNITY_Community 537|Community 537]]
- [[_COMMUNITY_Community 538|Community 538]]
- [[_COMMUNITY_Community 539|Community 539]]
- [[_COMMUNITY_Community 540|Community 540]]
- [[_COMMUNITY_Community 541|Community 541]]
- [[_COMMUNITY_Community 542|Community 542]]
- [[_COMMUNITY_Community 543|Community 543]]
- [[_COMMUNITY_Community 544|Community 544]]
- [[_COMMUNITY_Community 545|Community 545]]
- [[_COMMUNITY_Community 546|Community 546]]
- [[_COMMUNITY_Community 547|Community 547]]
- [[_COMMUNITY_Community 548|Community 548]]
- [[_COMMUNITY_Community 549|Community 549]]
- [[_COMMUNITY_Community 550|Community 550]]
- [[_COMMUNITY_Community 551|Community 551]]
- [[_COMMUNITY_Community 552|Community 552]]
- [[_COMMUNITY_Community 553|Community 553]]
- [[_COMMUNITY_Community 554|Community 554]]
- [[_COMMUNITY_Community 555|Community 555]]
- [[_COMMUNITY_Community 556|Community 556]]
- [[_COMMUNITY_Community 557|Community 557]]
- [[_COMMUNITY_Community 558|Community 558]]
- [[_COMMUNITY_Community 559|Community 559]]
- [[_COMMUNITY_Community 560|Community 560]]
- [[_COMMUNITY_Community 561|Community 561]]
- [[_COMMUNITY_Community 562|Community 562]]
- [[_COMMUNITY_Community 563|Community 563]]
- [[_COMMUNITY_Community 564|Community 564]]
- [[_COMMUNITY_Community 565|Community 565]]
- [[_COMMUNITY_Community 566|Community 566]]
- [[_COMMUNITY_Community 567|Community 567]]
- [[_COMMUNITY_Community 568|Community 568]]
- [[_COMMUNITY_Community 569|Community 569]]
- [[_COMMUNITY_Community 570|Community 570]]
- [[_COMMUNITY_Community 571|Community 571]]
- [[_COMMUNITY_Community 572|Community 572]]
- [[_COMMUNITY_Community 573|Community 573]]
- [[_COMMUNITY_Community 574|Community 574]]
- [[_COMMUNITY_Community 575|Community 575]]
- [[_COMMUNITY_Community 576|Community 576]]
- [[_COMMUNITY_Community 577|Community 577]]
- [[_COMMUNITY_Community 578|Community 578]]
- [[_COMMUNITY_Community 579|Community 579]]
- [[_COMMUNITY_Community 580|Community 580]]
- [[_COMMUNITY_Community 581|Community 581]]
- [[_COMMUNITY_Community 582|Community 582]]
- [[_COMMUNITY_Community 583|Community 583]]
- [[_COMMUNITY_Community 584|Community 584]]
- [[_COMMUNITY_Community 585|Community 585]]
- [[_COMMUNITY_Community 586|Community 586]]
- [[_COMMUNITY_Community 589|Community 589]]
- [[_COMMUNITY_Community 590|Community 590]]
- [[_COMMUNITY_Community 591|Community 591]]
- [[_COMMUNITY_Community 592|Community 592]]
- [[_COMMUNITY_Community 593|Community 593]]
- [[_COMMUNITY_Community 594|Community 594]]
- [[_COMMUNITY_Community 595|Community 595]]
- [[_COMMUNITY_Community 596|Community 596]]
- [[_COMMUNITY_Community 597|Community 597]]
- [[_COMMUNITY_Community 598|Community 598]]
- [[_COMMUNITY_Community 599|Community 599]]
- [[_COMMUNITY_Community 600|Community 600]]
- [[_COMMUNITY_Community 601|Community 601]]
- [[_COMMUNITY_Community 602|Community 602]]
- [[_COMMUNITY_Community 603|Community 603]]
- [[_COMMUNITY_Community 604|Community 604]]
- [[_COMMUNITY_Community 605|Community 605]]
- [[_COMMUNITY_Community 606|Community 606]]
- [[_COMMUNITY_Community 607|Community 607]]
- [[_COMMUNITY_Community 608|Community 608]]
- [[_COMMUNITY_Community 609|Community 609]]
- [[_COMMUNITY_Community 610|Community 610]]
- [[_COMMUNITY_Community 611|Community 611]]
- [[_COMMUNITY_Community 612|Community 612]]
- [[_COMMUNITY_Community 613|Community 613]]
- [[_COMMUNITY_Community 614|Community 614]]
- [[_COMMUNITY_Community 615|Community 615]]
- [[_COMMUNITY_Community 616|Community 616]]
- [[_COMMUNITY_Community 617|Community 617]]
- [[_COMMUNITY_Community 618|Community 618]]
- [[_COMMUNITY_Community 619|Community 619]]
- [[_COMMUNITY_Community 620|Community 620]]
- [[_COMMUNITY_Community 621|Community 621]]
- [[_COMMUNITY_Community 622|Community 622]]
- [[_COMMUNITY_Community 623|Community 623]]
- [[_COMMUNITY_Community 624|Community 624]]
- [[_COMMUNITY_Community 625|Community 625]]
- [[_COMMUNITY_Community 626|Community 626]]
- [[_COMMUNITY_Community 627|Community 627]]
- [[_COMMUNITY_Community 628|Community 628]]
- [[_COMMUNITY_Community 629|Community 629]]
- [[_COMMUNITY_Community 630|Community 630]]
- [[_COMMUNITY_Community 631|Community 631]]
- [[_COMMUNITY_Community 632|Community 632]]
- [[_COMMUNITY_Community 633|Community 633]]
- [[_COMMUNITY_Community 634|Community 634]]
- [[_COMMUNITY_Community 635|Community 635]]
- [[_COMMUNITY_Community 636|Community 636]]
- [[_COMMUNITY_Community 637|Community 637]]
- [[_COMMUNITY_Community 638|Community 638]]
- [[_COMMUNITY_Community 639|Community 639]]
- [[_COMMUNITY_Community 640|Community 640]]
- [[_COMMUNITY_Community 641|Community 641]]
- [[_COMMUNITY_Community 642|Community 642]]
- [[_COMMUNITY_Community 643|Community 643]]
- [[_COMMUNITY_Community 644|Community 644]]
- [[_COMMUNITY_Community 645|Community 645]]
- [[_COMMUNITY_Community 646|Community 646]]
- [[_COMMUNITY_Community 647|Community 647]]
- [[_COMMUNITY_Community 648|Community 648]]
- [[_COMMUNITY_Community 649|Community 649]]
- [[_COMMUNITY_Community 650|Community 650]]
- [[_COMMUNITY_Community 651|Community 651]]
- [[_COMMUNITY_Community 652|Community 652]]
- [[_COMMUNITY_Community 653|Community 653]]
- [[_COMMUNITY_Community 654|Community 654]]
- [[_COMMUNITY_Community 655|Community 655]]
- [[_COMMUNITY_Community 656|Community 656]]
- [[_COMMUNITY_Community 657|Community 657]]
- [[_COMMUNITY_Community 658|Community 658]]
- [[_COMMUNITY_Community 659|Community 659]]
- [[_COMMUNITY_Community 660|Community 660]]
- [[_COMMUNITY_Community 661|Community 661]]
- [[_COMMUNITY_Community 662|Community 662]]
- [[_COMMUNITY_Community 663|Community 663]]
- [[_COMMUNITY_Community 664|Community 664]]
- [[_COMMUNITY_Community 665|Community 665]]
- [[_COMMUNITY_Community 666|Community 666]]
- [[_COMMUNITY_Community 667|Community 667]]
- [[_COMMUNITY_Community 668|Community 668]]
- [[_COMMUNITY_Community 669|Community 669]]
- [[_COMMUNITY_Community 670|Community 670]]
- [[_COMMUNITY_Community 671|Community 671]]
- [[_COMMUNITY_Community 672|Community 672]]
- [[_COMMUNITY_Community 673|Community 673]]
- [[_COMMUNITY_Community 674|Community 674]]
- [[_COMMUNITY_Community 675|Community 675]]
- [[_COMMUNITY_Community 676|Community 676]]
- [[_COMMUNITY_Community 677|Community 677]]
- [[_COMMUNITY_Community 678|Community 678]]
- [[_COMMUNITY_Community 679|Community 679]]
- [[_COMMUNITY_Community 680|Community 680]]
- [[_COMMUNITY_Community 681|Community 681]]
- [[_COMMUNITY_Community 682|Community 682]]
- [[_COMMUNITY_Community 683|Community 683]]
- [[_COMMUNITY_Community 684|Community 684]]
- [[_COMMUNITY_Community 685|Community 685]]
- [[_COMMUNITY_Community 686|Community 686]]
- [[_COMMUNITY_Community 687|Community 687]]
- [[_COMMUNITY_Community 688|Community 688]]
- [[_COMMUNITY_Community 689|Community 689]]
- [[_COMMUNITY_Community 690|Community 690]]
- [[_COMMUNITY_Community 691|Community 691]]
- [[_COMMUNITY_Community 692|Community 692]]
- [[_COMMUNITY_Community 693|Community 693]]
- [[_COMMUNITY_Community 694|Community 694]]
- [[_COMMUNITY_Community 695|Community 695]]
- [[_COMMUNITY_Community 696|Community 696]]
- [[_COMMUNITY_Community 697|Community 697]]
- [[_COMMUNITY_Community 698|Community 698]]
- [[_COMMUNITY_Community 699|Community 699]]
- [[_COMMUNITY_Community 700|Community 700]]
- [[_COMMUNITY_Community 701|Community 701]]
- [[_COMMUNITY_Community 702|Community 702]]
- [[_COMMUNITY_Community 703|Community 703]]
- [[_COMMUNITY_Community 704|Community 704]]
- [[_COMMUNITY_Community 705|Community 705]]
- [[_COMMUNITY_Community 706|Community 706]]
- [[_COMMUNITY_Community 707|Community 707]]
- [[_COMMUNITY_Community 708|Community 708]]
- [[_COMMUNITY_Community 709|Community 709]]
- [[_COMMUNITY_Community 710|Community 710]]
- [[_COMMUNITY_Community 711|Community 711]]
- [[_COMMUNITY_Community 712|Community 712]]
- [[_COMMUNITY_Community 713|Community 713]]
- [[_COMMUNITY_Community 714|Community 714]]
- [[_COMMUNITY_Community 715|Community 715]]
- [[_COMMUNITY_Community 716|Community 716]]
- [[_COMMUNITY_Community 717|Community 717]]
- [[_COMMUNITY_Community 718|Community 718]]
- [[_COMMUNITY_Community 719|Community 719]]
- [[_COMMUNITY_Community 720|Community 720]]
- [[_COMMUNITY_Community 721|Community 721]]
- [[_COMMUNITY_Community 722|Community 722]]
- [[_COMMUNITY_Community 723|Community 723]]
- [[_COMMUNITY_Community 724|Community 724]]
- [[_COMMUNITY_Community 725|Community 725]]
- [[_COMMUNITY_Community 726|Community 726]]
- [[_COMMUNITY_Community 727|Community 727]]
- [[_COMMUNITY_Community 728|Community 728]]
- [[_COMMUNITY_Community 729|Community 729]]
- [[_COMMUNITY_Community 730|Community 730]]
- [[_COMMUNITY_Community 731|Community 731]]
- [[_COMMUNITY_Community 732|Community 732]]
- [[_COMMUNITY_Community 733|Community 733]]
- [[_COMMUNITY_Community 734|Community 734]]
- [[_COMMUNITY_Community 735|Community 735]]
- [[_COMMUNITY_Community 736|Community 736]]
- [[_COMMUNITY_Community 737|Community 737]]
- [[_COMMUNITY_Community 738|Community 738]]
- [[_COMMUNITY_Community 739|Community 739]]
- [[_COMMUNITY_Community 740|Community 740]]
- [[_COMMUNITY_Community 741|Community 741]]
- [[_COMMUNITY_Community 742|Community 742]]
- [[_COMMUNITY_Community 743|Community 743]]
- [[_COMMUNITY_Community 744|Community 744]]
- [[_COMMUNITY_Community 745|Community 745]]
- [[_COMMUNITY_Community 746|Community 746]]
- [[_COMMUNITY_Community 747|Community 747]]
- [[_COMMUNITY_Community 748|Community 748]]
- [[_COMMUNITY_Community 749|Community 749]]
- [[_COMMUNITY_Community 750|Community 750]]
- [[_COMMUNITY_Community 751|Community 751]]
- [[_COMMUNITY_Community 752|Community 752]]
- [[_COMMUNITY_Community 753|Community 753]]
- [[_COMMUNITY_Community 754|Community 754]]
- [[_COMMUNITY_Community 755|Community 755]]
- [[_COMMUNITY_Community 758|Community 758]]
- [[_COMMUNITY_Community 759|Community 759]]
- [[_COMMUNITY_Community 760|Community 760]]
- [[_COMMUNITY_Community 761|Community 761]]
- [[_COMMUNITY_Community 762|Community 762]]
- [[_COMMUNITY_Community 764|Community 764]]
- [[_COMMUNITY_Community 766|Community 766]]
- [[_COMMUNITY_Community 767|Community 767]]
- [[_COMMUNITY_Community 768|Community 768]]
- [[_COMMUNITY_Community 769|Community 769]]
- [[_COMMUNITY_Community 770|Community 770]]
- [[_COMMUNITY_Community 771|Community 771]]
- [[_COMMUNITY_Community 772|Community 772]]
- [[_COMMUNITY_Community 773|Community 773]]
- [[_COMMUNITY_Community 774|Community 774]]
- [[_COMMUNITY_Community 775|Community 775]]
- [[_COMMUNITY_Community 776|Community 776]]
- [[_COMMUNITY_Community 777|Community 777]]
- [[_COMMUNITY_Community 778|Community 778]]
- [[_COMMUNITY_Community 779|Community 779]]
- [[_COMMUNITY_Community 780|Community 780]]
- [[_COMMUNITY_Community 781|Community 781]]
- [[_COMMUNITY_Community 782|Community 782]]
- [[_COMMUNITY_Community 783|Community 783]]
- [[_COMMUNITY_Community 784|Community 784]]
- [[_COMMUNITY_Community 785|Community 785]]
- [[_COMMUNITY_Community 786|Community 786]]
- [[_COMMUNITY_Community 787|Community 787]]
- [[_COMMUNITY_Community 788|Community 788]]
- [[_COMMUNITY_Community 789|Community 789]]
- [[_COMMUNITY_Community 790|Community 790]]
- [[_COMMUNITY_Community 791|Community 791]]
- [[_COMMUNITY_Community 792|Community 792]]
- [[_COMMUNITY_Community 793|Community 793]]
- [[_COMMUNITY_Community 794|Community 794]]
- [[_COMMUNITY_Community 795|Community 795]]
- [[_COMMUNITY_Community 796|Community 796]]
- [[_COMMUNITY_Community 797|Community 797]]
- [[_COMMUNITY_Community 798|Community 798]]
- [[_COMMUNITY_Community 799|Community 799]]
- [[_COMMUNITY_Community 800|Community 800]]
- [[_COMMUNITY_Community 801|Community 801]]
- [[_COMMUNITY_Community 802|Community 802]]
- [[_COMMUNITY_Community 803|Community 803]]
- [[_COMMUNITY_Community 804|Community 804]]
- [[_COMMUNITY_Community 805|Community 805]]
- [[_COMMUNITY_Community 806|Community 806]]
- [[_COMMUNITY_Community 807|Community 807]]
- [[_COMMUNITY_Community 808|Community 808]]
- [[_COMMUNITY_Community 809|Community 809]]
- [[_COMMUNITY_Community 810|Community 810]]
- [[_COMMUNITY_Community 811|Community 811]]
- [[_COMMUNITY_Community 812|Community 812]]
- [[_COMMUNITY_Community 813|Community 813]]
- [[_COMMUNITY_Community 814|Community 814]]
- [[_COMMUNITY_Community 815|Community 815]]
- [[_COMMUNITY_Community 816|Community 816]]
- [[_COMMUNITY_Community 817|Community 817]]
- [[_COMMUNITY_Community 818|Community 818]]
- [[_COMMUNITY_Community 819|Community 819]]
- [[_COMMUNITY_Community 820|Community 820]]
- [[_COMMUNITY_Community 821|Community 821]]
- [[_COMMUNITY_Community 822|Community 822]]
- [[_COMMUNITY_Community 823|Community 823]]
- [[_COMMUNITY_Community 824|Community 824]]
- [[_COMMUNITY_Community 825|Community 825]]
- [[_COMMUNITY_Community 826|Community 826]]
- [[_COMMUNITY_Community 827|Community 827]]
- [[_COMMUNITY_Community 828|Community 828]]
- [[_COMMUNITY_Community 829|Community 829]]
- [[_COMMUNITY_Community 830|Community 830]]
- [[_COMMUNITY_Community 831|Community 831]]
- [[_COMMUNITY_Community 832|Community 832]]
- [[_COMMUNITY_Community 833|Community 833]]
- [[_COMMUNITY_Community 834|Community 834]]
- [[_COMMUNITY_Community 835|Community 835]]
- [[_COMMUNITY_Community 836|Community 836]]
- [[_COMMUNITY_Community 837|Community 837]]
- [[_COMMUNITY_Community 838|Community 838]]
- [[_COMMUNITY_Community 839|Community 839]]
- [[_COMMUNITY_Community 840|Community 840]]
- [[_COMMUNITY_Community 841|Community 841]]
- [[_COMMUNITY_Community 842|Community 842]]
- [[_COMMUNITY_Community 843|Community 843]]
- [[_COMMUNITY_Community 844|Community 844]]
- [[_COMMUNITY_Community 845|Community 845]]
- [[_COMMUNITY_Community 846|Community 846]]
- [[_COMMUNITY_Community 847|Community 847]]
- [[_COMMUNITY_Community 848|Community 848]]
- [[_COMMUNITY_Community 849|Community 849]]
- [[_COMMUNITY_Community 850|Community 850]]
- [[_COMMUNITY_Community 851|Community 851]]
- [[_COMMUNITY_Community 852|Community 852]]
- [[_COMMUNITY_Community 853|Community 853]]
- [[_COMMUNITY_Community 854|Community 854]]
- [[_COMMUNITY_Community 855|Community 855]]
- [[_COMMUNITY_Community 856|Community 856]]
- [[_COMMUNITY_Community 857|Community 857]]
- [[_COMMUNITY_Community 858|Community 858]]
- [[_COMMUNITY_Community 859|Community 859]]
- [[_COMMUNITY_Community 860|Community 860]]
- [[_COMMUNITY_Community 861|Community 861]]
- [[_COMMUNITY_Community 862|Community 862]]
- [[_COMMUNITY_Community 863|Community 863]]
- [[_COMMUNITY_Community 864|Community 864]]
- [[_COMMUNITY_Community 865|Community 865]]
- [[_COMMUNITY_Community 866|Community 866]]
- [[_COMMUNITY_Community 867|Community 867]]
- [[_COMMUNITY_Community 868|Community 868]]
- [[_COMMUNITY_Community 869|Community 869]]
- [[_COMMUNITY_Community 870|Community 870]]
- [[_COMMUNITY_Community 871|Community 871]]
- [[_COMMUNITY_Community 872|Community 872]]
- [[_COMMUNITY_Community 873|Community 873]]
- [[_COMMUNITY_Community 874|Community 874]]
- [[_COMMUNITY_Community 875|Community 875]]
- [[_COMMUNITY_Community 876|Community 876]]
- [[_COMMUNITY_Community 877|Community 877]]
- [[_COMMUNITY_Community 878|Community 878]]
- [[_COMMUNITY_Community 879|Community 879]]
- [[_COMMUNITY_Community 880|Community 880]]
- [[_COMMUNITY_Community 881|Community 881]]
- [[_COMMUNITY_Community 882|Community 882]]
- [[_COMMUNITY_Community 883|Community 883]]
- [[_COMMUNITY_Community 884|Community 884]]
- [[_COMMUNITY_Community 885|Community 885]]
- [[_COMMUNITY_Community 886|Community 886]]
- [[_COMMUNITY_Community 887|Community 887]]
- [[_COMMUNITY_Community 888|Community 888]]
- [[_COMMUNITY_Community 889|Community 889]]
- [[_COMMUNITY_Community 890|Community 890]]
- [[_COMMUNITY_Community 891|Community 891]]
- [[_COMMUNITY_Community 892|Community 892]]
- [[_COMMUNITY_Community 893|Community 893]]
- [[_COMMUNITY_Community 894|Community 894]]
- [[_COMMUNITY_Community 895|Community 895]]
- [[_COMMUNITY_Community 896|Community 896]]
- [[_COMMUNITY_Community 897|Community 897]]
- [[_COMMUNITY_Community 898|Community 898]]
- [[_COMMUNITY_Community 899|Community 899]]
- [[_COMMUNITY_Community 900|Community 900]]
- [[_COMMUNITY_Community 901|Community 901]]
- [[_COMMUNITY_Community 902|Community 902]]
- [[_COMMUNITY_Community 903|Community 903]]
- [[_COMMUNITY_Community 904|Community 904]]
- [[_COMMUNITY_Community 905|Community 905]]
- [[_COMMUNITY_Community 906|Community 906]]
- [[_COMMUNITY_Community 907|Community 907]]
- [[_COMMUNITY_Community 908|Community 908]]
- [[_COMMUNITY_Community 909|Community 909]]
- [[_COMMUNITY_Community 910|Community 910]]
- [[_COMMUNITY_Community 911|Community 911]]
- [[_COMMUNITY_Community 912|Community 912]]
- [[_COMMUNITY_Community 913|Community 913]]
- [[_COMMUNITY_Community 914|Community 914]]
- [[_COMMUNITY_Community 915|Community 915]]
- [[_COMMUNITY_Community 916|Community 916]]
- [[_COMMUNITY_Community 917|Community 917]]
- [[_COMMUNITY_Community 918|Community 918]]
- [[_COMMUNITY_Community 919|Community 919]]
- [[_COMMUNITY_Community 920|Community 920]]
- [[_COMMUNITY_Community 921|Community 921]]
- [[_COMMUNITY_Community 922|Community 922]]
- [[_COMMUNITY_Community 923|Community 923]]
- [[_COMMUNITY_Community 924|Community 924]]
- [[_COMMUNITY_Community 925|Community 925]]
- [[_COMMUNITY_Community 926|Community 926]]
- [[_COMMUNITY_Community 927|Community 927]]
- [[_COMMUNITY_Community 928|Community 928]]
- [[_COMMUNITY_Community 929|Community 929]]
- [[_COMMUNITY_Community 930|Community 930]]
- [[_COMMUNITY_Community 931|Community 931]]
- [[_COMMUNITY_Community 932|Community 932]]
- [[_COMMUNITY_Community 933|Community 933]]
- [[_COMMUNITY_Community 934|Community 934]]
- [[_COMMUNITY_Community 935|Community 935]]
- [[_COMMUNITY_Community 936|Community 936]]
- [[_COMMUNITY_Community 937|Community 937]]
- [[_COMMUNITY_Community 938|Community 938]]
- [[_COMMUNITY_Community 939|Community 939]]
- [[_COMMUNITY_Community 940|Community 940]]
- [[_COMMUNITY_Community 941|Community 941]]
- [[_COMMUNITY_Community 942|Community 942]]
- [[_COMMUNITY_Community 943|Community 943]]
- [[_COMMUNITY_Community 944|Community 944]]
- [[_COMMUNITY_Community 945|Community 945]]
- [[_COMMUNITY_Community 946|Community 946]]
- [[_COMMUNITY_Community 947|Community 947]]
- [[_COMMUNITY_Community 948|Community 948]]
- [[_COMMUNITY_Community 949|Community 949]]
- [[_COMMUNITY_Community 950|Community 950]]
- [[_COMMUNITY_Community 951|Community 951]]
- [[_COMMUNITY_Community 952|Community 952]]
- [[_COMMUNITY_Community 953|Community 953]]
- [[_COMMUNITY_Community 954|Community 954]]
- [[_COMMUNITY_Community 955|Community 955]]
- [[_COMMUNITY_Community 956|Community 956]]
- [[_COMMUNITY_Community 957|Community 957]]
- [[_COMMUNITY_Community 958|Community 958]]
- [[_COMMUNITY_Community 959|Community 959]]
- [[_COMMUNITY_Community 960|Community 960]]
- [[_COMMUNITY_Community 961|Community 961]]
- [[_COMMUNITY_Community 962|Community 962]]
- [[_COMMUNITY_Community 963|Community 963]]
- [[_COMMUNITY_Community 964|Community 964]]
- [[_COMMUNITY_Community 965|Community 965]]
- [[_COMMUNITY_Community 966|Community 966]]
- [[_COMMUNITY_Community 967|Community 967]]
- [[_COMMUNITY_Community 968|Community 968]]
- [[_COMMUNITY_Community 969|Community 969]]
- [[_COMMUNITY_Community 970|Community 970]]
- [[_COMMUNITY_Community 971|Community 971]]
- [[_COMMUNITY_Community 972|Community 972]]
- [[_COMMUNITY_Community 973|Community 973]]
- [[_COMMUNITY_Community 974|Community 974]]
- [[_COMMUNITY_Community 975|Community 975]]
- [[_COMMUNITY_Community 976|Community 976]]
- [[_COMMUNITY_Community 977|Community 977]]
- [[_COMMUNITY_Community 978|Community 978]]
- [[_COMMUNITY_Community 979|Community 979]]
- [[_COMMUNITY_Community 980|Community 980]]
- [[_COMMUNITY_Community 981|Community 981]]
- [[_COMMUNITY_Community 982|Community 982]]
- [[_COMMUNITY_Community 983|Community 983]]
- [[_COMMUNITY_Community 984|Community 984]]
- [[_COMMUNITY_Community 985|Community 985]]
- [[_COMMUNITY_Community 986|Community 986]]
- [[_COMMUNITY_Community 987|Community 987]]
- [[_COMMUNITY_Community 988|Community 988]]
- [[_COMMUNITY_Community 989|Community 989]]
- [[_COMMUNITY_Community 990|Community 990]]
- [[_COMMUNITY_Community 991|Community 991]]
- [[_COMMUNITY_Community 992|Community 992]]
- [[_COMMUNITY_Community 993|Community 993]]
- [[_COMMUNITY_Community 994|Community 994]]
- [[_COMMUNITY_Community 995|Community 995]]
- [[_COMMUNITY_Community 996|Community 996]]
- [[_COMMUNITY_Community 997|Community 997]]
- [[_COMMUNITY_Community 998|Community 998]]
- [[_COMMUNITY_Community 999|Community 999]]
- [[_COMMUNITY_Community 1000|Community 1000]]
- [[_COMMUNITY_Community 1001|Community 1001]]
- [[_COMMUNITY_Community 1002|Community 1002]]
- [[_COMMUNITY_Community 1003|Community 1003]]
- [[_COMMUNITY_Community 1004|Community 1004]]
- [[_COMMUNITY_Community 1005|Community 1005]]
- [[_COMMUNITY_Community 1006|Community 1006]]
- [[_COMMUNITY_Community 1007|Community 1007]]
- [[_COMMUNITY_Community 1008|Community 1008]]
- [[_COMMUNITY_Community 1009|Community 1009]]
- [[_COMMUNITY_Community 1010|Community 1010]]
- [[_COMMUNITY_Community 1011|Community 1011]]
- [[_COMMUNITY_Community 1012|Community 1012]]
- [[_COMMUNITY_Community 1013|Community 1013]]
- [[_COMMUNITY_Community 1014|Community 1014]]
- [[_COMMUNITY_Community 1015|Community 1015]]
- [[_COMMUNITY_Community 1016|Community 1016]]
- [[_COMMUNITY_Community 1017|Community 1017]]
- [[_COMMUNITY_Community 1018|Community 1018]]
- [[_COMMUNITY_Community 1019|Community 1019]]
- [[_COMMUNITY_Community 1020|Community 1020]]
- [[_COMMUNITY_Community 1021|Community 1021]]
- [[_COMMUNITY_Community 1022|Community 1022]]
- [[_COMMUNITY_Community 1023|Community 1023]]
- [[_COMMUNITY_Community 1024|Community 1024]]
- [[_COMMUNITY_Community 1025|Community 1025]]
- [[_COMMUNITY_Community 1026|Community 1026]]
- [[_COMMUNITY_Community 1027|Community 1027]]
- [[_COMMUNITY_Community 1028|Community 1028]]
- [[_COMMUNITY_Community 1029|Community 1029]]
- [[_COMMUNITY_Community 1030|Community 1030]]
- [[_COMMUNITY_Community 1031|Community 1031]]
- [[_COMMUNITY_Community 1032|Community 1032]]
- [[_COMMUNITY_Community 1033|Community 1033]]
- [[_COMMUNITY_Community 1034|Community 1034]]
- [[_COMMUNITY_Community 1035|Community 1035]]
- [[_COMMUNITY_Community 1036|Community 1036]]
- [[_COMMUNITY_Community 1037|Community 1037]]
- [[_COMMUNITY_Community 1038|Community 1038]]
- [[_COMMUNITY_Community 1039|Community 1039]]
- [[_COMMUNITY_Community 1040|Community 1040]]
- [[_COMMUNITY_Community 1041|Community 1041]]
- [[_COMMUNITY_Community 1042|Community 1042]]
- [[_COMMUNITY_Community 1043|Community 1043]]
- [[_COMMUNITY_Community 1044|Community 1044]]
- [[_COMMUNITY_Community 1045|Community 1045]]
- [[_COMMUNITY_Community 1046|Community 1046]]
- [[_COMMUNITY_Community 1047|Community 1047]]
- [[_COMMUNITY_Community 1048|Community 1048]]
- [[_COMMUNITY_Community 1049|Community 1049]]
- [[_COMMUNITY_Community 1050|Community 1050]]
- [[_COMMUNITY_Community 1051|Community 1051]]
- [[_COMMUNITY_Community 1052|Community 1052]]
- [[_COMMUNITY_Community 1053|Community 1053]]
- [[_COMMUNITY_Community 1054|Community 1054]]
- [[_COMMUNITY_Community 1055|Community 1055]]
- [[_COMMUNITY_Community 1056|Community 1056]]
- [[_COMMUNITY_Community 1057|Community 1057]]
- [[_COMMUNITY_Community 1058|Community 1058]]
- [[_COMMUNITY_Community 1059|Community 1059]]
- [[_COMMUNITY_Community 1060|Community 1060]]
- [[_COMMUNITY_Community 1061|Community 1061]]
- [[_COMMUNITY_Community 1062|Community 1062]]
- [[_COMMUNITY_Community 1063|Community 1063]]
- [[_COMMUNITY_Community 1064|Community 1064]]
- [[_COMMUNITY_Community 1065|Community 1065]]
- [[_COMMUNITY_Community 1066|Community 1066]]
- [[_COMMUNITY_Community 1067|Community 1067]]
- [[_COMMUNITY_Community 1068|Community 1068]]
- [[_COMMUNITY_Community 1069|Community 1069]]
- [[_COMMUNITY_Community 1070|Community 1070]]
- [[_COMMUNITY_Community 1071|Community 1071]]
- [[_COMMUNITY_Community 1072|Community 1072]]
- [[_COMMUNITY_Community 1073|Community 1073]]
- [[_COMMUNITY_Community 1074|Community 1074]]
- [[_COMMUNITY_Community 1075|Community 1075]]
- [[_COMMUNITY_Community 1076|Community 1076]]
- [[_COMMUNITY_Community 1077|Community 1077]]
- [[_COMMUNITY_Community 1078|Community 1078]]
- [[_COMMUNITY_Community 1079|Community 1079]]
- [[_COMMUNITY_Community 1080|Community 1080]]
- [[_COMMUNITY_Community 1081|Community 1081]]
- [[_COMMUNITY_Community 1082|Community 1082]]
- [[_COMMUNITY_Community 1083|Community 1083]]
- [[_COMMUNITY_Community 1084|Community 1084]]
- [[_COMMUNITY_Community 1085|Community 1085]]
- [[_COMMUNITY_Community 1086|Community 1086]]
- [[_COMMUNITY_Community 1087|Community 1087]]
- [[_COMMUNITY_Community 1088|Community 1088]]
- [[_COMMUNITY_Community 1089|Community 1089]]
- [[_COMMUNITY_Community 1090|Community 1090]]
- [[_COMMUNITY_Community 1091|Community 1091]]
- [[_COMMUNITY_Community 1092|Community 1092]]
- [[_COMMUNITY_Community 1093|Community 1093]]
- [[_COMMUNITY_Community 1094|Community 1094]]
- [[_COMMUNITY_Community 1095|Community 1095]]
- [[_COMMUNITY_Community 1096|Community 1096]]
- [[_COMMUNITY_Community 1097|Community 1097]]
- [[_COMMUNITY_Community 1098|Community 1098]]
- [[_COMMUNITY_Community 1099|Community 1099]]
- [[_COMMUNITY_Community 1100|Community 1100]]
- [[_COMMUNITY_Community 1101|Community 1101]]
- [[_COMMUNITY_Community 1102|Community 1102]]
- [[_COMMUNITY_Community 1103|Community 1103]]
- [[_COMMUNITY_Community 1104|Community 1104]]
- [[_COMMUNITY_Community 1105|Community 1105]]
- [[_COMMUNITY_Community 1106|Community 1106]]
- [[_COMMUNITY_Community 1107|Community 1107]]
- [[_COMMUNITY_Community 1108|Community 1108]]
- [[_COMMUNITY_Community 1109|Community 1109]]
- [[_COMMUNITY_Community 1110|Community 1110]]
- [[_COMMUNITY_Community 1111|Community 1111]]
- [[_COMMUNITY_Community 1112|Community 1112]]
- [[_COMMUNITY_Community 1113|Community 1113]]
- [[_COMMUNITY_Community 1114|Community 1114]]
- [[_COMMUNITY_Community 1115|Community 1115]]
- [[_COMMUNITY_Community 1116|Community 1116]]
- [[_COMMUNITY_Community 1117|Community 1117]]
- [[_COMMUNITY_Community 1118|Community 1118]]
- [[_COMMUNITY_Community 1119|Community 1119]]
- [[_COMMUNITY_Community 1120|Community 1120]]
- [[_COMMUNITY_Community 1121|Community 1121]]
- [[_COMMUNITY_Community 1122|Community 1122]]
- [[_COMMUNITY_Community 1123|Community 1123]]
- [[_COMMUNITY_Community 1124|Community 1124]]
- [[_COMMUNITY_Community 1125|Community 1125]]
- [[_COMMUNITY_Community 1126|Community 1126]]
- [[_COMMUNITY_Community 1127|Community 1127]]
- [[_COMMUNITY_Community 1128|Community 1128]]
- [[_COMMUNITY_Community 1129|Community 1129]]
- [[_COMMUNITY_Community 1130|Community 1130]]
- [[_COMMUNITY_Community 1131|Community 1131]]
- [[_COMMUNITY_Community 1132|Community 1132]]
- [[_COMMUNITY_Community 1133|Community 1133]]
- [[_COMMUNITY_Community 1134|Community 1134]]
- [[_COMMUNITY_Community 1135|Community 1135]]
- [[_COMMUNITY_Community 1136|Community 1136]]
- [[_COMMUNITY_Community 1137|Community 1137]]
- [[_COMMUNITY_Community 1138|Community 1138]]
- [[_COMMUNITY_Community 1139|Community 1139]]
- [[_COMMUNITY_Community 1140|Community 1140]]
- [[_COMMUNITY_Community 1141|Community 1141]]
- [[_COMMUNITY_Community 1142|Community 1142]]
- [[_COMMUNITY_Community 1143|Community 1143]]
- [[_COMMUNITY_Community 1144|Community 1144]]
- [[_COMMUNITY_Community 1145|Community 1145]]
- [[_COMMUNITY_Community 1146|Community 1146]]
- [[_COMMUNITY_Community 1147|Community 1147]]
- [[_COMMUNITY_Community 1148|Community 1148]]
- [[_COMMUNITY_Community 1149|Community 1149]]
- [[_COMMUNITY_Community 1150|Community 1150]]
- [[_COMMUNITY_Community 1151|Community 1151]]
- [[_COMMUNITY_Community 1152|Community 1152]]
- [[_COMMUNITY_Community 1153|Community 1153]]
- [[_COMMUNITY_Community 1154|Community 1154]]
- [[_COMMUNITY_Community 1155|Community 1155]]
- [[_COMMUNITY_Community 1156|Community 1156]]
- [[_COMMUNITY_Community 1157|Community 1157]]
- [[_COMMUNITY_Community 1158|Community 1158]]
- [[_COMMUNITY_Community 1159|Community 1159]]
- [[_COMMUNITY_Community 1160|Community 1160]]
- [[_COMMUNITY_Community 1161|Community 1161]]
- [[_COMMUNITY_Community 1162|Community 1162]]
- [[_COMMUNITY_Community 1163|Community 1163]]
- [[_COMMUNITY_Community 1164|Community 1164]]
- [[_COMMUNITY_Community 1165|Community 1165]]
- [[_COMMUNITY_Community 1166|Community 1166]]
- [[_COMMUNITY_Community 1167|Community 1167]]
- [[_COMMUNITY_Community 1168|Community 1168]]
- [[_COMMUNITY_Community 1169|Community 1169]]
- [[_COMMUNITY_Community 1170|Community 1170]]
- [[_COMMUNITY_Community 1171|Community 1171]]
- [[_COMMUNITY_Community 1172|Community 1172]]
- [[_COMMUNITY_Community 1173|Community 1173]]
- [[_COMMUNITY_Community 1174|Community 1174]]
- [[_COMMUNITY_Community 1175|Community 1175]]
- [[_COMMUNITY_Community 1176|Community 1176]]
- [[_COMMUNITY_Community 1177|Community 1177]]
- [[_COMMUNITY_Community 1178|Community 1178]]
- [[_COMMUNITY_Community 1179|Community 1179]]
- [[_COMMUNITY_Community 1180|Community 1180]]
- [[_COMMUNITY_Community 1181|Community 1181]]
- [[_COMMUNITY_Community 1182|Community 1182]]
- [[_COMMUNITY_Community 1183|Community 1183]]
- [[_COMMUNITY_Community 1184|Community 1184]]
- [[_COMMUNITY_Community 1185|Community 1185]]
- [[_COMMUNITY_Community 1186|Community 1186]]
- [[_COMMUNITY_Community 1187|Community 1187]]
- [[_COMMUNITY_Community 1188|Community 1188]]
- [[_COMMUNITY_Community 1189|Community 1189]]
- [[_COMMUNITY_Community 1190|Community 1190]]
- [[_COMMUNITY_Community 1191|Community 1191]]
- [[_COMMUNITY_Community 1192|Community 1192]]
- [[_COMMUNITY_Community 1193|Community 1193]]
- [[_COMMUNITY_Community 1194|Community 1194]]
- [[_COMMUNITY_Community 1195|Community 1195]]
- [[_COMMUNITY_Community 1196|Community 1196]]
- [[_COMMUNITY_Community 1197|Community 1197]]
- [[_COMMUNITY_Community 1198|Community 1198]]
- [[_COMMUNITY_Community 1199|Community 1199]]
- [[_COMMUNITY_Community 1200|Community 1200]]
- [[_COMMUNITY_Community 1201|Community 1201]]
- [[_COMMUNITY_Community 1202|Community 1202]]
- [[_COMMUNITY_Community 1203|Community 1203]]
- [[_COMMUNITY_Community 1204|Community 1204]]
- [[_COMMUNITY_Community 1205|Community 1205]]
- [[_COMMUNITY_Community 1206|Community 1206]]
- [[_COMMUNITY_Community 1207|Community 1207]]
- [[_COMMUNITY_Community 1208|Community 1208]]
- [[_COMMUNITY_Community 1209|Community 1209]]
- [[_COMMUNITY_Community 1210|Community 1210]]
- [[_COMMUNITY_Community 1211|Community 1211]]
- [[_COMMUNITY_Community 1212|Community 1212]]
- [[_COMMUNITY_Community 1213|Community 1213]]
- [[_COMMUNITY_Community 1214|Community 1214]]
- [[_COMMUNITY_Community 1215|Community 1215]]
- [[_COMMUNITY_Community 1216|Community 1216]]
- [[_COMMUNITY_Community 1217|Community 1217]]
- [[_COMMUNITY_Community 1218|Community 1218]]
- [[_COMMUNITY_Community 1219|Community 1219]]
- [[_COMMUNITY_Community 1220|Community 1220]]
- [[_COMMUNITY_Community 1221|Community 1221]]
- [[_COMMUNITY_Community 1222|Community 1222]]
- [[_COMMUNITY_Community 1223|Community 1223]]
- [[_COMMUNITY_Community 1224|Community 1224]]
- [[_COMMUNITY_Community 1225|Community 1225]]
- [[_COMMUNITY_Community 1226|Community 1226]]
- [[_COMMUNITY_Community 1227|Community 1227]]
- [[_COMMUNITY_Community 1228|Community 1228]]
- [[_COMMUNITY_Community 1229|Community 1229]]
- [[_COMMUNITY_Community 1230|Community 1230]]
- [[_COMMUNITY_Community 1231|Community 1231]]
- [[_COMMUNITY_Community 1232|Community 1232]]
- [[_COMMUNITY_Community 1233|Community 1233]]
- [[_COMMUNITY_Community 1234|Community 1234]]
- [[_COMMUNITY_Community 1235|Community 1235]]
- [[_COMMUNITY_Community 1236|Community 1236]]
- [[_COMMUNITY_Community 1237|Community 1237]]
- [[_COMMUNITY_Community 1238|Community 1238]]
- [[_COMMUNITY_Community 1239|Community 1239]]
- [[_COMMUNITY_Community 1240|Community 1240]]
- [[_COMMUNITY_Community 1241|Community 1241]]
- [[_COMMUNITY_Community 1242|Community 1242]]
- [[_COMMUNITY_Community 1243|Community 1243]]
- [[_COMMUNITY_Community 1244|Community 1244]]
- [[_COMMUNITY_Community 1245|Community 1245]]
- [[_COMMUNITY_Community 1246|Community 1246]]
- [[_COMMUNITY_Community 1247|Community 1247]]
- [[_COMMUNITY_Community 1248|Community 1248]]
- [[_COMMUNITY_Community 1249|Community 1249]]
- [[_COMMUNITY_Community 1250|Community 1250]]
- [[_COMMUNITY_Community 1251|Community 1251]]
- [[_COMMUNITY_Community 1252|Community 1252]]
- [[_COMMUNITY_Community 1253|Community 1253]]
- [[_COMMUNITY_Community 1254|Community 1254]]
- [[_COMMUNITY_Community 1255|Community 1255]]
- [[_COMMUNITY_Community 1256|Community 1256]]
- [[_COMMUNITY_Community 1257|Community 1257]]
- [[_COMMUNITY_Community 1258|Community 1258]]
- [[_COMMUNITY_Community 1259|Community 1259]]
- [[_COMMUNITY_Community 1260|Community 1260]]
- [[_COMMUNITY_Community 1261|Community 1261]]
- [[_COMMUNITY_Community 1262|Community 1262]]
- [[_COMMUNITY_Community 1263|Community 1263]]
- [[_COMMUNITY_Community 1264|Community 1264]]
- [[_COMMUNITY_Community 1265|Community 1265]]
- [[_COMMUNITY_Community 1266|Community 1266]]
- [[_COMMUNITY_Community 1267|Community 1267]]
- [[_COMMUNITY_Community 1268|Community 1268]]
- [[_COMMUNITY_Community 1269|Community 1269]]
- [[_COMMUNITY_Community 1270|Community 1270]]
- [[_COMMUNITY_Community 1271|Community 1271]]
- [[_COMMUNITY_Community 1272|Community 1272]]
- [[_COMMUNITY_Community 1273|Community 1273]]
- [[_COMMUNITY_Community 1274|Community 1274]]
- [[_COMMUNITY_Community 1275|Community 1275]]
- [[_COMMUNITY_Community 1276|Community 1276]]
- [[_COMMUNITY_Community 1277|Community 1277]]
- [[_COMMUNITY_Community 1278|Community 1278]]
- [[_COMMUNITY_Community 1279|Community 1279]]
- [[_COMMUNITY_Community 1280|Community 1280]]
- [[_COMMUNITY_Community 1281|Community 1281]]
- [[_COMMUNITY_Community 1282|Community 1282]]
- [[_COMMUNITY_Community 1283|Community 1283]]
- [[_COMMUNITY_Community 1284|Community 1284]]
- [[_COMMUNITY_Community 1285|Community 1285]]
- [[_COMMUNITY_Community 1286|Community 1286]]
- [[_COMMUNITY_Community 1287|Community 1287]]
- [[_COMMUNITY_Community 1288|Community 1288]]
- [[_COMMUNITY_Community 1289|Community 1289]]
- [[_COMMUNITY_Community 1290|Community 1290]]
- [[_COMMUNITY_Community 1291|Community 1291]]
- [[_COMMUNITY_Community 1292|Community 1292]]
- [[_COMMUNITY_Community 1293|Community 1293]]
- [[_COMMUNITY_Community 1294|Community 1294]]
- [[_COMMUNITY_Community 1295|Community 1295]]
- [[_COMMUNITY_Community 1296|Community 1296]]
- [[_COMMUNITY_Community 1297|Community 1297]]
- [[_COMMUNITY_Community 1298|Community 1298]]
- [[_COMMUNITY_Community 1299|Community 1299]]
- [[_COMMUNITY_Community 1300|Community 1300]]
- [[_COMMUNITY_Community 1301|Community 1301]]
- [[_COMMUNITY_Community 1302|Community 1302]]
- [[_COMMUNITY_Community 1303|Community 1303]]
- [[_COMMUNITY_Community 1304|Community 1304]]
- [[_COMMUNITY_Community 1305|Community 1305]]
- [[_COMMUNITY_Community 1306|Community 1306]]
- [[_COMMUNITY_Community 1307|Community 1307]]
- [[_COMMUNITY_Community 1308|Community 1308]]
- [[_COMMUNITY_Community 1309|Community 1309]]
- [[_COMMUNITY_Community 1310|Community 1310]]
- [[_COMMUNITY_Community 1311|Community 1311]]
- [[_COMMUNITY_Community 1312|Community 1312]]
- [[_COMMUNITY_Community 1313|Community 1313]]
- [[_COMMUNITY_Community 1314|Community 1314]]
- [[_COMMUNITY_Community 1315|Community 1315]]
- [[_COMMUNITY_Community 1316|Community 1316]]
- [[_COMMUNITY_Community 1317|Community 1317]]
- [[_COMMUNITY_Community 1318|Community 1318]]
- [[_COMMUNITY_Community 1319|Community 1319]]
- [[_COMMUNITY_Community 1320|Community 1320]]
- [[_COMMUNITY_Community 1321|Community 1321]]
- [[_COMMUNITY_Community 1322|Community 1322]]
- [[_COMMUNITY_Community 1323|Community 1323]]
- [[_COMMUNITY_Community 1324|Community 1324]]
- [[_COMMUNITY_Community 1325|Community 1325]]
- [[_COMMUNITY_Community 1326|Community 1326]]
- [[_COMMUNITY_Community 1327|Community 1327]]
- [[_COMMUNITY_Community 1328|Community 1328]]
- [[_COMMUNITY_Community 1329|Community 1329]]
- [[_COMMUNITY_Community 1330|Community 1330]]
- [[_COMMUNITY_Community 1331|Community 1331]]
- [[_COMMUNITY_Community 1332|Community 1332]]
- [[_COMMUNITY_Community 1333|Community 1333]]
- [[_COMMUNITY_Community 1334|Community 1334]]
- [[_COMMUNITY_Community 1335|Community 1335]]
- [[_COMMUNITY_Community 1336|Community 1336]]
- [[_COMMUNITY_Community 1337|Community 1337]]
- [[_COMMUNITY_Community 1338|Community 1338]]
- [[_COMMUNITY_Community 1339|Community 1339]]
- [[_COMMUNITY_Community 1340|Community 1340]]
- [[_COMMUNITY_Community 1341|Community 1341]]
- [[_COMMUNITY_Community 1342|Community 1342]]
- [[_COMMUNITY_Community 1343|Community 1343]]
- [[_COMMUNITY_Community 1344|Community 1344]]
- [[_COMMUNITY_Community 1345|Community 1345]]
- [[_COMMUNITY_Community 1346|Community 1346]]
- [[_COMMUNITY_Community 1347|Community 1347]]
- [[_COMMUNITY_Community 1348|Community 1348]]
- [[_COMMUNITY_Community 1349|Community 1349]]
- [[_COMMUNITY_Community 1350|Community 1350]]
- [[_COMMUNITY_Community 1351|Community 1351]]
- [[_COMMUNITY_Community 1352|Community 1352]]
- [[_COMMUNITY_Community 1353|Community 1353]]
- [[_COMMUNITY_Community 1354|Community 1354]]
- [[_COMMUNITY_Community 1355|Community 1355]]
- [[_COMMUNITY_Community 1356|Community 1356]]
- [[_COMMUNITY_Community 1357|Community 1357]]
- [[_COMMUNITY_Community 1358|Community 1358]]
- [[_COMMUNITY_Community 1359|Community 1359]]
- [[_COMMUNITY_Community 1360|Community 1360]]
- [[_COMMUNITY_Community 1361|Community 1361]]
- [[_COMMUNITY_Community 1362|Community 1362]]
- [[_COMMUNITY_Community 1363|Community 1363]]
- [[_COMMUNITY_Community 1364|Community 1364]]
- [[_COMMUNITY_Community 1365|Community 1365]]
- [[_COMMUNITY_Community 1366|Community 1366]]
- [[_COMMUNITY_Community 1367|Community 1367]]
- [[_COMMUNITY_Community 1368|Community 1368]]
- [[_COMMUNITY_Community 1369|Community 1369]]
- [[_COMMUNITY_Community 1370|Community 1370]]
- [[_COMMUNITY_Community 1371|Community 1371]]
- [[_COMMUNITY_Community 1372|Community 1372]]
- [[_COMMUNITY_Community 1373|Community 1373]]
- [[_COMMUNITY_Community 1374|Community 1374]]
- [[_COMMUNITY_Community 1375|Community 1375]]
- [[_COMMUNITY_Community 1376|Community 1376]]
- [[_COMMUNITY_Community 1377|Community 1377]]
- [[_COMMUNITY_Community 1378|Community 1378]]
- [[_COMMUNITY_Community 1379|Community 1379]]
- [[_COMMUNITY_Community 1380|Community 1380]]
- [[_COMMUNITY_Community 1381|Community 1381]]
- [[_COMMUNITY_Community 1382|Community 1382]]
- [[_COMMUNITY_Community 1383|Community 1383]]
- [[_COMMUNITY_Community 1384|Community 1384]]
- [[_COMMUNITY_Community 1385|Community 1385]]
- [[_COMMUNITY_Community 1386|Community 1386]]
- [[_COMMUNITY_Community 1387|Community 1387]]
- [[_COMMUNITY_Community 1388|Community 1388]]
- [[_COMMUNITY_Community 1389|Community 1389]]
- [[_COMMUNITY_Community 1390|Community 1390]]
- [[_COMMUNITY_Community 1391|Community 1391]]
- [[_COMMUNITY_Community 1392|Community 1392]]
- [[_COMMUNITY_Community 1393|Community 1393]]
- [[_COMMUNITY_Community 1394|Community 1394]]
- [[_COMMUNITY_Community 1395|Community 1395]]
- [[_COMMUNITY_Community 1396|Community 1396]]
- [[_COMMUNITY_Community 1397|Community 1397]]
- [[_COMMUNITY_Community 1398|Community 1398]]
- [[_COMMUNITY_Community 1399|Community 1399]]
- [[_COMMUNITY_Community 1400|Community 1400]]
- [[_COMMUNITY_Community 1401|Community 1401]]
- [[_COMMUNITY_Community 1402|Community 1402]]
- [[_COMMUNITY_Community 1403|Community 1403]]
- [[_COMMUNITY_Community 1404|Community 1404]]
- [[_COMMUNITY_Community 1405|Community 1405]]
- [[_COMMUNITY_Community 1406|Community 1406]]
- [[_COMMUNITY_Community 1407|Community 1407]]
- [[_COMMUNITY_Community 1408|Community 1408]]
- [[_COMMUNITY_Community 1409|Community 1409]]
- [[_COMMUNITY_Community 1410|Community 1410]]
- [[_COMMUNITY_Community 1411|Community 1411]]
- [[_COMMUNITY_Community 1412|Community 1412]]
- [[_COMMUNITY_Community 1413|Community 1413]]
- [[_COMMUNITY_Community 1414|Community 1414]]
- [[_COMMUNITY_Community 1415|Community 1415]]
- [[_COMMUNITY_Community 1416|Community 1416]]
- [[_COMMUNITY_Community 1417|Community 1417]]
- [[_COMMUNITY_Community 1418|Community 1418]]
- [[_COMMUNITY_Community 1419|Community 1419]]
- [[_COMMUNITY_Community 1420|Community 1420]]
- [[_COMMUNITY_Community 1421|Community 1421]]
- [[_COMMUNITY_Community 1422|Community 1422]]
- [[_COMMUNITY_Community 1423|Community 1423]]
- [[_COMMUNITY_Community 1424|Community 1424]]
- [[_COMMUNITY_Community 1425|Community 1425]]
- [[_COMMUNITY_Community 1426|Community 1426]]
- [[_COMMUNITY_Community 1427|Community 1427]]
- [[_COMMUNITY_Community 1428|Community 1428]]
- [[_COMMUNITY_Community 1429|Community 1429]]
- [[_COMMUNITY_Community 1430|Community 1430]]
- [[_COMMUNITY_Community 1431|Community 1431]]
- [[_COMMUNITY_Community 1432|Community 1432]]
- [[_COMMUNITY_Community 1433|Community 1433]]
- [[_COMMUNITY_Community 1434|Community 1434]]
- [[_COMMUNITY_Community 1435|Community 1435]]
- [[_COMMUNITY_Community 1436|Community 1436]]
- [[_COMMUNITY_Community 1437|Community 1437]]
- [[_COMMUNITY_Community 1438|Community 1438]]
- [[_COMMUNITY_Community 1439|Community 1439]]
- [[_COMMUNITY_Community 1440|Community 1440]]
- [[_COMMUNITY_Community 1441|Community 1441]]
- [[_COMMUNITY_Community 1442|Community 1442]]
- [[_COMMUNITY_Community 1443|Community 1443]]
- [[_COMMUNITY_Community 1444|Community 1444]]
- [[_COMMUNITY_Community 1445|Community 1445]]
- [[_COMMUNITY_Community 1446|Community 1446]]
- [[_COMMUNITY_Community 1447|Community 1447]]
- [[_COMMUNITY_Community 1448|Community 1448]]
- [[_COMMUNITY_Community 1449|Community 1449]]
- [[_COMMUNITY_Community 1450|Community 1450]]
- [[_COMMUNITY_Community 1451|Community 1451]]
- [[_COMMUNITY_Community 1452|Community 1452]]
- [[_COMMUNITY_Community 1453|Community 1453]]
- [[_COMMUNITY_Community 1454|Community 1454]]
- [[_COMMUNITY_Community 1455|Community 1455]]
- [[_COMMUNITY_Community 1456|Community 1456]]
- [[_COMMUNITY_Community 1457|Community 1457]]
- [[_COMMUNITY_Community 1458|Community 1458]]
- [[_COMMUNITY_Community 1459|Community 1459]]
- [[_COMMUNITY_Community 1460|Community 1460]]
- [[_COMMUNITY_Community 1461|Community 1461]]
- [[_COMMUNITY_Community 1462|Community 1462]]
- [[_COMMUNITY_Community 1463|Community 1463]]
- [[_COMMUNITY_Community 1464|Community 1464]]
- [[_COMMUNITY_Community 1465|Community 1465]]
- [[_COMMUNITY_Community 1466|Community 1466]]
- [[_COMMUNITY_Community 1467|Community 1467]]
- [[_COMMUNITY_Community 1468|Community 1468]]
- [[_COMMUNITY_Community 1469|Community 1469]]
- [[_COMMUNITY_Community 1470|Community 1470]]
- [[_COMMUNITY_Community 1471|Community 1471]]
- [[_COMMUNITY_Community 1472|Community 1472]]
- [[_COMMUNITY_Community 1473|Community 1473]]
- [[_COMMUNITY_Community 1474|Community 1474]]
- [[_COMMUNITY_Community 1475|Community 1475]]
- [[_COMMUNITY_Community 1476|Community 1476]]
- [[_COMMUNITY_Community 1477|Community 1477]]
- [[_COMMUNITY_Community 1478|Community 1478]]
- [[_COMMUNITY_Community 1479|Community 1479]]
- [[_COMMUNITY_Community 1480|Community 1480]]
- [[_COMMUNITY_Community 1481|Community 1481]]
- [[_COMMUNITY_Community 1482|Community 1482]]
- [[_COMMUNITY_Community 1483|Community 1483]]
- [[_COMMUNITY_Community 1484|Community 1484]]
- [[_COMMUNITY_Community 1485|Community 1485]]
- [[_COMMUNITY_Community 1486|Community 1486]]
- [[_COMMUNITY_Community 1487|Community 1487]]
- [[_COMMUNITY_Community 1488|Community 1488]]
- [[_COMMUNITY_Community 1489|Community 1489]]
- [[_COMMUNITY_Community 1490|Community 1490]]
- [[_COMMUNITY_Community 1491|Community 1491]]
- [[_COMMUNITY_Community 1492|Community 1492]]
- [[_COMMUNITY_Community 1493|Community 1493]]
- [[_COMMUNITY_Community 1494|Community 1494]]
- [[_COMMUNITY_Community 1495|Community 1495]]
- [[_COMMUNITY_Community 1496|Community 1496]]
- [[_COMMUNITY_Community 1497|Community 1497]]
- [[_COMMUNITY_Community 1498|Community 1498]]
- [[_COMMUNITY_Community 1499|Community 1499]]
- [[_COMMUNITY_Community 1500|Community 1500]]
- [[_COMMUNITY_Community 1501|Community 1501]]
- [[_COMMUNITY_Community 1502|Community 1502]]
- [[_COMMUNITY_Community 1503|Community 1503]]
- [[_COMMUNITY_Community 1504|Community 1504]]
- [[_COMMUNITY_Community 1505|Community 1505]]
- [[_COMMUNITY_Community 1506|Community 1506]]
- [[_COMMUNITY_Community 1507|Community 1507]]
- [[_COMMUNITY_Community 1508|Community 1508]]
- [[_COMMUNITY_Community 1509|Community 1509]]
- [[_COMMUNITY_Community 1510|Community 1510]]
- [[_COMMUNITY_Community 1511|Community 1511]]
- [[_COMMUNITY_Community 1512|Community 1512]]
- [[_COMMUNITY_Community 1513|Community 1513]]
- [[_COMMUNITY_Community 1514|Community 1514]]
- [[_COMMUNITY_Community 1515|Community 1515]]
- [[_COMMUNITY_Community 1516|Community 1516]]
- [[_COMMUNITY_Community 1517|Community 1517]]
- [[_COMMUNITY_Community 1518|Community 1518]]
- [[_COMMUNITY_Community 1519|Community 1519]]
- [[_COMMUNITY_Community 1520|Community 1520]]
- [[_COMMUNITY_Community 1521|Community 1521]]
- [[_COMMUNITY_Community 1522|Community 1522]]
- [[_COMMUNITY_Community 1523|Community 1523]]
- [[_COMMUNITY_Community 1524|Community 1524]]
- [[_COMMUNITY_Community 1525|Community 1525]]
- [[_COMMUNITY_Community 1526|Community 1526]]
- [[_COMMUNITY_Community 1527|Community 1527]]
- [[_COMMUNITY_Community 1528|Community 1528]]
- [[_COMMUNITY_Community 1529|Community 1529]]
- [[_COMMUNITY_Community 1530|Community 1530]]
- [[_COMMUNITY_Community 1531|Community 1531]]
- [[_COMMUNITY_Community 1532|Community 1532]]
- [[_COMMUNITY_Community 1533|Community 1533]]
- [[_COMMUNITY_Community 1534|Community 1534]]
- [[_COMMUNITY_Community 1535|Community 1535]]
- [[_COMMUNITY_Community 1536|Community 1536]]
- [[_COMMUNITY_Community 1537|Community 1537]]
- [[_COMMUNITY_Community 1538|Community 1538]]
- [[_COMMUNITY_Community 1539|Community 1539]]
- [[_COMMUNITY_Community 1540|Community 1540]]
- [[_COMMUNITY_Community 1541|Community 1541]]
- [[_COMMUNITY_Community 1542|Community 1542]]
- [[_COMMUNITY_Community 1543|Community 1543]]
- [[_COMMUNITY_Community 1544|Community 1544]]
- [[_COMMUNITY_Community 1545|Community 1545]]
- [[_COMMUNITY_Community 1546|Community 1546]]
- [[_COMMUNITY_Community 1547|Community 1547]]
- [[_COMMUNITY_Community 1548|Community 1548]]
- [[_COMMUNITY_Community 1549|Community 1549]]
- [[_COMMUNITY_Community 1550|Community 1550]]
- [[_COMMUNITY_Community 1551|Community 1551]]
- [[_COMMUNITY_Community 1552|Community 1552]]
- [[_COMMUNITY_Community 1553|Community 1553]]
- [[_COMMUNITY_Community 1554|Community 1554]]
- [[_COMMUNITY_Community 1555|Community 1555]]
- [[_COMMUNITY_Community 1556|Community 1556]]
- [[_COMMUNITY_Community 1557|Community 1557]]
- [[_COMMUNITY_Community 1558|Community 1558]]
- [[_COMMUNITY_Community 1559|Community 1559]]
- [[_COMMUNITY_Community 1560|Community 1560]]
- [[_COMMUNITY_Community 1561|Community 1561]]
- [[_COMMUNITY_Community 1562|Community 1562]]
- [[_COMMUNITY_Community 1563|Community 1563]]
- [[_COMMUNITY_Community 1564|Community 1564]]
- [[_COMMUNITY_Community 1565|Community 1565]]
- [[_COMMUNITY_Community 1566|Community 1566]]
- [[_COMMUNITY_Community 1567|Community 1567]]
- [[_COMMUNITY_Community 1568|Community 1568]]
- [[_COMMUNITY_Community 1569|Community 1569]]
- [[_COMMUNITY_Community 1570|Community 1570]]
- [[_COMMUNITY_Community 1571|Community 1571]]
- [[_COMMUNITY_Community 1572|Community 1572]]
- [[_COMMUNITY_Community 1573|Community 1573]]
- [[_COMMUNITY_Community 1574|Community 1574]]
- [[_COMMUNITY_Community 1575|Community 1575]]
- [[_COMMUNITY_Community 1576|Community 1576]]
- [[_COMMUNITY_Community 1577|Community 1577]]
- [[_COMMUNITY_Community 1578|Community 1578]]
- [[_COMMUNITY_Community 1579|Community 1579]]
- [[_COMMUNITY_Community 1580|Community 1580]]
- [[_COMMUNITY_Community 1581|Community 1581]]
- [[_COMMUNITY_Community 1582|Community 1582]]
- [[_COMMUNITY_Community 1583|Community 1583]]
- [[_COMMUNITY_Community 1584|Community 1584]]
- [[_COMMUNITY_Community 1585|Community 1585]]
- [[_COMMUNITY_Community 1586|Community 1586]]
- [[_COMMUNITY_Community 1587|Community 1587]]
- [[_COMMUNITY_Community 1588|Community 1588]]
- [[_COMMUNITY_Community 1589|Community 1589]]
- [[_COMMUNITY_Community 1590|Community 1590]]
- [[_COMMUNITY_Community 1591|Community 1591]]
- [[_COMMUNITY_Community 1592|Community 1592]]
- [[_COMMUNITY_Community 1593|Community 1593]]
- [[_COMMUNITY_Community 1594|Community 1594]]
- [[_COMMUNITY_Community 1595|Community 1595]]
- [[_COMMUNITY_Community 1596|Community 1596]]
- [[_COMMUNITY_Community 1597|Community 1597]]
- [[_COMMUNITY_Community 1598|Community 1598]]
- [[_COMMUNITY_Community 1599|Community 1599]]
- [[_COMMUNITY_Community 1600|Community 1600]]
- [[_COMMUNITY_Community 1601|Community 1601]]
- [[_COMMUNITY_Community 1602|Community 1602]]
- [[_COMMUNITY_Community 1603|Community 1603]]
- [[_COMMUNITY_Community 1604|Community 1604]]
- [[_COMMUNITY_Community 1605|Community 1605]]
- [[_COMMUNITY_Community 1606|Community 1606]]
- [[_COMMUNITY_Community 1607|Community 1607]]
- [[_COMMUNITY_Community 1608|Community 1608]]
- [[_COMMUNITY_Community 1609|Community 1609]]
- [[_COMMUNITY_Community 1610|Community 1610]]
- [[_COMMUNITY_Community 1611|Community 1611]]
- [[_COMMUNITY_Community 1612|Community 1612]]
- [[_COMMUNITY_Community 1613|Community 1613]]
- [[_COMMUNITY_Community 1614|Community 1614]]
- [[_COMMUNITY_Community 1615|Community 1615]]
- [[_COMMUNITY_Community 1616|Community 1616]]
- [[_COMMUNITY_Community 1617|Community 1617]]
- [[_COMMUNITY_Community 1618|Community 1618]]
- [[_COMMUNITY_Community 1619|Community 1619]]
- [[_COMMUNITY_Community 1620|Community 1620]]
- [[_COMMUNITY_Community 1621|Community 1621]]
- [[_COMMUNITY_Community 1622|Community 1622]]
- [[_COMMUNITY_Community 1623|Community 1623]]
- [[_COMMUNITY_Community 1624|Community 1624]]
- [[_COMMUNITY_Community 1625|Community 1625]]
- [[_COMMUNITY_Community 1626|Community 1626]]
- [[_COMMUNITY_Community 1627|Community 1627]]
- [[_COMMUNITY_Community 1628|Community 1628]]
- [[_COMMUNITY_Community 1629|Community 1629]]
- [[_COMMUNITY_Community 1630|Community 1630]]
- [[_COMMUNITY_Community 1631|Community 1631]]
- [[_COMMUNITY_Community 1632|Community 1632]]
- [[_COMMUNITY_Community 1633|Community 1633]]
- [[_COMMUNITY_Community 1634|Community 1634]]
- [[_COMMUNITY_Community 1635|Community 1635]]
- [[_COMMUNITY_Community 1636|Community 1636]]
- [[_COMMUNITY_Community 1637|Community 1637]]
- [[_COMMUNITY_Community 1638|Community 1638]]
- [[_COMMUNITY_Community 1639|Community 1639]]
- [[_COMMUNITY_Community 1640|Community 1640]]
- [[_COMMUNITY_Community 1641|Community 1641]]
- [[_COMMUNITY_Community 1642|Community 1642]]
- [[_COMMUNITY_Community 1643|Community 1643]]
- [[_COMMUNITY_Community 1644|Community 1644]]
- [[_COMMUNITY_Community 1645|Community 1645]]
- [[_COMMUNITY_Community 1646|Community 1646]]
- [[_COMMUNITY_Community 1647|Community 1647]]
- [[_COMMUNITY_Community 1648|Community 1648]]
- [[_COMMUNITY_Community 1649|Community 1649]]
- [[_COMMUNITY_Community 1650|Community 1650]]
- [[_COMMUNITY_Community 1651|Community 1651]]
- [[_COMMUNITY_Community 1652|Community 1652]]
- [[_COMMUNITY_Community 1653|Community 1653]]
- [[_COMMUNITY_Community 1654|Community 1654]]
- [[_COMMUNITY_Community 1655|Community 1655]]
- [[_COMMUNITY_Community 1656|Community 1656]]
- [[_COMMUNITY_Community 1657|Community 1657]]
- [[_COMMUNITY_Community 1658|Community 1658]]
- [[_COMMUNITY_Community 1659|Community 1659]]
- [[_COMMUNITY_Community 1660|Community 1660]]
- [[_COMMUNITY_Community 1661|Community 1661]]
- [[_COMMUNITY_Community 1662|Community 1662]]
- [[_COMMUNITY_Community 1663|Community 1663]]
- [[_COMMUNITY_Community 1664|Community 1664]]
- [[_COMMUNITY_Community 1665|Community 1665]]
- [[_COMMUNITY_Community 1666|Community 1666]]
- [[_COMMUNITY_Community 1667|Community 1667]]
- [[_COMMUNITY_Community 1668|Community 1668]]
- [[_COMMUNITY_Community 1669|Community 1669]]
- [[_COMMUNITY_Community 1670|Community 1670]]
- [[_COMMUNITY_Community 1671|Community 1671]]
- [[_COMMUNITY_Community 1672|Community 1672]]
- [[_COMMUNITY_Community 1673|Community 1673]]
- [[_COMMUNITY_Community 1674|Community 1674]]
- [[_COMMUNITY_Community 1675|Community 1675]]
- [[_COMMUNITY_Community 1676|Community 1676]]
- [[_COMMUNITY_Community 1677|Community 1677]]
- [[_COMMUNITY_Community 1678|Community 1678]]
- [[_COMMUNITY_Community 1679|Community 1679]]
- [[_COMMUNITY_Community 1680|Community 1680]]
- [[_COMMUNITY_Community 1681|Community 1681]]
- [[_COMMUNITY_Community 1682|Community 1682]]
- [[_COMMUNITY_Community 1683|Community 1683]]
- [[_COMMUNITY_Community 1684|Community 1684]]
- [[_COMMUNITY_Community 1685|Community 1685]]
- [[_COMMUNITY_Community 1686|Community 1686]]
- [[_COMMUNITY_Community 1687|Community 1687]]
- [[_COMMUNITY_Community 1688|Community 1688]]
- [[_COMMUNITY_Community 1689|Community 1689]]
- [[_COMMUNITY_Community 1690|Community 1690]]
- [[_COMMUNITY_Community 1691|Community 1691]]
- [[_COMMUNITY_Community 1692|Community 1692]]
- [[_COMMUNITY_Community 1693|Community 1693]]
- [[_COMMUNITY_Community 1694|Community 1694]]
- [[_COMMUNITY_Community 1695|Community 1695]]
- [[_COMMUNITY_Community 1696|Community 1696]]
- [[_COMMUNITY_Community 1697|Community 1697]]
- [[_COMMUNITY_Community 1698|Community 1698]]
- [[_COMMUNITY_Community 1699|Community 1699]]
- [[_COMMUNITY_Community 1700|Community 1700]]
- [[_COMMUNITY_Community 1701|Community 1701]]
- [[_COMMUNITY_Community 1702|Community 1702]]
- [[_COMMUNITY_Community 1703|Community 1703]]
- [[_COMMUNITY_Community 1704|Community 1704]]
- [[_COMMUNITY_Community 1705|Community 1705]]
- [[_COMMUNITY_Community 1706|Community 1706]]
- [[_COMMUNITY_Community 1707|Community 1707]]
- [[_COMMUNITY_Community 1708|Community 1708]]
- [[_COMMUNITY_Community 1709|Community 1709]]
- [[_COMMUNITY_Community 1710|Community 1710]]
- [[_COMMUNITY_Community 1711|Community 1711]]
- [[_COMMUNITY_Community 1712|Community 1712]]
- [[_COMMUNITY_Community 1713|Community 1713]]
- [[_COMMUNITY_Community 1714|Community 1714]]
- [[_COMMUNITY_Community 1715|Community 1715]]
- [[_COMMUNITY_Community 1716|Community 1716]]
- [[_COMMUNITY_Community 1717|Community 1717]]
- [[_COMMUNITY_Community 1718|Community 1718]]
- [[_COMMUNITY_Community 1719|Community 1719]]
- [[_COMMUNITY_Community 1720|Community 1720]]
- [[_COMMUNITY_Community 1721|Community 1721]]
- [[_COMMUNITY_Community 1722|Community 1722]]
- [[_COMMUNITY_Community 1723|Community 1723]]
- [[_COMMUNITY_Community 1724|Community 1724]]
- [[_COMMUNITY_Community 1725|Community 1725]]
- [[_COMMUNITY_Community 1726|Community 1726]]
- [[_COMMUNITY_Community 1727|Community 1727]]
- [[_COMMUNITY_Community 1728|Community 1728]]
- [[_COMMUNITY_Community 1729|Community 1729]]
- [[_COMMUNITY_Community 1730|Community 1730]]
- [[_COMMUNITY_Community 1731|Community 1731]]
- [[_COMMUNITY_Community 1732|Community 1732]]
- [[_COMMUNITY_Community 1733|Community 1733]]
- [[_COMMUNITY_Community 1734|Community 1734]]
- [[_COMMUNITY_Community 1735|Community 1735]]
- [[_COMMUNITY_Community 1736|Community 1736]]
- [[_COMMUNITY_Community 1737|Community 1737]]
- [[_COMMUNITY_Community 1738|Community 1738]]
- [[_COMMUNITY_Community 1739|Community 1739]]
- [[_COMMUNITY_Community 1740|Community 1740]]
- [[_COMMUNITY_Community 1741|Community 1741]]
- [[_COMMUNITY_Community 1742|Community 1742]]
- [[_COMMUNITY_Community 1743|Community 1743]]
- [[_COMMUNITY_Community 1744|Community 1744]]
- [[_COMMUNITY_Community 1745|Community 1745]]
- [[_COMMUNITY_Community 1746|Community 1746]]
- [[_COMMUNITY_Community 1747|Community 1747]]
- [[_COMMUNITY_Community 1748|Community 1748]]
- [[_COMMUNITY_Community 1749|Community 1749]]
- [[_COMMUNITY_Community 1750|Community 1750]]
- [[_COMMUNITY_Community 1751|Community 1751]]
- [[_COMMUNITY_Community 1752|Community 1752]]
- [[_COMMUNITY_Community 1753|Community 1753]]
- [[_COMMUNITY_Community 1754|Community 1754]]
- [[_COMMUNITY_Community 1755|Community 1755]]
- [[_COMMUNITY_Community 1756|Community 1756]]
- [[_COMMUNITY_Community 1757|Community 1757]]
- [[_COMMUNITY_Community 1758|Community 1758]]
- [[_COMMUNITY_Community 1759|Community 1759]]
- [[_COMMUNITY_Community 1760|Community 1760]]
- [[_COMMUNITY_Community 1761|Community 1761]]
- [[_COMMUNITY_Community 1762|Community 1762]]
- [[_COMMUNITY_Community 1763|Community 1763]]
- [[_COMMUNITY_Community 1764|Community 1764]]
- [[_COMMUNITY_Community 1765|Community 1765]]
- [[_COMMUNITY_Community 1766|Community 1766]]
- [[_COMMUNITY_Community 1767|Community 1767]]
- [[_COMMUNITY_Community 1768|Community 1768]]
- [[_COMMUNITY_Community 1769|Community 1769]]
- [[_COMMUNITY_Community 1770|Community 1770]]
- [[_COMMUNITY_Community 1771|Community 1771]]
- [[_COMMUNITY_Community 1772|Community 1772]]
- [[_COMMUNITY_Community 1773|Community 1773]]
- [[_COMMUNITY_Community 1774|Community 1774]]
- [[_COMMUNITY_Community 1775|Community 1775]]
- [[_COMMUNITY_Community 1776|Community 1776]]
- [[_COMMUNITY_Community 1777|Community 1777]]
- [[_COMMUNITY_Community 1778|Community 1778]]
- [[_COMMUNITY_Community 1779|Community 1779]]
- [[_COMMUNITY_Community 1780|Community 1780]]
- [[_COMMUNITY_Community 1781|Community 1781]]
- [[_COMMUNITY_Community 1782|Community 1782]]
- [[_COMMUNITY_Community 1783|Community 1783]]
- [[_COMMUNITY_Community 1784|Community 1784]]
- [[_COMMUNITY_Community 1785|Community 1785]]
- [[_COMMUNITY_Community 1786|Community 1786]]
- [[_COMMUNITY_Community 1787|Community 1787]]
- [[_COMMUNITY_Community 1788|Community 1788]]
- [[_COMMUNITY_Community 1789|Community 1789]]
- [[_COMMUNITY_Community 1790|Community 1790]]
- [[_COMMUNITY_Community 1791|Community 1791]]
- [[_COMMUNITY_Community 1792|Community 1792]]
- [[_COMMUNITY_Community 1793|Community 1793]]
- [[_COMMUNITY_Community 1794|Community 1794]]
- [[_COMMUNITY_Community 1795|Community 1795]]
- [[_COMMUNITY_Community 1796|Community 1796]]
- [[_COMMUNITY_Community 1797|Community 1797]]
- [[_COMMUNITY_Community 1798|Community 1798]]
- [[_COMMUNITY_Community 1799|Community 1799]]
- [[_COMMUNITY_Community 1800|Community 1800]]
- [[_COMMUNITY_Community 1801|Community 1801]]
- [[_COMMUNITY_Community 1802|Community 1802]]
- [[_COMMUNITY_Community 1803|Community 1803]]
- [[_COMMUNITY_Community 1804|Community 1804]]
- [[_COMMUNITY_Community 1805|Community 1805]]
- [[_COMMUNITY_Community 1806|Community 1806]]
- [[_COMMUNITY_Community 1807|Community 1807]]
- [[_COMMUNITY_Community 1808|Community 1808]]
- [[_COMMUNITY_Community 1809|Community 1809]]
- [[_COMMUNITY_Community 1810|Community 1810]]
- [[_COMMUNITY_Community 1811|Community 1811]]
- [[_COMMUNITY_Community 1812|Community 1812]]
- [[_COMMUNITY_Community 1813|Community 1813]]
- [[_COMMUNITY_Community 1814|Community 1814]]
- [[_COMMUNITY_Community 1815|Community 1815]]
- [[_COMMUNITY_Community 1816|Community 1816]]
- [[_COMMUNITY_Community 1817|Community 1817]]
- [[_COMMUNITY_Community 1818|Community 1818]]
- [[_COMMUNITY_Community 1819|Community 1819]]
- [[_COMMUNITY_Community 1820|Community 1820]]
- [[_COMMUNITY_Community 1821|Community 1821]]
- [[_COMMUNITY_Community 1822|Community 1822]]
- [[_COMMUNITY_Community 1823|Community 1823]]
- [[_COMMUNITY_Community 1824|Community 1824]]
- [[_COMMUNITY_Community 1825|Community 1825]]
- [[_COMMUNITY_Community 1826|Community 1826]]
- [[_COMMUNITY_Community 1827|Community 1827]]
- [[_COMMUNITY_Community 1828|Community 1828]]
- [[_COMMUNITY_Community 1829|Community 1829]]
- [[_COMMUNITY_Community 1830|Community 1830]]
- [[_COMMUNITY_Community 1831|Community 1831]]
- [[_COMMUNITY_Community 1832|Community 1832]]
- [[_COMMUNITY_Community 1833|Community 1833]]
- [[_COMMUNITY_Community 1834|Community 1834]]
- [[_COMMUNITY_Community 1835|Community 1835]]
- [[_COMMUNITY_Community 1836|Community 1836]]
- [[_COMMUNITY_Community 1837|Community 1837]]
- [[_COMMUNITY_Community 1838|Community 1838]]
- [[_COMMUNITY_Community 1839|Community 1839]]
- [[_COMMUNITY_Community 1840|Community 1840]]
- [[_COMMUNITY_Community 1841|Community 1841]]
- [[_COMMUNITY_Community 1842|Community 1842]]
- [[_COMMUNITY_Community 1843|Community 1843]]
- [[_COMMUNITY_Community 1844|Community 1844]]
- [[_COMMUNITY_Community 1845|Community 1845]]
- [[_COMMUNITY_Community 1846|Community 1846]]
- [[_COMMUNITY_Community 1847|Community 1847]]
- [[_COMMUNITY_Community 1848|Community 1848]]
- [[_COMMUNITY_Community 1849|Community 1849]]
- [[_COMMUNITY_Community 1850|Community 1850]]
- [[_COMMUNITY_Community 1851|Community 1851]]
- [[_COMMUNITY_Community 1852|Community 1852]]
- [[_COMMUNITY_Community 1853|Community 1853]]
- [[_COMMUNITY_Community 1854|Community 1854]]
- [[_COMMUNITY_Community 1855|Community 1855]]
- [[_COMMUNITY_Community 1856|Community 1856]]
- [[_COMMUNITY_Community 1857|Community 1857]]
- [[_COMMUNITY_Community 1858|Community 1858]]
- [[_COMMUNITY_Community 1859|Community 1859]]
- [[_COMMUNITY_Community 1860|Community 1860]]
- [[_COMMUNITY_Community 1861|Community 1861]]
- [[_COMMUNITY_Community 1862|Community 1862]]
- [[_COMMUNITY_Community 1863|Community 1863]]
- [[_COMMUNITY_Community 1864|Community 1864]]
- [[_COMMUNITY_Community 1865|Community 1865]]
- [[_COMMUNITY_Community 1866|Community 1866]]
- [[_COMMUNITY_Community 1867|Community 1867]]
- [[_COMMUNITY_Community 1868|Community 1868]]
- [[_COMMUNITY_Community 1869|Community 1869]]
- [[_COMMUNITY_Community 1870|Community 1870]]
- [[_COMMUNITY_Community 1871|Community 1871]]
- [[_COMMUNITY_Community 1872|Community 1872]]
- [[_COMMUNITY_Community 1873|Community 1873]]
- [[_COMMUNITY_Community 1874|Community 1874]]
- [[_COMMUNITY_Community 1875|Community 1875]]
- [[_COMMUNITY_Community 1876|Community 1876]]
- [[_COMMUNITY_Community 1877|Community 1877]]
- [[_COMMUNITY_Community 1878|Community 1878]]
- [[_COMMUNITY_Community 1879|Community 1879]]
- [[_COMMUNITY_Community 1880|Community 1880]]
- [[_COMMUNITY_Community 1881|Community 1881]]
- [[_COMMUNITY_Community 1882|Community 1882]]
- [[_COMMUNITY_Community 1883|Community 1883]]
- [[_COMMUNITY_Community 1884|Community 1884]]
- [[_COMMUNITY_Community 1885|Community 1885]]
- [[_COMMUNITY_Community 1886|Community 1886]]
- [[_COMMUNITY_Community 1887|Community 1887]]
- [[_COMMUNITY_Community 1888|Community 1888]]
- [[_COMMUNITY_Community 1889|Community 1889]]
- [[_COMMUNITY_Community 1890|Community 1890]]
- [[_COMMUNITY_Community 1891|Community 1891]]
- [[_COMMUNITY_Community 1892|Community 1892]]
- [[_COMMUNITY_Community 1893|Community 1893]]
- [[_COMMUNITY_Community 1894|Community 1894]]
- [[_COMMUNITY_Community 1895|Community 1895]]
- [[_COMMUNITY_Community 1896|Community 1896]]
- [[_COMMUNITY_Community 1897|Community 1897]]
- [[_COMMUNITY_Community 1898|Community 1898]]
- [[_COMMUNITY_Community 1899|Community 1899]]
- [[_COMMUNITY_Community 1900|Community 1900]]
- [[_COMMUNITY_Community 1901|Community 1901]]
- [[_COMMUNITY_Community 1902|Community 1902]]
- [[_COMMUNITY_Community 1903|Community 1903]]
- [[_COMMUNITY_Community 1904|Community 1904]]
- [[_COMMUNITY_Community 1905|Community 1905]]
- [[_COMMUNITY_Community 1906|Community 1906]]
- [[_COMMUNITY_Community 1907|Community 1907]]
- [[_COMMUNITY_Community 1908|Community 1908]]
- [[_COMMUNITY_Community 1909|Community 1909]]
- [[_COMMUNITY_Community 1910|Community 1910]]
- [[_COMMUNITY_Community 1911|Community 1911]]
- [[_COMMUNITY_Community 1912|Community 1912]]
- [[_COMMUNITY_Community 1913|Community 1913]]
- [[_COMMUNITY_Community 1914|Community 1914]]
- [[_COMMUNITY_Community 1915|Community 1915]]
- [[_COMMUNITY_Community 1916|Community 1916]]
- [[_COMMUNITY_Community 1917|Community 1917]]
- [[_COMMUNITY_Community 1918|Community 1918]]
- [[_COMMUNITY_Community 1919|Community 1919]]
- [[_COMMUNITY_Community 1920|Community 1920]]
- [[_COMMUNITY_Community 1921|Community 1921]]
- [[_COMMUNITY_Community 1922|Community 1922]]
- [[_COMMUNITY_Community 1923|Community 1923]]
- [[_COMMUNITY_Community 1924|Community 1924]]
- [[_COMMUNITY_Community 1925|Community 1925]]
- [[_COMMUNITY_Community 1926|Community 1926]]
- [[_COMMUNITY_Community 1927|Community 1927]]
- [[_COMMUNITY_Community 1928|Community 1928]]
- [[_COMMUNITY_Community 1929|Community 1929]]
- [[_COMMUNITY_Community 1930|Community 1930]]
- [[_COMMUNITY_Community 1931|Community 1931]]
- [[_COMMUNITY_Community 1932|Community 1932]]
- [[_COMMUNITY_Community 1933|Community 1933]]
- [[_COMMUNITY_Community 1934|Community 1934]]
- [[_COMMUNITY_Community 1935|Community 1935]]
- [[_COMMUNITY_Community 1936|Community 1936]]
- [[_COMMUNITY_Community 1937|Community 1937]]
- [[_COMMUNITY_Community 1938|Community 1938]]
- [[_COMMUNITY_Community 1939|Community 1939]]
- [[_COMMUNITY_Community 1940|Community 1940]]
- [[_COMMUNITY_Community 1941|Community 1941]]
- [[_COMMUNITY_Community 1942|Community 1942]]
- [[_COMMUNITY_Community 1943|Community 1943]]
- [[_COMMUNITY_Community 1944|Community 1944]]
- [[_COMMUNITY_Community 1945|Community 1945]]
- [[_COMMUNITY_Community 1946|Community 1946]]
- [[_COMMUNITY_Community 1947|Community 1947]]
- [[_COMMUNITY_Community 1948|Community 1948]]
- [[_COMMUNITY_Community 1949|Community 1949]]
- [[_COMMUNITY_Community 1950|Community 1950]]
- [[_COMMUNITY_Community 1951|Community 1951]]
- [[_COMMUNITY_Community 1952|Community 1952]]
- [[_COMMUNITY_Community 1953|Community 1953]]
- [[_COMMUNITY_Community 1954|Community 1954]]
- [[_COMMUNITY_Community 1955|Community 1955]]
- [[_COMMUNITY_Community 1956|Community 1956]]
- [[_COMMUNITY_Community 1957|Community 1957]]
- [[_COMMUNITY_Community 1958|Community 1958]]
- [[_COMMUNITY_Community 1959|Community 1959]]
- [[_COMMUNITY_Community 1960|Community 1960]]
- [[_COMMUNITY_Community 1961|Community 1961]]
- [[_COMMUNITY_Community 1962|Community 1962]]
- [[_COMMUNITY_Community 1963|Community 1963]]
- [[_COMMUNITY_Community 1964|Community 1964]]
- [[_COMMUNITY_Community 1965|Community 1965]]
- [[_COMMUNITY_Community 1966|Community 1966]]
- [[_COMMUNITY_Community 1967|Community 1967]]
- [[_COMMUNITY_Community 1968|Community 1968]]
- [[_COMMUNITY_Community 1969|Community 1969]]
- [[_COMMUNITY_Community 1970|Community 1970]]
- [[_COMMUNITY_Community 1971|Community 1971]]
- [[_COMMUNITY_Community 1972|Community 1972]]
- [[_COMMUNITY_Community 1973|Community 1973]]
- [[_COMMUNITY_Community 1974|Community 1974]]
- [[_COMMUNITY_Community 1975|Community 1975]]
- [[_COMMUNITY_Community 1976|Community 1976]]
- [[_COMMUNITY_Community 1977|Community 1977]]
- [[_COMMUNITY_Community 1978|Community 1978]]
- [[_COMMUNITY_Community 1979|Community 1979]]
- [[_COMMUNITY_Community 1980|Community 1980]]
- [[_COMMUNITY_Community 1981|Community 1981]]
- [[_COMMUNITY_Community 1982|Community 1982]]
- [[_COMMUNITY_Community 1983|Community 1983]]
- [[_COMMUNITY_Community 1984|Community 1984]]
- [[_COMMUNITY_Community 1985|Community 1985]]
- [[_COMMUNITY_Community 1986|Community 1986]]
- [[_COMMUNITY_Community 1987|Community 1987]]
- [[_COMMUNITY_Community 1988|Community 1988]]
- [[_COMMUNITY_Community 1989|Community 1989]]
- [[_COMMUNITY_Community 1990|Community 1990]]
- [[_COMMUNITY_Community 1991|Community 1991]]
- [[_COMMUNITY_Community 1992|Community 1992]]
- [[_COMMUNITY_Community 1993|Community 1993]]
- [[_COMMUNITY_Community 1994|Community 1994]]
- [[_COMMUNITY_Community 1995|Community 1995]]
- [[_COMMUNITY_Community 1996|Community 1996]]
- [[_COMMUNITY_Community 1997|Community 1997]]
- [[_COMMUNITY_Community 1998|Community 1998]]
- [[_COMMUNITY_Community 1999|Community 1999]]
- [[_COMMUNITY_Community 2000|Community 2000]]
- [[_COMMUNITY_Community 2001|Community 2001]]
- [[_COMMUNITY_Community 2002|Community 2002]]
- [[_COMMUNITY_Community 2003|Community 2003]]
- [[_COMMUNITY_Community 2004|Community 2004]]
- [[_COMMUNITY_Community 2005|Community 2005]]
- [[_COMMUNITY_Community 2006|Community 2006]]
- [[_COMMUNITY_Community 2007|Community 2007]]
- [[_COMMUNITY_Community 2008|Community 2008]]
- [[_COMMUNITY_Community 2009|Community 2009]]
- [[_COMMUNITY_Community 2010|Community 2010]]
- [[_COMMUNITY_Community 2011|Community 2011]]
- [[_COMMUNITY_Community 2012|Community 2012]]
- [[_COMMUNITY_Community 2013|Community 2013]]
- [[_COMMUNITY_Community 2014|Community 2014]]
- [[_COMMUNITY_Community 2015|Community 2015]]
- [[_COMMUNITY_Community 2016|Community 2016]]
- [[_COMMUNITY_Community 2017|Community 2017]]
- [[_COMMUNITY_Community 2018|Community 2018]]
- [[_COMMUNITY_Community 2019|Community 2019]]
- [[_COMMUNITY_Community 2020|Community 2020]]
- [[_COMMUNITY_Community 2021|Community 2021]]
- [[_COMMUNITY_Community 2022|Community 2022]]
- [[_COMMUNITY_Community 2023|Community 2023]]
- [[_COMMUNITY_Community 2024|Community 2024]]
- [[_COMMUNITY_Community 2025|Community 2025]]
- [[_COMMUNITY_Community 2026|Community 2026]]
- [[_COMMUNITY_Community 2027|Community 2027]]
- [[_COMMUNITY_Community 2028|Community 2028]]
- [[_COMMUNITY_Community 2029|Community 2029]]
- [[_COMMUNITY_Community 2030|Community 2030]]
- [[_COMMUNITY_Community 2031|Community 2031]]
- [[_COMMUNITY_Community 2032|Community 2032]]
- [[_COMMUNITY_Community 2033|Community 2033]]
- [[_COMMUNITY_Community 2034|Community 2034]]
- [[_COMMUNITY_Community 2035|Community 2035]]
- [[_COMMUNITY_Community 2036|Community 2036]]
- [[_COMMUNITY_Community 2037|Community 2037]]
- [[_COMMUNITY_Community 2038|Community 2038]]
- [[_COMMUNITY_Community 2039|Community 2039]]
- [[_COMMUNITY_Community 2040|Community 2040]]
- [[_COMMUNITY_Community 2041|Community 2041]]
- [[_COMMUNITY_Community 2042|Community 2042]]
- [[_COMMUNITY_Community 2043|Community 2043]]
- [[_COMMUNITY_Community 2044|Community 2044]]
- [[_COMMUNITY_Community 2045|Community 2045]]
- [[_COMMUNITY_Community 2046|Community 2046]]
- [[_COMMUNITY_Community 2047|Community 2047]]
- [[_COMMUNITY_Community 2048|Community 2048]]
- [[_COMMUNITY_Community 2049|Community 2049]]
- [[_COMMUNITY_Community 2050|Community 2050]]
- [[_COMMUNITY_Community 2051|Community 2051]]
- [[_COMMUNITY_Community 2052|Community 2052]]
- [[_COMMUNITY_Community 2053|Community 2053]]
- [[_COMMUNITY_Community 2054|Community 2054]]
- [[_COMMUNITY_Community 2055|Community 2055]]
- [[_COMMUNITY_Community 2056|Community 2056]]
- [[_COMMUNITY_Community 2057|Community 2057]]
- [[_COMMUNITY_Community 2058|Community 2058]]
- [[_COMMUNITY_Community 2059|Community 2059]]
- [[_COMMUNITY_Community 2060|Community 2060]]
- [[_COMMUNITY_Community 2061|Community 2061]]
- [[_COMMUNITY_Community 2062|Community 2062]]
- [[_COMMUNITY_Community 2063|Community 2063]]
- [[_COMMUNITY_Community 2064|Community 2064]]
- [[_COMMUNITY_Community 2065|Community 2065]]
- [[_COMMUNITY_Community 2066|Community 2066]]
- [[_COMMUNITY_Community 2067|Community 2067]]
- [[_COMMUNITY_Community 2068|Community 2068]]
- [[_COMMUNITY_Community 2069|Community 2069]]
- [[_COMMUNITY_Community 2070|Community 2070]]
- [[_COMMUNITY_Community 2071|Community 2071]]
- [[_COMMUNITY_Community 2072|Community 2072]]
- [[_COMMUNITY_Community 2073|Community 2073]]
- [[_COMMUNITY_Community 2074|Community 2074]]
- [[_COMMUNITY_Community 2075|Community 2075]]
- [[_COMMUNITY_Community 2076|Community 2076]]
- [[_COMMUNITY_Community 2077|Community 2077]]
- [[_COMMUNITY_Community 2078|Community 2078]]
- [[_COMMUNITY_Community 2079|Community 2079]]
- [[_COMMUNITY_Community 2080|Community 2080]]
- [[_COMMUNITY_Community 2081|Community 2081]]
- [[_COMMUNITY_Community 2082|Community 2082]]
- [[_COMMUNITY_Community 2083|Community 2083]]
- [[_COMMUNITY_Community 2084|Community 2084]]
- [[_COMMUNITY_Community 2085|Community 2085]]
- [[_COMMUNITY_Community 2086|Community 2086]]
- [[_COMMUNITY_Community 2087|Community 2087]]
- [[_COMMUNITY_Community 2088|Community 2088]]
- [[_COMMUNITY_Community 2089|Community 2089]]
- [[_COMMUNITY_Community 2090|Community 2090]]
- [[_COMMUNITY_Community 2091|Community 2091]]
- [[_COMMUNITY_Community 2092|Community 2092]]
- [[_COMMUNITY_Community 2093|Community 2093]]
- [[_COMMUNITY_Community 2094|Community 2094]]
- [[_COMMUNITY_Community 2095|Community 2095]]
- [[_COMMUNITY_Community 2096|Community 2096]]
- [[_COMMUNITY_Community 2097|Community 2097]]
- [[_COMMUNITY_Community 2098|Community 2098]]
- [[_COMMUNITY_Community 2099|Community 2099]]
- [[_COMMUNITY_Community 2100|Community 2100]]
- [[_COMMUNITY_Community 2101|Community 2101]]
- [[_COMMUNITY_Community 2102|Community 2102]]
- [[_COMMUNITY_Community 2103|Community 2103]]
- [[_COMMUNITY_Community 2104|Community 2104]]
- [[_COMMUNITY_Community 2105|Community 2105]]
- [[_COMMUNITY_Community 2106|Community 2106]]
- [[_COMMUNITY_Community 2107|Community 2107]]
- [[_COMMUNITY_Community 2108|Community 2108]]
- [[_COMMUNITY_Community 2109|Community 2109]]
- [[_COMMUNITY_Community 2110|Community 2110]]
- [[_COMMUNITY_Community 2111|Community 2111]]
- [[_COMMUNITY_Community 2112|Community 2112]]
- [[_COMMUNITY_Community 2113|Community 2113]]
- [[_COMMUNITY_Community 2114|Community 2114]]
- [[_COMMUNITY_Community 2115|Community 2115]]
- [[_COMMUNITY_Community 2116|Community 2116]]
- [[_COMMUNITY_Community 2117|Community 2117]]
- [[_COMMUNITY_Community 2118|Community 2118]]
- [[_COMMUNITY_Community 2119|Community 2119]]
- [[_COMMUNITY_Community 2120|Community 2120]]
- [[_COMMUNITY_Community 2121|Community 2121]]
- [[_COMMUNITY_Community 2122|Community 2122]]
- [[_COMMUNITY_Community 2123|Community 2123]]
- [[_COMMUNITY_Community 2124|Community 2124]]
- [[_COMMUNITY_Community 2125|Community 2125]]
- [[_COMMUNITY_Community 2126|Community 2126]]
- [[_COMMUNITY_Community 2127|Community 2127]]
- [[_COMMUNITY_Community 2128|Community 2128]]
- [[_COMMUNITY_Community 2129|Community 2129]]
- [[_COMMUNITY_Community 2130|Community 2130]]
- [[_COMMUNITY_Community 2131|Community 2131]]
- [[_COMMUNITY_Community 2132|Community 2132]]
- [[_COMMUNITY_Community 2133|Community 2133]]
- [[_COMMUNITY_Community 2134|Community 2134]]
- [[_COMMUNITY_Community 2135|Community 2135]]
- [[_COMMUNITY_Community 2136|Community 2136]]
- [[_COMMUNITY_Community 2137|Community 2137]]
- [[_COMMUNITY_Community 2139|Community 2139]]
- [[_COMMUNITY_Community 2140|Community 2140]]
- [[_COMMUNITY_Community 2141|Community 2141]]
- [[_COMMUNITY_Community 2142|Community 2142]]
- [[_COMMUNITY_Community 2143|Community 2143]]
- [[_COMMUNITY_Community 2144|Community 2144]]
- [[_COMMUNITY_Community 2146|Community 2146]]
- [[_COMMUNITY_Community 2147|Community 2147]]
- [[_COMMUNITY_Community 2148|Community 2148]]
- [[_COMMUNITY_Community 2149|Community 2149]]
- [[_COMMUNITY_Community 2150|Community 2150]]
- [[_COMMUNITY_Community 2151|Community 2151]]
- [[_COMMUNITY_Community 2152|Community 2152]]
- [[_COMMUNITY_Community 2153|Community 2153]]
- [[_COMMUNITY_Community 2154|Community 2154]]
- [[_COMMUNITY_Community 2155|Community 2155]]
- [[_COMMUNITY_Community 2156|Community 2156]]
- [[_COMMUNITY_Community 2157|Community 2157]]
- [[_COMMUNITY_Community 2158|Community 2158]]
- [[_COMMUNITY_Community 2159|Community 2159]]
- [[_COMMUNITY_Community 2160|Community 2160]]
- [[_COMMUNITY_Community 2161|Community 2161]]
- [[_COMMUNITY_Community 2162|Community 2162]]
- [[_COMMUNITY_Community 2163|Community 2163]]
- [[_COMMUNITY_Community 2164|Community 2164]]
- [[_COMMUNITY_Community 2165|Community 2165]]
- [[_COMMUNITY_Community 2166|Community 2166]]
- [[_COMMUNITY_Community 2167|Community 2167]]
- [[_COMMUNITY_Community 2168|Community 2168]]
- [[_COMMUNITY_Community 2169|Community 2169]]
- [[_COMMUNITY_Community 2170|Community 2170]]
- [[_COMMUNITY_Community 2171|Community 2171]]
- [[_COMMUNITY_Community 2172|Community 2172]]
- [[_COMMUNITY_Community 2173|Community 2173]]
- [[_COMMUNITY_Community 2174|Community 2174]]
- [[_COMMUNITY_Community 2175|Community 2175]]
- [[_COMMUNITY_Community 2176|Community 2176]]
- [[_COMMUNITY_Community 2177|Community 2177]]
- [[_COMMUNITY_Community 2178|Community 2178]]
- [[_COMMUNITY_Community 2179|Community 2179]]
- [[_COMMUNITY_Community 2180|Community 2180]]
- [[_COMMUNITY_Community 2181|Community 2181]]
- [[_COMMUNITY_Community 2182|Community 2182]]
- [[_COMMUNITY_Community 2183|Community 2183]]
- [[_COMMUNITY_Community 2184|Community 2184]]
- [[_COMMUNITY_Community 2185|Community 2185]]
- [[_COMMUNITY_Community 2186|Community 2186]]
- [[_COMMUNITY_Community 2187|Community 2187]]
- [[_COMMUNITY_Community 2188|Community 2188]]
- [[_COMMUNITY_Community 2189|Community 2189]]
- [[_COMMUNITY_Community 2190|Community 2190]]
- [[_COMMUNITY_Community 2191|Community 2191]]
- [[_COMMUNITY_Community 2192|Community 2192]]
- [[_COMMUNITY_Community 2193|Community 2193]]
- [[_COMMUNITY_Community 2194|Community 2194]]
- [[_COMMUNITY_Community 2195|Community 2195]]
- [[_COMMUNITY_Community 2196|Community 2196]]
- [[_COMMUNITY_Community 2197|Community 2197]]
- [[_COMMUNITY_Community 2198|Community 2198]]
- [[_COMMUNITY_Community 2199|Community 2199]]
- [[_COMMUNITY_Community 2200|Community 2200]]
- [[_COMMUNITY_Community 2201|Community 2201]]
- [[_COMMUNITY_Community 2202|Community 2202]]
- [[_COMMUNITY_Community 2203|Community 2203]]
- [[_COMMUNITY_Community 2204|Community 2204]]
- [[_COMMUNITY_Community 2205|Community 2205]]
- [[_COMMUNITY_Community 2206|Community 2206]]
- [[_COMMUNITY_Community 2207|Community 2207]]
- [[_COMMUNITY_Community 2208|Community 2208]]
- [[_COMMUNITY_Community 2209|Community 2209]]
- [[_COMMUNITY_Community 2210|Community 2210]]
- [[_COMMUNITY_Community 2211|Community 2211]]
- [[_COMMUNITY_Community 2212|Community 2212]]
- [[_COMMUNITY_Community 2213|Community 2213]]
- [[_COMMUNITY_Community 2214|Community 2214]]
- [[_COMMUNITY_Community 2215|Community 2215]]
- [[_COMMUNITY_Community 2216|Community 2216]]
- [[_COMMUNITY_Community 2217|Community 2217]]
- [[_COMMUNITY_Community 2218|Community 2218]]
- [[_COMMUNITY_Community 2219|Community 2219]]
- [[_COMMUNITY_Community 2220|Community 2220]]
- [[_COMMUNITY_Community 2221|Community 2221]]
- [[_COMMUNITY_Community 2222|Community 2222]]
- [[_COMMUNITY_Community 2223|Community 2223]]
- [[_COMMUNITY_Community 2224|Community 2224]]
- [[_COMMUNITY_Community 2225|Community 2225]]
- [[_COMMUNITY_Community 2226|Community 2226]]
- [[_COMMUNITY_Community 2227|Community 2227]]
- [[_COMMUNITY_Community 2228|Community 2228]]
- [[_COMMUNITY_Community 2229|Community 2229]]
- [[_COMMUNITY_Community 2230|Community 2230]]
- [[_COMMUNITY_Community 2231|Community 2231]]
- [[_COMMUNITY_Community 2232|Community 2232]]
- [[_COMMUNITY_Community 2233|Community 2233]]
- [[_COMMUNITY_Community 2234|Community 2234]]
- [[_COMMUNITY_Community 2235|Community 2235]]
- [[_COMMUNITY_Community 2236|Community 2236]]
- [[_COMMUNITY_Community 2237|Community 2237]]
- [[_COMMUNITY_Community 2238|Community 2238]]
- [[_COMMUNITY_Community 2239|Community 2239]]
- [[_COMMUNITY_Community 2240|Community 2240]]
- [[_COMMUNITY_Community 2241|Community 2241]]
- [[_COMMUNITY_Community 2242|Community 2242]]
- [[_COMMUNITY_Community 2243|Community 2243]]
- [[_COMMUNITY_Community 2244|Community 2244]]
- [[_COMMUNITY_Community 2245|Community 2245]]
- [[_COMMUNITY_Community 2246|Community 2246]]
- [[_COMMUNITY_Community 2247|Community 2247]]
- [[_COMMUNITY_Community 2248|Community 2248]]
- [[_COMMUNITY_Community 2249|Community 2249]]
- [[_COMMUNITY_Community 2250|Community 2250]]
- [[_COMMUNITY_Community 2251|Community 2251]]
- [[_COMMUNITY_Community 2252|Community 2252]]
- [[_COMMUNITY_Community 2253|Community 2253]]
- [[_COMMUNITY_Community 2254|Community 2254]]
- [[_COMMUNITY_Community 2255|Community 2255]]
- [[_COMMUNITY_Community 2256|Community 2256]]
- [[_COMMUNITY_Community 2257|Community 2257]]
- [[_COMMUNITY_Community 2258|Community 2258]]
- [[_COMMUNITY_Community 2259|Community 2259]]
- [[_COMMUNITY_Community 2260|Community 2260]]
- [[_COMMUNITY_Community 2261|Community 2261]]
- [[_COMMUNITY_Community 2262|Community 2262]]
- [[_COMMUNITY_Community 2263|Community 2263]]
- [[_COMMUNITY_Community 2264|Community 2264]]
- [[_COMMUNITY_Community 2265|Community 2265]]
- [[_COMMUNITY_Community 2266|Community 2266]]
- [[_COMMUNITY_Community 2267|Community 2267]]
- [[_COMMUNITY_Community 2268|Community 2268]]
- [[_COMMUNITY_Community 2269|Community 2269]]
- [[_COMMUNITY_Community 2270|Community 2270]]
- [[_COMMUNITY_Community 2271|Community 2271]]
- [[_COMMUNITY_Community 2272|Community 2272]]
- [[_COMMUNITY_Community 2273|Community 2273]]
- [[_COMMUNITY_Community 2274|Community 2274]]
- [[_COMMUNITY_Community 2275|Community 2275]]
- [[_COMMUNITY_Community 2276|Community 2276]]
- [[_COMMUNITY_Community 2277|Community 2277]]
- [[_COMMUNITY_Community 2278|Community 2278]]
- [[_COMMUNITY_Community 2279|Community 2279]]
- [[_COMMUNITY_Community 2280|Community 2280]]
- [[_COMMUNITY_Community 2281|Community 2281]]
- [[_COMMUNITY_Community 2282|Community 2282]]
- [[_COMMUNITY_Community 2283|Community 2283]]
- [[_COMMUNITY_Community 2284|Community 2284]]
- [[_COMMUNITY_Community 2285|Community 2285]]
- [[_COMMUNITY_Community 2286|Community 2286]]
- [[_COMMUNITY_Community 2287|Community 2287]]
- [[_COMMUNITY_Community 2288|Community 2288]]
- [[_COMMUNITY_Community 2289|Community 2289]]
- [[_COMMUNITY_Community 2290|Community 2290]]
- [[_COMMUNITY_Community 2291|Community 2291]]
- [[_COMMUNITY_Community 2292|Community 2292]]
- [[_COMMUNITY_Community 2293|Community 2293]]
- [[_COMMUNITY_Community 2294|Community 2294]]
- [[_COMMUNITY_Community 2295|Community 2295]]
- [[_COMMUNITY_Community 2296|Community 2296]]
- [[_COMMUNITY_Community 2297|Community 2297]]
- [[_COMMUNITY_Community 2298|Community 2298]]
- [[_COMMUNITY_Community 2299|Community 2299]]
- [[_COMMUNITY_Community 2300|Community 2300]]
- [[_COMMUNITY_Community 2301|Community 2301]]
- [[_COMMUNITY_Community 2302|Community 2302]]
- [[_COMMUNITY_Community 2303|Community 2303]]
- [[_COMMUNITY_Community 2304|Community 2304]]
- [[_COMMUNITY_Community 2305|Community 2305]]
- [[_COMMUNITY_Community 2306|Community 2306]]
- [[_COMMUNITY_Community 2307|Community 2307]]
- [[_COMMUNITY_Community 2308|Community 2308]]
- [[_COMMUNITY_Community 2309|Community 2309]]
- [[_COMMUNITY_Community 2310|Community 2310]]
- [[_COMMUNITY_Community 2311|Community 2311]]
- [[_COMMUNITY_Community 2312|Community 2312]]
- [[_COMMUNITY_Community 2313|Community 2313]]
- [[_COMMUNITY_Community 2314|Community 2314]]
- [[_COMMUNITY_Community 2315|Community 2315]]
- [[_COMMUNITY_Community 2316|Community 2316]]
- [[_COMMUNITY_Community 2317|Community 2317]]
- [[_COMMUNITY_Community 2318|Community 2318]]
- [[_COMMUNITY_Community 2319|Community 2319]]
- [[_COMMUNITY_Community 2320|Community 2320]]
- [[_COMMUNITY_Community 2321|Community 2321]]
- [[_COMMUNITY_Community 2322|Community 2322]]
- [[_COMMUNITY_Community 2323|Community 2323]]
- [[_COMMUNITY_Community 2324|Community 2324]]
- [[_COMMUNITY_Community 2325|Community 2325]]
- [[_COMMUNITY_Community 2326|Community 2326]]
- [[_COMMUNITY_Community 2327|Community 2327]]
- [[_COMMUNITY_Community 2328|Community 2328]]
- [[_COMMUNITY_Community 2329|Community 2329]]
- [[_COMMUNITY_Community 2330|Community 2330]]
- [[_COMMUNITY_Community 2331|Community 2331]]
- [[_COMMUNITY_Community 2332|Community 2332]]
- [[_COMMUNITY_Community 2333|Community 2333]]
- [[_COMMUNITY_Community 2334|Community 2334]]
- [[_COMMUNITY_Community 2335|Community 2335]]
- [[_COMMUNITY_Community 2336|Community 2336]]
- [[_COMMUNITY_Community 2337|Community 2337]]
- [[_COMMUNITY_Community 2338|Community 2338]]
- [[_COMMUNITY_Community 2339|Community 2339]]
- [[_COMMUNITY_Community 2340|Community 2340]]
- [[_COMMUNITY_Community 2341|Community 2341]]
- [[_COMMUNITY_Community 2342|Community 2342]]
- [[_COMMUNITY_Community 2343|Community 2343]]
- [[_COMMUNITY_Community 2344|Community 2344]]
- [[_COMMUNITY_Community 2345|Community 2345]]
- [[_COMMUNITY_Community 2346|Community 2346]]
- [[_COMMUNITY_Community 2347|Community 2347]]
- [[_COMMUNITY_Community 2348|Community 2348]]
- [[_COMMUNITY_Community 2349|Community 2349]]
- [[_COMMUNITY_Community 2350|Community 2350]]
- [[_COMMUNITY_Community 2351|Community 2351]]
- [[_COMMUNITY_Community 2352|Community 2352]]
- [[_COMMUNITY_Community 2353|Community 2353]]
- [[_COMMUNITY_Community 2354|Community 2354]]
- [[_COMMUNITY_Community 2355|Community 2355]]
- [[_COMMUNITY_Community 2356|Community 2356]]
- [[_COMMUNITY_Community 2357|Community 2357]]
- [[_COMMUNITY_Community 2358|Community 2358]]
- [[_COMMUNITY_Community 2359|Community 2359]]
- [[_COMMUNITY_Community 2360|Community 2360]]
- [[_COMMUNITY_Community 2361|Community 2361]]
- [[_COMMUNITY_Community 2362|Community 2362]]
- [[_COMMUNITY_Community 2363|Community 2363]]
- [[_COMMUNITY_Community 2364|Community 2364]]
- [[_COMMUNITY_Community 2365|Community 2365]]
- [[_COMMUNITY_Community 2366|Community 2366]]
- [[_COMMUNITY_Community 2367|Community 2367]]
- [[_COMMUNITY_Community 2368|Community 2368]]
- [[_COMMUNITY_Community 2369|Community 2369]]
- [[_COMMUNITY_Community 2370|Community 2370]]
- [[_COMMUNITY_Community 2371|Community 2371]]
- [[_COMMUNITY_Community 2372|Community 2372]]
- [[_COMMUNITY_Community 2373|Community 2373]]
- [[_COMMUNITY_Community 2374|Community 2374]]
- [[_COMMUNITY_Community 2375|Community 2375]]
- [[_COMMUNITY_Community 2376|Community 2376]]
- [[_COMMUNITY_Community 2377|Community 2377]]
- [[_COMMUNITY_Community 2378|Community 2378]]
- [[_COMMUNITY_Community 2379|Community 2379]]
- [[_COMMUNITY_Community 2380|Community 2380]]
- [[_COMMUNITY_Community 2381|Community 2381]]
- [[_COMMUNITY_Community 2382|Community 2382]]
- [[_COMMUNITY_Community 2383|Community 2383]]
- [[_COMMUNITY_Community 2384|Community 2384]]
- [[_COMMUNITY_Community 2385|Community 2385]]
- [[_COMMUNITY_Community 2386|Community 2386]]
- [[_COMMUNITY_Community 2387|Community 2387]]
- [[_COMMUNITY_Community 2388|Community 2388]]
- [[_COMMUNITY_Community 2389|Community 2389]]
- [[_COMMUNITY_Community 2390|Community 2390]]
- [[_COMMUNITY_Community 2391|Community 2391]]
- [[_COMMUNITY_Community 2392|Community 2392]]
- [[_COMMUNITY_Community 2393|Community 2393]]
- [[_COMMUNITY_Community 2394|Community 2394]]
- [[_COMMUNITY_Community 2395|Community 2395]]
- [[_COMMUNITY_Community 2396|Community 2396]]
- [[_COMMUNITY_Community 2397|Community 2397]]
- [[_COMMUNITY_Community 2398|Community 2398]]
- [[_COMMUNITY_Community 2399|Community 2399]]
- [[_COMMUNITY_Community 2400|Community 2400]]
- [[_COMMUNITY_Community 2401|Community 2401]]
- [[_COMMUNITY_Community 2402|Community 2402]]
- [[_COMMUNITY_Community 2403|Community 2403]]
- [[_COMMUNITY_Community 2404|Community 2404]]
- [[_COMMUNITY_Community 2405|Community 2405]]
- [[_COMMUNITY_Community 2406|Community 2406]]
- [[_COMMUNITY_Community 2407|Community 2407]]
- [[_COMMUNITY_Community 2408|Community 2408]]
- [[_COMMUNITY_Community 2409|Community 2409]]
- [[_COMMUNITY_Community 2410|Community 2410]]
- [[_COMMUNITY_Community 2411|Community 2411]]
- [[_COMMUNITY_Community 2412|Community 2412]]
- [[_COMMUNITY_Community 2413|Community 2413]]
- [[_COMMUNITY_Community 2414|Community 2414]]
- [[_COMMUNITY_Community 2415|Community 2415]]
- [[_COMMUNITY_Community 2416|Community 2416]]
- [[_COMMUNITY_Community 2417|Community 2417]]
- [[_COMMUNITY_Community 2418|Community 2418]]
- [[_COMMUNITY_Community 2419|Community 2419]]
- [[_COMMUNITY_Community 2420|Community 2420]]
- [[_COMMUNITY_Community 2421|Community 2421]]
- [[_COMMUNITY_Community 2422|Community 2422]]
- [[_COMMUNITY_Community 2423|Community 2423]]
- [[_COMMUNITY_Community 2424|Community 2424]]
- [[_COMMUNITY_Community 2425|Community 2425]]
- [[_COMMUNITY_Community 2426|Community 2426]]
- [[_COMMUNITY_Community 2427|Community 2427]]
- [[_COMMUNITY_Community 2428|Community 2428]]
- [[_COMMUNITY_Community 2429|Community 2429]]
- [[_COMMUNITY_Community 2430|Community 2430]]
- [[_COMMUNITY_Community 2431|Community 2431]]
- [[_COMMUNITY_Community 2432|Community 2432]]
- [[_COMMUNITY_Community 2433|Community 2433]]
- [[_COMMUNITY_Community 2434|Community 2434]]
- [[_COMMUNITY_Community 2435|Community 2435]]
- [[_COMMUNITY_Community 2436|Community 2436]]
- [[_COMMUNITY_Community 2437|Community 2437]]
- [[_COMMUNITY_Community 2438|Community 2438]]
- [[_COMMUNITY_Community 2439|Community 2439]]
- [[_COMMUNITY_Community 2440|Community 2440]]
- [[_COMMUNITY_Community 2441|Community 2441]]
- [[_COMMUNITY_Community 2442|Community 2442]]
- [[_COMMUNITY_Community 2443|Community 2443]]
- [[_COMMUNITY_Community 2444|Community 2444]]
- [[_COMMUNITY_Community 2445|Community 2445]]
- [[_COMMUNITY_Community 2446|Community 2446]]
- [[_COMMUNITY_Community 2447|Community 2447]]
- [[_COMMUNITY_Community 2448|Community 2448]]
- [[_COMMUNITY_Community 2449|Community 2449]]
- [[_COMMUNITY_Community 2450|Community 2450]]
- [[_COMMUNITY_Community 2451|Community 2451]]
- [[_COMMUNITY_Community 2452|Community 2452]]
- [[_COMMUNITY_Community 2453|Community 2453]]
- [[_COMMUNITY_Community 2454|Community 2454]]
- [[_COMMUNITY_Community 2455|Community 2455]]
- [[_COMMUNITY_Community 2456|Community 2456]]
- [[_COMMUNITY_Community 2457|Community 2457]]
- [[_COMMUNITY_Community 2458|Community 2458]]
- [[_COMMUNITY_Community 2459|Community 2459]]
- [[_COMMUNITY_Community 2460|Community 2460]]
- [[_COMMUNITY_Community 2461|Community 2461]]
- [[_COMMUNITY_Community 2462|Community 2462]]
- [[_COMMUNITY_Community 2463|Community 2463]]
- [[_COMMUNITY_Community 2464|Community 2464]]
- [[_COMMUNITY_Community 2465|Community 2465]]
- [[_COMMUNITY_Community 2466|Community 2466]]
- [[_COMMUNITY_Community 2467|Community 2467]]
- [[_COMMUNITY_Community 2468|Community 2468]]
- [[_COMMUNITY_Community 2469|Community 2469]]
- [[_COMMUNITY_Community 2470|Community 2470]]
- [[_COMMUNITY_Community 2471|Community 2471]]
- [[_COMMUNITY_Community 2472|Community 2472]]
- [[_COMMUNITY_Community 2473|Community 2473]]
- [[_COMMUNITY_Community 2474|Community 2474]]
- [[_COMMUNITY_Community 2475|Community 2475]]
- [[_COMMUNITY_Community 2476|Community 2476]]
- [[_COMMUNITY_Community 2477|Community 2477]]
- [[_COMMUNITY_Community 2478|Community 2478]]
- [[_COMMUNITY_Community 2480|Community 2480]]
- [[_COMMUNITY_Community 2481|Community 2481]]
- [[_COMMUNITY_Community 2482|Community 2482]]
- [[_COMMUNITY_Community 2486|Community 2486]]
- [[_COMMUNITY_Community 2487|Community 2487]]
- [[_COMMUNITY_Community 2488|Community 2488]]
- [[_COMMUNITY_Community 2490|Community 2490]]
- [[_COMMUNITY_Community 2491|Community 2491]]
- [[_COMMUNITY_Community 2492|Community 2492]]
- [[_COMMUNITY_Community 2493|Community 2493]]
- [[_COMMUNITY_Community 2494|Community 2494]]
- [[_COMMUNITY_Community 2495|Community 2495]]
- [[_COMMUNITY_Community 2496|Community 2496]]
- [[_COMMUNITY_Community 2497|Community 2497]]
- [[_COMMUNITY_Community 2498|Community 2498]]
- [[_COMMUNITY_Community 2499|Community 2499]]
- [[_COMMUNITY_Community 2500|Community 2500]]
- [[_COMMUNITY_Community 2501|Community 2501]]
- [[_COMMUNITY_Community 2502|Community 2502]]
- [[_COMMUNITY_Community 2503|Community 2503]]
- [[_COMMUNITY_Community 2504|Community 2504]]
- [[_COMMUNITY_Community 2505|Community 2505]]
- [[_COMMUNITY_Community 2506|Community 2506]]
- [[_COMMUNITY_Community 2507|Community 2507]]
- [[_COMMUNITY_Community 2508|Community 2508]]
- [[_COMMUNITY_Community 2509|Community 2509]]
- [[_COMMUNITY_Community 2510|Community 2510]]
- [[_COMMUNITY_Community 2511|Community 2511]]
- [[_COMMUNITY_Community 2512|Community 2512]]
- [[_COMMUNITY_Community 2513|Community 2513]]
- [[_COMMUNITY_Community 2514|Community 2514]]
- [[_COMMUNITY_Community 2515|Community 2515]]
- [[_COMMUNITY_Community 2516|Community 2516]]
- [[_COMMUNITY_Community 2517|Community 2517]]
- [[_COMMUNITY_Community 2518|Community 2518]]
- [[_COMMUNITY_Community 2519|Community 2519]]
- [[_COMMUNITY_Community 2520|Community 2520]]
- [[_COMMUNITY_Community 2521|Community 2521]]
- [[_COMMUNITY_Community 2522|Community 2522]]
- [[_COMMUNITY_Community 2523|Community 2523]]
- [[_COMMUNITY_Community 2524|Community 2524]]
- [[_COMMUNITY_Community 2525|Community 2525]]
- [[_COMMUNITY_Community 2526|Community 2526]]
- [[_COMMUNITY_Community 2527|Community 2527]]
- [[_COMMUNITY_Community 2528|Community 2528]]
- [[_COMMUNITY_Community 2529|Community 2529]]
- [[_COMMUNITY_Community 2530|Community 2530]]
- [[_COMMUNITY_Community 2531|Community 2531]]
- [[_COMMUNITY_Community 2532|Community 2532]]
- [[_COMMUNITY_Community 2533|Community 2533]]
- [[_COMMUNITY_Community 2534|Community 2534]]
- [[_COMMUNITY_Community 2535|Community 2535]]
- [[_COMMUNITY_Community 2536|Community 2536]]
- [[_COMMUNITY_Community 2537|Community 2537]]
- [[_COMMUNITY_Community 2538|Community 2538]]
- [[_COMMUNITY_Community 2539|Community 2539]]
- [[_COMMUNITY_Community 2540|Community 2540]]
- [[_COMMUNITY_Community 2541|Community 2541]]
- [[_COMMUNITY_Community 2542|Community 2542]]
- [[_COMMUNITY_Community 2543|Community 2543]]
- [[_COMMUNITY_Community 2544|Community 2544]]
- [[_COMMUNITY_Community 2545|Community 2545]]
- [[_COMMUNITY_Community 2546|Community 2546]]
- [[_COMMUNITY_Community 2547|Community 2547]]
- [[_COMMUNITY_Community 2548|Community 2548]]
- [[_COMMUNITY_Community 2549|Community 2549]]
- [[_COMMUNITY_Community 2550|Community 2550]]
- [[_COMMUNITY_Community 2551|Community 2551]]
- [[_COMMUNITY_Community 2552|Community 2552]]
- [[_COMMUNITY_Community 2553|Community 2553]]
- [[_COMMUNITY_Community 2554|Community 2554]]
- [[_COMMUNITY_Community 2555|Community 2555]]
- [[_COMMUNITY_Community 2556|Community 2556]]
- [[_COMMUNITY_Community 2557|Community 2557]]
- [[_COMMUNITY_Community 2558|Community 2558]]
- [[_COMMUNITY_Community 2559|Community 2559]]
- [[_COMMUNITY_Community 2560|Community 2560]]
- [[_COMMUNITY_Community 2561|Community 2561]]
- [[_COMMUNITY_Community 2562|Community 2562]]
- [[_COMMUNITY_Community 2563|Community 2563]]
- [[_COMMUNITY_Community 2564|Community 2564]]
- [[_COMMUNITY_Community 2565|Community 2565]]
- [[_COMMUNITY_Community 2566|Community 2566]]
- [[_COMMUNITY_Community 2567|Community 2567]]
- [[_COMMUNITY_Community 2568|Community 2568]]
- [[_COMMUNITY_Community 2569|Community 2569]]
- [[_COMMUNITY_Community 2570|Community 2570]]
- [[_COMMUNITY_Community 2571|Community 2571]]
- [[_COMMUNITY_Community 2572|Community 2572]]
- [[_COMMUNITY_Community 2573|Community 2573]]
- [[_COMMUNITY_Community 2574|Community 2574]]
- [[_COMMUNITY_Community 2575|Community 2575]]
- [[_COMMUNITY_Community 2576|Community 2576]]
- [[_COMMUNITY_Community 2577|Community 2577]]
- [[_COMMUNITY_Community 2578|Community 2578]]
- [[_COMMUNITY_Community 2579|Community 2579]]
- [[_COMMUNITY_Community 2580|Community 2580]]
- [[_COMMUNITY_Community 2581|Community 2581]]
- [[_COMMUNITY_Community 2582|Community 2582]]
- [[_COMMUNITY_Community 2583|Community 2583]]
- [[_COMMUNITY_Community 2584|Community 2584]]
- [[_COMMUNITY_Community 2585|Community 2585]]
- [[_COMMUNITY_Community 2586|Community 2586]]
- [[_COMMUNITY_Community 2587|Community 2587]]
- [[_COMMUNITY_Community 2588|Community 2588]]
- [[_COMMUNITY_Community 2589|Community 2589]]
- [[_COMMUNITY_Community 2590|Community 2590]]
- [[_COMMUNITY_Community 2591|Community 2591]]
- [[_COMMUNITY_Community 2592|Community 2592]]
- [[_COMMUNITY_Community 2593|Community 2593]]
- [[_COMMUNITY_Community 2594|Community 2594]]
- [[_COMMUNITY_Community 2595|Community 2595]]
- [[_COMMUNITY_Community 2596|Community 2596]]
- [[_COMMUNITY_Community 2597|Community 2597]]
- [[_COMMUNITY_Community 2598|Community 2598]]
- [[_COMMUNITY_Community 2599|Community 2599]]
- [[_COMMUNITY_Community 2600|Community 2600]]
- [[_COMMUNITY_Community 2601|Community 2601]]
- [[_COMMUNITY_Community 2602|Community 2602]]
- [[_COMMUNITY_Community 2603|Community 2603]]
- [[_COMMUNITY_Community 2604|Community 2604]]
- [[_COMMUNITY_Community 2605|Community 2605]]
- [[_COMMUNITY_Community 2606|Community 2606]]
- [[_COMMUNITY_Community 2607|Community 2607]]
- [[_COMMUNITY_Community 2608|Community 2608]]
- [[_COMMUNITY_Community 2609|Community 2609]]
- [[_COMMUNITY_Community 2610|Community 2610]]
- [[_COMMUNITY_Community 2611|Community 2611]]
- [[_COMMUNITY_Community 2612|Community 2612]]
- [[_COMMUNITY_Community 2613|Community 2613]]
- [[_COMMUNITY_Community 2614|Community 2614]]
- [[_COMMUNITY_Community 2615|Community 2615]]
- [[_COMMUNITY_Community 2616|Community 2616]]
- [[_COMMUNITY_Community 2617|Community 2617]]
- [[_COMMUNITY_Community 2618|Community 2618]]
- [[_COMMUNITY_Community 2619|Community 2619]]
- [[_COMMUNITY_Community 2620|Community 2620]]
- [[_COMMUNITY_Community 2621|Community 2621]]
- [[_COMMUNITY_Community 2622|Community 2622]]
- [[_COMMUNITY_Community 2623|Community 2623]]
- [[_COMMUNITY_Community 2624|Community 2624]]
- [[_COMMUNITY_Community 2625|Community 2625]]
- [[_COMMUNITY_Community 2626|Community 2626]]
- [[_COMMUNITY_Community 2627|Community 2627]]
- [[_COMMUNITY_Community 2628|Community 2628]]
- [[_COMMUNITY_Community 2629|Community 2629]]
- [[_COMMUNITY_Community 2630|Community 2630]]
- [[_COMMUNITY_Community 2631|Community 2631]]
- [[_COMMUNITY_Community 2632|Community 2632]]
- [[_COMMUNITY_Community 2633|Community 2633]]
- [[_COMMUNITY_Community 2634|Community 2634]]
- [[_COMMUNITY_Community 2635|Community 2635]]
- [[_COMMUNITY_Community 2636|Community 2636]]
- [[_COMMUNITY_Community 2637|Community 2637]]
- [[_COMMUNITY_Community 2638|Community 2638]]
- [[_COMMUNITY_Community 2639|Community 2639]]
- [[_COMMUNITY_Community 2640|Community 2640]]
- [[_COMMUNITY_Community 2641|Community 2641]]
- [[_COMMUNITY_Community 2642|Community 2642]]
- [[_COMMUNITY_Community 2643|Community 2643]]
- [[_COMMUNITY_Community 2644|Community 2644]]
- [[_COMMUNITY_Community 2645|Community 2645]]
- [[_COMMUNITY_Community 2646|Community 2646]]
- [[_COMMUNITY_Community 2647|Community 2647]]
- [[_COMMUNITY_Community 2648|Community 2648]]
- [[_COMMUNITY_Community 2649|Community 2649]]
- [[_COMMUNITY_Community 2650|Community 2650]]
- [[_COMMUNITY_Community 2651|Community 2651]]
- [[_COMMUNITY_Community 2652|Community 2652]]
- [[_COMMUNITY_Community 2653|Community 2653]]
- [[_COMMUNITY_Community 2654|Community 2654]]
- [[_COMMUNITY_Community 2655|Community 2655]]
- [[_COMMUNITY_Community 2656|Community 2656]]
- [[_COMMUNITY_Community 2657|Community 2657]]
- [[_COMMUNITY_Community 2658|Community 2658]]
- [[_COMMUNITY_Community 2659|Community 2659]]
- [[_COMMUNITY_Community 2660|Community 2660]]
- [[_COMMUNITY_Community 2661|Community 2661]]
- [[_COMMUNITY_Community 2662|Community 2662]]
- [[_COMMUNITY_Community 2663|Community 2663]]
- [[_COMMUNITY_Community 2664|Community 2664]]
- [[_COMMUNITY_Community 2665|Community 2665]]
- [[_COMMUNITY_Community 2666|Community 2666]]
- [[_COMMUNITY_Community 2667|Community 2667]]
- [[_COMMUNITY_Community 2668|Community 2668]]
- [[_COMMUNITY_Community 2669|Community 2669]]
- [[_COMMUNITY_Community 2670|Community 2670]]
- [[_COMMUNITY_Community 2671|Community 2671]]
- [[_COMMUNITY_Community 2672|Community 2672]]
- [[_COMMUNITY_Community 2673|Community 2673]]
- [[_COMMUNITY_Community 2674|Community 2674]]
- [[_COMMUNITY_Community 2675|Community 2675]]
- [[_COMMUNITY_Community 2676|Community 2676]]
- [[_COMMUNITY_Community 2677|Community 2677]]
- [[_COMMUNITY_Community 2678|Community 2678]]
- [[_COMMUNITY_Community 2679|Community 2679]]
- [[_COMMUNITY_Community 2680|Community 2680]]
- [[_COMMUNITY_Community 2681|Community 2681]]
- [[_COMMUNITY_Community 2682|Community 2682]]
- [[_COMMUNITY_Community 2683|Community 2683]]
- [[_COMMUNITY_Community 2684|Community 2684]]
- [[_COMMUNITY_Community 2685|Community 2685]]
- [[_COMMUNITY_Community 2686|Community 2686]]
- [[_COMMUNITY_Community 2687|Community 2687]]
- [[_COMMUNITY_Community 2688|Community 2688]]
- [[_COMMUNITY_Community 2689|Community 2689]]
- [[_COMMUNITY_Community 2690|Community 2690]]
- [[_COMMUNITY_Community 2691|Community 2691]]
- [[_COMMUNITY_Community 2692|Community 2692]]
- [[_COMMUNITY_Community 2693|Community 2693]]
- [[_COMMUNITY_Community 2694|Community 2694]]
- [[_COMMUNITY_Community 2695|Community 2695]]
- [[_COMMUNITY_Community 2696|Community 2696]]
- [[_COMMUNITY_Community 2697|Community 2697]]
- [[_COMMUNITY_Community 2698|Community 2698]]
- [[_COMMUNITY_Community 2699|Community 2699]]
- [[_COMMUNITY_Community 2700|Community 2700]]
- [[_COMMUNITY_Community 2701|Community 2701]]
- [[_COMMUNITY_Community 2702|Community 2702]]
- [[_COMMUNITY_Community 2703|Community 2703]]
- [[_COMMUNITY_Community 2704|Community 2704]]
- [[_COMMUNITY_Community 2705|Community 2705]]
- [[_COMMUNITY_Community 2706|Community 2706]]
- [[_COMMUNITY_Community 2707|Community 2707]]
- [[_COMMUNITY_Community 2708|Community 2708]]
- [[_COMMUNITY_Community 2709|Community 2709]]
- [[_COMMUNITY_Community 2710|Community 2710]]
- [[_COMMUNITY_Community 2711|Community 2711]]
- [[_COMMUNITY_Community 2712|Community 2712]]
- [[_COMMUNITY_Community 2713|Community 2713]]
- [[_COMMUNITY_Community 2714|Community 2714]]
- [[_COMMUNITY_Community 2715|Community 2715]]
- [[_COMMUNITY_Community 2716|Community 2716]]
- [[_COMMUNITY_Community 2717|Community 2717]]
- [[_COMMUNITY_Community 2718|Community 2718]]
- [[_COMMUNITY_Community 2719|Community 2719]]
- [[_COMMUNITY_Community 2720|Community 2720]]
- [[_COMMUNITY_Community 2721|Community 2721]]
- [[_COMMUNITY_Community 2722|Community 2722]]
- [[_COMMUNITY_Community 2723|Community 2723]]
- [[_COMMUNITY_Community 2724|Community 2724]]
- [[_COMMUNITY_Community 2725|Community 2725]]
- [[_COMMUNITY_Community 2726|Community 2726]]
- [[_COMMUNITY_Community 2727|Community 2727]]
- [[_COMMUNITY_Community 2728|Community 2728]]
- [[_COMMUNITY_Community 2729|Community 2729]]
- [[_COMMUNITY_Community 2730|Community 2730]]
- [[_COMMUNITY_Community 2731|Community 2731]]
- [[_COMMUNITY_Community 2732|Community 2732]]
- [[_COMMUNITY_Community 2733|Community 2733]]
- [[_COMMUNITY_Community 2734|Community 2734]]
- [[_COMMUNITY_Community 2735|Community 2735]]
- [[_COMMUNITY_Community 2736|Community 2736]]
- [[_COMMUNITY_Community 2737|Community 2737]]
- [[_COMMUNITY_Community 2738|Community 2738]]
- [[_COMMUNITY_Community 2739|Community 2739]]
- [[_COMMUNITY_Community 2740|Community 2740]]
- [[_COMMUNITY_Community 2741|Community 2741]]
- [[_COMMUNITY_Community 2742|Community 2742]]
- [[_COMMUNITY_Community 2743|Community 2743]]
- [[_COMMUNITY_Community 2744|Community 2744]]
- [[_COMMUNITY_Community 2745|Community 2745]]
- [[_COMMUNITY_Community 2746|Community 2746]]
- [[_COMMUNITY_Community 2747|Community 2747]]
- [[_COMMUNITY_Community 2748|Community 2748]]
- [[_COMMUNITY_Community 2749|Community 2749]]
- [[_COMMUNITY_Community 2750|Community 2750]]
- [[_COMMUNITY_Community 2751|Community 2751]]
- [[_COMMUNITY_Community 2752|Community 2752]]
- [[_COMMUNITY_Community 2753|Community 2753]]
- [[_COMMUNITY_Community 2754|Community 2754]]
- [[_COMMUNITY_Community 2755|Community 2755]]
- [[_COMMUNITY_Community 2756|Community 2756]]
- [[_COMMUNITY_Community 2757|Community 2757]]
- [[_COMMUNITY_Community 2758|Community 2758]]
- [[_COMMUNITY_Community 2759|Community 2759]]
- [[_COMMUNITY_Community 2760|Community 2760]]
- [[_COMMUNITY_Community 2761|Community 2761]]
- [[_COMMUNITY_Community 2762|Community 2762]]
- [[_COMMUNITY_Community 2763|Community 2763]]
- [[_COMMUNITY_Community 2764|Community 2764]]
- [[_COMMUNITY_Community 2765|Community 2765]]
- [[_COMMUNITY_Community 2766|Community 2766]]
- [[_COMMUNITY_Community 2767|Community 2767]]
- [[_COMMUNITY_Community 2768|Community 2768]]
- [[_COMMUNITY_Community 2769|Community 2769]]
- [[_COMMUNITY_Community 2770|Community 2770]]
- [[_COMMUNITY_Community 2771|Community 2771]]
- [[_COMMUNITY_Community 2772|Community 2772]]
- [[_COMMUNITY_Community 2773|Community 2773]]
- [[_COMMUNITY_Community 2774|Community 2774]]
- [[_COMMUNITY_Community 2775|Community 2775]]
- [[_COMMUNITY_Community 2776|Community 2776]]
- [[_COMMUNITY_Community 2777|Community 2777]]
- [[_COMMUNITY_Community 2778|Community 2778]]
- [[_COMMUNITY_Community 2779|Community 2779]]
- [[_COMMUNITY_Community 2780|Community 2780]]
- [[_COMMUNITY_Community 2781|Community 2781]]
- [[_COMMUNITY_Community 2782|Community 2782]]
- [[_COMMUNITY_Community 2783|Community 2783]]
- [[_COMMUNITY_Community 2784|Community 2784]]
- [[_COMMUNITY_Community 2785|Community 2785]]
- [[_COMMUNITY_Community 2786|Community 2786]]
- [[_COMMUNITY_Community 2787|Community 2787]]
- [[_COMMUNITY_Community 2788|Community 2788]]
- [[_COMMUNITY_Community 2789|Community 2789]]
- [[_COMMUNITY_Community 2790|Community 2790]]
- [[_COMMUNITY_Community 2791|Community 2791]]
- [[_COMMUNITY_Community 2792|Community 2792]]
- [[_COMMUNITY_Community 2793|Community 2793]]
- [[_COMMUNITY_Community 2794|Community 2794]]
- [[_COMMUNITY_Community 2795|Community 2795]]
- [[_COMMUNITY_Community 2796|Community 2796]]
- [[_COMMUNITY_Community 2797|Community 2797]]
- [[_COMMUNITY_Community 2798|Community 2798]]
- [[_COMMUNITY_Community 2799|Community 2799]]
- [[_COMMUNITY_Community 2800|Community 2800]]
- [[_COMMUNITY_Community 2801|Community 2801]]
- [[_COMMUNITY_Community 2802|Community 2802]]
- [[_COMMUNITY_Community 2803|Community 2803]]
- [[_COMMUNITY_Community 2804|Community 2804]]
- [[_COMMUNITY_Community 2805|Community 2805]]
- [[_COMMUNITY_Community 2806|Community 2806]]
- [[_COMMUNITY_Community 2807|Community 2807]]
- [[_COMMUNITY_Community 2808|Community 2808]]
- [[_COMMUNITY_Community 2809|Community 2809]]
- [[_COMMUNITY_Community 2810|Community 2810]]
- [[_COMMUNITY_Community 2811|Community 2811]]
- [[_COMMUNITY_Community 2812|Community 2812]]
- [[_COMMUNITY_Community 2813|Community 2813]]
- [[_COMMUNITY_Community 2814|Community 2814]]
- [[_COMMUNITY_Community 2815|Community 2815]]
- [[_COMMUNITY_Community 2816|Community 2816]]
- [[_COMMUNITY_Community 2817|Community 2817]]
- [[_COMMUNITY_Community 2818|Community 2818]]
- [[_COMMUNITY_Community 2819|Community 2819]]
- [[_COMMUNITY_Community 2820|Community 2820]]
- [[_COMMUNITY_Community 2821|Community 2821]]
- [[_COMMUNITY_Community 2822|Community 2822]]
- [[_COMMUNITY_Community 2823|Community 2823]]
- [[_COMMUNITY_Community 2824|Community 2824]]
- [[_COMMUNITY_Community 2825|Community 2825]]
- [[_COMMUNITY_Community 2826|Community 2826]]
- [[_COMMUNITY_Community 2827|Community 2827]]
- [[_COMMUNITY_Community 2828|Community 2828]]
- [[_COMMUNITY_Community 2829|Community 2829]]
- [[_COMMUNITY_Community 2830|Community 2830]]
- [[_COMMUNITY_Community 2831|Community 2831]]
- [[_COMMUNITY_Community 2832|Community 2832]]
- [[_COMMUNITY_Community 2833|Community 2833]]
- [[_COMMUNITY_Community 2834|Community 2834]]
- [[_COMMUNITY_Community 2835|Community 2835]]
- [[_COMMUNITY_Community 2836|Community 2836]]
- [[_COMMUNITY_Community 2837|Community 2837]]
- [[_COMMUNITY_Community 2838|Community 2838]]
- [[_COMMUNITY_Community 2839|Community 2839]]
- [[_COMMUNITY_Community 2840|Community 2840]]
- [[_COMMUNITY_Community 2841|Community 2841]]
- [[_COMMUNITY_Community 2842|Community 2842]]
- [[_COMMUNITY_Community 2843|Community 2843]]
- [[_COMMUNITY_Community 2844|Community 2844]]
- [[_COMMUNITY_Community 2845|Community 2845]]
- [[_COMMUNITY_Community 2846|Community 2846]]
- [[_COMMUNITY_Community 2847|Community 2847]]
- [[_COMMUNITY_Community 2848|Community 2848]]
- [[_COMMUNITY_Community 2849|Community 2849]]
- [[_COMMUNITY_Community 2850|Community 2850]]
- [[_COMMUNITY_Community 2851|Community 2851]]
- [[_COMMUNITY_Community 2852|Community 2852]]
- [[_COMMUNITY_Community 2853|Community 2853]]
- [[_COMMUNITY_Community 2854|Community 2854]]
- [[_COMMUNITY_Community 2855|Community 2855]]
- [[_COMMUNITY_Community 2856|Community 2856]]
- [[_COMMUNITY_Community 2857|Community 2857]]
- [[_COMMUNITY_Community 2858|Community 2858]]
- [[_COMMUNITY_Community 2859|Community 2859]]
- [[_COMMUNITY_Community 2860|Community 2860]]
- [[_COMMUNITY_Community 2861|Community 2861]]
- [[_COMMUNITY_Community 2862|Community 2862]]
- [[_COMMUNITY_Community 2863|Community 2863]]
- [[_COMMUNITY_Community 2864|Community 2864]]
- [[_COMMUNITY_Community 2865|Community 2865]]
- [[_COMMUNITY_Community 2866|Community 2866]]
- [[_COMMUNITY_Community 2867|Community 2867]]
- [[_COMMUNITY_Community 2868|Community 2868]]
- [[_COMMUNITY_Community 2869|Community 2869]]
- [[_COMMUNITY_Community 2870|Community 2870]]
- [[_COMMUNITY_Community 2871|Community 2871]]
- [[_COMMUNITY_Community 2872|Community 2872]]
- [[_COMMUNITY_Community 2873|Community 2873]]
- [[_COMMUNITY_Community 2874|Community 2874]]
- [[_COMMUNITY_Community 2875|Community 2875]]
- [[_COMMUNITY_Community 2876|Community 2876]]
- [[_COMMUNITY_Community 2877|Community 2877]]
- [[_COMMUNITY_Community 2878|Community 2878]]
- [[_COMMUNITY_Community 2879|Community 2879]]
- [[_COMMUNITY_Community 2880|Community 2880]]
- [[_COMMUNITY_Community 2881|Community 2881]]
- [[_COMMUNITY_Community 2882|Community 2882]]
- [[_COMMUNITY_Community 2883|Community 2883]]
- [[_COMMUNITY_Community 2884|Community 2884]]
- [[_COMMUNITY_Community 2885|Community 2885]]
- [[_COMMUNITY_Community 2886|Community 2886]]
- [[_COMMUNITY_Community 2887|Community 2887]]
- [[_COMMUNITY_Community 2888|Community 2888]]
- [[_COMMUNITY_Community 2889|Community 2889]]
- [[_COMMUNITY_Community 2890|Community 2890]]
- [[_COMMUNITY_Community 2891|Community 2891]]
- [[_COMMUNITY_Community 2892|Community 2892]]
- [[_COMMUNITY_Community 2893|Community 2893]]
- [[_COMMUNITY_Community 2894|Community 2894]]
- [[_COMMUNITY_Community 2895|Community 2895]]
- [[_COMMUNITY_Community 2896|Community 2896]]
- [[_COMMUNITY_Community 2897|Community 2897]]
- [[_COMMUNITY_Community 2898|Community 2898]]
- [[_COMMUNITY_Community 2899|Community 2899]]
- [[_COMMUNITY_Community 2900|Community 2900]]
- [[_COMMUNITY_Community 2901|Community 2901]]
- [[_COMMUNITY_Community 2902|Community 2902]]
- [[_COMMUNITY_Community 2903|Community 2903]]
- [[_COMMUNITY_Community 2904|Community 2904]]
- [[_COMMUNITY_Community 2905|Community 2905]]
- [[_COMMUNITY_Community 2906|Community 2906]]
- [[_COMMUNITY_Community 2907|Community 2907]]
- [[_COMMUNITY_Community 2908|Community 2908]]
- [[_COMMUNITY_Community 2909|Community 2909]]
- [[_COMMUNITY_Community 2910|Community 2910]]
- [[_COMMUNITY_Community 2911|Community 2911]]
- [[_COMMUNITY_Community 2912|Community 2912]]
- [[_COMMUNITY_Community 2913|Community 2913]]
- [[_COMMUNITY_Community 2914|Community 2914]]
- [[_COMMUNITY_Community 2915|Community 2915]]
- [[_COMMUNITY_Community 2916|Community 2916]]
- [[_COMMUNITY_Community 2917|Community 2917]]
- [[_COMMUNITY_Community 2918|Community 2918]]
- [[_COMMUNITY_Community 2919|Community 2919]]
- [[_COMMUNITY_Community 2920|Community 2920]]
- [[_COMMUNITY_Community 2921|Community 2921]]
- [[_COMMUNITY_Community 2922|Community 2922]]
- [[_COMMUNITY_Community 2923|Community 2923]]
- [[_COMMUNITY_Community 2924|Community 2924]]
- [[_COMMUNITY_Community 2925|Community 2925]]
- [[_COMMUNITY_Community 2926|Community 2926]]
- [[_COMMUNITY_Community 2927|Community 2927]]
- [[_COMMUNITY_Community 2928|Community 2928]]
- [[_COMMUNITY_Community 2929|Community 2929]]
- [[_COMMUNITY_Community 2930|Community 2930]]
- [[_COMMUNITY_Community 2931|Community 2931]]
- [[_COMMUNITY_Community 2932|Community 2932]]
- [[_COMMUNITY_Community 2933|Community 2933]]
- [[_COMMUNITY_Community 2934|Community 2934]]
- [[_COMMUNITY_Community 2935|Community 2935]]
- [[_COMMUNITY_Community 2936|Community 2936]]
- [[_COMMUNITY_Community 2937|Community 2937]]
- [[_COMMUNITY_Community 2938|Community 2938]]
- [[_COMMUNITY_Community 2939|Community 2939]]
- [[_COMMUNITY_Community 2940|Community 2940]]
- [[_COMMUNITY_Community 2941|Community 2941]]
- [[_COMMUNITY_Community 2942|Community 2942]]
- [[_COMMUNITY_Community 2943|Community 2943]]
- [[_COMMUNITY_Community 2944|Community 2944]]
- [[_COMMUNITY_Community 2945|Community 2945]]
- [[_COMMUNITY_Community 2946|Community 2946]]
- [[_COMMUNITY_Community 2947|Community 2947]]
- [[_COMMUNITY_Community 2948|Community 2948]]
- [[_COMMUNITY_Community 2949|Community 2949]]
- [[_COMMUNITY_Community 2950|Community 2950]]
- [[_COMMUNITY_Community 2951|Community 2951]]
- [[_COMMUNITY_Community 2952|Community 2952]]
- [[_COMMUNITY_Community 2953|Community 2953]]
- [[_COMMUNITY_Community 2954|Community 2954]]
- [[_COMMUNITY_Community 2955|Community 2955]]
- [[_COMMUNITY_Community 2956|Community 2956]]
- [[_COMMUNITY_Community 2957|Community 2957]]
- [[_COMMUNITY_Community 2958|Community 2958]]
- [[_COMMUNITY_Community 2959|Community 2959]]
- [[_COMMUNITY_Community 2960|Community 2960]]
- [[_COMMUNITY_Community 2961|Community 2961]]
- [[_COMMUNITY_Community 2962|Community 2962]]
- [[_COMMUNITY_Community 2963|Community 2963]]
- [[_COMMUNITY_Community 2964|Community 2964]]
- [[_COMMUNITY_Community 2965|Community 2965]]
- [[_COMMUNITY_Community 2966|Community 2966]]
- [[_COMMUNITY_Community 2967|Community 2967]]
- [[_COMMUNITY_Community 2968|Community 2968]]
- [[_COMMUNITY_Community 2969|Community 2969]]
- [[_COMMUNITY_Community 2970|Community 2970]]
- [[_COMMUNITY_Community 2971|Community 2971]]
- [[_COMMUNITY_Community 2972|Community 2972]]
- [[_COMMUNITY_Community 2973|Community 2973]]
- [[_COMMUNITY_Community 2974|Community 2974]]
- [[_COMMUNITY_Community 2975|Community 2975]]
- [[_COMMUNITY_Community 2976|Community 2976]]
- [[_COMMUNITY_Community 2977|Community 2977]]
- [[_COMMUNITY_Community 2978|Community 2978]]
- [[_COMMUNITY_Community 2979|Community 2979]]
- [[_COMMUNITY_Community 2980|Community 2980]]
- [[_COMMUNITY_Community 2981|Community 2981]]
- [[_COMMUNITY_Community 2982|Community 2982]]
- [[_COMMUNITY_Community 2983|Community 2983]]
- [[_COMMUNITY_Community 2984|Community 2984]]
- [[_COMMUNITY_Community 2985|Community 2985]]
- [[_COMMUNITY_Community 2986|Community 2986]]
- [[_COMMUNITY_Community 2987|Community 2987]]
- [[_COMMUNITY_Community 2988|Community 2988]]
- [[_COMMUNITY_Community 2989|Community 2989]]
- [[_COMMUNITY_Community 2990|Community 2990]]
- [[_COMMUNITY_Community 2991|Community 2991]]
- [[_COMMUNITY_Community 2992|Community 2992]]
- [[_COMMUNITY_Community 2993|Community 2993]]
- [[_COMMUNITY_Community 2994|Community 2994]]
- [[_COMMUNITY_Community 2995|Community 2995]]
- [[_COMMUNITY_Community 2996|Community 2996]]
- [[_COMMUNITY_Community 2997|Community 2997]]
- [[_COMMUNITY_Community 2998|Community 2998]]
- [[_COMMUNITY_Community 2999|Community 2999]]
- [[_COMMUNITY_Community 3000|Community 3000]]
- [[_COMMUNITY_Community 3001|Community 3001]]
- [[_COMMUNITY_Community 3002|Community 3002]]
- [[_COMMUNITY_Community 3003|Community 3003]]
- [[_COMMUNITY_Community 3004|Community 3004]]
- [[_COMMUNITY_Community 3005|Community 3005]]
- [[_COMMUNITY_Community 3006|Community 3006]]
- [[_COMMUNITY_Community 3007|Community 3007]]
- [[_COMMUNITY_Community 3008|Community 3008]]
- [[_COMMUNITY_Community 3009|Community 3009]]
- [[_COMMUNITY_Community 3010|Community 3010]]
- [[_COMMUNITY_Community 3011|Community 3011]]
- [[_COMMUNITY_Community 3012|Community 3012]]
- [[_COMMUNITY_Community 3013|Community 3013]]
- [[_COMMUNITY_Community 3014|Community 3014]]
- [[_COMMUNITY_Community 3015|Community 3015]]
- [[_COMMUNITY_Community 3016|Community 3016]]
- [[_COMMUNITY_Community 3017|Community 3017]]
- [[_COMMUNITY_Community 3018|Community 3018]]
- [[_COMMUNITY_Community 3019|Community 3019]]
- [[_COMMUNITY_Community 3020|Community 3020]]
- [[_COMMUNITY_Community 3021|Community 3021]]
- [[_COMMUNITY_Community 3022|Community 3022]]
- [[_COMMUNITY_Community 3023|Community 3023]]
- [[_COMMUNITY_Community 3024|Community 3024]]
- [[_COMMUNITY_Community 3025|Community 3025]]
- [[_COMMUNITY_Community 3026|Community 3026]]
- [[_COMMUNITY_Community 3027|Community 3027]]
- [[_COMMUNITY_Community 3028|Community 3028]]
- [[_COMMUNITY_Community 3029|Community 3029]]
- [[_COMMUNITY_Community 3030|Community 3030]]
- [[_COMMUNITY_Community 3031|Community 3031]]
- [[_COMMUNITY_Community 3032|Community 3032]]
- [[_COMMUNITY_Community 3033|Community 3033]]
- [[_COMMUNITY_Community 3034|Community 3034]]
- [[_COMMUNITY_Community 3035|Community 3035]]
- [[_COMMUNITY_Community 3036|Community 3036]]
- [[_COMMUNITY_Community 3037|Community 3037]]
- [[_COMMUNITY_Community 3038|Community 3038]]
- [[_COMMUNITY_Community 3039|Community 3039]]
- [[_COMMUNITY_Community 3040|Community 3040]]
- [[_COMMUNITY_Community 3041|Community 3041]]
- [[_COMMUNITY_Community 3042|Community 3042]]
- [[_COMMUNITY_Community 3043|Community 3043]]
- [[_COMMUNITY_Community 3044|Community 3044]]
- [[_COMMUNITY_Community 3045|Community 3045]]
- [[_COMMUNITY_Community 3046|Community 3046]]
- [[_COMMUNITY_Community 3047|Community 3047]]
- [[_COMMUNITY_Community 3048|Community 3048]]
- [[_COMMUNITY_Community 3049|Community 3049]]
- [[_COMMUNITY_Community 3050|Community 3050]]
- [[_COMMUNITY_Community 3051|Community 3051]]
- [[_COMMUNITY_Community 3052|Community 3052]]
- [[_COMMUNITY_Community 3053|Community 3053]]
- [[_COMMUNITY_Community 3054|Community 3054]]
- [[_COMMUNITY_Community 3055|Community 3055]]
- [[_COMMUNITY_Community 3056|Community 3056]]
- [[_COMMUNITY_Community 3057|Community 3057]]
- [[_COMMUNITY_Community 3058|Community 3058]]
- [[_COMMUNITY_Community 3059|Community 3059]]
- [[_COMMUNITY_Community 3060|Community 3060]]
- [[_COMMUNITY_Community 3061|Community 3061]]
- [[_COMMUNITY_Community 3062|Community 3062]]
- [[_COMMUNITY_Community 3063|Community 3063]]
- [[_COMMUNITY_Community 3064|Community 3064]]
- [[_COMMUNITY_Community 3065|Community 3065]]
- [[_COMMUNITY_Community 3066|Community 3066]]
- [[_COMMUNITY_Community 3067|Community 3067]]
- [[_COMMUNITY_Community 3068|Community 3068]]
- [[_COMMUNITY_Community 3069|Community 3069]]
- [[_COMMUNITY_Community 3070|Community 3070]]
- [[_COMMUNITY_Community 3071|Community 3071]]
- [[_COMMUNITY_Community 3072|Community 3072]]
- [[_COMMUNITY_Community 3073|Community 3073]]
- [[_COMMUNITY_Community 3074|Community 3074]]
- [[_COMMUNITY_Community 3075|Community 3075]]
- [[_COMMUNITY_Community 3076|Community 3076]]
- [[_COMMUNITY_Community 3077|Community 3077]]
- [[_COMMUNITY_Community 3078|Community 3078]]
- [[_COMMUNITY_Community 3079|Community 3079]]
- [[_COMMUNITY_Community 3080|Community 3080]]
- [[_COMMUNITY_Community 3081|Community 3081]]
- [[_COMMUNITY_Community 3082|Community 3082]]
- [[_COMMUNITY_Community 3083|Community 3083]]
- [[_COMMUNITY_Community 3084|Community 3084]]
- [[_COMMUNITY_Community 3085|Community 3085]]
- [[_COMMUNITY_Community 3086|Community 3086]]
- [[_COMMUNITY_Community 3087|Community 3087]]
- [[_COMMUNITY_Community 3088|Community 3088]]
- [[_COMMUNITY_Community 3089|Community 3089]]
- [[_COMMUNITY_Community 3090|Community 3090]]
- [[_COMMUNITY_Community 3091|Community 3091]]
- [[_COMMUNITY_Community 3092|Community 3092]]
- [[_COMMUNITY_Community 3093|Community 3093]]
- [[_COMMUNITY_Community 3094|Community 3094]]
- [[_COMMUNITY_Community 3095|Community 3095]]
- [[_COMMUNITY_Community 3096|Community 3096]]
- [[_COMMUNITY_Community 3097|Community 3097]]
- [[_COMMUNITY_Community 3098|Community 3098]]
- [[_COMMUNITY_Community 3099|Community 3099]]
- [[_COMMUNITY_Community 3100|Community 3100]]
- [[_COMMUNITY_Community 3101|Community 3101]]
- [[_COMMUNITY_Community 3102|Community 3102]]
- [[_COMMUNITY_Community 3103|Community 3103]]
- [[_COMMUNITY_Community 3104|Community 3104]]
- [[_COMMUNITY_Community 3105|Community 3105]]
- [[_COMMUNITY_Community 3106|Community 3106]]
- [[_COMMUNITY_Community 3107|Community 3107]]
- [[_COMMUNITY_Community 3108|Community 3108]]
- [[_COMMUNITY_Community 3109|Community 3109]]
- [[_COMMUNITY_Community 3110|Community 3110]]
- [[_COMMUNITY_Community 3111|Community 3111]]
- [[_COMMUNITY_Community 3112|Community 3112]]
- [[_COMMUNITY_Community 3113|Community 3113]]
- [[_COMMUNITY_Community 3114|Community 3114]]
- [[_COMMUNITY_Community 3115|Community 3115]]
- [[_COMMUNITY_Community 3116|Community 3116]]
- [[_COMMUNITY_Community 3117|Community 3117]]
- [[_COMMUNITY_Community 3118|Community 3118]]
- [[_COMMUNITY_Community 3119|Community 3119]]
- [[_COMMUNITY_Community 3120|Community 3120]]
- [[_COMMUNITY_Community 3121|Community 3121]]
- [[_COMMUNITY_Community 3122|Community 3122]]
- [[_COMMUNITY_Community 3123|Community 3123]]
- [[_COMMUNITY_Community 3124|Community 3124]]
- [[_COMMUNITY_Community 3125|Community 3125]]
- [[_COMMUNITY_Community 3126|Community 3126]]
- [[_COMMUNITY_Community 3127|Community 3127]]
- [[_COMMUNITY_Community 3128|Community 3128]]
- [[_COMMUNITY_Community 3129|Community 3129]]
- [[_COMMUNITY_Community 3130|Community 3130]]
- [[_COMMUNITY_Community 3131|Community 3131]]
- [[_COMMUNITY_Community 3132|Community 3132]]
- [[_COMMUNITY_Community 3133|Community 3133]]
- [[_COMMUNITY_Community 3134|Community 3134]]
- [[_COMMUNITY_Community 3135|Community 3135]]
- [[_COMMUNITY_Community 3136|Community 3136]]
- [[_COMMUNITY_Community 3137|Community 3137]]
- [[_COMMUNITY_Community 3138|Community 3138]]
- [[_COMMUNITY_Community 3139|Community 3139]]
- [[_COMMUNITY_Community 3140|Community 3140]]
- [[_COMMUNITY_Community 3141|Community 3141]]
- [[_COMMUNITY_Community 3142|Community 3142]]
- [[_COMMUNITY_Community 3143|Community 3143]]
- [[_COMMUNITY_Community 3144|Community 3144]]
- [[_COMMUNITY_Community 3145|Community 3145]]
- [[_COMMUNITY_Community 3146|Community 3146]]
- [[_COMMUNITY_Community 3147|Community 3147]]
- [[_COMMUNITY_Community 3148|Community 3148]]
- [[_COMMUNITY_Community 3149|Community 3149]]
- [[_COMMUNITY_Community 3150|Community 3150]]
- [[_COMMUNITY_Community 3151|Community 3151]]
- [[_COMMUNITY_Community 3152|Community 3152]]
- [[_COMMUNITY_Community 3153|Community 3153]]
- [[_COMMUNITY_Community 3154|Community 3154]]
- [[_COMMUNITY_Community 3155|Community 3155]]
- [[_COMMUNITY_Community 3156|Community 3156]]
- [[_COMMUNITY_Community 3157|Community 3157]]
- [[_COMMUNITY_Community 3158|Community 3158]]
- [[_COMMUNITY_Community 3159|Community 3159]]
- [[_COMMUNITY_Community 3160|Community 3160]]
- [[_COMMUNITY_Community 3161|Community 3161]]
- [[_COMMUNITY_Community 3162|Community 3162]]
- [[_COMMUNITY_Community 3163|Community 3163]]
- [[_COMMUNITY_Community 3164|Community 3164]]
- [[_COMMUNITY_Community 3165|Community 3165]]
- [[_COMMUNITY_Community 3166|Community 3166]]
- [[_COMMUNITY_Community 3167|Community 3167]]
- [[_COMMUNITY_Community 3168|Community 3168]]
- [[_COMMUNITY_Community 3169|Community 3169]]
- [[_COMMUNITY_Community 3170|Community 3170]]
- [[_COMMUNITY_Community 3171|Community 3171]]
- [[_COMMUNITY_Community 3172|Community 3172]]
- [[_COMMUNITY_Community 3173|Community 3173]]
- [[_COMMUNITY_Community 3174|Community 3174]]
- [[_COMMUNITY_Community 3175|Community 3175]]
- [[_COMMUNITY_Community 3176|Community 3176]]
- [[_COMMUNITY_Community 3177|Community 3177]]
- [[_COMMUNITY_Community 3178|Community 3178]]
- [[_COMMUNITY_Community 3179|Community 3179]]
- [[_COMMUNITY_Community 3180|Community 3180]]
- [[_COMMUNITY_Community 3181|Community 3181]]
- [[_COMMUNITY_Community 3182|Community 3182]]
- [[_COMMUNITY_Community 3183|Community 3183]]
- [[_COMMUNITY_Community 3184|Community 3184]]
- [[_COMMUNITY_Community 3185|Community 3185]]
- [[_COMMUNITY_Community 3186|Community 3186]]
- [[_COMMUNITY_Community 3187|Community 3187]]
- [[_COMMUNITY_Community 3188|Community 3188]]
- [[_COMMUNITY_Community 3189|Community 3189]]
- [[_COMMUNITY_Community 3190|Community 3190]]
- [[_COMMUNITY_Community 3191|Community 3191]]
- [[_COMMUNITY_Community 3192|Community 3192]]
- [[_COMMUNITY_Community 3193|Community 3193]]
- [[_COMMUNITY_Community 3194|Community 3194]]
- [[_COMMUNITY_Community 3195|Community 3195]]
- [[_COMMUNITY_Community 3196|Community 3196]]
- [[_COMMUNITY_Community 3197|Community 3197]]
- [[_COMMUNITY_Community 3198|Community 3198]]
- [[_COMMUNITY_Community 3199|Community 3199]]
- [[_COMMUNITY_Community 3200|Community 3200]]
- [[_COMMUNITY_Community 3201|Community 3201]]
- [[_COMMUNITY_Community 3202|Community 3202]]
- [[_COMMUNITY_Community 3203|Community 3203]]
- [[_COMMUNITY_Community 3204|Community 3204]]
- [[_COMMUNITY_Community 3205|Community 3205]]
- [[_COMMUNITY_Community 3206|Community 3206]]
- [[_COMMUNITY_Community 3207|Community 3207]]
- [[_COMMUNITY_Community 3208|Community 3208]]
- [[_COMMUNITY_Community 3209|Community 3209]]
- [[_COMMUNITY_Community 3210|Community 3210]]
- [[_COMMUNITY_Community 3211|Community 3211]]
- [[_COMMUNITY_Community 3212|Community 3212]]
- [[_COMMUNITY_Community 3213|Community 3213]]
- [[_COMMUNITY_Community 3214|Community 3214]]
- [[_COMMUNITY_Community 3215|Community 3215]]
- [[_COMMUNITY_Community 3216|Community 3216]]
- [[_COMMUNITY_Community 3217|Community 3217]]
- [[_COMMUNITY_Community 3218|Community 3218]]
- [[_COMMUNITY_Community 3219|Community 3219]]
- [[_COMMUNITY_Community 3220|Community 3220]]
- [[_COMMUNITY_Community 3221|Community 3221]]
- [[_COMMUNITY_Community 3222|Community 3222]]
- [[_COMMUNITY_Community 3223|Community 3223]]
- [[_COMMUNITY_Community 3224|Community 3224]]
- [[_COMMUNITY_Community 3225|Community 3225]]
- [[_COMMUNITY_Community 3226|Community 3226]]
- [[_COMMUNITY_Community 3227|Community 3227]]
- [[_COMMUNITY_Community 3228|Community 3228]]
- [[_COMMUNITY_Community 3229|Community 3229]]
- [[_COMMUNITY_Community 3230|Community 3230]]
- [[_COMMUNITY_Community 3231|Community 3231]]
- [[_COMMUNITY_Community 3232|Community 3232]]
- [[_COMMUNITY_Community 3233|Community 3233]]
- [[_COMMUNITY_Community 3234|Community 3234]]
- [[_COMMUNITY_Community 3235|Community 3235]]
- [[_COMMUNITY_Community 3236|Community 3236]]
- [[_COMMUNITY_Community 3237|Community 3237]]
- [[_COMMUNITY_Community 3238|Community 3238]]
- [[_COMMUNITY_Community 3239|Community 3239]]
- [[_COMMUNITY_Community 3240|Community 3240]]
- [[_COMMUNITY_Community 3241|Community 3241]]
- [[_COMMUNITY_Community 3242|Community 3242]]
- [[_COMMUNITY_Community 3243|Community 3243]]
- [[_COMMUNITY_Community 3244|Community 3244]]
- [[_COMMUNITY_Community 3245|Community 3245]]
- [[_COMMUNITY_Community 3246|Community 3246]]
- [[_COMMUNITY_Community 3247|Community 3247]]
- [[_COMMUNITY_Community 3248|Community 3248]]
- [[_COMMUNITY_Community 3249|Community 3249]]
- [[_COMMUNITY_Community 3250|Community 3250]]
- [[_COMMUNITY_Community 3251|Community 3251]]
- [[_COMMUNITY_Community 3252|Community 3252]]
- [[_COMMUNITY_Community 3253|Community 3253]]
- [[_COMMUNITY_Community 3254|Community 3254]]
- [[_COMMUNITY_Community 3255|Community 3255]]
- [[_COMMUNITY_Community 3256|Community 3256]]
- [[_COMMUNITY_Community 3257|Community 3257]]
- [[_COMMUNITY_Community 3258|Community 3258]]
- [[_COMMUNITY_Community 3259|Community 3259]]
- [[_COMMUNITY_Community 3260|Community 3260]]
- [[_COMMUNITY_Community 3261|Community 3261]]
- [[_COMMUNITY_Community 3262|Community 3262]]
- [[_COMMUNITY_Community 3263|Community 3263]]
- [[_COMMUNITY_Community 3264|Community 3264]]
- [[_COMMUNITY_Community 3265|Community 3265]]
- [[_COMMUNITY_Community 3266|Community 3266]]
- [[_COMMUNITY_Community 3267|Community 3267]]
- [[_COMMUNITY_Community 3268|Community 3268]]
- [[_COMMUNITY_Community 3269|Community 3269]]
- [[_COMMUNITY_Community 3270|Community 3270]]
- [[_COMMUNITY_Community 3271|Community 3271]]
- [[_COMMUNITY_Community 3272|Community 3272]]
- [[_COMMUNITY_Community 3273|Community 3273]]
- [[_COMMUNITY_Community 3274|Community 3274]]
- [[_COMMUNITY_Community 3275|Community 3275]]
- [[_COMMUNITY_Community 3276|Community 3276]]
- [[_COMMUNITY_Community 3277|Community 3277]]
- [[_COMMUNITY_Community 3278|Community 3278]]
- [[_COMMUNITY_Community 3279|Community 3279]]
- [[_COMMUNITY_Community 3280|Community 3280]]
- [[_COMMUNITY_Community 3281|Community 3281]]
- [[_COMMUNITY_Community 3282|Community 3282]]
- [[_COMMUNITY_Community 3283|Community 3283]]
- [[_COMMUNITY_Community 3284|Community 3284]]
- [[_COMMUNITY_Community 3285|Community 3285]]
- [[_COMMUNITY_Community 3298|Community 3298]]
- [[_COMMUNITY_Community 3301|Community 3301]]
- [[_COMMUNITY_Community 3306|Community 3306]]
- [[_COMMUNITY_Community 3307|Community 3307]]
- [[_COMMUNITY_Community 3308|Community 3308]]
- [[_COMMUNITY_Community 3309|Community 3309]]
- [[_COMMUNITY_Community 3310|Community 3310]]
- [[_COMMUNITY_Community 3311|Community 3311]]
- [[_COMMUNITY_Community 3312|Community 3312]]
- [[_COMMUNITY_Community 3313|Community 3313]]
- [[_COMMUNITY_Community 3314|Community 3314]]
- [[_COMMUNITY_Community 3315|Community 3315]]
- [[_COMMUNITY_Community 3316|Community 3316]]
- [[_COMMUNITY_Community 3317|Community 3317]]
- [[_COMMUNITY_Community 3318|Community 3318]]
- [[_COMMUNITY_Community 3319|Community 3319]]
- [[_COMMUNITY_Community 3320|Community 3320]]
- [[_COMMUNITY_Community 3321|Community 3321]]
- [[_COMMUNITY_Community 3322|Community 3322]]
- [[_COMMUNITY_Community 3323|Community 3323]]
- [[_COMMUNITY_Community 3324|Community 3324]]
- [[_COMMUNITY_Community 3325|Community 3325]]
- [[_COMMUNITY_Community 3326|Community 3326]]
- [[_COMMUNITY_Community 3327|Community 3327]]
- [[_COMMUNITY_Community 3328|Community 3328]]
- [[_COMMUNITY_Community 3329|Community 3329]]
- [[_COMMUNITY_Community 3330|Community 3330]]
- [[_COMMUNITY_Community 3331|Community 3331]]
- [[_COMMUNITY_Community 3332|Community 3332]]
- [[_COMMUNITY_Community 3333|Community 3333]]
- [[_COMMUNITY_Community 3334|Community 3334]]
- [[_COMMUNITY_Community 3335|Community 3335]]
- [[_COMMUNITY_Community 3336|Community 3336]]
- [[_COMMUNITY_Community 3337|Community 3337]]
- [[_COMMUNITY_Community 3338|Community 3338]]
- [[_COMMUNITY_Community 3339|Community 3339]]
- [[_COMMUNITY_Community 3340|Community 3340]]
- [[_COMMUNITY_Community 3341|Community 3341]]
- [[_COMMUNITY_Community 3342|Community 3342]]
- [[_COMMUNITY_Community 3343|Community 3343]]
- [[_COMMUNITY_Community 3344|Community 3344]]
- [[_COMMUNITY_Community 3345|Community 3345]]
- [[_COMMUNITY_Community 3346|Community 3346]]
- [[_COMMUNITY_Community 3347|Community 3347]]
- [[_COMMUNITY_Community 3348|Community 3348]]
- [[_COMMUNITY_Community 3349|Community 3349]]
- [[_COMMUNITY_Community 3350|Community 3350]]
- [[_COMMUNITY_Community 3351|Community 3351]]
- [[_COMMUNITY_Community 3352|Community 3352]]
- [[_COMMUNITY_Community 3353|Community 3353]]
- [[_COMMUNITY_Community 3354|Community 3354]]
- [[_COMMUNITY_Community 3355|Community 3355]]
- [[_COMMUNITY_Community 3356|Community 3356]]
- [[_COMMUNITY_Community 3357|Community 3357]]
- [[_COMMUNITY_Community 3358|Community 3358]]
- [[_COMMUNITY_Community 3359|Community 3359]]
- [[_COMMUNITY_Community 3361|Community 3361]]
- [[_COMMUNITY_Community 3362|Community 3362]]
- [[_COMMUNITY_Community 3363|Community 3363]]
- [[_COMMUNITY_Community 3364|Community 3364]]
- [[_COMMUNITY_Community 3365|Community 3365]]
- [[_COMMUNITY_Community 3366|Community 3366]]
- [[_COMMUNITY_Community 3367|Community 3367]]
- [[_COMMUNITY_Community 3368|Community 3368]]
- [[_COMMUNITY_Community 3369|Community 3369]]
- [[_COMMUNITY_Community 3370|Community 3370]]
- [[_COMMUNITY_Community 3371|Community 3371]]
- [[_COMMUNITY_Community 3372|Community 3372]]
- [[_COMMUNITY_Community 3373|Community 3373]]
- [[_COMMUNITY_Community 3374|Community 3374]]
- [[_COMMUNITY_Community 3375|Community 3375]]
- [[_COMMUNITY_Community 3376|Community 3376]]
- [[_COMMUNITY_Community 3377|Community 3377]]
- [[_COMMUNITY_Community 3378|Community 3378]]
- [[_COMMUNITY_Community 3379|Community 3379]]
- [[_COMMUNITY_Community 3381|Community 3381]]
- [[_COMMUNITY_Community 3382|Community 3382]]
- [[_COMMUNITY_Community 3383|Community 3383]]
- [[_COMMUNITY_Community 3384|Community 3384]]
- [[_COMMUNITY_Community 3385|Community 3385]]
- [[_COMMUNITY_Community 3386|Community 3386]]
- [[_COMMUNITY_Community 3387|Community 3387]]
- [[_COMMUNITY_Community 3388|Community 3388]]
- [[_COMMUNITY_Community 3389|Community 3389]]
- [[_COMMUNITY_Community 3390|Community 3390]]
- [[_COMMUNITY_Community 3391|Community 3391]]
- [[_COMMUNITY_Community 3392|Community 3392]]
- [[_COMMUNITY_Community 3393|Community 3393]]
- [[_COMMUNITY_Community 3394|Community 3394]]
- [[_COMMUNITY_Community 3395|Community 3395]]
- [[_COMMUNITY_Community 3396|Community 3396]]
- [[_COMMUNITY_Community 3397|Community 3397]]
- [[_COMMUNITY_Community 3398|Community 3398]]
- [[_COMMUNITY_Community 3399|Community 3399]]
- [[_COMMUNITY_Community 3400|Community 3400]]
- [[_COMMUNITY_Community 3401|Community 3401]]
- [[_COMMUNITY_Community 3402|Community 3402]]
- [[_COMMUNITY_Community 3403|Community 3403]]
- [[_COMMUNITY_Community 3404|Community 3404]]
- [[_COMMUNITY_Community 3405|Community 3405]]
- [[_COMMUNITY_Community 3406|Community 3406]]
- [[_COMMUNITY_Community 3407|Community 3407]]
- [[_COMMUNITY_Community 3408|Community 3408]]
- [[_COMMUNITY_Community 3409|Community 3409]]
- [[_COMMUNITY_Community 3410|Community 3410]]
- [[_COMMUNITY_Community 3411|Community 3411]]
- [[_COMMUNITY_Community 3412|Community 3412]]
- [[_COMMUNITY_Community 3413|Community 3413]]
- [[_COMMUNITY_Community 3414|Community 3414]]
- [[_COMMUNITY_Community 3415|Community 3415]]
- [[_COMMUNITY_Community 3416|Community 3416]]
- [[_COMMUNITY_Community 3417|Community 3417]]
- [[_COMMUNITY_Community 3418|Community 3418]]
- [[_COMMUNITY_Community 3419|Community 3419]]
- [[_COMMUNITY_Community 3420|Community 3420]]
- [[_COMMUNITY_Community 3421|Community 3421]]
- [[_COMMUNITY_Community 3422|Community 3422]]
- [[_COMMUNITY_Community 3423|Community 3423]]
- [[_COMMUNITY_Community 3424|Community 3424]]
- [[_COMMUNITY_Community 3425|Community 3425]]
- [[_COMMUNITY_Community 3426|Community 3426]]
- [[_COMMUNITY_Community 3427|Community 3427]]
- [[_COMMUNITY_Community 3428|Community 3428]]
- [[_COMMUNITY_Community 3429|Community 3429]]
- [[_COMMUNITY_Community 3430|Community 3430]]
- [[_COMMUNITY_Community 3431|Community 3431]]
- [[_COMMUNITY_Community 3432|Community 3432]]
- [[_COMMUNITY_Community 3433|Community 3433]]
- [[_COMMUNITY_Community 3434|Community 3434]]
- [[_COMMUNITY_Community 3435|Community 3435]]
- [[_COMMUNITY_Community 3436|Community 3436]]
- [[_COMMUNITY_Community 3437|Community 3437]]
- [[_COMMUNITY_Community 3438|Community 3438]]
- [[_COMMUNITY_Community 3439|Community 3439]]
- [[_COMMUNITY_Community 3440|Community 3440]]
- [[_COMMUNITY_Community 3441|Community 3441]]
- [[_COMMUNITY_Community 3442|Community 3442]]
- [[_COMMUNITY_Community 3443|Community 3443]]
- [[_COMMUNITY_Community 3444|Community 3444]]
- [[_COMMUNITY_Community 3445|Community 3445]]
- [[_COMMUNITY_Community 3446|Community 3446]]
- [[_COMMUNITY_Community 3447|Community 3447]]
- [[_COMMUNITY_Community 3448|Community 3448]]
- [[_COMMUNITY_Community 3449|Community 3449]]
- [[_COMMUNITY_Community 3450|Community 3450]]
- [[_COMMUNITY_Community 3451|Community 3451]]
- [[_COMMUNITY_Community 3452|Community 3452]]
- [[_COMMUNITY_Community 3453|Community 3453]]
- [[_COMMUNITY_Community 3454|Community 3454]]
- [[_COMMUNITY_Community 3455|Community 3455]]
- [[_COMMUNITY_Community 3456|Community 3456]]
- [[_COMMUNITY_Community 3457|Community 3457]]
- [[_COMMUNITY_Community 3458|Community 3458]]
- [[_COMMUNITY_Community 3459|Community 3459]]
- [[_COMMUNITY_Community 3460|Community 3460]]
- [[_COMMUNITY_Community 3461|Community 3461]]
- [[_COMMUNITY_Community 3462|Community 3462]]
- [[_COMMUNITY_Community 3463|Community 3463]]
- [[_COMMUNITY_Community 3464|Community 3464]]
- [[_COMMUNITY_Community 3465|Community 3465]]
- [[_COMMUNITY_Community 3466|Community 3466]]
- [[_COMMUNITY_Community 3467|Community 3467]]
- [[_COMMUNITY_Community 3468|Community 3468]]
- [[_COMMUNITY_Community 3469|Community 3469]]
- [[_COMMUNITY_Community 3470|Community 3470]]
- [[_COMMUNITY_Community 3471|Community 3471]]
- [[_COMMUNITY_Community 3472|Community 3472]]
- [[_COMMUNITY_Community 3473|Community 3473]]
- [[_COMMUNITY_Community 3474|Community 3474]]
- [[_COMMUNITY_Community 3475|Community 3475]]
- [[_COMMUNITY_Community 3476|Community 3476]]
- [[_COMMUNITY_Community 3477|Community 3477]]
- [[_COMMUNITY_Community 3478|Community 3478]]
- [[_COMMUNITY_Community 3479|Community 3479]]
- [[_COMMUNITY_Community 3480|Community 3480]]
- [[_COMMUNITY_Community 3481|Community 3481]]
- [[_COMMUNITY_Community 3482|Community 3482]]
- [[_COMMUNITY_Community 3483|Community 3483]]
- [[_COMMUNITY_Community 3484|Community 3484]]
- [[_COMMUNITY_Community 3485|Community 3485]]
- [[_COMMUNITY_Community 3486|Community 3486]]
- [[_COMMUNITY_Community 3487|Community 3487]]
- [[_COMMUNITY_Community 3488|Community 3488]]
- [[_COMMUNITY_Community 3489|Community 3489]]
- [[_COMMUNITY_Community 3490|Community 3490]]
- [[_COMMUNITY_Community 3491|Community 3491]]
- [[_COMMUNITY_Community 3492|Community 3492]]
- [[_COMMUNITY_Community 3493|Community 3493]]
- [[_COMMUNITY_Community 3494|Community 3494]]
- [[_COMMUNITY_Community 3495|Community 3495]]
- [[_COMMUNITY_Community 3496|Community 3496]]
- [[_COMMUNITY_Community 3497|Community 3497]]
- [[_COMMUNITY_Community 3498|Community 3498]]
- [[_COMMUNITY_Community 3499|Community 3499]]
- [[_COMMUNITY_Community 3500|Community 3500]]
- [[_COMMUNITY_Community 3501|Community 3501]]
- [[_COMMUNITY_Community 3502|Community 3502]]
- [[_COMMUNITY_Community 3503|Community 3503]]
- [[_COMMUNITY_Community 3504|Community 3504]]
- [[_COMMUNITY_Community 3505|Community 3505]]
- [[_COMMUNITY_Community 3506|Community 3506]]
- [[_COMMUNITY_Community 3507|Community 3507]]
- [[_COMMUNITY_Community 3508|Community 3508]]
- [[_COMMUNITY_Community 3509|Community 3509]]
- [[_COMMUNITY_Community 3510|Community 3510]]
- [[_COMMUNITY_Community 3511|Community 3511]]
- [[_COMMUNITY_Community 3512|Community 3512]]
- [[_COMMUNITY_Community 3513|Community 3513]]
- [[_COMMUNITY_Community 3514|Community 3514]]
- [[_COMMUNITY_Community 3515|Community 3515]]
- [[_COMMUNITY_Community 3516|Community 3516]]
- [[_COMMUNITY_Community 3517|Community 3517]]
- [[_COMMUNITY_Community 3518|Community 3518]]
- [[_COMMUNITY_Community 3519|Community 3519]]
- [[_COMMUNITY_Community 3520|Community 3520]]
- [[_COMMUNITY_Community 3521|Community 3521]]
- [[_COMMUNITY_Community 3522|Community 3522]]
- [[_COMMUNITY_Community 3523|Community 3523]]
- [[_COMMUNITY_Community 3524|Community 3524]]
- [[_COMMUNITY_Community 3525|Community 3525]]
- [[_COMMUNITY_Community 3526|Community 3526]]
- [[_COMMUNITY_Community 3527|Community 3527]]
- [[_COMMUNITY_Community 3528|Community 3528]]
- [[_COMMUNITY_Community 3529|Community 3529]]
- [[_COMMUNITY_Community 3530|Community 3530]]
- [[_COMMUNITY_Community 3531|Community 3531]]
- [[_COMMUNITY_Community 3532|Community 3532]]
- [[_COMMUNITY_Community 3533|Community 3533]]
- [[_COMMUNITY_Community 3534|Community 3534]]
- [[_COMMUNITY_Community 3535|Community 3535]]
- [[_COMMUNITY_Community 3536|Community 3536]]
- [[_COMMUNITY_Community 3537|Community 3537]]
- [[_COMMUNITY_Community 3538|Community 3538]]
- [[_COMMUNITY_Community 3539|Community 3539]]
- [[_COMMUNITY_Community 3540|Community 3540]]
- [[_COMMUNITY_Community 3541|Community 3541]]
- [[_COMMUNITY_Community 3542|Community 3542]]
- [[_COMMUNITY_Community 3543|Community 3543]]
- [[_COMMUNITY_Community 3544|Community 3544]]
- [[_COMMUNITY_Community 3545|Community 3545]]
- [[_COMMUNITY_Community 3546|Community 3546]]
- [[_COMMUNITY_Community 3547|Community 3547]]
- [[_COMMUNITY_Community 3548|Community 3548]]
- [[_COMMUNITY_Community 3549|Community 3549]]
- [[_COMMUNITY_Community 3550|Community 3550]]
- [[_COMMUNITY_Community 3551|Community 3551]]
- [[_COMMUNITY_Community 3552|Community 3552]]
- [[_COMMUNITY_Community 3553|Community 3553]]
- [[_COMMUNITY_Community 3554|Community 3554]]
- [[_COMMUNITY_Community 3555|Community 3555]]
- [[_COMMUNITY_Community 3556|Community 3556]]
- [[_COMMUNITY_Community 3557|Community 3557]]
- [[_COMMUNITY_Community 3558|Community 3558]]
- [[_COMMUNITY_Community 3559|Community 3559]]
- [[_COMMUNITY_Community 3560|Community 3560]]
- [[_COMMUNITY_Community 3561|Community 3561]]
- [[_COMMUNITY_Community 3562|Community 3562]]
- [[_COMMUNITY_Community 3563|Community 3563]]
- [[_COMMUNITY_Community 3564|Community 3564]]
- [[_COMMUNITY_Community 3565|Community 3565]]
- [[_COMMUNITY_Community 3566|Community 3566]]
- [[_COMMUNITY_Community 3567|Community 3567]]
- [[_COMMUNITY_Community 3568|Community 3568]]
- [[_COMMUNITY_Community 3569|Community 3569]]
- [[_COMMUNITY_Community 3570|Community 3570]]
- [[_COMMUNITY_Community 3571|Community 3571]]
- [[_COMMUNITY_Community 3572|Community 3572]]
- [[_COMMUNITY_Community 3573|Community 3573]]
- [[_COMMUNITY_Community 3574|Community 3574]]
- [[_COMMUNITY_Community 3575|Community 3575]]
- [[_COMMUNITY_Community 3576|Community 3576]]
- [[_COMMUNITY_Community 3577|Community 3577]]
- [[_COMMUNITY_Community 3578|Community 3578]]
- [[_COMMUNITY_Community 3579|Community 3579]]
- [[_COMMUNITY_Community 3580|Community 3580]]
- [[_COMMUNITY_Community 3581|Community 3581]]
- [[_COMMUNITY_Community 3582|Community 3582]]
- [[_COMMUNITY_Community 3583|Community 3583]]
- [[_COMMUNITY_Community 3584|Community 3584]]
- [[_COMMUNITY_Community 3585|Community 3585]]
- [[_COMMUNITY_Community 3586|Community 3586]]
- [[_COMMUNITY_Community 3587|Community 3587]]
- [[_COMMUNITY_Community 3588|Community 3588]]
- [[_COMMUNITY_Community 3589|Community 3589]]
- [[_COMMUNITY_Community 3590|Community 3590]]
- [[_COMMUNITY_Community 3591|Community 3591]]
- [[_COMMUNITY_Community 3592|Community 3592]]
- [[_COMMUNITY_Community 3593|Community 3593]]
- [[_COMMUNITY_Community 3594|Community 3594]]
- [[_COMMUNITY_Community 3595|Community 3595]]
- [[_COMMUNITY_Community 3596|Community 3596]]
- [[_COMMUNITY_Community 3597|Community 3597]]
- [[_COMMUNITY_Community 3598|Community 3598]]
- [[_COMMUNITY_Community 3599|Community 3599]]
- [[_COMMUNITY_Community 3600|Community 3600]]
- [[_COMMUNITY_Community 3601|Community 3601]]
- [[_COMMUNITY_Community 3602|Community 3602]]
- [[_COMMUNITY_Community 3603|Community 3603]]
- [[_COMMUNITY_Community 3604|Community 3604]]
- [[_COMMUNITY_Community 3605|Community 3605]]
- [[_COMMUNITY_Community 3606|Community 3606]]
- [[_COMMUNITY_Community 3607|Community 3607]]
- [[_COMMUNITY_Community 3608|Community 3608]]
- [[_COMMUNITY_Community 3609|Community 3609]]
- [[_COMMUNITY_Community 3610|Community 3610]]
- [[_COMMUNITY_Community 3611|Community 3611]]
- [[_COMMUNITY_Community 3612|Community 3612]]
- [[_COMMUNITY_Community 3613|Community 3613]]
- [[_COMMUNITY_Community 3614|Community 3614]]
- [[_COMMUNITY_Community 3615|Community 3615]]
- [[_COMMUNITY_Community 3616|Community 3616]]
- [[_COMMUNITY_Community 3617|Community 3617]]
- [[_COMMUNITY_Community 3618|Community 3618]]
- [[_COMMUNITY_Community 3619|Community 3619]]
- [[_COMMUNITY_Community 3620|Community 3620]]
- [[_COMMUNITY_Community 3621|Community 3621]]
- [[_COMMUNITY_Community 3622|Community 3622]]
- [[_COMMUNITY_Community 3623|Community 3623]]
- [[_COMMUNITY_Community 3624|Community 3624]]
- [[_COMMUNITY_Community 3625|Community 3625]]
- [[_COMMUNITY_Community 3626|Community 3626]]
- [[_COMMUNITY_Community 3627|Community 3627]]
- [[_COMMUNITY_Community 3628|Community 3628]]
- [[_COMMUNITY_Community 3629|Community 3629]]
- [[_COMMUNITY_Community 3630|Community 3630]]
- [[_COMMUNITY_Community 3631|Community 3631]]
- [[_COMMUNITY_Community 3632|Community 3632]]
- [[_COMMUNITY_Community 3633|Community 3633]]
- [[_COMMUNITY_Community 3634|Community 3634]]
- [[_COMMUNITY_Community 3635|Community 3635]]
- [[_COMMUNITY_Community 3636|Community 3636]]
- [[_COMMUNITY_Community 3637|Community 3637]]
- [[_COMMUNITY_Community 3638|Community 3638]]
- [[_COMMUNITY_Community 3639|Community 3639]]

## God Nodes (most connected - your core abstractions)
1. `9. Architecture Decisions` - 260 edges
2. `NewKnowledgeGraph()` - 157 edges
3. `DefaultTree()` - 153 edges
4. `NewRequest()` - 152 edges
5. `registerMCPTools()` - 133 edges
6. `newDefaultConfig()` - 100 edges
7. `RED phase` - 91 edges
8. `GREEN phase` - 91 edges
9. `OpenPrograms()` - 89 edges
10. `RegisterAction()` - 84 edges

## Surprising Connections (you probably didn't know these)
- `TestHandleHealth_UsesDashboardHealthJSON()` --calls--> `NewRequest()`  [INFERRED]
  cmd/bt-dashboard/main_test.go → internal/hitl/request.go
- `TestDashboardAPIRoutesHaveOpenAPICoverage()` --calls--> `DashboardRoutes()`  [INFERRED]
  cmd/bt-dashboard/main_test.go → internal/api/openapi.go
- `TestGardenerRollbackTool_CallRestoresSnapshot()` --calls--> `SnapshotTree()`  [INFERRED]
  cmd/bt-gardener/main_test.go → internal/evolution/quality_gate.go
- `finalizeAutomationApproval()` --calls--> `Warn()`  [INFERRED]
  cmd/bt-agent/autopilot.go → internal/engine/app_logger.go
- `handleSprintExecute()` --calls--> `ResolveAgentName()`  [INFERRED]
  cmd/bt-dashboard/main.go → internal/dashboard/executor.go

## Communities (7317 total, 1223 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.01
Nodes (260): 9. Architecture Decisions, ADR-001: Behavior Trees as Core Execution Model, ADR-002: MCP as External Interface, ADR-003: File-Based Persistence over SQL, ADR-004: YAML-Defined Agent Platform, ADR-005: Stockfish-Adapted Evolution Engine, ADR-006: ChainAction — LLM Integration via BT Nodes, ADR-007: Reliability Architecture — Circuit Breakers, Retry, DLQ (+252 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (119): evaluatorServer, TestIntegration_EdgeCases(), TestIntegration_MutationOperators(), TestIntegration_QualityGatesFullFlow(), TestIntegration_ReflectionAndPersistence(), TestContainsWord_EmptySource(), TestContainsWord_EmptyWord(), TestContainsWord_ExactMatch() (+111 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (17): agentTestMockLLM, chainMockLLM, countedErrorMockLLM, ctxAwareFusionMockLLM, emptyDecomposeMockLLM, errorMockLLM, flakyDecomposeMockLLM, flakyOnceMockLLM (+9 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (32): expandChainStateTemplates(), recordChainHistory(), DemoChainTree(), TestChainAction_DemoTree(), TestExpandChainStateTemplates_Basic(), TestExpandChainStateTemplates_MissingKey(), TestExpandChainStateTemplates_MultipleSubstitutions(), TestExpandChainStateTemplates_NilChainState() (+24 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (71): newDefaultConfig(), TestCheckRuntime_AllEmptyPaths(), TestCheckRuntime_AllOk(), TestCheckRuntime_ConfigFileNotFound(), TestCheckRuntime_CreatedDir_Valid(), TestCheckRuntime_DeepSeekEmptyHost(), TestCheckRuntime_DeepSeekNoOllamaCheck(), TestCheckRuntime_DeepSeekReachable() (+63 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (98): handleLogout(), auditKey, bearerPrincipalKey, IPFilter, IPFilterMode, RateLimiter, requestIDKey, TestReviewChunkedBodyLimit() (+90 more)

### Community 6 - "Community 6"
Cohesion: 0.03
Nodes (79): TestMatchBlockPattern(), newProductionPopulation(), TestNewProductionPopulationAttachesSpecialists(), AntiPattern, countTreeNodes(), TestCrossover_Single(), TestDTAnalyzer_New(), TestExpertKnowledge_DetectAntiPatterns() (+71 more)

### Community 7 - "Community 7"
Cohesion: 0.04
Nodes (83): TestKnowledgeGraphRegistersAllDomainTrees(), TestKnowledgeGraphRegistersResolverSpecialCaseTrees(), TestAutoCreateTree_ConfidenceThreshold(), TestAutoCreateTree_Existing(), TestAutoCreateTree_New(), TestBreed(), TestBreed_FromArchetype(), TestBreed_FromArchetype_Fallback() (+75 more)

### Community 8 - "Community 8"
Cohesion: 0.05
Nodes (84): mcpErr(), registerBlockTools(), resolveStrategyTree(), blockOnHeldServerLock(), TestBTBlocksComposeRejectsUnknownStrategyTree(), TestBTBlocksComposeSaveGatesActivation(), TestBTBlocksComposeSaveHoldsServerLock(), TestBTHITLComposeTaskSaveHoldsServerLock() (+76 more)

### Community 9 - "Community 9"
Cohesion: 0.04
Nodes (11): code:block3 (FILES_CHANGED:), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN phase complete, GREEN_RESULTS, NOTES, RED phase (+3 more)

### Community 10 - "Community 10"
Cohesion: 0.05
Nodes (70): AgentBus, AgentEvent, InitAgentBus(), PublishHealthAlert(), PublishServiceDown(), TestAgentBus_AllSubscribersReceiveAll(), TestAgentBus_CloseSubscriberChannelsClosed(), TestAgentBus_ConcurrentPublish() (+62 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (65): agentExecuteResult(), buildDashboardKnowledgeGraph(), dashboardAPIKey(), dashboardCSRFMiddleware(), dashboardLocalAgentResult(), decodeMutation(), getHomeDir(), handleAgentCreate() (+57 more)

### Community 12 - "Community 12"
Cohesion: 0.03
Nodes (53): init(), registerGoDevActions(), containsAnyStr(), findChildByName(), TestContainsAnyLower_BasicMatch(), TestContainsAnyLower_CaseInsensitive(), TestContainsAnyLower_EmptyInput(), TestContainsAnyLower_MultipleKeywords() (+45 more)

### Community 13 - "Community 13"
Cohesion: 0.05
Nodes (76): RealLLM(), TestTauBenchBuiltinRetailAndDefaultEntries(), BuiltinBFCLV3(), EvaluateBFCLV3(), minInt(), TestBFCLV3_LongContext(), TestBFCLV3_MultiTurn_Basic(), TestBFCLV3_MultiTurn_Composite() (+68 more)

### Community 14 - "Community 14"
Cohesion: 0.04
Nodes (63): ApplyJitter(), DecorrelatedJitter(), DefaultRetryPolicy(), EqualJitter(), FullJitter(), GetErrorContext(), NewErrorContext(), TestApplyJitter_AllStrategiesDeterministicRange() (+55 more)

### Community 15 - "Community 15"
Cohesion: 0.03
Nodes (44): cacheCheckAction(), cacheResultAction(), execLLMCallAction(), execRefineAction(), generatePlanAction(), knowledgeQueryAction(), NewEngine(), reflectOnOutcomeAction() (+36 more)

### Community 16 - "Community 16"
Cohesion: 0.07
Nodes (59): defaultPRShepherdDeps(), fleetPRBranch(), fleetPRTitleAndBody(), githubTokenFromCredentialStore(), init(), loadPRShepherdState(), newGitHubPRClientFromEnv(), parseGitHubRemote() (+51 more)

### Community 17 - "Community 17"
Cohesion: 0.06
Nodes (71): TestSet_ByteLimitTooLargeStillErrors(), TestSet_EvictsOldestOnEntryLimit(), TestSet_EvictsOnByteLimit(), TestSet_NoEvictKeepsStrictLimit(), TestSet_UpdateDoesNotEvictAtEntryLimit(), NewManager(), TestNotebookLMActions_SkipWhileQuotaCached(), loadGoapChargeStampsDurable() (+63 more)

### Community 18 - "Community 18"
Cohesion: 0.04
Nodes (60): TestArc42SectionsAgainstRealRepoDocs(), TestNewFileReadTool_Structure(), TestNewFileWriteTool_Structure(), TestNewGoBuildTool_Structure(), TestNewGoTestTool_Structure(), TestNewGoVetTool_Structure(), TestNewShellExecTool_Structure(), TestNewWebSearchTool_Structure() (+52 more)

### Community 19 - "Community 19"
Cohesion: 0.04
Nodes (61): AgentDefinition, ContentType, TestAddTag(), TestAddTag_EmptyDescription(), TestContentResponse_200(), TestContentResponse_Non200(), TestMustParseAgentDefinition_PanicsOnInvalid(), TestMustParseAgentDefinition_Valid() (+53 more)

### Community 20 - "Community 20"
Cohesion: 0.05
Nodes (71): ChainConfig, ChainKind, availableToolNames(), bbRunID(), bbTemplateDisplay(), bbTemplateValue(), buildMapReduceFallback(), buildToolList() (+63 more)

### Community 21 - "Community 21"
Cohesion: 0.05
Nodes (41): TestGoalFactory(), ChangeCallback, ConfigWatcher, NewConfigWatcher(), TestConfigWatcher_ConfigChangeWithDotEnv(), TestConfigWatcher_DotEnvChangeDoesNotTriggerOnNoChange(), TestConfigWatcher_DotEnvFileDisappears(), TestConfigWatcher_DotEnvHotReload() (+33 more)

### Community 22 - "Community 22"
Cohesion: 0.05
Nodes (72): ABDelta, ABTest, absDiff(), AgentMonitorSuite(), AlertRouterSuite(), AnnotateMetrics(), Arc42DocsyncSuite(), Arc42SeederSuite() (+64 more)

### Community 23 - "Community 23"
Cohesion: 0.05
Nodes (40): apiKey, KeyHash(), NewKeyRing(), NewKeyRotationScheduler(), sha256Hex(), TestExpireKey_NotFound(), TestExpireKey_Success(), TestExpiringKeys_EmptyRing() (+32 more)

### Community 24 - "Community 24"
Cohesion: 0.05
Nodes (72): conditionDescriptionGaps(), conditionGuardEdgeGaps(), coverageRootName(), describableDomainTrees(), edgeMetadataGaps(), findNode(), idEqualityLiterals(), nonRegistrySmokeTestableTrees() (+64 more)

### Community 25 - "Community 25"
Cohesion: 0.08
Nodes (62): TestRunSuiteIsSandboxed(), init(), loadBanditStats(), TestBanditSelector_CachesStatsAcrossTicksNoPerTickReload(), TestBanditSelector_ColdStartTriesEveryArmOnce(), TestBanditSelector_ConcurrentTicksNoLostOutcomes(), TestBanditSelector_ConcurrentTicksNoLostOutcomesEnabled(), TestBanditSelector_DisabledMatchesSelectorSemantics() (+54 more)

### Community 26 - "Community 26"
Cohesion: 0.06
Nodes (26): TestCheckConfidence_ConditionExists(), Individual, CloneMetadata(), cloneTree(), collectNodeNames(), containsCrisisReason(), Crossover(), ExperienceRetrievalHits() (+18 more)

### Community 27 - "Community 27"
Cohesion: 0.05
Nodes (55): condCase, condTest(), simpleContainsCond(), TestCondBulk_StandardConditions(), TestCondFallback_HasModelToolIssues(), TestCondFallback_HasNewContent(), TestCondFallback_HasSkillGaps(), TestCondFallback_HasStackTrace() (+47 more)

### Community 28 - "Community 28"
Cohesion: 0.04
Nodes (50): FeedbackFile(), TestLocalAgentResultNilResultAndSuccessPathsUnchanged(), TestLocalAgentResultPreservesOutcomeOnError(), attemptOutcomeError(), buildSchedulerConfig(), dlqReplayOutcomeError(), endpointsFromCards(), experienceBankDir() (+42 more)

### Community 29 - "Community 29"
Cohesion: 0.05
Nodes (56): clearSuperpowersPlanState(), buildDeterministicDesign(), buildSuperpowersFinishReport(), currentSuperpowersForEachTask(), defaultNlmGrillAuthGuard(), ensureSuperpowersForEachTaskSetup(), goapFusionApplyAlreadyLanded(), init() (+48 more)

### Community 30 - "Community 30"
Cohesion: 0.06
Nodes (63): TestArc42ReadGraphReport_UsesCanonicalGoapFusionReportPath(), TestPrioritizeGoapGoals_ConcurrentWithPersistGoapProgramAllSurvive(), appendGoapResearchGoals(), completeGoapProgramMilestone(), extractGoapResearchGoals(), fallbackGoapGoal(), goapFusionNotebookLMGoalsFromGaps(), goapResearchGapLines() (+55 more)

### Community 31 - "Community 31"
Cohesion: 0.07
Nodes (65): biddingExecutor, NewAgentRouter(), NewLocalExecutor(), NewRouterFromEndpoints(), ScoreOutcome(), TestAgentRouter_AllExecutorsFail(), TestAgentRouter_EmptyHealth(), TestAgentRouter_Executors() (+57 more)

### Community 32 - "Community 32"
Cohesion: 0.06
Nodes (54): TestPromoteRunToAgentScope(), TestReviewRunScopeReleasedAfterConsumers(), TestEngineBlackboardAppendTool(), TestEngineBlackboardToolsRoundTrip(), TestEngineExpandBBTemplate_InLLMCall(), TestEnginePrepareBlackboard_Idempotent(), NewHandle(), DefaultManager() (+46 more)

### Community 33 - "Community 33"
Cohesion: 0.07
Nodes (62): NewAgent(), goalEntry, assertError(), TestAgentActionFails(), TestAgentCallbacks(), TestAgentGetSetState(), TestAgentHistory(), TestAgentNoPlan() (+54 more)

### Community 34 - "Community 34"
Cohesion: 0.07
Nodes (63): cohensD(), DefaultMock(), fishersExact(), GoDevSuite(), minF(), QuickValidate(), RunABTest(), ScoreMutation() (+55 more)

### Community 35 - "Community 35"
Cohesion: 0.06
Nodes (52): deregisterLiveRun(), EnqueueLiveMutation(), ListLiveRuns(), LiveMutationJournal(), registerLiveRun(), TestBuildNodeCapture(), TestLiveRunEnqueueAndDrain(), TestLiveRunOpCap() (+44 more)

### Community 36 - "Community 36"
Cohesion: 0.06
Nodes (51): BlackboardDir(), CircuitBreakersFile(), DLQFile(), HistoryDir(), HomeDir(), JobsDir(), LogsDir(), MemoryDir() (+43 more)

### Community 37 - "Community 37"
Cohesion: 0.05
Nodes (40): TestBuildDashboardKnowledgeGraph_SetsExpectedDomainsAndSurfacesGaps(), runPipelineAgentStep(), AgentMetrics, AgentStats, TestBuildIdentityStampedRevisionFallback(), BuildIdentity, Counter, Gauge (+32 more)

### Community 38 - "Community 38"
Cohesion: 0.05
Nodes (55): okCommand, panicCommand, BuildParallel(), parallelCommand, ParallelMode, parallelState, intSliceFromInterface(), BuildReactiveParallel() (+47 more)

### Community 39 - "Community 39"
Cohesion: 0.06
Nodes (48): FileJobStore, NewHistory(), TestScheduler_RemoveNonexistent(), TestScheduler_UnknownAgent(), JobStore, TestFileJobStore_AllowsEmptySaveOnFreshStore(), TestFileJobStore_AllowsLegitimateEmptyAfterOwnership(), TestFileJobStore_RefusesEmptyOverwriteFromBlindProcess() (+40 more)

### Community 40 - "Community 40"
Cohesion: 0.09
Nodes (49): TestBTPG_QualityMetrics_AllDomainTrees(), TestBFCL_AllDomainTrees_Accuracy(), Arc42DocsyncTree(), TestArc42DocsyncTreeRegistered(), TestArc42DocsyncTreeShape(), Arc42SeederTree(), TestArc42SeederTreeRegistered(), TestArc42SeederTreeShape() (+41 more)

### Community 41 - "Community 41"
Cohesion: 0.08
Nodes (43): TestAllSuites_Complete(), AllSuites(), CodeReview(), Cron(), DataPipeline(), DevOps(), Evolution(), Finance() (+35 more)

### Community 42 - "Community 42"
Cohesion: 0.06
Nodes (21): code:block1 (delegate_hooks_test.go:20: AgentMemoryBaseDir zero value = "), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/benchmark	0.064s), code:go (lower := strings.ToLower(response)          // haystack lowe), code:block4 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block5 (now=14:59:00 -> 2026-08-24 15:02:00  matches=true), FILES_CHANGED, FILES_CHANGED, FILES_CHANGED (+13 more)

### Community 43 - "Community 43"
Cohesion: 0.07
Nodes (48): perTreeStats(), setupGardener(), TestBaseNodeCount(), TestBenchmarkMockIntegration(), TestEvolveTreeV2_WithRealTree(), TestMetricsTracker_BestFitnessTracking(), TestMetricsTracker_ImprovementRate(), TestMetricsTracker_LastFitnessTracking() (+40 more)

### Community 44 - "Community 44"
Cohesion: 0.08
Nodes (58): cloneTreeForGardener(), assertIslandAdoptionSkipped(), chainTree(), crisisMetricsGardener(), crisisV2Config(), eliteSeedGardener(), eliteSeedTree(), eliteSeedV2Config() (+50 more)

### Community 45 - "Community 45"
Cohesion: 0.05
Nodes (40): BuildTaskFromInputs(), ParseInputParams(), TestBuildTaskFromInputs(), TestParseInputParams(), TestValidateInputs_Default(), TestValidateInputs_File(), TestValidateInputs_JSON(), TestValidateInputs_Required() (+32 more)

### Community 46 - "Community 46"
Cohesion: 0.06
Nodes (48): Approval, mockOrch, mockTTOrch, NewWorkflow(), sortTasks(), TestApproval_Fields(), TestApproveTask(), TestExecuteSprint() (+40 more)

### Community 47 - "Community 47"
Cohesion: 0.06
Nodes (39): BTOptimizer, TestBTOptimizer_New(), collectSelectorChildConditions(), collectSelectors(), conditionOverlap(), extractCondition(), findMainSelector(), TestExtractCondition() (+31 more)

### Community 48 - "Community 48"
Cohesion: 0.05
Nodes (42): banditChildKey(), banditLockFor(), banditStatsDir(), banditStatsPath(), banditUCB1Order(), BuildBanditSelector(), recordBanditOutcome(), saveBanditStats() (+34 more)

### Community 49 - "Community 49"
Cohesion: 0.05
Nodes (41): ActionFunc, ActionProvider, TestArc42DocsyncNodesRegistered(), Blackboard, ConditionFunc, ConditionProvider, Engine, TestAssignComplexityAction_NoLLM_DefaultsMedium() (+33 more)

### Community 50 - "Community 50"
Cohesion: 0.07
Nodes (25): DefaultLLM(), attachSuperpowersLogRecorder(), attachLogHandler(), buildBaseHandler(), buildLogger(), Debug(), envLogLevel(), Error() (+17 more)

### Community 51 - "Community 51"
Cohesion: 0.09
Nodes (41): chargeGoapResearchGoalFailure(), goapAbandonedResearchGoal(), goapGoalFailureNote(), goapResearchGoalKey(), stripGoapGoalScopeSuffix(), isolateGoapProgramStore(), reloadGoalBudget(), seedGoalBudget() (+33 more)

### Community 52 - "Community 52"
Cohesion: 0.07
Nodes (43): TestDebugPhase_DryRunSkipsClaudeAndWritesMarker(), TestDebugPhases_WriteEvidenceFiles(), TestDiscardWorktree_RefusesEmptyPath(), TestDiscardWorktree_RefusesMainRepoPath(), TestDiscardWorktree_RemovesWorktreeAndDeletesBranch(), TestPushBranchAndCreatePR_InvokesGitPushAndGhPr(), withSwappedSuperpowersRunners(), init() (+35 more)

### Community 53 - "Community 53"
Cohesion: 0.08
Nodes (50): Duration(), TestDuration(), BuildCircuitBreaker(), BuildTimeout(), extractTreeType(), jaccardSimilarity(), NewExperienceBank(), bankHasEntry() (+42 more)

### Community 54 - "Community 54"
Cohesion: 0.08
Nodes (32): Collect(), indexOf(), GoapFusionPreflightNode(), TestSuperpowersRuntime_ActionsRegistered_ScheduledGoapFusionGatesClaudeImplementation(), TestSuperpowersRuntime_ActionsRegistered_ScheduledGoapFusionNotebookGuardsNonFatal(), TestSuperpowersRuntime_ActionsRegistered_ScheduledGoapFusionPreflightComposesCircuitBreaker(), TestSuperpowersRuntime_ActionsRegistered_ScheduledGoapFusionPreflightComposesCircuitPolicy(), TestSuperpowersRuntime_ActionsRegistered_ScheduledGoapFusionPreflightComposesGitRemote() (+24 more)

### Community 55 - "Community 55"
Cohesion: 0.06
Nodes (43): brokenTool, BuildChainAction(), buildChainActionFn(), generateTemplateOutput(), TestGenerateTemplateOutput_Basic(), TestGenerateTemplateOutput_NilChainState(), TestGenerateTemplateOutput_NoArc42Section(), TestGenerateTemplateOutput_TruncatedCache() (+35 more)

### Community 56 - "Community 56"
Cohesion: 0.05
Nodes (40): ErrorCategory, ClassifyError(), contextDeadlineExceeded(), ExecuteWithPolicy(), isAuthError(), isLLMError(), isNetworkError(), isRateLimitError() (+32 more)

### Community 57 - "Community 57"
Cohesion: 0.07
Nodes (34): HTTPMethod, collectViolations(), fieldPath(), findResponse(), NewRouteIndex(), pathOrRoot(), ResponseValidator(), routeKey() (+26 more)

### Community 58 - "Community 58"
Cohesion: 0.06
Nodes (38): adoptionStamp, TestAdoptDriftOnIdle_AdoptsSynchronously(), TestAdoptDriftOnIdle_SwallowsErrors(), AdoptDriftOnIdle(), adoptionStampHead(), AutoRebuildEnabled(), AutoRestartEnabled(), TestAdoptionStamps_InertUnderTestWithoutOptIn() (+30 more)

### Community 59 - "Community 59"
Cohesion: 0.07
Nodes (48): TestBuildPlannerNode_AllFail(), TestBuildPlannerNode_FailThenSuccess(), TestScoreChild_GoalAlignmentBranch(), TestScoreChild_NilChainState(), TestScoreChildren_BasicRanking(), TestValidateOutputQuality_BlankResult(), TestValidateOutputQuality_ErrorPattern(), TestValidateOutputQuality_MarkdownStructure() (+40 more)

### Community 60 - "Community 60"
Cohesion: 0.07
Nodes (44): buildGoapFusionNotebookLMQuery(), isGoapNotebookLMQuotaError(), TestIsGoapNotebookLMQuotaError(), extractConversationID(), extractGoapGoals(), extractGoapNotebookLMRecommendation(), extractJSONStringField(), extractNotebookLMAnswer() (+36 more)

### Community 61 - "Community 61"
Cohesion: 0.12
Nodes (44): handleTreeStructure(), TestRouting_Research(), TestTree_Research(), TestIntegration_AllTreesExecute(), TestIntegration_AllTreesExecute(), namedTree, allPlatformTrees(), sprintRecover() (+36 more)

### Community 62 - "Community 62"
Cohesion: 0.09
Nodes (45): buildSelfReviewPrompt(), canonicalSelfReviewSig(), defaultSelfReviewDeps(), extractSelfReviewFindingsArray(), filterAutonomousCommits(), init(), loadSelfReviewState(), normalizeSelfReviewTitle() (+37 more)

### Community 63 - "Community 63"
Cohesion: 0.1
Nodes (44): defaultGoapReviewDeps(), gatherFailureReviewBody(), gatherReviewContext(), init(), loadLastReviewedSHA(), nextNlmQuotaReset(), nlmQuotaExhausted(), nlmQuotaExhaustedUntil() (+36 more)

### Community 64 - "Community 64"
Cohesion: 0.07
Nodes (36): NewMemoryStore(), priorityWeight(), summarizeOutput(), storeForTest(), TestContextBlock_EmptyStoreReturnsEmpty(), TestContextBlock_IncludesFactsPitfallsPatterns(), TestContextBlock_SkipsNonHighPriorityItems(), TestDelete_PersistsAfterReopen() (+28 more)

### Community 65 - "Community 65"
Cohesion: 0.08
Nodes (43): NewConnPool(), NewSharedConnPool(), TestAgentRouter_PooledExecutors_LeastConnections(), TestAgentRouter_WithSharedPool(), TestConnPool_Close(), TestConnPool_CloseIdleConnections(), TestConnPool_CustomConfig(), TestConnPool_DefaultConfig() (+35 more)

### Community 66 - "Community 66"
Cohesion: 0.06
Nodes (35): ChildTick, childTickLog, collectNames(), contains(), TestBlackboard_AllFields(), TestOutcome_EmptyTaskFails(), TestOutcome_Success(), TestRouting_CodeReview() (+27 more)

### Community 67 - "Community 67"
Cohesion: 0.08
Nodes (39): NewCrisisDetector(), containsReason(), TestCrisisDetector_Detect_DeclineRunsFireStagnation(), TestCrisisDetector_Detect_DiversityCollapse(), TestCrisisDetector_Detect_DiversityZero(), TestCrisisDetector_Detect_FlatFitnessIsPlateauNotStagnation(), TestCrisisDetector_Detect_ImprovementResetsStagnation(), TestCrisisDetector_Detect_NoCrisis() (+31 more)

### Community 68 - "Community 68"
Cohesion: 0.1
Nodes (38): NewEvolvedAgent(), saveRecordWithDelay(), TestConfig_AllFields(), TestCreateAgentTool_Call_NilFactory(), TestEvolvedAgent_Run(), TestEvolvedAgent_Run_Error(), TestEvolvedAgent_Run_WithAutoEvolve(), TestEvolvedAgent_StructFields() (+30 more)

### Community 69 - "Community 69"
Cohesion: 0.08
Nodes (42): dashboardMux(), dashboardSessionAuth(), TestReviewCSRFOnlyAuthenticatedKeyExempt(), TestReviewPrivilegedDiagnosticsAuthenticated(), TestReviewSafeMethodsCannotMutate(), TestReviewRemoteExecutorProductionMiddleware(), TestSecurityDashboardConfigKeyLogin(), TestSecurityDashboardEmptyKeyRejectsSession() (+34 more)

### Community 70 - "Community 70"
Cohesion: 0.12
Nodes (37): main(), run(), Check, dependabotConfig, workflow, boolDetail(), containsExactOrKey(), errDetail() (+29 more)

### Community 71 - "Community 71"
Cohesion: 0.08
Nodes (40): TestSnapshotFilesKeepRestrictivePerms(), GateResult, NewLocalSearcher(), TestLocalSearcher_HillClimb_CrossesStepPlateau(), TestLocalSearcher_RefineGated_AcceptsImprovement(), TestLocalSearcher_RefineGated_RejectsNonImprovement(), TestMemeticEvolve_ResurrectsExtinctSpecialist(), TestQualityGate_Probe_DoesNotRecordFailures() (+32 more)

### Community 72 - "Community 72"
Cohesion: 0.07
Nodes (5): ConcurrencyLimiter, DeadLetterQueue, PriorityQueue, SchedulerState, TaskQueue

### Community 73 - "Community 73"
Cohesion: 0.08
Nodes (38): goapBacktickValueAfter(), goapFusionImplDegradedSection(), goapFusionMaterializerSnapshotsSection(), goapFusionParkedBranchesSection(), init(), loadGoapFusionReportedSnapshots(), markGoapFusionReportedSnapshots(), registerGoapFusionProductionAdditions() (+30 more)

### Community 74 - "Community 74"
Cohesion: 0.11
Nodes (43): headMilestoneAttempts(), redPassBB(), seedChargedDeliverableProgram(), seedDeliverableProgram(), stubRepoFileState(), TestPrecheck_MissingDeliverableNeitherCompletesNorKeepsEvidence(), TestRedPass_GoalWithoutTestDeliverableIsUnaffected(), TestRedPass_MissingDeliverableChargesTheAttempt() (+35 more)

### Community 75 - "Community 75"
Cohesion: 0.04
Nodes (45): code:go (package agent), code:go (package agent), code:bash (git add internal/agent/concurrency_group.go internal/agent/c), code:go (package agent), code:go (package agent), code:bash (git add internal/agent/lanes.go internal/agent/lanes_test.go), code:go (package agent), code:go (lanes      *laneState        // admission control for concur) (+37 more)

### Community 76 - "Community 76"
Cohesion: 0.08
Nodes (35): NewRunDeps(), isolateRunDepsEnv(), TestNewRunDeps_ConfigLoadFailure_FallsBackToZeroConfig(), TestNewRunDeps_PopulatesAllFields(), TestNewRunDeps_ResolveTree_MatchesDomainsResolver(), TestNewRunDeps_ResolveTreeForUser_MatchesDomainsResolver(), TestNewRunDeps_SharedReflectionsRoot(), TestGoapFusionLoopTreeIsProductionWired() (+27 more)

### Community 77 - "Community 77"
Cohesion: 0.1
Nodes (7): Manager, isPersistentScope(), Request, Status, Store, cloneRequest(), TestFileLockLegacyWaitsThroughContention()

### Community 78 - "Community 78"
Cohesion: 0.09
Nodes (31): BehavioralDescriptor, FeatureDimension, Bucket(), Descriptor(), NewMAPElitesGrid(), cellFitnesses(), insertPersistNiche(), newPersistElite() (+23 more)

### Community 79 - "Community 79"
Cohesion: 0.06
Nodes (13): Capability, Edge, evolvedFitness(), blendedSelectionFitness(), coldStartConfidence(), coldStartWeightedFitness(), structuralGate(), KnowledgeGraph (+5 more)

### Community 80 - "Community 80"
Cohesion: 0.09
Nodes (23): AgentRunner, Checkpoint, extractCycleFacts(), TestExtractCycleFacts(), truncateForLog(), RunContext, ScheduledJob, Scheduler (+15 more)

### Community 81 - "Community 81"
Cohesion: 0.08
Nodes (36): ctxAwareBlockingRunner, buildErrorHandlerPrompt(), errorHandlerCooldown(), errorHandlerEnabled(), errorHandlerMaxNodes(), errorHandlerNodeDepth(), firstTickedLeaf(), parseErrorHandlerProposal() (+28 more)

### Community 82 - "Community 82"
Cohesion: 0.09
Nodes (38): TestBuildGoalDrivenImplementationPlanIgnoresReuseNotePathsForFileScope(), TestBrainstormExpansionAcceptedWhenDeeper(), TestBrainstormNotCalledWhenPlanAlreadyFull(), TestBrainstormRejectedWhenFilesNotGoPath(), TestBrainstormRejectedWhenMalformedOrProse(), TestBrainstormRejectedWhenNotDeeper(), TestBrainstormRejectedWhenRunOmitsShort(), withBrainstorm() (+30 more)

### Community 83 - "Community 83"
Cohesion: 0.09
Nodes (37): fakeClaude, fakeCommitRunner, hookFailureClass, applySuperpowersRunFromBareRepo(), blockingMainRepoDirtySummary(), captureSuperpowersWorktreePatch(), commitAppliedSuperpowersRun(), discardSuperpowersWorktree() (+29 more)

### Community 84 - "Community 84"
Cohesion: 0.12
Nodes (39): escalateFlaggedTreeForReview(), recordUserFeedback(), registerFeedbackTools(), newFeedbackDeps(), TestBTFeedbackRegistered(), TestPersistGeneratedTreeForUser_UserWorkspaceAndFallback(), TestRecordUserFeedback_FeedsSatisfactionAndFlags(), TestRecordUserFeedback_FlaggedForReviewDoesNotReEscalateWhilePending() (+31 more)

### Community 85 - "Community 85"
Cohesion: 0.07
Nodes (26): BlockConfig, ApplyDiffMutation(), DefaultBlockConfig(), findBlockForNode(), NewMetaPromptEvolver(), NewMutationContext(), TestApplyDiffMutation(), TestBlockConfig_FilterMutations() (+18 more)

### Community 86 - "Community 86"
Cohesion: 0.07
Nodes (38): TestBuiltinSWELite_CoverageAndUniqueness(), TestMax1(), benchmarkPlanTree(), benchmarkSuccessTree(), TestBFCLV3LoadErrorsAndEmptyEvaluation(), TestBFCLV3LoadFlattenAndEvaluate(), TestBuiltinBFCLSuitesHaveStableShape(), TestBuiltinBFCLV3StableShape() (+30 more)

### Community 87 - "Community 87"
Cohesion: 0.05
Nodes (23): A2AHandoffBlock(), TestA2AHandoffBlock_BuildAndValidate(), TestA2AHandoffBlock_Structure(), builtinBlocks(), TestBuiltinBlocks_Count(), TestBuiltinBlocks_UniqueNonEmptyIDs(), DelegateBlock(), TestDelegateBlock_BuildAndValidate() (+15 more)

### Community 88 - "Community 88"
Cohesion: 0.09
Nodes (37): runFusionAction(), TestFusionResearchRoutingConditions(), TestQueryNotebookLMResearchSurfacesOnlyNewVaultNotes(), TestReportNoNewResearchSucceedsWithoutReportWrite(), TestSearchForBTPatternsOnlyReportsUnknownFindings(), TestSearchForBTPatternsQueriesArc42AnchoredResearch(), TestSearchForBTPatternsSurvivesResearchFailure(), TestWithFusionKnowledgeStubsLiveNotebookLMResearch() (+29 more)

### Community 89 - "Community 89"
Cohesion: 0.09
Nodes (36): registerA2ANodes(), TestDelegateToA2A_ContextPropagates(), TestDelegateToA2A_FnNotConfigured(), TestDelegateToA2A_FnReturnsError(), TestDelegateToA2A_MissingTaskEmptyChainState(), TestDelegateToA2A_MissingURL(), TestDelegateToA2A_RateLimitCarryoverPropagates(), TestDelegateToA2A_Success() (+28 more)

### Community 90 - "Community 90"
Cohesion: 0.09
Nodes (28): handleSecurityAudit(), CaptureAuditEvent(), CountEvents(), GlobalAuditBuffer(), NewAuditBuffer(), SetGlobalAuditBuffer(), TestAuditBuffer_Capacity(), TestAuditBuffer_EmptyBuffer() (+20 more)

### Community 91 - "Community 91"
Cohesion: 0.06
Nodes (21): AgentScore, Runner, Suite, SuiteResult, TestCase, TestKind, TestResult, collectErrors() (+13 more)

### Community 92 - "Community 92"
Cohesion: 0.08
Nodes (33): killedClaudeRunner, scriptedClaudeRunner, scriptedSuperpowersRunner, buildSuperpowersGreenPrompt(), buildSuperpowersRedPrompt(), buildSuperpowersTaskPrompt(), buildSuperpowersVerificationChecks(), buildVerifyLintFixPrompt() (+25 more)

### Community 93 - "Community 93"
Cohesion: 0.1
Nodes (34): cmdImpact(), impactedTestsForSource(), TestImpactedTestsForSource_AbsoluteSourceUnderRoot(), writeImpactCLIFixture(), impactTests(), registerImpactTools(), TestBTImpactTestsRegistered(), TestImpactTests_NormalizesAbsoluteSourceUnderRoot() (+26 more)

### Community 95 - "Community 95"
Cohesion: 0.09
Nodes (30): TestRunOnce_EmitsRunRootSpan(), TestRunJob_WebhookSpanJoinsRunTrace(), TestNestedActionSpansParentCorrectly(), TestPanickingActionRestoresTraceContextAndEndsSpan(), TestRegisteredActionEmitsSpan(), TestRegisteredConditionEmitsSpan(), withRecordingGlobalTracer(), TestTracedLLM_GenerateWithMaxTokens_ForwardsAndEmitsSpan() (+22 more)

### Community 96 - "Community 96"
Cohesion: 0.09
Nodes (32): TestArchiveAndDeleteSuperpowersBranch_LogsForceReap(), orderingSweepRunner, archiveAndDeleteSuperpowersBranch(), TestResolveSuperpowersWorktreeBaseDefault(), TestResolveSuperpowersWorktreeBaseEnvOverride(), TestShepherdFixWorktreePathUsesBase(), cleanupAppliedSuperpowersWorktree(), cleanupSuperpowersWorktree() (+24 more)

### Community 97 - "Community 97"
Cohesion: 0.09
Nodes (36): FileRead(), ShellExec(), splitPipeline(), TestFileRead_EmptyFile(), TestFileRead_NotFound(), TestFileRead_Success(), TestFileRead_Truncated(), TestShellExec_Failure() (+28 more)

### Community 99 - "Community 99"
Cohesion: 0.11
Nodes (14): TestDetermineCategory(), TestExtractKeywords(), Factory, AutoCreateTreeWith(), containsAnyStr(), determineCategory(), extractKeywords(), extractSubtree() (+6 more)

### Community 100 - "Community 100"
Cohesion: 0.11
Nodes (33): collapseToSingleLine(), scopeGoapGoalLine(), TestScopeGoapGoalLineAppendsGrepMatches(), graphifyComponentsPromptBlock(), graphifyGoalQueryTopic(), graphifyReuseTopic(), graphifyScopeGoalLine(), isGraphifyNoiseComponent() (+25 more)

### Community 101 - "Community 101"
Cohesion: 0.07
Nodes (34): stringContains(), TestBuildGoapStepPrompt(), TestPlanStepsToStrings(), TestPlanStepsToStrings_Empty(), TestWorldStateFromMap(), buildGoapStepPrompt(), getStepResults(), planStepsToStrings() (+26 more)

### Community 102 - "Community 102"
Cohesion: 0.11
Nodes (36): TestWalkValidate_DepthLimitExceeded(), TestWalkValidate_DestroySideEffectNoGate(), TestWalkValidate_ExternalSideEffectNoGate(), TestWalkValidate_HumanApprovalGateSetsFlag(), TestWalkValidate_KnownNodeTypePasses(), TestWalkValidate_MaxRetriesLimitExceeded(), TestWalkValidate_NilNode(), TestWalkValidate_NodeCountLimitExceeded() (+28 more)

### Community 103 - "Community 103"
Cohesion: 0.05
Nodes (38): Binaries, code:go (type Counter struct { ... }), code:go (type Tracer interface {), code:go (type ContentType string), code:go (type KnowledgeGraph struct { ... }), code:go (type LLM interface {), code:go (type Alert struct {), code:go (type ThinkTank struct { ... }) (+30 more)

### Community 104 - "Community 104"
Cohesion: 0.1
Nodes (29): TestComposedTaskTree_BuildExpand_EmptyTaskFails(), TestComposedTaskTree_BuildExpand_ValidTaskRuns(), TestComposedTaskTreeAgentic_SetsPlan(), InitRegistry(), ApplyHITLPolicy(), TestHumanApprovalGate_AutoApprove(), TestHumanApprovalGate_ManualApprove(), TestHumanApprovalGate_PostPhase() (+21 more)

### Community 105 - "Community 105"
Cohesion: 0.12
Nodes (32): AlertRouterTree(), Arc42Trees(), chain(), section10Quality(), section11Risks(), section12Glossary(), section1IntroGoals(), section2Constraints() (+24 more)

### Community 106 - "Community 106"
Cohesion: 0.08
Nodes (27): collectSelectorNames(), DecisionTreeStatsFile(), SelectorStatsFile(), selectorTelemetryPath(), TestRunOnceFlushesDecisionTreeTelemetry(), TestRunOnceFlushesSelectorTelemetry(), TestSelectorStatsFileSanitizesTreeID(), TestPackageTestsIsolateHomeDir() (+19 more)

### Community 107 - "Community 107"
Cohesion: 0.1
Nodes (32): callArc42Action(), isolateGoapFusionGraphReport(), TestArc42Action_DetectHardware(), TestArc42Action_DetectProcesses(), TestArc42Action_GitHistory(), TestArc42Action_ListBinaries(), TestArc42Action_ListExternalAPIs(), TestArc42Action_ListMCPTools() (+24 more)

### Community 108 - "Community 108"
Cohesion: 0.1
Nodes (35): TestApplyMutations_Batch(), TestApplyMutations_DuplicateFallbackRejected(), TestApplyMutations_NoOpDoesNotCountAsApplied(), TestApplyMutations_PromptToolIterationMutationsAreBounded(), TestCountNodes(), TestDefaultTree_Structure(), TestMutation_AddAfter(), TestMutation_AddBefore() (+27 more)

### Community 109 - "Community 109"
Cohesion: 0.08
Nodes (30): ConcurrencyLimiterStats, QueueStats, NewConcurrencyLimiter(), NewWorkerPool(), TestConcurrencyLimiter_AcquireRelease(), TestConcurrencyLimiter_Available(), TestConcurrencyLimiter_MultipleReleaseNoUnderflow(), TestConcurrencyLimiter_ReleaseWhenEmpty() (+22 more)

### Community 110 - "Community 110"
Cohesion: 0.06
Nodes (15): TestActionForName_BulkCoverage(), TestInitAction_HealthCheckAgent(), TestInitAction_MetricsCollectionAgent(), DefaultPanicHandler(), Recover(), SafeGo(), TestRecover_CatchesPanic(), TestRecover_NoPanic() (+7 more)

### Community 111 - "Community 111"
Cohesion: 0.1
Nodes (31): TestMultiNode_ConcurrentAgentExecution(), TestMultiNode_ProbeFailsOnEmptyNodeURL(), TestMultiNode_ProbeFailsOnLessThanTwoNodes(), TestMultiNode_ScalabilityStatusRouterReporting(), TestMultiNodeExecutionPipeline(), MultiNodeProbeConfig, MultiNodeProbeReport, NodeProbeStatus (+23 more)

### Community 112 - "Community 112"
Cohesion: 0.05
Nodes (36): Chunk 1: Backend Domain Models & File-Based Storage, Chunk 2: AI Intent Parsing & Page Generation (LLM + Fallback), Chunk 3: REST API Handlers & Routing Integration, Chunk 4: Page-First Web Interface & Page Template Library, Chunk 5: E2E Playwright Verification, code:go (package doormate), code:go (package doormate), code:go (package doormate) (+28 more)

### Community 113 - "Community 113"
Cohesion: 0.08
Nodes (22): auctionAnnouncement(), auctionCandidates(), AuctionWinnerName(), betterBid(), candidateContext(), candidateOverride(), cardURL(), NewPersistentAuctioneer() (+14 more)

### Community 114 - "Community 114"
Cohesion: 0.07
Nodes (23): BTAgentClient, treeSkillName(), ConfigurePlatformClient(), interpretSendResult(), NewBTAgentClient(), safetyGetMessageText(), sameOrigin(), TestInterpretSendResult_HonestlyReportsFailureStates() (+15 more)

### Community 115 - "Community 115"
Cohesion: 0.09
Nodes (25): BenchEvalFunc, countConditionsActions(), DefaultCascadeConfig(), maxTreeDepthEval(), NewCascadeEvaluator(), StructuralQuickEval(), buildDeepTree(), buildOptimalTree() (+17 more)

### Community 116 - "Community 116"
Cohesion: 0.11
Nodes (34): growthFitness(), FitnessDimension, MultiIndividual, clampScore(), countActions(), countConditions(), NewMultiFitness(), NewParetoFront() (+26 more)

### Community 117 - "Community 117"
Cohesion: 0.06
Nodes (35): code:go (package engine), code:go (// goapReviewContext is what the Claude review fallback will), code:bash (git add internal/engine/actions_goap_fusion_claude_review.go), code:go (type fakeReviewClaudeRunner struct {), code:go (// goapReviewAllowedTools keeps the review run read-only: th), code:bash (git add internal/engine/actions_goap_fusion_claude_review.go), code:go (func TestNotebookLMActions_SkipWhileQuotaCached(t *testing.T), code:go (if nlmQuotaExhausted(bb) {) (+27 more)

### Community 118 - "Community 118"
Cohesion: 0.09
Nodes (28): TestClient_AnalyzeComplexity_FallbackOnError(), TestClient_Generate_ConnectionRefused(), TestClient_GeneratePlan_FallbackOnError(), TestClient_Reflect_FallbackOnError(), TestDeepSeekClient_AnalyzeComplexity(), TestDeepSeekClient_Generate_APIError(), TestDeepSeekClient_Generate_ConnectionRefused(), TestDeepSeekClient_Generate_EmptyChoices() (+20 more)

### Community 119 - "Community 119"
Cohesion: 0.12
Nodes (30): ScoreAnnouncement(), TestExecuteAgentNameFromCtxWinsOverContextID(), TestReviewA2ACallerCancellationReachesTree(), rpcRequest, rpcResponse, SetTreeResolver(), bidArtifact(), drainExecute() (+22 more)

### Community 120 - "Community 120"
Cohesion: 0.1
Nodes (18): AuctionDelegate(), cardWithURL(), newAuctionTransport(), TestAuctionDelegate_AwardsWinnerFromCardRegistry(), TestAuctionDelegate_ChainStateCandidateOverride(), TestAuctionDelegate_DispatchRetryExhaustedFallsBack(), TestAuctionDelegate_NoCandidatesFallsBack(), TestAuctionDelegate_NoEligibleBidsFallsBack() (+10 more)

### Community 121 - "Community 121"
Cohesion: 0.12
Nodes (31): buildGrillRound1Query(), buildClaudeReviewPrompt(), TestBuildClaudeReviewPrompt_ContractMarkers(), arc42GoalsPromptBlock(), arc42ResearchTopics(), loadArc42QualityGoals(), TestArc42GoalsPromptBlockContainsEveryGoal(), TestArc42GoalsPromptBlockEmptyWhenDocMissing() (+23 more)

### Community 122 - "Community 122"
Cohesion: 0.11
Nodes (22): crowdingDistances(), NewNSGAIIPopulation(), NewNSGAIISorter(), nonDominatedSort(), frontIndexSets(), sameFronts(), TestNewNSGAIIPopulation(), TestNewNSGAIIPopulation_SeedsSpecialists() (+14 more)

### Community 123 - "Community 123"
Cohesion: 0.08
Nodes (31): handleScalability(), TestHandleScalability_ReflectsInjectedQueueAndRouter(), TestPriorityQueue_PersistAndReloadViaInterface(), TestPriorityTaskQueue_DequeueEmpty(), TestPriorityTaskQueue_EnqueueDequeue(), TestPriorityTaskQueue_Peek(), TestPriorityTaskQueue_PeekEmpty(), TestPriorityTaskQueue_PurgeViaInterface() (+23 more)

### Community 124 - "Community 124"
Cohesion: 0.1
Nodes (33): introspectionResult, DefaultOAuth2DiscoveryConfig(), DefaultOAuth2IntrospectionConfig(), DiscoverOAuth2IntrospectionConfig(), OAuth2DiscoveryValidator(), OAuth2IntrospectionValidator(), TestDefaultOAuth2DiscoveryConfig(), TestDiscoverOAuth2IntrospectionConfig_Defaults() (+25 more)

### Community 125 - "Community 125"
Cohesion: 0.1
Nodes (26): registerHITLTools(), setupHITLFinalizationTest(), TestHandleHITL_ApproveActivatesAutomation(), TestHandleHITL_ApproveResumesFeedbackEscalation(), TestHandleHITL_RejectQuarantinesAutomationTree(), encodeJSON(), finalizeHITLResolution(), HandleHITL() (+18 more)

### Community 126 - "Community 126"
Cohesion: 0.12
Nodes (20): ChildStats, NodeExecutionRecord, applyOrderToNode(), collectSelectorNames(), Entropy(), GiniImpurity(), InformationGain(), isSelectorFallback() (+12 more)

### Community 127 - "Community 127"
Cohesion: 0.1
Nodes (27): composeOrderedWithMiddle(), ComposePreset(), ComposePresetWithTools(), ComposeTaskTreeAgentic(), ComposeTaskTreeFull(), ListToolProfileBlocks(), PipelineWithToolsProfile(), profileOrDefault() (+19 more)

### Community 128 - "Community 128"
Cohesion: 0.14
Nodes (28): NewMCTSMutator(), hasNodeWithPrefix(), mctsSelectorTree(), mockFitnessEvaluator(), testBaseTree(), TestMCTSMutation_FitnessGap(), TestMCTSMutator_Backpropagate(), TestMCTSMutator_BuildMutationOps() (+20 more)

### Community 129 - "Community 129"
Cohesion: 0.12
Nodes (32): TestReviewA2AHandlerBoundsChunkedBody(), TestReviewGlobalDiscoveryAdvertisesNoUnservedRPC(), CompressionMiddleware(), gzipDecompress(), TestCompressionMiddleware_AlreadyCompressed(), TestCompressionMiddleware_BinaryContentType(), TestCompressionMiddleware_EmptyBody(), TestCompressionMiddleware_GzipRequest() (+24 more)

### Community 130 - "Community 130"
Cohesion: 0.06
Nodes (33): dart:convert, _ActivityCard, _ApproveButton, BTStudioApp, build, _buildBody, _buildOverview, Card (+25 more)

### Community 131 - "Community 131"
Cohesion: 0.06
Nodes (33): ClaudeErrorHandler Node Implementation Plan, code:go (// internal/engine/error_handler_store_test.go), code:bash (git add internal/engine/registry.go internal/engine/error_ha), code:go (// internal/engine/error_handler_claude_test.go), code:go (// internal/engine/error_handler_claude.go), code:bash (git add internal/engine/error_handler_claude.go internal/eng), code:go (// internal/engine/error_handler_node_test.go), code:go (// ClaudeErrorHandler — self-extending recovery decorator: c) (+25 more)

### Community 132 - "Community 132"
Cohesion: 0.15
Nodes (28): AllSLOMetrics(), GetSLOMetrics(), TestAllSLOMetrics(), gateObjective, DefaultValidationGateConfig(), TestValidationGate_EvidenceForOtherTreeOnly_FailsClosed(), TestValidationGate_FileEvidence_AggregatesAcrossAgents(), TestValidationGate_FileEvidence_LowSuccessRate_Rejects() (+20 more)

### Community 133 - "Community 133"
Cohesion: 0.2
Nodes (31): BuildInverter(), BuildRepeater(), BuildRunner(), BuildSucceeder(), actionChild(), decoratorsTestNode(), init(), newDecoratorsBB() (+23 more)

### Community 134 - "Community 134"
Cohesion: 0.15
Nodes (30): deriveGraphifyReuseTopic(), TestDeriveGraphifyReuseTopicPrefersConcreteTask(), deriveNotebookLMResearchQuery(), isBoilerplateResearchTopic(), loadNlmQueryCache(), loadNlmUsage(), nlmBudgetFor(), nlmExtractQuery() (+22 more)

### Community 135 - "Community 135"
Cohesion: 0.14
Nodes (26): bareApplyScriptRunner, bareSyncRunner, applySuperpowersRunToMainRepo(), assertNoForcedMasterWrite(), bareTestRun(), TestApplySuperpowersRunBareRepoDetachedHeadDoesNotRebase(), TestApplySuperpowersRunBareRepoFfRebaseAttemptsBounded(), TestApplySuperpowersRunBareRepoRebaseConflictAbortsAndParks() (+18 more)

### Community 136 - "Community 136"
Cohesion: 0.11
Nodes (27): buildGardenerConfig(), experienceBankDir(), TestBuildGardenerConfig_ExperienceBankSharedWithDaemon(), TestBuildGardenerConfig_FeedbackPersistenceArmed(), TestBuildGardenerConfig_KnowledgeGraphWired(), TestBuildGardenerConfig_SafetyComponentsWired(), TestBuildGardenerConfig_SnapshotDirCreated(), TestBuildGardenerConfig_TranspositionTableWired() (+19 more)

### Community 137 - "Community 137"
Cohesion: 0.09
Nodes (31): CosineSimilarity(), hammerRegister(), TestBuildIndex_ConcurrentRegisterNoRace(), TestBuildIndex_PanicRecovered(), TestCosineSimilarity_BothZero(), TestCosineSimilarity_DifferentLengths(), TestCosineSimilarity_Empty(), TestCosineSimilarity_Identical() (+23 more)

### Community 139 - "Community 139"
Cohesion: 0.07
Nodes (24): TestDeprecatedHandlerFunc(), TestSchemaToMap_Format(), DeprecationHeader, DeprecatedHandlerFunc(), httpStatusText(), schemaToMap(), TestDeprecatedHandlerFunc_WithSunset(), TestHTTPStatusText() (+16 more)

### Community 140 - "Community 140"
Cohesion: 0.18
Nodes (31): TestRouteBuilder_Build(), NewOpenAPIGenerator(), NewRoute(), StringSchema(), mapKeys(), TestDashboardRoutes_AllRoutesBuildWithoutPanic(), TestDashboardRoutes_AuthConsistency(), TestDashboardRoutes_GeneratesValidSpec() (+23 more)

### Community 141 - "Community 141"
Cohesion: 0.11
Nodes (28): dashboardMiddleware(), inFlightMiddleware(), finishProbe(), headerEquals(), headerPresent(), probeCSRFCookie(), ProbeDashboard(), probeSessionCookies() (+20 more)

### Community 142 - "Community 142"
Cohesion: 0.14
Nodes (22): cooldownStepIndex(), DefaultLoginThrottleConfig(), NewLoginThrottle(), stripPort(), TestCooldownStepIndex(), TestLoginThrottle_CleanupExpired(), TestLoginThrottle_ConcurrentAccess(), TestLoginThrottle_DecayWindowReset() (+14 more)

### Community 143 - "Community 143"
Cohesion: 0.06
Nodes (31): Brainstorm Grill-Driven Design-Improvement Loop — Implementation Plan, code:go (// internal/engine/superpowers_runner.go), code:go (// Package engine — grill-driven design-improvement loop act), code:bash (git add internal/engine/actions_superpowers_design_loop.go i), code:block12 (=== CLEAR DESIGN ===), code:go (const splitFakeOutput = `=== CLEAR DESIGN ===), code:go (RegisterAction("SplitDesignArtifact", func(ctx *btcore.BTCon), code:go (func parseSplitOutput(out string) (clear, followup, program ) (+23 more)

### Community 144 - "Community 144"
Cohesion: 0.1
Nodes (17): Catalog, extractYAMLField(), inferTree(), NewCatalog(), splitTags(), TestCatalog_Export(), TestCatalog_ListInstalled(), TestCatalog_ListTemplates() (+9 more)

### Community 145 - "Community 145"
Cohesion: 0.12
Nodes (21): TestNonRegistryDescriptionsHaveNoOrphans(), HermesSelfEvolutionTree(), HermesObsidianOptimizerTree(), KanbanAutoPilotTree(), KanbanBoardMonitorTree(), KanbanQATree(), KanbanRefinerTree(), KanbanTaskCreatorTree() (+13 more)

### Community 146 - "Community 146"
Cohesion: 0.24
Nodes (30): TestClaudeErrorHandler_DoesNotClobberExistingCategory(), errorHandlerSignatureFromBB(), swapErrorHandlerRunner(), TestRequestErrorHandlerProposal_RespectsCallerContext(), TestRequestErrorHandlerProposal_StampsLedgerOnEveryOutcome(), ehTestCodeFixJSON(), ehTestHandlerNode(), ehTestProposalJSON() (+22 more)

### Community 147 - "Community 147"
Cohesion: 0.15
Nodes (26): precheckGoapStaleMilestones(), reloadPrecheckMilestone(), seedPrecheckProgram(), stubRedPrecheck(), TestPrecheckGoapStaleMilestones_DefaultRunnerInertUnderTest(), TestPrecheckGoapStaleMilestones_ReleasesClaimHeldByPriorCycle(), TestPrioritizeGoapGoals_RedPrecheckCompletesStaleMilestone(), TestPrioritizeGoapGoals_RedPrecheckFailingRedClearsHypothesis() (+18 more)

### Community 148 - "Community 148"
Cohesion: 0.11
Nodes (12): FitnessFunc, firstNodeNameByType(), firstOpName(), maxInt(), MergeScoredMutations(), SelectStructuralStrategy(), TestMergeScoredMutations_OneCompetition(), MCTSMetrics (+4 more)

### Community 149 - "Community 149"
Cohesion: 0.11
Nodes (24): convertGoapChildren(), FromGoapNode(), GOAPDevOpsTree(), GOAPPlanningTree(), GOAPResearchTree(), Blackboard, BlackboardBridge, BTNodeType (+16 more)

### Community 150 - "Community 150"
Cohesion: 0.09
Nodes (12): Embedding, EmbeddingClient, DeepSeekClient, OpenAICompatClient, OpenAICompatConfig, openAICompatMessage, openAICompatRequest, openAICompatResponse (+4 more)

### Community 151 - "Community 151"
Cohesion: 0.11
Nodes (30): ClearGoapFusionStateHashes(), goapFusionCircuitBreakerVerdict(), goapFusionCircuitBreakerWindow(), goapFusionCircuitPolicyVerdict(), goapFusionCircuitPolicyVerdictWithBypass(), goapFusionImplementationGateWired(), goapFusionNoopPatchStreak(), goapFusionPreflightWired() (+22 more)

### Community 152 - "Community 152"
Cohesion: 0.09
Nodes (12): NewPageAgent(), TestAgentLLMGeneration(), TestAgentMockFallback(), TestBubbleRecommendation(), Handler, NewHandler(), TestHandleBookmark(), TestHandleIntent() (+4 more)

### Community 153 - "Community 153"
Cohesion: 0.1
Nodes (6): Task, priorityRank(), resolveHITLAudit(), TaskStore, Workflow, newTaskNonce()

### Community 154 - "Community 154"
Cohesion: 0.06
Nodes (30): code:go (// internal/engine/tree_mutation_test.go), code:go (// internal/engine/run_task_mutable.go), code:go (if bb.liveRun != nil {), code:bash (cd /home/nico/go-bt-evolve && git add internal/engine/run_ta), code:go (// internal/agent/mutated_trees_test.go), code:go (// internal/agent/mutated_trees.go), code:go (runCtx := ctx), code:go (func resolveTree(id string) *evolution.SerializableNode {) (+22 more)

### Community 155 - "Community 155"
Cohesion: 0.15
Nodes (20): extractBulletPoints(), extractFirstLine(), extractListSection(), extractSection(), findNextSection(), parseDebateTranscript(), parseProbability(), parseReport() (+12 more)

### Community 156 - "Community 156"
Cohesion: 0.1
Nodes (26): RunSuite(), RunSuiteWithLLM(), TestDetectPath_FromCurrentPath(), TestDetectPath_FromVisitedPaths(), TestDetectPath_KeywordFallback(), detectPath(), pathMatches(), TestDetectPath_CronCapacityPlanningKeywordCollision() (+18 more)

### Community 157 - "Community 157"
Cohesion: 0.11
Nodes (19): TreeStore, exportSLOMetrics(), TestExportSLOMetrics_CreatesMissingParentDirs(), TestExportSLOMetrics_RoundTrip(), LoadJSON(), SaveJSONAtomic(), SaveJSONAtomicMode(), effectiveUmask() (+11 more)

### Community 159 - "Community 159"
Cohesion: 0.11
Nodes (25): CollectBlockIDs(), FitnessRanking(), RecordTaskBlockFitness(), ScoreFromBlackboard(), TestRecordTaskBlockFitness(), BlockFitnessRanking(), BlockFitnessSnapshot(), BlockMetricsSnapshot() (+17 more)

### Community 160 - "Community 160"
Cohesion: 0.11
Nodes (24): Block, TestBlockMutationsInsert(), TestComposeTaskTreeWithStrategy(), TestPromoteSubtree(), TestWrapReliable_HasTimeoutAndFallbacks(), Category, Compose(), ComposeFromRefs() (+16 more)

### Community 161 - "Community 161"
Cohesion: 0.19
Nodes (28): readErrorHandlerJSONStrict(), TestSeedCodeFixProgram_ConcurrentWithPersistGoapProgramAllSurvive(), acquireSelfFixStoreLock(), seedCodeFixProgram(), selfFixCooldown(), selfFixDir(), selfFixEnabled(), selfFixLedgerPath() (+20 more)

### Community 162 - "Community 162"
Cohesion: 0.11
Nodes (26): gitOutput(), TestNoCommittedRootBinaries(), stripGoapGoalTransientNotes(), extractTaskID(), init(), nlmMarkResearchQueryDone(), nlmResearchQueryKeyContent(), nlmResearchQueryRecentlySeen() (+18 more)

### Community 163 - "Community 163"
Cohesion: 0.12
Nodes (25): ApplyParameters(), cholesky(), collectParams(), ExtractParameters(), invertCholesky(), multiplyCholesky(), navigateToPath(), NewCMAESOptimizer() (+17 more)

### Community 164 - "Community 164"
Cohesion: 0.07
Nodes (25): AgentEndpoint, AgentExecutor, AgentResult, CircuitBreakerOptions, CircuitSummary, DeadLetterEntry, executorFailureState, ExecutorHealthDetail (+17 more)

### Community 165 - "Community 165"
Cohesion: 0.12
Nodes (22): handleFellows(), NewMockLLM(), TestMockLLMGenerateCtxAndTimeoutDelegate(), TestMockLLMGenerateIgnoresPrompt(), TestMockLLMGeneratePrecedence(), TestMockLLMGenerateWithTimeoutDoesNotBlock(), TestMockLLMPassthroughAccessors(), TestMockLLMResponseLengths() (+14 more)

### Community 166 - "Community 166"
Cohesion: 0.07
Nodes (8): CreateAgentTool, EvolveTool, FitnessTool, GetReflectionsTool, GetTreeTool, truncateStr(), ReflectTool, RunTaskTool

### Community 167 - "Community 167"
Cohesion: 0.09
Nodes (26): applyDotEnvFiles(), applyDotEnvToConfig(), applyEnvOverrides(), Load(), LoadFile(), LoadFileWithDotEnv(), parseBool(), TestLoadFile_Basic() (+18 more)

### Community 168 - "Community 168"
Cohesion: 0.13
Nodes (21): docgenState, allSections(), buildSectionMap(), contains(), fileHash(), hashSectionSources(), isSectionDone(), main() (+13 more)

### Community 169 - "Community 169"
Cohesion: 0.09
Nodes (26): collectDiscoverIDs(), TestDiscover_RegisterNotStarvedBySlowEmbeddingCall(), TestDiscoverRelated_Bidirectional(), TestDiscoverRelated_ConnectedFrom(), TestDiscoverRelated_ConnectedTo(), TestDiscoverRelated_Deduplicates(), TestDiscoverRelated_EmptyGraph(), TestDiscoverRelated_NonExistentTree() (+18 more)

### Community 170 - "Community 170"
Cohesion: 0.16
Nodes (24): NewAuctioneer(), newDeadlineProbeTransport(), TestAuctioneer_RunAuction_AppliesDefaultDispatchDeadline(), TestAuctioneer_RunAuction_BoundsWinnerDispatchWithAnnouncementDeadline(), TestAuctioneer_RunAuction_DispatchesToWinnerAndReturnsResult(), TestAuctioneer_RunAuction_NoEligibleBidsDispatchesNothing(), TestAuctioneer_RunAuction_RejectsEmptyDescription(), TestCollectBids_AppliesDefaultDeadlineWhenAnnouncementHasNone() (+16 more)

### Community 171 - "Community 171"
Cohesion: 0.1
Nodes (20): TestDefaultRebuildTargets_PinsFullList(), buildStampLdflags(), copyFile(), DashboardRebuildTargets(), defaultRebuildBuild(), DefaultRebuildTargets(), GardenerRebuildTargets(), RebuildBinaries() (+12 more)

### Community 172 - "Community 172"
Cohesion: 0.11
Nodes (24): AllTools(), bindToolCommandCancellation(), DevTools(), DiskUsage(), HTTPGet(), MemoryUsage(), MonitorTools(), ProcessCheck() (+16 more)

### Community 173 - "Community 173"
Cohesion: 0.1
Nodes (10): TestNewLocalAgentExecutor_ThreadsContextIntoRunOnce(), TestReviewRealToolRunningCancellation(), TestReviewToolHonorsCancellation(), TestWaitForRequest_Approve(), TestWaitForRequest_Escalate(), redisID(), RedisPriorityQueue, RedisQueue (+2 more)

### Community 174 - "Community 174"
Cohesion: 0.07
Nodes (27): pushToDLQAction(), TestPushToDLQAction_StampsBuildRevision(), TestDeadLetterQueue_AutoClassify(), TestDeadLetterQueue_CategoryCounts(), TestDeadLetterQueue_CategoryCounts_Empty(), TestDeadLetterQueue_EmptyErrorNoCategory(), TestDeadLetterQueue_PersistenceRoundtrip(), TestDeadLetterQueue_PreserveCategory() (+19 more)

### Community 175 - "Community 175"
Cohesion: 0.11
Nodes (23): ClaudeRunner, CommandResult, CommandRunner, execClaudeRunner, execCommandRunner, TestSuperpowersCommandTimeout_DefaultExceedsInnerTestTimeouts(), TestSuperpowersCommandTimeout_EnvOverride(), resolvedSuperpowersClaudeEffort() (+15 more)

### Community 177 - "Community 177"
Cohesion: 0.1
Nodes (20): ChildTick, DecisionTrace, NewTraceStore(), TestExplainLastFailure_NoTrace(), TestExplainLastFailure_RendersPathFromChildTicks(), TestExplainLastFailure_TraceFound(), TestTraceStore_GetByTreeID(), TestTraceStore_GetEmpty() (+12 more)

### Community 179 - "Community 179"
Cohesion: 0.12
Nodes (22): graphReportBuiltCommit(), arc42OutputDir(), countCPUCores(), execWithTimeout(), getBoolChainState(), init(), registerArc42Nodes(), sectionFileExists() (+14 more)

### Community 180 - "Community 180"
Cohesion: 0.07
Nodes (27): Bubble System, Core User Journey, Data Model, DoorMate Page-First AI Assistant Design, Feedback Event, Follow-Up Bubbles, Generated Page, Generated Page (+19 more)

### Community 181 - "Community 181"
Cohesion: 0.12
Nodes (16): TestWorkflowApprovalWait_EscalatedIsNotApproved(), WorkflowApprovalTaskID(), WorkflowApprovalWait(), ApprovalWaitResult, Pipeline, PipelineResult, Runner, Step (+8 more)

### Community 182 - "Community 182"
Cohesion: 0.14
Nodes (21): UsersDir(), TestResolveGeneratedTree_EndToEnd(), TestResolveGeneratedTree_UserWorkspaceFallback(), automationApproved(), AutomationBlocked(), ReflectionsPath(), ResolveGeneratedTree(), ResolveGeneratedTreeForUser() (+13 more)

### Community 183 - "Community 183"
Cohesion: 0.09
Nodes (18): Config, CycleMetrics, TestAugmentWithMCTSCandidates_MergesSearchIntoOneCompetition(), maxInt(), NewRegistryWithUsers(), TestRegistry_Rescan_PicksUpTreeAddedAfterConstruction(), metricsDocument, MetricsTracker (+10 more)

### Community 184 - "Community 184"
Cohesion: 0.13
Nodes (6): TestWinnerBreakerStore_SaveLocksReadModifyWrite(), TestAgentCircuitBreakerStore_SaveLocksReadModifyWrite(), ExperienceBank, cappedCells(), TrackRecord, AcquireFileLock()

### Community 185 - "Community 185"
Cohesion: 0.1
Nodes (19): isKnownActionName(), isKnownConditionName(), TestIsKnownActionName(), TestIsKnownConditionName(), TestBuildNode_AbortOnEvent(), TestBuildNode_PlannerNode(), TestBuildNode_ReactiveParallel(), TestBuildNode_UtilitySelector() (+11 more)

### Community 186 - "Community 186"
Cohesion: 0.21
Nodes (25): acquireErrorHandlerClaudeLock(), acquireErrorHandlerFileLock(), acquireErrorHandlerStoreLock(), activeErrorHandlerExtensions(), appendErrorHandlerExtension(), errorHandlerDir(), errorHandlerExtensionsPath(), errorHandlerLedgerPath() (+17 more)

### Community 187 - "Community 187"
Cohesion: 0.11
Nodes (24): fakeGrillClaude, grillAnswerers, grillQuestion, grillResult, designBodyHash(), grillRoundHeading(), openCriticalDigest(), parseGrillQuestions() (+16 more)

### Community 188 - "Community 188"
Cohesion: 0.13
Nodes (23): arc42GuidelineFor(), classifyAffectedArc42Sections(), syncArc42Section(), syncArc42SectionsAndReadme(), syncReadme(), TestArc42GuidelineForMissingFileDegrades(), TestArc42GuidelineForSlicesOneSection(), TestClassifyAffectedArc42Sections() (+15 more)

### Community 189 - "Community 189"
Cohesion: 0.13
Nodes (24): NewDefaultCompany(), CEOTree(), CTOTree(), PMTree(), Roles(), companySummary(), TestCompanySimulation_Quarter(), TestCompanySimulation_Sprint() (+16 more)

### Community 190 - "Community 190"
Cohesion: 0.07
Nodes (26): BT Platform Video Walkthrough, Chapter 1 — Platform Health (0:00–1:00), Chapter 2 — Build and Test Confidence (1:00–2:30), Chapter 3 — Dashboard Tour (2:30–4:30), Chapter 4 — Behavior Tree Execution (4:30–6:30), Chapter 5 — Observability and Diagnostics (6:30–8:00), Chapter 6 — Security and Configuration (8:00–9:30), Chapter 7 — Scalability and Reliability (9:30–11:00) (+18 more)

### Community 191 - "Community 191"
Cohesion: 0.14
Nodes (9): code:block1 (go test ./internal/reliability ./cmd/bt-agent ./internal/age), code:block2 (ok  github.com/nico/go-bt-evolve/internal/reliability), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase (+1 more)

### Community 192 - "Community 192"
Cohesion: 0.16
Nodes (9): app, generator, buildGraph(), main(), newApp(), normalizeFlagArgs(), parseIntent(), printUsage() (+1 more)

### Community 193 - "Community 193"
Cohesion: 0.12
Nodes (16): injectPersonaContextLocked(), mineUserPatterns(), personaEmbedder(), personaError(), recordPersonaInteraction(), registerPersonaTools(), NewHabitMiner(), Interaction (+8 more)

### Community 194 - "Community 194"
Cohesion: 0.11
Nodes (14): cli(), configure(), main(), Unattended adapter for notebooklm-mcp-cli 0.10.1 (embedded in Go).  Only restore, recognized(), restore(), BaseClient, cli_main() (+6 more)

### Community 195 - "Community 195"
Cohesion: 0.1
Nodes (20): TestRunPipelineWithID_RecordsAgentMetrics(), hermesOutcome(), isAuctionShapedTask(), PickTreeForTask(), ResolveAgentName(), TestHermesOutcome(), TestPickTreeForTask_AuctionRoutingBeatsKnowledgeGraph(), TestPickTreeForTask_ConsultsKnowledgeGraphOverStaticSwitch() (+12 more)

### Community 196 - "Community 196"
Cohesion: 0.14
Nodes (15): Definition, InputSpec, Instance, OutputSpec, QualitySpec, Registry, cloneDefinition(), cloneInstance() (+7 more)

### Community 197 - "Community 197"
Cohesion: 0.1
Nodes (3): CircuitBreaker, CircuitBreakerStore, LocalExecutor

### Community 198 - "Community 198"
Cohesion: 0.13
Nodes (22): TestAnalyzeImprovementGaps_NoFabricatedEngineTestBlocker(), Analyzer, extractJSON(), NewAnalyzer(), NewAgentFactory(), TestFactory_RejectsInvalidGeneratedTreeBeforePersisting(), TestGenerator_CompilesSkillActionsToChainActions(), TestAnalyzer_EmptyResponse_Error() (+14 more)

### Community 199 - "Community 199"
Cohesion: 0.14
Nodes (25): NewNodeHeartbeat(), NewNodeHeartbeatWithCleanupInterval(), TestNodeHeartbeat_CleanupKeepsAliveNodes(), TestNodeHeartbeat_CleanupRemovesExpired(), TestNodeHeartbeat_ConcurrentAccess(), TestNodeHeartbeat_ConcurrentDeregister(), TestNodeHeartbeat_Deregister(), TestNodeHeartbeat_EmptyListAlive() (+17 more)

### Community 200 - "Community 200"
Cohesion: 0.1
Nodes (18): delegatingRunner, DelegationProvider, errInvalidProvider, fakeCodexRunner, TestDelegationBinary_Provider(), routingClaudeRunner, TestCodexDelegationWritesArtifactOptIn(), delegationBinary() (+10 more)

### Community 201 - "Community 201"
Cohesion: 0.08
Nodes (17): init(), modal, num, renderTab(), state, TAB_KEYS, renderAgents(), renderCompany() (+9 more)

### Community 202 - "Community 202"
Cohesion: 0.12
Nodes (7): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/benchmark	0.064s), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agentexec	0.040s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 203 - "Community 203"
Cohesion: 0.14
Nodes (11): code:block1 (cmd/bt-agent/main_test.go:374:8: undefined: jitteredDriftInt), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.235s), code:block3 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.2s), code:block4 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.356s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+3 more)

### Community 204 - "Community 204"
Cohesion: 0.14
Nodes (11): code:block1 (PATH=/usr/local/go/bin:$PATH /usr/local/go/bin/go test ./int), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.198s), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/agent	2.696s), FILES_CHANGED, Final Report, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+3 more)

### Community 205 - "Community 205"
Cohesion: 0.11
Nodes (6): ApprovalPolicy, TestSanitizeUserID(), Profile, SanitizeUserID(), Store, Workspace

### Community 206 - "Community 206"
Cohesion: 0.13
Nodes (25): TestNumberSchema(), ArraySchema(), BoolSchema(), IntSchema(), NumberSchema(), ObjectSchema(), TestConvenienceSchemaConstructors(), TestValidateResponse_ArrayValidation() (+17 more)

### Community 207 - "Community 207"
Cohesion: 0.12
Nodes (23): mcpDeps, addFailureContext(), benchmarkGateEvolvedWinner(), checkLLMHealth(), countTypes(), detectAntiPatternsInTree(), evolveHealthProjection(), expertArchivePath() (+15 more)

### Community 208 - "Community 208"
Cohesion: 0.09
Nodes (9): GardenerDeactivateAllTool, GardenerRecommendTool, GardenerRollbackTool, GardenerRunCycleTool, GardenerStatusTool, main(), newGardenerRunCycleTool(), truncateStr() (+1 more)

### Community 209 - "Community 209"
Cohesion: 0.1
Nodes (17): assertNoExecutePlanStubs(), TestGoapFusionLoopSeedsBeforeResearch(), TestGoapFusionLoopTree_ClaudeReviewFallback(), TestGoapFusionTreeHasResearchRouter(), containsNodeName(), TestGoapFusion_Structure(), GoapFusionTree(), GoapFusionLoopTree() (+9 more)

### Community 210 - "Community 210"
Cohesion: 0.15
Nodes (23): appendMissingRealTools(), inferToolsForTask(), init(), registerSetupActions(), TestAppendMissingRealTools_AllUnknownReturnsEmptySlice(), TestAppendMissingRealTools_NilBlackboardReturnsNil(), TestAppendMissingRealTools_SkipsEmptyExistingAndUnknownNames(), TestDiscoverAvailableTools_InitializesNilChainState() (+15 more)

### Community 211 - "Community 211"
Cohesion: 0.19
Nodes (22): islandTestPopulation(), islandTestTree(), TestExperienceBank_TransferExperiencesIsSourceTreeAware(), TestIslandModel_AddGetAndSingleIslandMigration(), TestIslandModel_DiversityEdgeCases(), TestIslandModel_DiversityStatsAndSummary(), TestIslandModel_EvolveAllEvaluatesPopulations(), TestIslandModel_LoadCapEvictionIncrementsEvictedIndividuals() (+14 more)

### Community 212 - "Community 212"
Cohesion: 0.09
Nodes (8): NewDocPlanner(), DocPlanner, DocPlannerWorldState, fakeBlackboard, TestDocPlannerAlreadyCompleteReturnsEmptyPlan(), TestNewDocPlannerProducesValidArc42Plan(), TestValidatePlanNilEmptyAndFinalStateFailure(), SectionMapping

### Community 213 - "Community 213"
Cohesion: 0.14
Nodes (17): fakeLLM, NewGoalFactory(), TestAutomationActions_ReachTaskAutomated(), TestGoalFactory_ArchetypeFallbackWhenLLMKeepsFailing(), TestGoalFactory_ArchetypeFallbackWithoutLLM(), TestGoalFactory_EmptyIntent(), TestGoalFactory_FromIntentLLM(), TestGoalFactory_FromPattern() (+9 more)

### Community 214 - "Community 214"
Cohesion: 0.12
Nodes (24): Analysis, Contradiction, CoveragePoint, TestJudge_ParsesStructuredJSON(), TestJudge_RejectsInvalidJSON(), TestSynthesize_UsesAnalysisAndResponses(), TestToolLoop_StopsAtMaxToolCalls(), ModelCaller (+16 more)

### Community 215 - "Community 215"
Cohesion: 0.28
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 216 - "Community 216"
Cohesion: 0.12
Nodes (16): code:block1 (--- FAIL: TestBuildCompositeChildren_GuardReevaluatedEachTic), code:block2 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block3 ("Should I deploy now"   -> lower="should i deploy now"   mat), code:block4 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase (+8 more)

### Community 217 - "Community 217"
Cohesion: 0.19
Nodes (22): CompilePlanToTree(), compileStep(), encodePairs(), encodePairsReadable(), executableNode(), outcomeSelector(), PlanHash(), preGate() (+14 more)

### Community 218 - "Community 218"
Cohesion: 0.08
Nodes (15): TestComputeAnalytics_BottleneckCarriesStructuredFailure(), TestComputeAnalytics_Bottlenecks(), TestComputeAnalytics_BottleneckWithoutTraceHasEmptyFailure(), TestComputeAnalytics_BottleneckWithTrace(), TestComputeAnalytics_Centrality(), TestComputeAnalytics_CoverageGapsIncludeResolverSpecialCases(), TestComputeAnalytics_CoverageGapsUseExpectedDomains(), TestComputeAnalytics_EmptyGraph() (+7 more)

### Community 219 - "Community 219"
Cohesion: 0.11
Nodes (23): outcomeScore(), TestOutcomeScore_ChainFailed(), TestOutcomeScore_ChainPanic(), TestOutcomeScore_ChainSuccess(), TestOutcomeScore_Default(), TestOutcomeScore_Failure(), TestOutcomeScore_SharedVocabulary(), TestOutcomeScore_Success() (+15 more)

### Community 220 - "Community 220"
Cohesion: 0.11
Nodes (18): CollectAgentSLOs(), deepSearchGardener(), deepSearchV2Config(), relocateDomainDescription(), TestCollectAgentSLOs_NoMemoryNoFile_ReturnsNil(), TestCollectAgentSLOs_ReadsPersistedFileEvidence(), TestEvolveTreeV2_DeepSearchMetrics_PopulatedWhenTTConfigured(), TestEvolveTreeV2_DeepSearchMetrics_ZeroWithoutTranspositionTable() (+10 more)

### Community 221 - "Community 221"
Cohesion: 0.16
Nodes (22): dataFileMetrics(), extractExistingDataPath(), init(), inspectDataSource(), registerDomainActions(), domainActionCases(), runDomainAction(), TestApplyTransform() (+14 more)

### Community 222 - "Community 222"
Cohesion: 0.18
Nodes (8): MaxDepth(), hasExecutionPathFirst(), hasUnboundedRetry(), MetaValidationDecision, MetaValidationIssue, MetaValidationReport, MetaValidator, MetaValidatorConfig

### Community 223 - "Community 223"
Cohesion: 0.14
Nodes (14): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block3 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+6 more)

### Community 224 - "Community 224"
Cohesion: 0.14
Nodes (12): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestBTEvolveBottlenecksPrioritizedAndCapped (0.00s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED phase complete (+4 more)

### Community 225 - "Community 225"
Cohesion: 0.13
Nodes (8): code:block1 (--- FAIL: TestBuildCompositeChildren_GuardReevaluatedEachTic), code:block2 (--- FAIL: TestClarifyGateBlock_LeafOrder (0.00s)), code:block3 ("Should I deploy now"   -> lower="should i deploy now"   mat), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT, Summary

### Community 226 - "Community 226"
Cohesion: 0.12
Nodes (14): tracingMiddleware(), ContextWithTraceParentHeader(), Endpoint(), InitFromEnv(), parseOTLPEndpoint(), ParseOTLPTarget(), newRecordingTracer(), TestContextWithTraceParentHeader() (+6 more)

### Community 227 - "Community 227"
Cohesion: 0.13
Nodes (9): DashboardRoutes(), TestDashboardRoutes_AgentsExecuteHasRequestBody(), TestDashboardRoutes_AllHaveOperationIDs(), TestDashboardRoutes_Completeness(), TestDashboardRoutes_OperationIDUniqueness(), TestDashboardRoutes_SwaggerRoute(), TestReviewDiagnosticsDiscoveryRequiresAuth(), TestReviewValidationCapturesCompleteBody() (+1 more)

### Community 228 - "Community 228"
Cohesion: 0.17
Nodes (19): localSearchGateArm(), localSearchGateTree(), metaValidatorWiringGardener(), TestBiasCandidatesWithExperience_NilOrEmptyBankKeepsOrder(), TestEvolveTreeV2_DeepSearchMutationRejectedByMetaValidatorNotPersisted(), TestEvolveTreeV2_DeepSearchMutationRejectedByValidationGateNotPersisted(), TestEvolveTreeV2_DeepSearchResultAppliedWhenGreedyLoopFindsNothing(), TestEvolveTreeV2_LocalSearchRefinementRespectsValidationGate() (+11 more)

### Community 229 - "Community 229"
Cohesion: 0.19
Nodes (18): FusionDeliberationTree(), collectFusionNames(), findFusionNode(), TestFusionDeliberationTree_HasFusionAndDirectPaths(), TestFusionDeliberationTree_UsesFusionChainAction(), NewAction(), NewAdapt(), NewChainAction() (+10 more)

### Community 230 - "Community 230"
Cohesion: 0.13
Nodes (3): goalHeap, GoalQueue, entryLess()

### Community 231 - "Community 231"
Cohesion: 0.12
Nodes (10): ContentItem, InputSchema, sanitizeArg(), Message, Property, RPCError, Server, ToolDef (+2 more)

### Community 232 - "Community 232"
Cohesion: 0.17
Nodes (20): evidenceVerificationReport, defaultStorageDir(), encodeJSON(), main(), run(), contains(), mustParseTime(), TestDefaultStorageDirUsesEnv() (+12 more)

### Community 233 - "Community 233"
Cohesion: 0.13
Nodes (7): BuildMutationPrompt(), EnsembleConfig, EnsembleStats, EvolutionContext, ModelEnsemble, ModelRole, PriorSolution

### Community 234 - "Community 234"
Cohesion: 0.16
Nodes (14): GoalArchetype, GoalFactory, clamp01(), containsAny(), DefaultInitialState(), DetectArchetype(), extractJSONObject(), goalSlug() (+6 more)

### Community 235 - "Community 235"
Cohesion: 0.13
Nodes (18): DiversityMetrics, EvolutionParameters, FitnessMetrics, BuildPopulationState(), BuildPopulationStateWithGrid(), clamp01(), clampMutationRate(), DefaultPhaseRanges() (+10 more)

### Community 236 - "Community 236"
Cohesion: 0.12
Nodes (15): agentNameFromBB(), init(), registerDelegateNodes(), init(), registerAuctionDelegateNode(), registerIsAuctionTaskCondition(), init(), init() (+7 more)

### Community 237 - "Community 237"
Cohesion: 0.12
Nodes (21): TestBuildChainActionFn_LlmCallNilLLM(), TestBuildChainActionFn_UnknownChainType(), TestComputeSubtreeMetrics_MixedChildren(), TestComputeSubtreeMetrics_NilInput(), TestComputeSubtreeMetrics_UpdatesMaxValues(), TestComputeTreeMetrics_MaxRetriesAndTimeout(), TestComputeTreeMetrics_NestedChildren(), TestComputeTreeMetrics_ParallelWidth() (+13 more)

### Community 238 - "Community 238"
Cohesion: 0.13
Nodes (15): fakeCaller, flakyCaller, remainingDeadline(), TestJudge_RetriesTransientFailure(), TestRun_JudgeAndSynthesizeGetOwnTimeoutBudget(), TestRunPanel_CircuitBreakerOpensAfterConsecutiveFailures(), TestRunPanel_FailsWhenAllModelsFail(), TestRunPanel_PreservesPerModelErrors() (+7 more)

### Community 239 - "Community 239"
Cohesion: 0.11
Nodes (20): envBool(), hasExplicitField(), mergeFileConfig(), TestEnvBool(), marshalConfig(), TestDiff_DifferentValues(), TestDiff_EmptyConfigs(), TestEnvBool_DefaultValue() (+12 more)

### Community 240 - "Community 240"
Cohesion: 0.17
Nodes (13): TestClient_AnalyzeComplexity_High(), TestClient_Reflect_FallbackSections(), TestClient_Reflect_OnlyWentWell(), fakeSpan, fakeTracer, mockOllamaServer(), newTestClient(), TestClient_AnalyzeComplexity() (+5 more)

### Community 241 - "Community 241"
Cohesion: 0.17
Nodes (11): TestLocalSearchMutableParamsAndTabu(), extractMutableParams(), getFloatMeta(), setFloatMeta(), TestExtractMutableParams_CoversStructFields(), toFloat64(), LocalSearcher, LocalSearchStrategy (+3 more)

### Community 242 - "Community 242"
Cohesion: 0.15
Nodes (15): allDigits(), consecutiveFailures(), diagnoseFailureMode(), getLatestTimestamp(), groupByTreeName(), inferAgentName(), init(), registerBTManagerActions() (+7 more)

### Community 243 - "Community 243"
Cohesion: 0.22
Nodes (19): NewRedisPriorityQueue(), NewRedisPriorityQueueFromClient(), NewRedisQueue(), NewRedisQueueFromClient(), testRedisAddr(), testRedisClient(), TestRedisPriorityQueue_DefaultKey(), TestRedisPriorityQueue_DequeueOrder() (+11 more)

### Community 244 - "Community 244"
Cohesion: 0.14
Nodes (14): langAgentServer, main(), newLangAgentServer(), newTestServer(), seedRecords(), TestHandleEvolve_DefaultTreeHasNoMatchingTargetsSoNothingApplies(), TestHandleEvolve_TreeWithAnalyzeTaskGetsWrapRetryApplied(), TestHandleFitness() (+6 more)

### Community 245 - "Community 245"
Cohesion: 0.18
Nodes (7): code:block1 (cmd/bt-dashboard/main_test.go:116:9: got.RequeuedAt undefine), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.025s), GREEN phase, GREEN phase complete, RED phase, RED phase complete, Summary

### Community 246 - "Community 246"
Cohesion: 0.13
Nodes (15): code:block1 (--- FAIL: TestAllDomainTreeConditionsHaveDescriptions (0.00s), code:block2 (--- FAIL: TestEveryResolverSelectableTreeIsDescribable/godev), code:block3 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+7 more)

### Community 248 - "Community 248"
Cohesion: 0.16
Nodes (18): TestEligibleBidders(), TestEligibleBidders_NoRequiredTagsMatchesAll(), BuildCardRegistry(), capabilityActionTags(), cardCoversTags(), ConvertToAgentCard(), EligibleBidders(), legacyTreeTags() (+10 more)

### Community 249 - "Community 249"
Cohesion: 0.12
Nodes (19): NewAgentCircuitBreakerStore(), TestAgentCircuitBreakerStore_LoadHalfOpenBecomesOpen(), TestAgentCircuitBreakerStore_LoadSkipsWinnerKeys(), TestAgentCircuitBreakerStore_SavePreservesForeignWinnerKeys(), TestRunJob_MissingAgentDoesNotLeakBreakerProbe(), TestAgentCircuitBreakerStore_Allowed(), TestAgentCircuitBreakerStore_Get_Creates(), TestAgentCircuitBreakerStore_GetReturnsReliabilityCircuitBreaker() (+11 more)

### Community 250 - "Community 250"
Cohesion: 0.12
Nodes (6): runDomainCondCases(), TestConditionsDomain_LowercasedTaskConditions(), TestConditionsDomain_ResultKeywordConditions(), TestConditionsDomain_TaskKeywordConditions(), TestConditionsDomain_TaskKeywordConditions_CaseInsensitive(), domainCondCase

### Community 251 - "Community 251"
Cohesion: 0.16
Nodes (14): DefaultRegressionConfig(), NewBaselineStore(), NewComparator(), TestBaselineStore_Load_EmptyFile(), TestBaselineStore_Load_MissingFile(), TestBaselineStore_Save_MissingParentDirErrors(), TestBaselineStore_SaveAndLoad_RoundTrip(), TestBaselineStore_UpdateBaseline() (+6 more)

### Community 252 - "Community 252"
Cohesion: 0.18
Nodes (15): fixture(), read(), TestCancellationTerminatesRestoreSubprocess(), TestCorruptCooldownFailsClosed(), TestCronUsesSameCrossProcessCooldown(), TestCrossProcessPolicy(), TestRecheckDeterminesFinalVerdict(), TestRestoreFailuresPreserveProfilesAndCooldown() (+7 more)

### Community 253 - "Community 253"
Cohesion: 0.14
Nodes (13): CodexRunner, resolvedSuperpowersCodexModel(), captureRunnerCodexArgs(), TestExecCodexRunnerBuildsReadOnlyArgs(), TestExecCodexRunnerBuildsWriteArgs(), TestExecCodexRunnerForceReadOnlyPinsSandbox(), TestExecCodexRunnerOmitsModelWhenAuto(), TestExecCodexRunnerSandboxEnvOverride() (+5 more)

### Community 254 - "Community 254"
Cohesion: 0.16
Nodes (14): circuitBreakerCmd, errorAwareCmd, init(), registerReliabilityNodes(), buildCircuitBreaker(), buildTimeout(), circuitBreakerFor(), cooldownFromNode() (+6 more)

### Community 255 - "Community 255"
Cohesion: 0.15
Nodes (12): CompanyOrchestrator, EngineerTree(), MarketingTree(), SalesTree(), StartupTrees(), clamp(), safeDiv(), TestClamp() (+4 more)

### Community 256 - "Community 256"
Cohesion: 0.1
Nodes (19): Agent Categories, Architecture, BT Agent Platform, code:block1 (Agent YAML → Registry → Scheduler → resolveTree() → BuildTre), code:bash (# Prerequisites: Go 1.26.5+, Ollama (optional for LLM agents), code:block3 (┌─────────────────────────────────────────────────────────┐), code:block4 (cmd/                          # Entrypoints (7 binaries)), code:block5 (Sequence "Main") (+11 more)

### Community 257 - "Community 257"
Cohesion: 0.16
Nodes (8): code:block1 (--- FAIL: TestBuildSeedProgramPromptCarriesGraphAnalyticalSe), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase, Summary

### Community 258 - "Community 258"
Cohesion: 0.18
Nodes (8): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard ./internal/reli), code:block2 (--- FAIL: TestHandleDLQReplay_RequeuesEntryForCrossProcessRe), code:block3 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.023s), GREEN phase, RED phase, RED phase complete, RED phase result, Summary

### Community 259 - "Community 259"
Cohesion: 0.14
Nodes (7): code:block1 (go test ./internal/reliability ./cmd/bt-agent ./internal/age), code:block2 (ok  github.com/nico/go-bt-evolve/internal/reliability), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 260 - "Community 260"
Cohesion: 0.2
Nodes (13): LoadSLOEvidence(), readSLOFile(), resetSLORegistry(), TestSaveSLOMetrics_CorruptExistingFileDoesNotBlockTheWrite(), TestSaveSLOMetrics_OwnKeysWinOverStaleDiskEntries(), TestSaveSLOMetrics_PreservesOtherProcessesEvidence(), writeSLOFile(), SaveSLOMetrics() (+5 more)

### Community 261 - "Community 261"
Cohesion: 0.13
Nodes (5): Server, NewHTTPServer(), ListenerAddress(), TestSecurityListenerAddress(), TestReviewHTTPServerDeadlines()

### Community 262 - "Community 262"
Cohesion: 0.19
Nodes (16): cardSignatureValid(), loadOrCreateSigningKey(), SignAgentCard(), TestConvertToAgentCard_AttachesSignature(), TestHandleGlobalAgentCard_ResponseIsSigned(), TestLoadOrCreateSigningKey_LogsWarningOnPersistFailure(), TestSignAgentCard(), TestSignAgentCard_Deterministic() (+8 more)

### Community 263 - "Community 263"
Cohesion: 0.17
Nodes (10): TestIsBreakerSuccess(), newCatchupFixture(), TestCycleBreakerSuccess_HealthyOutcomesDoNotTripBreaker(), TestCycleBreakerSuccessTreatsRateLimitCarryoverAsHealthy(), TestSchedule_PreservesCrashRecoveryImmediateRun(), TestSchedule_PreservesMissedNextRunForCatchUp(), TestSchedule_RecomputesNextRunOnScheduleChange(), IsBreakerSuccess() (+2 more)

### Community 264 - "Community 264"
Cohesion: 0.15
Nodes (9): fanoutHandler, InitLogExport(), logExportTarget(), newTraceContextHandler(), TestInitLogExport_Idempotent(), TestLogExportTarget(), TestTraceContextHandler_InjectsTraceIDs(), traceContextHandler (+1 more)

### Community 265 - "Community 265"
Cohesion: 0.12
Nodes (11): DLQEscalateBlock(), FitnessProbeBlock(), AuditLogBlock(), TestAuditLogBlock_Structure(), TestTraceCheckpointBlock_BuildAndValidate(), TestTraceCheckpointBlock_Structure(), TestValidateTreeFull_SameNameLeafIdiomNoCycle(), TraceCheckpointBlock() (+3 more)

### Community 266 - "Community 266"
Cohesion: 0.18
Nodes (16): Entry, Limits, Scope, ScopeKind, DefaultLimits(), bbManager(), bbManagerAt(), cmdBB() (+8 more)

### Community 267 - "Community 267"
Cohesion: 0.13
Nodes (3): Milestone, Program, ProgramStore

### Community 268 - "Community 268"
Cohesion: 0.23
Nodes (17): registerDecisionTreeAction(), TestDecisionTreeRoutesByChainStateMatch(), TestDecisionTreeUsesDefaultBranchWhenNoMatch(), TestDecisionTreeValidatesAsKnownNodeType(), RouteHistory(), modelRouterTree(), TestCollectBranchLabels_SkipsDefault(), TestModelRouting_HighConfidenceRoutesToModelLabel() (+9 more)

### Community 269 - "Community 269"
Cohesion: 0.18
Nodes (17): NewOpenAICompatClient(), TestOpenAICompat_ClientErrorsDoNotTripBreaker(), TestOpenAICompat_ErrorResponse(), TestOpenAICompat_GenerateWithModel_RetryPolicyByStatusCode(), TestOpenAICompat_GenerateWithModel_SendsChatCompletion(), TestOpenAICompat_NonJSONServerError_IsRetried(), TestOpenAICompat_NonRetryableProbeErrorDoesNotWedgeBreaker(), TestOpenAICompat_OpenBreakerRejectsWithoutRequest() (+9 more)

### Community 270 - "Community 270"
Cohesion: 0.2
Nodes (16): toast(), apiPost(), approveHITL(), approveTask(), executeSprint(), pollSprintStatus(), refreshHITL(), refreshTasks() (+8 more)

### Community 271 - "Community 271"
Cohesion: 0.18
Nodes (17): AgentMetric, AlertReport, MetricsJSON, evaluateAgentAlerts(), EvaluateAlerts(), EvaluateFromJSON(), evaluateGlobalAlerts(), evaluateHTTPAlerts() (+9 more)

### Community 272 - "Community 272"
Cohesion: 0.11
Nodes (18): 1. Prerequisites, 2. Install, 3. Run tests, 4. Start the dashboard, 5. Run your first task, API Endpoints, Architecture, code:bash (git clone https://github.com/nico/go-bt-evolve.git) (+10 more)

### Community 273 - "Community 273"
Cohesion: 0.11
Nodes (18): 8.10 Autonomous Landing Pipeline, 8.11 Observability, 8.12 A2A Auction Task Allocation, 8.13 Leaf-Node Structural Validation, 8.14 Fleet-Wide Node Description Coverage, 8.15 Validation-Gated Composition Activation, 8.16 Self-Extending Claude Error Recovery, 8.17 Composable Blocks and Expand-at-Build (+10 more)

### Community 274 - "Community 274"
Cohesion: 0.11
Nodes (18): Architecture, Bounds and reload semantics, code:json ({), Configuration, Dispatch, Dispatch guards, Env (boot defaults), Exclusion groups (+10 more)

### Community 275 - "Community 275"
Cohesion: 0.12
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine -shor), code:block2 (--- FAIL: TestTelegramInit_HasQuestion_ShouldIMarker (0.00s)), RED phase result

### Community 276 - "Community 276"
Cohesion: 0.16
Nodes (14): code:block1 (internal/gardener/evolve_v2_test.go:2497:3: unknown field Lo), code:block2 (/usr/local/go/bin/go test ./internal/evolution ./internal/ga), code:block3 (/usr/local/go/bin/go test ./internal/evolution ./internal/ga), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN_COMMANDS / GREEN_RESULTS, GREEN phase (+6 more)

### Community 277 - "Community 277"
Cohesion: 0.17
Nodes (11): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard ./internal/reli), code:block2 (cmd/bt-dashboard/main_test.go:115:16: entries[0].RequeuedAt ), code:block3 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.021s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase (+3 more)

### Community 278 - "Community 278"
Cohesion: 0.16
Nodes (12): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (/usr/local/go/bin/go test ./internal/knowledge ./cmd/bt-agen), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.085s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+4 more)

### Community 279 - "Community 279"
Cohesion: 0.19
Nodes (15): circuitBreakersFileForTest, NewAgentCircuitBreaker(), TestAgentCircuitBreaker_Allow_Closed(), TestAgentCircuitBreaker_Allow_HalfOpen_SecondCall(), TestAgentCircuitBreaker_Allow_Open_Expired(), TestAgentCircuitBreaker_Allow_Open_NotExpired(), TestAgentCircuitBreaker_FailureCount(), TestAgentCircuitBreaker_IsReliabilityCircuitBreaker() (+7 more)

### Community 280 - "Community 280"
Cohesion: 0.16
Nodes (17): handleAgentsList(), AgentHistoryEntry, AgentInfo, agentStatus(), ListAgents(), ListAgentsWithCB(), listRegistryAgents(), loadCircuitBreakers() (+9 more)

### Community 281 - "Community 281"
Cohesion: 0.19
Nodes (12): CleanupOldLogs(), NewRotatingWriter(), TestCleanupOldLogs(), TestCleanupOldLogs_NoBackups(), TestNewRotatingWriter_AppendToExisting(), TestNewRotatingWriter_CreatesFile(), TestRotatingWriter_Close(), TestRotatingWriter_MaxBackups() (+4 more)

### Community 282 - "Community 282"
Cohesion: 0.19
Nodes (7): newScopedStore(), normalizeKey(), truncateSummary(), loadScopeFile(), safeFilename(), scopedStore, scopeFile

### Community 283 - "Community 283"
Cohesion: 0.14
Nodes (14): TestComposeAndExpand(), TestExpand_BlockMetadata(), Expand(), expandNode(), HasSubTreeRefs(), init(), annotateBlockSource(), traceBlockOp() (+6 more)

### Community 284 - "Community 284"
Cohesion: 0.2
Nodes (14): extractWorldState(), hasWorldState(), NewCheckpointVerifier(), readPostconditions(), restoreState(), snapshotState(), TestCheckpointVerifier_DefaultMaxRetries(), TestCheckpointVerifier_ExhaustsRetriesReturnsFailure() (+6 more)

### Community 285 - "Community 285"
Cohesion: 0.2
Nodes (17): NewEventBus(), itoa(), TestEventBus_Close(), TestEventBus_CloseMultipleSubscribers(), TestEventBus_ConcurrentAccess(), TestEventBus_DropOnFullChannel(), TestEventBus_HasFired(), TestEventBus_HasFiredAfterReset() (+9 more)

### Community 286 - "Community 286"
Cohesion: 0.21
Nodes (17): evaluateEventCondition(), TestEvaluateEventCondition_BoolAsFloat(), TestEvaluateEventCondition_BoolEvalFalse(), TestEvaluateEventCondition_BoolEvalNonBool(), TestEvaluateEventCondition_BoolEvalTrue(), TestEvaluateEventCondition_IntValue(), TestEvaluateEventCondition_KeyNotExists(), TestEvaluateEventCondition_NilChainState() (+9 more)

### Community 287 - "Community 287"
Cohesion: 0.14
Nodes (16): TestValidate_BanditSelectorRequiresUniqueName(), TestCachedCondition_RefusesHITLConditions(), TestCachedCondition_RefusesNestedHITLConditions(), TestValidateTree_NestedChildren(), TestValidateTree_ValidTree(), TestValidate_MemoryNodesDuplicateNames(), TestValidate_MemoryNodesRequireUniqueNames(), TestValidateNode_SequenceSkip() (+8 more)

### Community 288 - "Community 288"
Cohesion: 0.19
Nodes (8): sameJSONID(), scanJSONLines(), startScanJSONLines(), TestStartScanJSONLines_ReaderPanicRecoveredOnScanErr(), writeACPMessage(), ACPClient, ACPConfig, syncBuffer

### Community 289 - "Community 289"
Cohesion: 0.24
Nodes (16): bbError(), bbManager(), parseBBScope(), registerBlackboardTools(), invokeBB(), newBBServer(), TestBBManagerRequiresAgentRunner(), TestBTBBDeleteMissingKeyErrors() (+8 more)

### Community 290 - "Community 290"
Cohesion: 0.11
Nodes (17): BT Agents — Operator Guide, code:block1 (Agent YAML  →  Registry  →  Scheduler (optional)  →  resolve), DoorMate (not a BT agent), Evolution and agents, Execution path comparison, Known limitations, MCP agent tools (quick reference), Mental model (+9 more)

### Community 291 - "Community 291"
Cohesion: 0.11
Nodes (17): 10. Testing, 11. Known baseline caveat, 1. Node semantics, 2. Error signature, 3. Persistence (ADR-003), 4. Claude invocation, 5. Validation (strict, before any graft), 6. Guardrails (+9 more)

### Community 292 - "Community 292"
Cohesion: 0.11
Nodes (17): arc42 conformance restructures (from the official tips), arc42 Docs Consolidation & Per-Section Sync Nodes — Design, B1. Read-side repoint, B2. Section manifest + shared sync engine (new files in `internal/engine`), B3. Registered nodes + tree, B4. Pipeline integration, B5. Retirements & docgen, B6. Drift script (+9 more)

### Community 293 - "Community 293"
Cohesion: 0.14
Nodes (14): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (--- FAIL: TestPrefixedDomainIDsAreDescribable/domain:code_re), code:block3 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+6 more)

### Community 294 - "Community 294"
Cohesion: 0.2
Nodes (5): code:block1 (--- FAIL: TestBuildGoalDrivenPlan_InjectsPreviousFailureNote), GREEN phase, RED phase, Summary, Summary

### Community 295 - "Community 295"
Cohesion: 0.16
Nodes (10): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.106s), code:block2 (/usr/local/go/bin/go test ./internal/agent ./cmd/bt-agent -s), code:block3 (ok  github.com/nico/go-bt-evolve/internal/agent   1.539s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+2 more)

### Community 296 - "Community 296"
Cohesion: 0.25
Nodes (16): addFusionNewCount(), addFusionNewItems(), btFusionPatternQuestion(), btFusionResearchFindings(), fusionMarkdown(), fusionNewCount(), fusionTitle(), init() (+8 more)

### Community 297 - "Community 297"
Cohesion: 0.21
Nodes (14): goapFusionStateHashes(), loadGoapFusionStateHashes(), saveGoapFusionStateHashes(), TestGoapFusionStateHashes_DurableAcrossCronTicks(), TestGoapFusionStateHashes_HistoryCappedOnDisk(), TestPublishGoapFusionStateHash_ActiveWorkStillAccumulates(), TestPublishGoapFusionStateHash_IdleDoesNotAccumulate(), TestPublishGoapFusionStateHash_ResultReflectsDurableDepth() (+6 more)

### Community 298 - "Community 298"
Cohesion: 0.21
Nodes (15): buildRunActivitySummary(), fencedItems(), isFactLine(), joinCapped(), salientOutputLines(), TestBuildRunActivitySummaryCapsLongFencedLists(), TestBuildRunActivitySummaryEmptyEverything(), TestBuildRunActivitySummaryFailureLeadsWithReason() (+7 more)

### Community 299 - "Community 299"
Cohesion: 0.35
Nodes (15): autofixScriptRunner, autofixScriptStep, TestVerifySuperpowersRunBudgetExhaustedMarker(), VerifySuperpowersRunRuntime(), autofixVerifyRun(), TestVerifyLintFixMaxAttemptsZeroDisablesClaude(), TestVerifySuperpowersRunAutofixesLintOnlyFailure(), TestVerifySuperpowersRunClaudeRepairStillFailsIsFinal() (+7 more)

### Community 300 - "Community 300"
Cohesion: 0.18
Nodes (13): reportFusionCycleBaseChainState(), TestReportFusionCycleListsAgedUnmergedSuperpowersBranchesAsPendingPatch(), TestReportFusionCycleListsMaterializerSnapshotsFilenameSizeAndChangedCount(), TestVerifyGoapFusionEvidenceAcceptsCommittedPROpened(), TestVerifyGoapFusionEvidenceAcceptsDelegatedVerification(), TestVerifyGoapFusionEvidenceKeysOnNormalizedDelegationToken(), TestVerifyGoapFusionEvidenceRejectsBogusVerification(), writeGoapFusionCycleReport() (+5 more)

### Community 301 - "Community 301"
Cohesion: 0.14
Nodes (14): DiskInfo, GardenerMetrics, MemInfo, Metrics, loadGardenerMetrics(), rankTopWinners(), TestCollect_IncludesDLQCategoriesFromHook(), TestCollect_SurfacesTopEvolvedWinnersFromTrees() (+6 more)

### Community 303 - "Community 303"
Cohesion: 0.12
Nodes (16): TestValidate_CBCooldownSecs_TooHigh(), TestValidate_CBCooldownSecs_TooLow(), TestValidate_CBThreshold_TooHigh(), TestValidate_CBThreshold_TooLow(), TestValidate_DLQMaxEntries_TooHigh(), TestValidate_DLQMaxEntries_TooLow(), TestValidate_RetryBaseDelayMs_TooHigh(), TestValidate_RetryBaseDelayMs_TooLow() (+8 more)

### Community 304 - "Community 304"
Cohesion: 0.12
Nodes (16): code:bash (# 1. Get a runner token from GitHub), code:bash (# Check status), code:yaml (runs-on: [self-hosted, jetson, arm64]), code:bash (rm -rf ~/actions-runner/_work/_actions), Nightly tests failing with "Ollama not reachable", Prerequisites, Quick Setup, Runner appears offline (+8 more)

### Community 305 - "Community 305"
Cohesion: 0.12
Nodes (17): Agent Executor, Circuit Breaker, code:go (type CircuitBreaker struct { ... }), code:go (func RetryWithBackoff(fn func() error, maxAttempts int, base), code:go (type DeadLetterQueue struct { ... }), code:go (type WorkerPool struct { ... }), code:go (type ConcurrencyLimiter struct { ... }), code:go (type Queue interface {) (+9 more)

### Community 306 - "Community 306"
Cohesion: 0.12
Nodes (17): Audit, code:go (type RateLimiter struct { ... }), code:go (func SanitizeMiddleware(next http.Handler) http.Handler), code:go (type SecurityHeadersConfig struct {), code:go (func CrossOriginMiddleware(allowedOrigins []string) func(htt), code:go (type IPFilterMode int  // FilterAllowlist | FilterBlocklist), code:go (func AuditSecurityEvent(ctx context.Context, eventType strin), code:go (func GenerateRequestID() string            // crypto/rand, 1) (+9 more)

### Community 307 - "Community 307"
Cohesion: 0.12
Nodes (16): code:block1 (┌──────────────────────────────────────────────┐), Idle assets that the vision needs (build on, don't rebuild), Part 1 — Review: Current State vs arc42 Goals, Part 2 — Target Architecture, Part 3 — Phased Roadmap, Part 4 — Sequencing, Risks, Metrics, Personalized Self-Evolving Agents — Strategy and Implementation Plan, Phase 0 — Close the creation→execution loop (prerequisite, small) (+8 more)

### Community 308 - "Community 308"
Cohesion: 0.12
Nodes (16): Alternatives considered, ApproveDesign HITL enrichment, Brainstorm Grill-Driven Design-Improvement Loop — Design, code:block1 (MemSequence BrainstormBranch), Design, Error handling, Goals, GrillDesignArtifact (modified — becomes the ReviewCycle reviewer) (+8 more)

### Community 309 - "Community 309"
Cohesion: 0.13
Nodes (9): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestBTEvolveBottlenecksPrioritizedAndCapped (0.00s), FILES_CHANGED, NOTES, RED_COMMAND, RED phase complete, RED phase result, RED_RESULT (+1 more)

### Community 310 - "Community 310"
Cohesion: 0.15
Nodes (8): code:block1 (PATH=/usr/local/go/bin:$PATH /usr/local/go/bin/go test ./int), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.198s), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/agent	2.696s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 311 - "Community 311"
Cohesion: 0.23
Nodes (13): capNumPredictHandler(), TestClient_GenerateCtx_RetryPolicyByFailureKind(), TestClient_GenerateWithMaxTokens_SetsNumPredictOption(), TestClient_GenerateWithMaxTokens_ZeroLeavesRequestUnbounded(), TestErrorRecorder_GenerateWithMaxTokens_FallsBackToGenerateWhenInnerLacksSupport(), TestErrorRecorder_GenerateWithMaxTokens_ForwardsToCapableInnerLLM(), TestErrorRecorder_GenerateWithMaxTokens_RecordsError(), TestFallbackLLM_GenerateWithMaxTokens_FallsBackToUnboundedGenerateForIncapableModel() (+5 more)

### Community 313 - "Community 313"
Cohesion: 0.17
Nodes (5): enforceIslandCap(), mergeIslandPopulation(), islandArchive, IslandModel, IslandStats

### Community 314 - "Community 314"
Cohesion: 0.41
Nodes (13): partialLandingRunner, greenFail(), greenPass(), partialLandingRun(), redFail(), TestBatchFirstTaskFailureStaysAllOrNothing(), TestBatchFullSuccessUnwrapsSnapshots(), TestBatchMidBatchSnapshotDegradeStillUnwraps() (+5 more)

### Community 315 - "Community 315"
Cohesion: 0.22
Nodes (14): TestCurrentSuperpowersRunRefusesFinishedRun(), currentSuperpowersRun(), newSuperpowersRunID(), safeSlug(), superpowersPlanAttemptSaturated(), superpowersPlanAttemptSaturatedInDir(), superpowersTaskHashSuffix(), isolateSuperpowersRunsDir() (+6 more)

### Community 316 - "Community 316"
Cohesion: 0.23
Nodes (6): NewGoalStore(), TestGoalStore_AddRemoveRoundtrip(), TestGoalStore_LoadEmpty(), TestGoalStore_Queue(), TestGoalStore_ValidatesInput(), GoalStore

### Community 317 - "Community 317"
Cohesion: 0.13
Nodes (6): ActionFunc, ActionRegistry, Agent, AgentCallbacks, AgentRun, AgentState

### Community 318 - "Community 318"
Cohesion: 0.17
Nodes (14): IsKnownChainKind(), parseChainConfig(), TestChainAction_ParseConfig(), TestIsKnownChainKind(), TestSideEffectClassOfSubtree_DeepChild(), TestSideEffectClassOfSubtree_MultipleChildren(), TestSideEffectClassOfSubtree_NoSideEffect(), TestSideEffectClassOfSubtree_RootHasClass() (+6 more)

### Community 319 - "Community 319"
Cohesion: 0.24
Nodes (14): asFloat(), compiledGoapActionFor(), compiledGoapConditionFor(), goapValuesEqual(), goapWorldStateFrom(), isCompiledGoapAction(), isCompiledGoapCondition(), parseGoapPairs() (+6 more)

### Community 320 - "Community 320"
Cohesion: 0.19
Nodes (12): TestReviewSprintRetryReturnsExistingJob(), TestReviewTaskAdmissionDoesNotRevive(), TestReviewTaskClaimPersistenceFailure(), TestReviewTaskClaimsAtomic(), NewTaskStore(), newHITLTestStore(), TestNewTaskStore_PanicsOnCorruptFile(), TestTaskStore_ApprovedOrdersByPriorityThenSprint() (+4 more)

### Community 321 - "Community 321"
Cohesion: 0.23
Nodes (13): Alert, apiFetch(), esc(), getCookie(), sleep(), cbIcon(), createAgent(), deleteAgent() (+5 more)

### Community 322 - "Community 322"
Cohesion: 0.12
Nodes (14): code:bash (sudo mkdir -p /usr/local/go/bin), code:bash (export BT_API_KEY=dev-local-key), code:bash (tmux -f /exec-daemon/tmux.portal.conf new-session -d -s bt-d), Common commands, Cursor Cloud specific instructions, Dependency refresh (automatic), Long-running services, MCP binaries (`bt-agent`, etc.) (+6 more)

### Community 323 - "Community 323"
Cohesion: 0.12
Nodes (15): 5.0 Composable Blocks, 5.1 Whitebox Overall System, 5.2 Core Engine, 5.3 Evolution Engine, 5.4 Dashboard, 5.5 Chain Types, 5.6 Planned Building Blocks (ADR-133), 5. Building Block View (+7 more)

### Community 324 - "Community 324"
Cohesion: 0.12
Nodes (15): Architecture, Background constraints, code:go (// LiveRunInfo names the run for registry listing and persis), code:go (type MutationOp struct {), code:go (// internal/engine — nil-checked, wired from cmd/bt-agent.), Decisions (from brainstorming), Error handling, Goal (+7 more)

### Community 325 - "Community 325"
Cohesion: 0.2
Nodes (11): code:block1 (ollama_test.go:41: expected Generate to succeed after one ji), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/llm	21.140s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED_COMMAND (+3 more)

### Community 326 - "Community 326"
Cohesion: 0.19
Nodes (8): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/llm	3.844s), code:block2 (--- FAIL: TestChainAction_SingleShotExecutors_RetryOnTransie), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/engine	50-55s), GREEN phase, RED phase, Summary, Summary, Summary

### Community 327 - "Community 327"
Cohesion: 0.19
Nodes (7): FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase, Summary

### Community 328 - "Community 328"
Cohesion: 0.15
Nodes (7): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent ./cmd/bt-gardener .), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.235s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 329 - "Community 329"
Cohesion: 0.21
Nodes (6): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.086s), GREEN phase, RED phase, RED phase complete, RED phase result, Summary

### Community 330 - "Community 330"
Cohesion: 0.26
Nodes (8): NewFallbackLLM(), TestFallbackLLM_CircuitBreakerSkipsPersistentlyFailingModel(), TestFallbackLLM_GenerateAllPanicReturnsAggregatedError(), TestFallbackLLM_GenerateRecoversPanicAndTriesNextModel(), TestFallbackLLM_GenerateReturnsAllFailures(), TestFallbackLLM_GenerateUsesNextModelAfterPrimaryFailure(), TestFallbackLLM_ValidationErrorsDoNotTripBreaker(), stubLLM

### Community 331 - "Community 331"
Cohesion: 0.28
Nodes (13): activateAutomation(), automationAgentName(), considerAutomation(), finalizeAutomationApproval(), proposeAutomation(), registerAutomationTools(), suggestSchedule(), newAutopilotDeps() (+5 more)

### Community 332 - "Community 332"
Cohesion: 0.23
Nodes (13): autoApproveFromNode(), buildHumanApprovalGate(), childExecuted(), hitlPhase(), hitlStatus(), hitlStore(), init(), markChildExecuted() (+5 more)

### Community 333 - "Community 333"
Cohesion: 0.19
Nodes (6): AgentCircuitBreakerStore, reportAgentOutcome(), TestReportAgentOutcome_Failure(), TestReportAgentOutcome_NilStore(), TestReportAgentOutcome_OpensOnThreshold(), TestReportAgentOutcome_Success()

### Community 334 - "Community 334"
Cohesion: 0.31
Nodes (13): main(), newEvaluatorServer(), newTestEvaluatorServer(), sampleTree(), seedRecords(), TestHandleDeepen_DefaultsMaxDepthAndAutoSavesTT(), TestHandleDeepen_HonorsExplicitMaxDepth(), TestHandleEvaluate_NoTreeLoadedReturnsError() (+5 more)

### Community 335 - "Community 335"
Cohesion: 0.14
Nodes (10): BaselineStore, BenchmarkResult, HasRegressions(), HasWarnings(), pctChange(), TestHasRegressions(), TestHasWarnings(), Comparator (+2 more)

### Community 336 - "Community 336"
Cohesion: 0.19
Nodes (14): collectSystem(), buildRoutePrompt(), classifyTaskRoute(), exactLabel(), extractJSONObject(), fallbackDecision(), isModelRouted(), parseRouteResponse() (+6 more)

### Community 337 - "Community 337"
Cohesion: 0.13
Nodes (15): LoadDotEnv(), stripInlineComment(), TestLoadDotEnv_Basic(), TestLoadDotEnv_CommentsAndBlanks(), TestLoadDotEnv_EmptyFile(), TestLoadDotEnv_ExportPrefix(), TestLoadDotEnv_InlineComments(), TestLoadDotEnv_MissingFile() (+7 more)

### Community 338 - "Community 338"
Cohesion: 0.18
Nodes (6): saveBaseline(), TestRunCheckNoRegressionExitsClean(), TestRunCheckRejectsEmptyInput(), TestRunCheckSeverityExitCodes(), TestRunResetRemovesBaselineFile(), TestRunShowWithBaseline()

### Community 339 - "Community 339"
Cohesion: 0.13
Nodes (14): Claude Code, code:bash (# 1. Edit the unit's EnvironmentFile (operator-managed, outs), code:block2 (## Scheduled GOAP Fusion Runtime Preflight Passed), Codex CLI, Coding Delegation Providers (Claude / Codex), Configuring and restarting the daemon, Environment reference, Per-seam tool overrides (+6 more)

### Community 340 - "Community 340"
Cohesion: 0.13
Nodes (14): Binary Size Overview, code:block1 (Go version:), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-assistant	0.035s), code:block3 (-rwxrwxr-x 1 nico nico  13M Jun  2 08:32 /home/nico/go-bt-ev), code:block4 (bt-agent:), code:block5 (coverage: 100.0), code:block6 ({"packages":19,"status":"ok","trees":38,"uptime":"operationa), code:block7 (=== Doc Drift Validation ===) (+6 more)

### Community 341 - "Community 341"
Cohesion: 0.13
Nodes (14): 1. Shared primitive — `seedCodeFixProgram`, 2. Part A — Error-handler escalation (reactive), 3. Part B — Proactive self-audit agent (proactive), Architecture, code:block1 ([ClaudeErrorHandler]  tree -1, unresolvable + is-bug ──┐), code:go (func seedCodeFixProgram(sig, title, milestoneGoal, source st), Files, Goal (+6 more)

### Community 342 - "Community 342"
Cohesion: 0.13
Nodes (14): arc42 Docs Consolidation & Per-Section Sync Nodes — Implementation Plan, Canonical section layout (used by merge tasks, Go manifest, and drift script — keep all three in lockstep), code:bash (grep -F '## 10.1 Quality Tree' docs/arc42/10-quality.md && g), code:bash (grep -rn 'ADR-010' --include='*.go' internal/ cmd/), code:bash (PATH=/usr/local/go/bin:$PATH go build ./... && PATH=/usr/loc), code:bash (for f in 02-constraints 03-context-scope 04-solution-strateg), code:bash (PATH=/usr/local/go/bin:$PATH make check-full), code:bash (grep -c '^## 6\.' docs/arc42/06-runtime-view.md        # exp) (+6 more)

### Community 343 - "Community 343"
Cohesion: 0.18
Nodes (10): code:block1 (internal/agentexec/wiring_test.go:47:12: undefined: ResolveG), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./internal/agentexec ./), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/agentexec	0.018s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+2 more)

### Community 344 - "Community 344"
Cohesion: 0.13
Nodes (14): code:block1 (/usr/local/go/bin/go test -race ./internal/util -short -coun), code:block2 (# github.com/nico/go-bt-evolve/internal/util [github.com/nic), code:block3 (save error = commit .../fixture.json: rename .../fixture.jso), code:block4 (/usr/local/go/bin/go test -race ./internal/util -short -coun), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase (+6 more)

### Community 345 - "Community 345"
Cohesion: 0.13
Nodes (14): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestBuildCompositeChildren_GuardEvaluatedAtRunTime), code:go (if cond := guardConditionForChild(node.Edges, i); cond != ""), code:block4 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase (+6 more)

### Community 347 - "Community 347"
Cohesion: 0.23
Nodes (3): MultiFitness, cappedIndividuals(), ParetoFront

### Community 348 - "Community 348"
Cohesion: 0.23
Nodes (11): ComposeTaskTreeWithHITL(), HumanGateBlock(), TestComposeTaskTreeWithHITL_EmptyNameDefaults(), TestComposeTaskTreeWithHITL_IncludesHumanGate(), TestComposeTaskTreeWithHITL_NilRegistryUsesDefault(), TestComposeTaskTreeWithHITL_RootName(), TestComposeTaskTreeWithHITL_Validates(), TestComposeTaskTreeWithHITL_WithStrategy() (+3 more)

### Community 349 - "Community 349"
Cohesion: 0.26
Nodes (3): Registry, cloneTree(), NewRegistry()

### Community 350 - "Community 350"
Cohesion: 0.16
Nodes (10): applyScriptRunner, cleanApplyScriptRunner, TestApplySuperpowersRunToMainRepoAppliesVerifiesGraphifiesAndCommits(), TestApplySuperpowersRunToMainRepoSavesPendingPatchWhenMainRepoDirty(), TestCleanupGraphifyOutRemovesUntrackedRegeneratedArtifacts(), TestStageAllExceptGeneratedSucceedsWithIgnoredGraphifyOut(), runInDir(), TestScheduledGoapFusionBuildTreeMaterialized_HandlesDeletedTrackedFiles() (+2 more)

### Community 351 - "Community 351"
Cohesion: 0.2
Nodes (10): fakeSuperpowersReviewRunner, fixedDiffRunner, buildSuperpowersReviewPrompt(), parseSuperpowersReviewVerdict(), superpowersTaskReview(), TestParseSuperpowersReviewVerdict(), TestSuperpowersTaskGreenAction_InjectsReviewFeedbackWhenPresent(), TestSuperpowersTaskGreenAction_NoFeedback_PromptUnchanged() (+2 more)

### Community 352 - "Community 352"
Cohesion: 0.21
Nodes (5): LoadNotebookLMState(), loadNotebookLMStateAction(), QueryHash(), saveNotebookLMStateAction(), NotebookLMState

### Community 354 - "Community 354"
Cohesion: 0.14
Nodes (13): bookmarkBtn, camBtn, canvasBox, doormateTab, emptyState, inputEl, micBtn, pageContainer (+5 more)

### Community 355 - "Community 355"
Cohesion: 0.14
Nodes (13): code:go (sel("Phase1_Design",), code:markdown (### Task N: Title), code:go (evolution.SerializableNode{), code:text (ApproveSuperpowersPRPush (HumanApprovalGate, side_effect_cla), code:go (// Deprecated: use ExecuteSuperpowersTaskBatch.), Current Stub Surface To Replace, Done Definition, Migration / Cleanup Notes (+5 more)

### Community 356 - "Community 356"
Cohesion: 0.14
Nodes (13): 1. Tree change (`internal/domains/goap_fusion_loop.go`), 2. Quota memory (`internal/engine/actions_goap_fusion_claude_review.go`), 3. New action `RunClaudeCodeReviewResearch`, code:go (sel("ResearchRouter",), code:go (const goapReviewAllowedTools = "Read,Glob,Grep," +), Decisions (defaults chosen during design), Design, Error handling summary (+5 more)

### Community 357 - "Community 357"
Cohesion: 0.23
Nodes (5): code:block1 (internal/domains/domains_test.go:371:10: undefined: AuctionD), code:block2 (internal/domains/domains_test.go:367:10: undefined: AuctionD), GREEN phase, RED phase, Summary

### Community 358 - "Community 358"
Cohesion: 0.14
Nodes (13): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block3 (=== RUN 1 ===  ok  github.com/nico/go-bt-evolve/internal/dom), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+5 more)

### Community 359 - "Community 359"
Cohesion: 0.23
Nodes (10): code:block1 (/usr/local/go/bin/go test ./internal/benchmark -short -count), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/benchmark	0.065s), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.185s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+2 more)

### Community 360 - "Community 360"
Cohesion: 0.14
Nodes (13): code:block1 (/usr/local/go/bin/go test ./internal/security -short -count=), code:block2 (--- FAIL: TestRotateKey_ReplacementDoesNotSilentlyExpire (0.), code:block3 (/usr/local/go/bin/go test ./internal/security -short -count=), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+5 more)

### Community 361 - "Community 361"
Cohesion: 0.23
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (--- FAIL: TestEveryDomainTreeIsEngineBuildable/superpowers_w), FILES_CHANGED, NOTES, RED_COMMAND, RED Phase Result — Task 3, RED_RESULT

### Community 362 - "Community 362"
Cohesion: 0.14
Nodes (13): code:block1 (/usr/local/go/bin/go test ./internal/engine -run 'TestBuildR), code:block2 (--- FAIL: TestBuildRateLimit_IntervalResolution), code:block3 (/usr/local/go/bin/go test ./internal/engine -run 'TestBuildR), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+5 more)

### Community 363 - "Community 363"
Cohesion: 0.14
Nodes (13): code:block1 (/usr/local/go/bin/go test ./internal/agent ./internal/a2a -s), code:block2 (--- FAIL: TestAgentCircuitBreakerStore_SaveLocksReadModifyWr), code:block3 (/usr/local/go/bin/go test ./internal/agent ./internal/a2a -s), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+5 more)

### Community 364 - "Community 364"
Cohesion: 0.24
Nodes (5): Client, Config, LLM, DefaultConfig(), extractSection()

### Community 365 - "Community 365"
Cohesion: 0.17
Nodes (3): stringsContains(), TestGetSLOMetrics_SameInstance(), TestSLOMetrics_Summary()

### Community 367 - "Community 367"
Cohesion: 0.15
Nodes (3): CrisisDetector, CrisisState, InterveneAction

### Community 368 - "Community 368"
Cohesion: 0.21
Nodes (4): Clock, NewFakeClock(), FakeClock, RealClock

### Community 369 - "Community 369"
Cohesion: 0.18
Nodes (6): EdgeType, maxTokensFromMetadata(), ValidateEdge(), NodeValidationInfo, SerializableNode, TypedEdge

### Community 370 - "Community 370"
Cohesion: 0.21
Nodes (3): Goal, Plan, WorldState

### Community 371 - "Community 371"
Cohesion: 0.24
Nodes (8): classify(), Ensure(), runCLI(), saveState(), stateError(), policy, Result, runner

### Community 372 - "Community 372"
Cohesion: 0.18
Nodes (4): CategorizedError, ErrorContext, TestJitterStrategy_String(), JitterStrategy

### Community 373 - "Community 373"
Cohesion: 0.17
Nodes (12): GetCategory(), NewCategorizedError(), TestCategorizedError_Error(), TestCategorizedError_NilError(), TestCategorizedError_Unwrap(), TestClassifyError_CategorizedChain(), TestErrorContext_Chained(), TestGetCategory() (+4 more)

### Community 374 - "Community 374"
Cohesion: 0.26
Nodes (9): computeLayout(), loadMindMap(), nodeColors, pushDescendantsDown(), renderTree(), resolveCollisions(), shorten(), subtreeSpan() (+1 more)

### Community 375 - "Community 375"
Cohesion: 0.15
Nodes (12): Added, Added, Changed, Changelog, Chores, Fixed, Fixed, Miscellaneous (+4 more)

### Community 376 - "Community 376"
Cohesion: 0.15
Nodes (13): Algorithm Engines, code:go (type TreeStore struct { ... }), code:go (func DefaultTree() *SerializableNode           // 17-node ge), code:go (func PitchAgentTree() *SerializableNode), code:go (// ExpertKnowledge: 6 design patterns, 5 anti-patterns, 10 h), code:go (func ContainsWord(s, word string) bool      // case-insensit), code:go (func ApplyMutation(tree *SerializableNode, op string) *Seria), Finance Trees (10) (+5 more)

### Community 377 - "Community 377"
Cohesion: 0.15
Nodes (12): 7.1 Infrastructure Level 1, 7.2.1 Process Tree, 7.2.2 Storage Layout, 7.2.3 Network Topology, 7.2 Infrastructure Level 2, 7. Deployment View, code:block1 (systemd --user), code:block2 (~/.go-bt-evolve/) (+4 more)

### Community 378 - "Community 378"
Cohesion: 0.15
Nodes (12): code:block1 (token absent ──────────────► skip (pr_shepherd_no_token, no_), Config, Files, Fix loop (CI red), GitHub API client, Goal, Non-goals, Placement (per Nico: extend the goap-fusion tree) (+4 more)

### Community 379 - "Community 379"
Cohesion: 0.15
Nodes (12): code:go (func TestParseClaudeRateLimitReset(t *testing.T) {), code:go (const claudeResetMargin = 2 * time.Minute), code:go (func isolateClaudeBackoffStore(t *testing.T) {), code:go (type sharedClaudeBackoff struct {), code:go (func saveClaudeBackoffState(bb *Blackboard, until time.Time)), code:go (// actions_superpowers_prod.go), Global Constraints, Shared Claude Rate-Limit Backoff Implementation Plan (+4 more)

### Community 380 - "Community 380"
Cohesion: 0.15
Nodes (7): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.106s), code:block2 (ok  github.com/nico/go-bt-evolve/internal/agent   1.539s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 381 - "Community 381"
Cohesion: 0.18
Nodes (5): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard ./internal/reli), code:block2 (--- FAIL: TestHandleDLQReplay_RequeuesEntryForCrossProcessRe), RED phase complete, RED phase result, Summary

### Community 382 - "Community 382"
Cohesion: 0.15
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.025s), GREEN phase complete, Summary

### Community 383 - "Community 383"
Cohesion: 0.27
Nodes (4): code:block1 (--- FAIL: TestVerifyScheduledGoapFusionBuildTreeMaterialized), GREEN phase, RED phase, Summary

### Community 384 - "Community 384"
Cohesion: 0.22
Nodes (11): code:block1 (/usr/local/go/bin/go test ./internal/evolution ./internal/ga), code:block2 (internal/evolution/multi_objective_test.go:353:15: undefined), code:block3 (--- FAIL: TestValidationGate_ParetoAcceptsRecoveryForSuccess), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+3 more)

### Community 385 - "Community 385"
Cohesion: 0.21
Nodes (11): code:block1 (/usr/local/go/bin/go test ./internal/engine ./cmd/benchcmp .), code:block2 (--- FAIL: TestPrioritizeGoapGoals_NoImportCycleFalseGoalFrom), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED_COMMAND (+3 more)

### Community 386 - "Community 386"
Cohesion: 0.18
Nodes (10): code:block1 (internal/evolution/island_test.go:85:23: im.Stats().Resurrec), code:block2 (--- FAIL: TestEvolveWithExperience_ResurrectsExtinctSpeciali), code:block3 (/usr/local/go/bin/go test ./internal/evolution ./cmd/bt-agen), code:block4 (ok  github.com/nico/go-bt-evolve/internal/evolution  4.429s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+2 more)

### Community 387 - "Community 387"
Cohesion: 0.15
Nodes (7): code:block1 (--- FAIL: TestAllDomainTreeConditionsHaveDescriptions (0.00s), code:block2 (--- FAIL: TestEveryResolverSelectableTreeIsDescribable/godev), FILES_CHANGED, NOTES, RED_COMMAND, RED Phase Result, RED_RESULT

### Community 388 - "Community 388"
Cohesion: 0.17
Nodes (11): code:block1 (/usr/local/go/bin/go test ./internal/a2a ./internal/agentexe), code:block2 (--- FAIL: TestDaemonConfiguresAuctionDelegateHook (0.00s)), code:block3 (/usr/local/go/bin/go test ./internal/a2a ./internal/agentexe), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+3 more)

### Community 389 - "Community 389"
Cohesion: 0.15
Nodes (12): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-dashb), code:block2 (--- FAIL: TestRegistry_DomainDescriptionsResolveThroughDescr), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+4 more)

### Community 390 - "Community 390"
Cohesion: 0.15
Nodes (12): code:block1 (/usr/local/go/bin/go test -race ./internal/evolution ./inter), code:block2 (/usr/local/go/bin/go test -race ./internal/evolution ./inter), FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+4 more)

### Community 391 - "Community 391"
Cohesion: 0.21
Nodes (10): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/security ./c), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/security	4.021s), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/security	4.057s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+2 more)

### Community 392 - "Community 392"
Cohesion: 0.24
Nodes (10): TestNewProviderCreatesACPClient(), TestNewProvider_BuildsFallbackChainFromConfiguredModels(), fallbackSpec, maxTokensCapable, buildProvider(), isKnownProvider(), NewProvider(), parseFallbackModels() (+2 more)

### Community 393 - "Community 393"
Cohesion: 0.24
Nodes (7): agentNameKey, BTAgentExecutor, cardResponder, failureEventMessage(), parseAnnouncement(), RespondToAnnouncement(), TestFailureEventMessage_CarryoverKeepsSentinel()

### Community 394 - "Community 394"
Cohesion: 0.32
Nodes (10): NotebookLMTree(), findChildByName(), hasNode(), TestNotebookLMTreeIncludesIdempotencyStateActions(), TestNotebookLMTreeIsZeroLLM(), TestNotebookLMTreeRecordsRealPathDuringExecution(), TestNotebookLMTreeRoutesResearchBeforeIngestAndQuery(), TestNotebookLMTreeUsesDeterministicEvidenceGateBeforeSuccess() (+2 more)

### Community 395 - "Community 395"
Cohesion: 0.29
Nodes (10): MergeResultsBlock(), ParallelFanoutBlock(), TestFanoutBlocks_ReturnFreshValues(), TestMergeResultsBlock_Shape(), TestMergeResultsBlock_Validates(), TestParallelFanoutBlock_ChainAction(), TestParallelFanoutBlock_Children(), TestParallelFanoutBlock_ParallelMetadata() (+2 more)

### Community 396 - "Community 396"
Cohesion: 0.35
Nodes (9): docsyncRunner, syncDriftDocs(), docsyncTestRun(), TestDocDriftSyncFixesDriftViaClaudePass(), TestDocDriftSyncSkipsDryRunAndMissingScript(), TestDocDriftSyncSkipsWhenInSync(), TestDocDriftSyncStillFailingIsReported(), TestVerificationChecksIncludeDocDriftGate() (+1 more)

### Community 398 - "Community 398"
Cohesion: 0.21
Nodes (4): appendLatency(), avgLatency(), nlmMetricsReportAction(), NotebookLMMetrics

### Community 399 - "Community 399"
Cohesion: 0.26
Nodes (10): NewACPClient(), countCrashInvocations(), TestACPClientCircuitBreakerShortCircuitsRepeatedSubprocessFailures(), TestACPClientGenerateCtx_StderrBufferRace(), TestACPClientGenerateReturnsErrorWhenSessionMissing(), TestACPClientGenerateTalksToACPServer(), TestACPClientGenerateTalksToHermesACP(), TestACPHelperProcess() (+2 more)

### Community 402 - "Community 402"
Cohesion: 0.17
Nodes (11): Agent template, Blackboard outputs, code:yaml (params:), code:bash (export BT_LLM_PROVIDER=openrouter), code:bash (/usr/local/go/bin/go test ./internal/fusion ./internal/llm .), Configuration, Fusion BT Agent, Routing triggers (+3 more)

### Community 403 - "Community 403"
Cohesion: 0.17
Nodes (11): 6.1 Task Execution Scenario, 6.2 Evolution Cycle, 6.3 Sprint Execution, 6.4 Self-Improvement Cycle (goap-fusion loop), 6.5 Error Recovery, 6. Runtime View, code:block1 (Hermes Agent                    bt-agent (MCP)              ), code:block2 (bt-gardener                    bt-evaluator (MCP)           ) (+3 more)

### Community 404 - "Community 404"
Cohesion: 0.17
Nodes (11): Behavior matrix, code:json ({"until":"2026-07-14T22:44:33Z","set_by":"goap-fusion-loop-r), Design, Fleet-wide store (ADR-003), Goals, Non-goals, Ops, Problem (+3 more)

### Community 405 - "Community 405"
Cohesion: 0.23
Nodes (8): code:block1 (internal/evolution/island_test.go:154:6: im.ExpertKnowledge ), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase, Summary

### Community 406 - "Community 406"
Cohesion: 0.23
Nodes (7): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.746s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 407 - "Community 407"
Cohesion: 0.21
Nodes (9): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block2 (internal/evolution/learning_test.go:206:3: unknown field Spe), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT (+1 more)

### Community 408 - "Community 408"
Cohesion: 0.23
Nodes (8): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-ci-doctor -sho), code:block2 (# github.com/nico/go-bt-evolve/cmd/bt-ci-doctor [github.com/), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 409 - "Community 409"
Cohesion: 0.17
Nodes (11): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener ./c), code:block2 (cmd/bt-gardener/main_test.go:81:16: undefined: wireSelectorO), code:block3 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block4 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.083s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS (+3 more)

### Community 412 - "Community 412"
Cohesion: 0.22
Nodes (9): init(), TestWireSelectorReorder_SelectsStrategyFromEnv(), TestWireSelectorReorderIsOptIn(), TestWireSelectorReorderWiresDTStatsPathFn(), wireSelectorReorder(), TestDaemonConfiguresGoalPlanBrainstorm(), GoalPlanBrainstormWired(), WireGoalPlanBrainstorm() (+1 more)

### Community 413 - "Community 413"
Cohesion: 0.22
Nodes (4): History, splitLines(), RunRecord, RunStats

### Community 414 - "Community 414"
Cohesion: 0.31
Nodes (9): extractJSONPayload(), TestValidateOutputs_Empty(), TestValidateOutputs_JSONDirect(), TestValidateOutputs_JSONFence(), TestValidateOutputs_JSONInvalid(), TestValidateOutputs_Markdown(), TestValidateOutputs_NoSpec(), validateOutputJSON() (+1 more)

### Community 416 - "Community 416"
Cohesion: 0.29
Nodes (8): foreachTaskLoop(), SuperpowersWorkflowTree(), systematicDebugging(), tddTask(), TestSuperpowersWorkflowTree_GrillLoopShape(), TestSuperpowersWorkflowTree_ValidatesAndCoversPhases(), withMatch(), TestSuperpowersWorkflowTree_BuildsAndValidates()

### Community 417 - "Community 417"
Cohesion: 0.18
Nodes (3): EvolutionMetadata, FitnessRecord, MutationLog

### Community 418 - "Community 418"
Cohesion: 0.29
Nodes (5): Entry, Store, Key(), normalize(), truncateRunes()

### Community 420 - "Community 420"
Cohesion: 0.22
Nodes (4): Outcome, Record, FilterByTreeName(), Store

### Community 421 - "Community 421"
Cohesion: 0.25
Nodes (7): TaskLogPath(), Entry, auditLogAction(), init(), registerOpsActions(), traceCheckpointAction(), truncateStr()

### Community 422 - "Community 422"
Cohesion: 0.24
Nodes (5): ParamAction, bindTemplate(), TestBindTemplate_NoPlaceholders(), TestBindTemplate_Simple(), ParamPlannerNode

### Community 423 - "Community 423"
Cohesion: 0.33
Nodes (10): runSuperpowersCondCases(), TestConditionsSuperpowers_CheckIndexInRange(), TestConditionsSuperpowers_DesignApproved(), TestConditionsSuperpowers_DesignExists(), TestConditionsSuperpowers_HITLAlreadyApproved(), TestConditionsSuperpowers_IsCreativeTask(), TestConditionsSuperpowers_PlanReady(), TestConditionsSuperpowers_VerificationFailed() (+2 more)

### Community 424 - "Community 424"
Cohesion: 0.25
Nodes (4): estimateTokensForTest(), TestBuildBudget_MaxTokens_TripsOnCumulativeUsage(), TestGenerateWithRetry_IncrementsBlackboardTokensUsed(), budgetTokensMockLLM

### Community 425 - "Community 425"
Cohesion: 0.18
Nodes (11): BuildEventDrivenAbort(), TestBuildEventDrivenAbort_BlackboardKeyCondition(), TestBuildEventDrivenAbort_BlackboardKeyNotPresent(), TestBuildEventDrivenAbort_EventChannelAbort(), TestBuildEventDrivenAbort_LegacyEventKey(), TestBuildEventDrivenAbort_NilChainStateInit(), TestBuildEventDrivenAbort_NoAbortNoMessage(), TestBuildEventDrivenAbort_NoChildren() (+3 more)

### Community 428 - "Community 428"
Cohesion: 0.18
Nodes (11): 3.1 Create the bin directory, 3.2 Build the core MCP servers, 3.3 Build the dashboard and gardener, 3.4 Build utility binaries, 3. Build All Binaries (2 min), code:bash (ls -lh bin/bt-*), code:bash (go build -o bin/bt-dashboard ./cmd/bt-dashboard/), code:bash (go build -o bin/benchcmp ./cmd/benchcmp/) (+3 more)

### Community 429 - "Community 429"
Cohesion: 0.18
Nodes (11): code:bash (export PATH=$PATH:/usr/local/go/bin), code:bash (# Check if Ollama is running), code:bash (# Find and kill the old process), code:bash (# Run only fast tests (no Ollama)), code:bash (# Verify Go version), Dashboard port already in use, "Failed to build", "go: command not found" (+3 more)

### Community 430 - "Community 430"
Cohesion: 0.18
Nodes (11): ActionFunc / ConditionFunc, Blackboard, Chain Types (ChainAction nodes), code:go (type Blackboard struct {), code:go (type SerializableNode struct {), code:go (type ActionFunc = func(ctx *btcore.BTContext[Blackboard]) in), code:go (// BuildTree constructs a go-bt Command from a SerializableN), Functions (+3 more)

### Community 431 - "Community 431"
Cohesion: 0.18
Nodes (11): Catalog, code:go (type Definition struct {), code:go (type Registry struct { ... }), code:go (type Catalog struct { ... }), code:go (type Scheduler struct { ... }), code:go (type History struct { ... }), History, Package: agent (+3 more)

### Community 432 - "Community 432"
Cohesion: 0.18
Nodes (10): Architecture, Context, Error handling, Infra (`monitoring/`), Instrumentation, Log correlation & cleanup, Logging & Tracing Overhaul — Design, Staging (one commit each) (+2 more)

### Community 433 - "Community 433"
Cohesion: 0.18
Nodes (10): code:go (package engine), code:go (// RegisterAction adds an action to the global registry, wra), code:bash (git add internal/engine/registry.go internal/engine/registry), code:go (engine.Init()), code:bash (git add internal/ cmd/), Global Constraints, Logging & Tracing Overhaul Implementation Plan, Self-Review Notes (+2 more)

### Community 434 - "Community 434"
Cohesion: 0.24
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (--- FAIL: TestPrefixedDomainIDsAreDescribable/domain:code_re), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT, Summary

### Community 435 - "Community 435"
Cohesion: 0.18
Nodes (6): code:block1 (internal/gardener/evolve_v2_test.go:2497:3: unknown field Lo), code:block2 (# github.com/nico/go-bt-evolve/internal/evolution [test]), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT — FAIL (intended)

### Community 436 - "Community 436"
Cohesion: 0.18
Nodes (4): code:block1 (cmd/bt-dashboard/main_test.go:116:9: got.RequeuedAt undefine), code:block2 (cmd/bt-dashboard/main_test.go:112:16: entries[0].RequeuedAt ), RED phase complete, Summary

### Community 437 - "Community 437"
Cohesion: 0.18
Nodes (10): code:block1 (--- FAIL: TestRunTask_PreservesRateLimitSentinelOnFailureCod), code:block2 (/usr/local/go/bin/go test ./internal/engine ./cmd/bt-agent -), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/engine	53.965s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase (+2 more)

### Community 438 - "Community 438"
Cohesion: 0.24
Nodes (9): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/engine	56.752s), code:block3 (--- PASS: TestClearSuperpowersPlanState_WipesDurableChargeSt), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+1 more)

### Community 439 - "Community 439"
Cohesion: 0.18
Nodes (10): FILES_CHANGED, FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, NOTES, RED_COMMAND (+2 more)

### Community 440 - "Community 440"
Cohesion: 0.22
Nodes (8): code:block1 (internal/evolution/pareto_test.go:341:15: pf.Load undefined ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.868s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 441 - "Community 441"
Cohesion: 0.18
Nodes (10): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./inte), code:block2 (internal/agent/runner_test.go:133:10: undefined: AuctionWinn), code:block3 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./inte), code:block4 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.815s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND (+2 more)

### Community 442 - "Community 442"
Cohesion: 0.27
Nodes (7): fakeExecutor, fakeLLM, newTestApp(), TestAgentLifecycleCommands(), TestAskIncludesPlatformContext(), TestNaturalLanguageIntentParser(), TestRunTreeCommandDelegatesToExecutor()

### Community 445 - "Community 445"
Cohesion: 0.22
Nodes (8): circuitBreakerFileEntry, circuitBreakersFile, newCircuitBreakerStore(), NewWinnerCircuitBreakerStore(), TestValidateAgentRun_Allowed(), TestValidateAgentRun_Blocked(), TestValidateAgentRun_NilStore(), validateAgentRun()

### Community 446 - "Community 446"
Cohesion: 0.27
Nodes (8): classifyErrorHandlerFailure(), isGoapFusionCycle(), TestClassifyErrorHandlerFailure(), TestClassifyReliabilityFallback(), TestClaudeErrorHandler_ClassifiesUnclassifiedFailure(), TestClaudeErrorHandler_GoapFailureGetsGoapCategory(), TestErrorHandlerSignature_ClassifiedWithoutLastErrorIsStable(), TestIsGoapFusionCycle()

### Community 448 - "Community 448"
Cohesion: 0.44
Nodes (9): BuildDecisionTree(), childMatchesDecision(), chooseDecisionBranch(), decisionKey(), decisionValue(), defaultDecisionBranch(), stringifyDecisionValue(), collectBranchLabels() (+1 more)

### Community 449 - "Community 449"
Cohesion: 0.44
Nodes (9): baselineCmd(), baselinePath(), checkCmd(), main(), printUsage(), resetCmd(), run(), showCmd() (+1 more)

### Community 450 - "Community 450"
Cohesion: 0.33
Nodes (6): GeneratedAgent, Generator, generatedFallbackChainAction(), generatedFallbackChainActionFromStep(), generatedSelfCorrectChainAction(), generatedStepChainAction()

### Community 451 - "Community 451"
Cohesion: 0.2
Nodes (9): Block, ChartDataPoint, DiagramEdge, DiagramNode, FeedbackEvent, GeneratedPage, IntentSession, PageSchema (+1 more)

### Community 452 - "Community 452"
Cohesion: 0.31
Nodes (8): initDoormateTab(), loadProfile(), ratePage(), renderBubbles(), renderGeneratedPage(), renderProfileTags(), sendPhrase(), toggleBookmark()

### Community 453 - "Community 453"
Cohesion: 0.2
Nodes (10): 9.1 Run with real Ollama (optional), 9.2 Start the evolution daemon, 9.3 Register with Hermes Agent, 9.4 Explore deeper, 9.5 Run continuous integration locally, 9. Next Steps, code:bash (# Pull the model), code:bash (./bin/bt-gardener &) (+2 more)

### Community 454 - "Community 454"
Cohesion: 0.2
Nodes (10): All LLM-dependent services crash immediately, code:bash (# Check current value), code:bash (# 1. Verify the binary works), code:bash (ps aux | grep 'bin/bt-' | awk '{print $2, $9, $NF}' | sort -), code:bash (jq -r '(.candidates[0].operation // (.[0].operation // "none), Duplicate MCP processes, Gateway restart loop (hundreds of restarts), jq `.[0]` fails with exit code 5 on ev_order_mutations (+2 more)

### Community 455 - "Community 455"
Cohesion: 0.2
Nodes (10): A2A (port 8686), `bt-agent-cli run`, code:json ({ "agent": "code-reviewer", "task": "Review this function fo), code:json ({ "tree": "domain:code_review", "task": "..." }), code:bash (go run ./cmd/bt-agent-cli/ run code-reviewer --input "Review), Dashboard “Run agent” / pipelines, Direct tree execution (no agent YAML), On-demand: MCP `bt_agent_run` (+2 more)

### Community 456 - "Community 456"
Cohesion: 0.2
Nodes (9): Acceptance Criteria, code:go (package fusion), code:bash (cd /home/nico/go-bt-evolve), Current Codebase Anchors, Final Verification Checklist, Fusion-Style BT Agent Implementation Plan, Production Safety Notes, Proposed Data Model (+1 more)

### Community 457 - "Community 457"
Cohesion: 0.2
Nodes (9): 1. Loop resilience — infra failures must not burn milestone attempts, 2. Duplicate work between the two runners — verified non-issue + staggering, 3. Doc-drift check validates the invoking worktree, 4. DLQ replay consumer (finishes program c8094002 ms1–3), 5. Selector telemetry + specialists: writer always on, consumers opt-in, 6. Small fixes, 7. Deployment & reconciliation (ops, after landing), Loop Resilience, Telemetry Wiring, and Fleet Streamlining — Design (+1 more)

### Community 458 - "Community 458"
Cohesion: 0.2
Nodes (10): Anti-patterns (don'ts), Anti-patterns to reject, arc42 Section 4 (Solution Strategy) — Conformance Checklist, arc42 Section 6 (Runtime View) — Conformance Checklist, Do's (from official tips), Does NOT belong here (goes elsewhere in arc42), Does NOT belong here (lives elsewhere in arc42), Required content (per arc42 template) (+2 more)

### Community 459 - "Community 459"
Cohesion: 0.2
Nodes (10): arc42 Section 1 (Introduction and Goals) — Conformance Checklist, arc42 Section 2 (Constraints) — Conformance Checklist, Belongs elsewhere in arc42 (move it out), Do's, Do's (from the official tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns, Don'ts / anti-patterns (+2 more)

### Community 460 - "Community 460"
Cohesion: 0.2
Nodes (10): Anti-patterns to reject, arc42 Section 10 (Quality Requirements) — Conformance Checklist, arc42 Section 8 (Crosscutting Concepts) — Conformance Checklist, Belongs elsewhere in arc42, Do's, Do's (from the official tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns (+2 more)

### Community 461 - "Community 461"
Cohesion: 0.2
Nodes (10): code:yaml (server:), code:yaml (auth_enabled: false), code:yaml (name: bt-observability), code:yaml (apiVersion: 1), code:yaml (apiVersion: 1), code:json ({), code:make (observability-up:), code:bash (make observability-up && sleep 20) (+2 more)

### Community 462 - "Community 462"
Cohesion: 0.2
Nodes (9): code:mermaid (flowchart TD), code:block2 (PersistentMemSequence "SuperpowersWorkflow_Main"          ← ), code:mermaid (flowchart TD), Global Constraints, Part A — Complete Superpowers Workflow Graph, Part B — The Same Graph as a Behavior Tree, Part C — Design Decision Tree, Part D — Grill-Me Record (gap analysis, branch by branch) (+1 more)

### Community 463 - "Community 463"
Cohesion: 0.33
Nodes (5): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.104s), code:block2 (internal/gardener/gardener_test.go:108:55: unknown field Cri), GREEN phase, RED phase, Summary

### Community 464 - "Community 464"
Cohesion: 0.33
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/llm -short -), GREEN phase, RED phase, Summary

### Community 465 - "Community 465"
Cohesion: 0.2
Nodes (9): code:block1 (/usr/local/go/bin/go test ./internal/gardener -short -count=), code:block2 (expected LocalSearchDelta==0 when ValidationGate rejects the), code:block3 (/usr/local/go/bin/go test ./internal/gardener -short -count=), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+1 more)

### Community 466 - "Community 466"
Cohesion: 0.24
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH gofmt -l internal/agentexec/wir), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 467 - "Community 467"
Cohesion: 0.22
Nodes (7): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard ./internal/reli), code:block2 (cmd/bt-dashboard/main_test.go:115:16: entries[0].RequeuedAt ), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT, Summary

### Community 468 - "Community 468"
Cohesion: 0.29
Nodes (6): code:block1 (--- FAIL: TestPrioritizeGoapGoals_ConcurrentWithPersistGoapP), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/reliability	16.21), GREEN phase, RED phase, Summary, Summary

### Community 469 - "Community 469"
Cohesion: 0.22
Nodes (3): code:block1 (cmd/bt-agent/main_test.go:374:8: undefined: jitteredDriftInt), code:block2 (# github.com/nico/go-bt-evolve/cmd/bt-agent [github.com/nico), Summary

### Community 470 - "Community 470"
Cohesion: 0.22
Nodes (8): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.126s), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT, Summary

### Community 472 - "Community 472"
Cohesion: 0.31
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/evolution ./internal/ga), FILES_CHANGED, GREEN_COMMANDS, GREEN_COMMANDS / GREEN_RESULTS, GREEN_RESULTS, NOTES

### Community 473 - "Community 473"
Cohesion: 0.22
Nodes (8): code:block1 (internal/reliability/reliability_test.go:1288:17: undefined:), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase, Summary

### Community 474 - "Community 474"
Cohesion: 0.2
Nodes (9): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./internal/evo), code:block2 (niche "n50|d50|" stores the live tree pointer itself instead), code:block3 (expected CycleMetrics.EliteReseed == true — an earlier cycle), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase (+1 more)

### Community 475 - "Community 475"
Cohesion: 0.2
Nodes (7): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/llm	7.495s), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/llm	21.140s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 476 - "Community 476"
Cohesion: 0.22
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/benchmark -short -count), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/benchmark	0.065s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 477 - "Community 477"
Cohesion: 0.2
Nodes (9): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/reliability ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.73), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.80), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+1 more)

### Community 478 - "Community 478"
Cohesion: 0.22
Nodes (8): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/security ./c), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/security	4.021s), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/security	4.057s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 479 - "Community 479"
Cohesion: 0.2
Nodes (9): code:block1 (--- FAIL: TestAdoptIslandWinner_SkipsWhenQualityGateDisabled), code:block2 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block3 (ok  github.com/nico/go-bt-evolve/internal/gardener  0.191s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+1 more)

### Community 480 - "Community 480"
Cohesion: 0.27
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES, Summary

### Community 481 - "Community 481"
Cohesion: 0.2
Nodes (9): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block2 (--- FAIL: TestRefineGated_RejectsWhenGateRefuses (0.00s)), code:block3 (/usr/local/go/bin/go test ./internal/evolution -short -count), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES (+1 more)

### Community 482 - "Community 482"
Cohesion: 0.44
Nodes (7): LoadMutatedTreeOverride(), mutatedTreesDir(), sanitizeTreeID(), SaveMutatedTree(), TestLoadMutatedTreeOverride_CorruptFileReturnsNil(), TestMutatedTreeFilenameSanitized(), TestMutatedTreeOverrideRoundTrip()

### Community 483 - "Community 483"
Cohesion: 0.28
Nodes (5): exportMemoryCategory(), exportPreviousRuns(), minInt(), TestExportPreviousRuns(), TestSeedMemoryToBlackboard_HistoryOffloaded()

### Community 485 - "Community 485"
Cohesion: 0.36
Nodes (7): ClarifyGateBlock(), clarifyLeafNames(), clarifyNodeCount(), TestClarifyGateBlock_Branches(), TestClarifyGateBlock_LeafOrder(), TestClarifyGateBlock_NodeCount(), TestClarifyGateBlock_Root()

### Community 486 - "Community 486"
Cohesion: 0.33
Nodes (7): financeActionCases(), runFinanceAction(), TestBuildDCFModel(), TestBuildLBOModel(), TestFinanceActions_FixedOutputs(), financeActionCase, financeSpyLLM

### Community 487 - "Community 487"
Cohesion: 0.28
Nodes (9): parseEventSources(), TestFromMap_NonMap(), TestParseEventSources_Empty(), TestParseEventSources_InvalidItemInEvents(), TestParseEventSources_LegacyOnly(), TestParseEventSources_List(), TestParseEventSources_NilMetadata(), TestParseEventSources_NonListRaw() (+1 more)

### Community 488 - "Community 488"
Cohesion: 0.22
Nodes (8): TestValAsFloat64_BoolFalse(), TestValAsFloat64_BoolTrue(), TestValAsFloat64_Float64(), TestValAsFloat64_Int(), TestValAsFloat64_Int64(), TestValAsFloat64_Nil(), TestValAsFloat64_String(), valAsFloat64()

### Community 489 - "Community 489"
Cohesion: 0.36
Nodes (6): HabitMiner, cosine(), jaccard(), keywordSet(), sortedByCount(), RecurringPattern

### Community 491 - "Community 491"
Cohesion: 0.36
Nodes (6): resolveGraphifyBin(), TestResolveGraphifyBinErrorsWhenGenuinelyAbsent(), TestResolveGraphifyBinFallsBackWhenAbsentFromPATH(), TestResolveGraphifyBinPrefersPATH(), TestResolveGraphifyBinRejectsNonExecutableFallback(), writeExecutableGraphify()

### Community 493 - "Community 493"
Cohesion: 0.22
Nodes (8): DebateTurn, Fellow, Report, ResearchFinding, ReviewComment, Scenario, Synthesis, ThinkTank

### Community 495 - "Community 495"
Cohesion: 0.28
Nodes (7): Config, EvolvedAgent, buildEvolvedPrompt(), toolDescriptions(), toolNames(), TestBuildEvolvedPrompt(), TestToolDescriptions()

### Community 496 - "Community 496"
Cohesion: 0.22
Nodes (9): 8.1 Overview tab, 8.2 ThinkTank tab, 8.3 Company tab, 8.4 Tasks tab, 8.5 Trees tab, 8.6 MindMap tab, 8.7 Evolution tab, 8.8 Chat panel (+1 more)

### Community 497 - "Community 497"
Cohesion: 0.22
Nodes (8): 7.1 Check the metrics tracker, 7.2 Explore the evolution algorithms, 7.3 View quality gates, 7. Understand Tree Evolution (5 min), code:bash (# If gardener has been running:), code:bash (grep -n "QualityGate\|MinComposite\|MaxRegression\|Consecuti), Go BT Platform — Interactive Tutorial, Tutorial Complete! 🎉

### Community 498 - "Community 498"
Cohesion: 0.22
Nodes (9): code:bash (go run ./cmd/bt-agent-cli/ install-templates   # copy repo a), code:bash (cp ~/.go-bt-evolve/agents/templates/code-reviewer.yaml ~/.go), code:json ({), code:json ({ "from_template": "code-reviewer" }), Creating agents, Option A — Copy a template into the registry, Option B — MCP `bt_agent_create` (custom), Option C — MCP `bt_agent_create` (from template) (+1 more)

### Community 499 - "Community 499"
Cohesion: 0.22
Nodes (8): code:bash (/usr/local/go/bin/go test ./internal/domains ./internal/engi), GOAP Fusion Superpowers Bridge Implementation Plan, Task 1: Restore and adapt dual-mode GOAP fusion tree, Task 2: Add Superpowers plan-writing action, Task 3: Add Claude implementation action using the Superpowers plan, Task 4: Harden shell and verification behavior, Task 5: Add regression tests, Task 6: Verified deployment

### Community 500 - "Community 500"
Cohesion: 0.22
Nodes (9): code:go (func TestSyncArc42SectionsAndReadmeRunsClassifiedSubset(t *t), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (// syncArc42SectionsAndReadme is the pipeline entry point re), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (// Keep the architecture documentation in the same commit as), code:go (// Keep the architecture documentation in the same commit as), code:bash (git rm internal/engine/superpowers_arc42.go internal/engine/), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -sho) (+1 more)

### Community 501 - "Community 501"
Cohesion: 0.22
Nodes (9): code:bash (grep -rn "ConsoleTracer\|ConfigureOTLPFromEnv\|NewTraceReade), code:bash (git rm internal/tracing/batcher.go internal/tracing/batcher_), code:go (// Package tracing is a thin facade over the OpenTelemetry S), code:go (// ── Tracing ──────────────────────────────────────────────), code:go (// ── Tracing (OTel SDK; no-op unless OTEL_EXPORTER_OTLP_END), code:go (traceCtx := context.Background()), code:go (package tracing), code:bash (git add -A internal/tracing/ cmd/ internal/engine/mcp_server) (+1 more)

### Community 502 - "Community 502"
Cohesion: 0.22
Nodes (9): code:bash (git add internal/engine/actions_superpowers_debug.go interna), code:bash (git add internal/engine/bandit_selector.go internal/engine/b), code:bash (git add docs/adr/ADR-010-superpowers-workflow-tree.md docs/a), Known deviations / follow-ups, Part E — Implementation Tasks, Self-Review (writing-plans checklist — executed 2026-07-02), Task 11: Debugging-phase + finish-option actions, Task 12: `BanditSelector` (flag-gated) + stats recorder (+1 more)

### Community 503 - "Community 503"
Cohesion: 0.36
Nodes (3): GREEN phase, RED phase, Summary

### Community 504 - "Community 504"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.143s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 505 - "Community 505"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./internal/a2a ./cmd/bt-agent -sho), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.264s), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 506 - "Community 506"
Cohesion: 0.31
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -short -), code:block2 (tools_test.go:1607: bt_evolve_pareto must report "benchmark_), GREEN phase, RED phase, Summary

### Community 507 - "Community 507"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestMockLLMResponseLengths (0.00s)), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 508 - "Community 508"
Cohesion: 0.22
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/evolution ./internal/ga), code:block2 (internal/evolution/multi_objective_test.go:353:15: undefined), code:block3 (--- FAIL: TestValidationGate_ParetoAcceptsRecoveryForSuccess), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 509 - "Community 509"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./internal/reliability ./internal/), code:block2 (ok  github.com/nico/go-bt-evolve/internal/reliability   12.5), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 510 - "Community 510"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestBTEvolveBottlenecksPersistsEvolvedFitness), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 511 - "Community 511"
Cohesion: 0.25
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/engine	56.752s), code:block3 (--- PASS: TestClearSuperpowersPlanState_WipesDurableChargeSt), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 512 - "Community 512"
Cohesion: 0.28
Nodes (7): code:block1 (--- FAIL: TestMAPElitesPopulation_EvolveMAPElitesCrisisInter), code:block2 (internal/evolution/learning_test.go:90:10: pop.LastMutationR), GREEN phase, RED phase, RED phase complete, RED phase result, Summary

### Community 513 - "Community 513"
Cohesion: 0.33
Nodes (6): code:block1 (go test ./internal/domains ./internal/benchmark -short -coun), code:block2 (ok  github.com/nico/go-bt-evolve/internal/domains    5.123s), GREEN phase, RED phase, RED phase complete, Summary

### Community 514 - "Community 514"
Cohesion: 0.22
Nodes (8): code:block1 (--- FAIL: TestEvolveMAPElites_SameSeedSameArchive (0.02s)), code:block2 (/usr/local/go/bin/go test ./internal/evolution -short -count), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 515 - "Community 515"
Cohesion: 0.22
Nodes (8): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/fusion -shor), code:block2 (--- FAIL: TestRun_JudgeAndSynthesizeGetOwnTimeoutBudget (0.4), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 516 - "Community 516"
Cohesion: 0.22
Nodes (8): code:block1 (/usr/local/go/bin/go test ./internal/engine -run 'TestStatus), code:block2 (--- FAIL: TestReactiveParallelFoldsAbortedAsFailure/success_), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 517 - "Community 517"
Cohesion: 0.36
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/engine ./cmd/benchcmp .), code:block2 (ok      github.com/nico/go-bt-evolve/internal/engine    29.6), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 518 - "Community 518"
Cohesion: 0.54
Nodes (7): collect_issues(), fix_errcheck(), fix_unused_parameter(), load_lines(), main(), save_lines(), wrap_errcheck_line()

### Community 520 - "Community 520"
Cohesion: 0.5
Nodes (7): structuralParent(), structuralTestFactory(), TestStructuralCrossover_CachesExtractedTemplates(), TestStructuralCrossover_ChildDoesNotAliasParent(), TestStructuralCrossover_SplicesRealSubtrees(), TestStructuralCrossover_UnregisteredParentsSkipped(), TestStructuralCrossover_ValidationGate()

### Community 521 - "Community 521"
Cohesion: 0.43
Nodes (6): NotebookLMFitness(), RegisterNotebookLMFitness(), TestRegisterNotebookLMFitness_DoesNotAffectOtherTrees(), TestRegisterNotebookLMFitness_WiresIntoKnowledgeGraph(), TestRegisterNotebookLMFitness_WiresIntoRealProductionTree(), NotebookLMRunSummary

### Community 522 - "Community 522"
Cohesion: 0.29
Nodes (6): Analytics, minInt(), BottleneckEntry, CentralityEntry, ContentionEntry, SelectionPressureEntry

### Community 523 - "Community 523"
Cohesion: 0.25
Nodes (3): TestInitEngineDelegate(), InitEngineDelegate(), TaskStateBridge

### Community 525 - "Community 525"
Cohesion: 0.39
Nodes (7): distributedDispatch, appendDispatchErr(), driveDistributedDispatch(), main(), parseNodes(), run(), multiNodeReport

### Community 526 - "Community 526"
Cohesion: 0.36
Nodes (4): isCompressibleContentType(), TestIsCompressibleContentType(), compressionResponseWriter, gzipResponseWriter

### Community 527 - "Community 527"
Cohesion: 0.25
Nodes (7): BT Agent Platform, code:bash (PATH=/usr/local/go/bin:$PATH make check-quick), Commands, Conventions, Known flake, MCP, Toolchain — read this first

### Community 528 - "Community 528"
Cohesion: 0.25
Nodes (8): 6.1 Explore the knowledge graph, 6.2 Create a tree programmatically (Go code), 6.3 Register your tree, 6. Create Your First Custom Behavior Tree (10 min), code:bash (# View knowledge graph stats (via API)), code:go (package main), code:bash (go run ./cmd/my-tree/), code:go (// In cmd/bt-agent/main.go, add your tree to the resolveTree)

### Community 529 - "Community 529"
Cohesion: 0.25
Nodes (8): 2.1 Fast test suite (no LLM needed), 2.2 Coverage snapshot, 2.3 Check for common issues, 2. Run Tests (3 min), code:bash (go test -short -count=1 -timeout 60s ./...), code:block5 (ok  	github.com/nico/go-bt-evolve/internal/engine	0.123s), code:bash (go test -short -count=1 -coverprofile=/tmp/bt-coverage.out .), code:bash (go vet ./...)

### Community 530 - "Community 530"
Cohesion: 0.25
Nodes (7): BT Platform Troubleshooting Guide, code:bash (# Is Ollama running?), Condition & Routing Issues, Domain tree silently fails (no LLM calls), Keyword overlap causing misrouting, Quick Diagnostics, See Also

### Community 531 - "Community 531"
Cohesion: 0.25
Nodes (7): arc42 goal anchoring (commit 604b1e4), Chronic runner fixes, Landing procedure, NotebookLM corpus hygiene, Research Process Overhaul — 2026-07-08, Research quality findings (review that motivated the overhaul), The runaway-backstop deadlock

### Community 532 - "Community 532"
Cohesion: 0.25
Nodes (8): Agent runs but quality always low, code:bash (# Verify tree ID in agent YAML), LLM errors on run, “no tree found for agent X”, Pipeline not found, Scheduled agent never runs, “template not found” on create, Troubleshooting

### Community 533 - "Community 533"
Cohesion: 0.25
Nodes (7): CI mapping, Environment, Gosec excludes, Implementation checklist (branch `cursor/reusable-tree-blocks-c122`), Quick reference, Security and linting — local checks vs CI, Tool versions

### Community 534 - "Community 534"
Cohesion: 0.25
Nodes (8): code:go (type Tool interface {), code:text (Thought: ...), code:text (Final Answer: ...), code:go (func TestToolLoop_StopsAtMaxToolCalls(t *testing.T)), code:text (Web tools unavailable in this run; state uncertainty explici), code:bash (go test ./internal/fusion -run 'TestToolLoop|TestRunPanel|Te), code:bash (git add internal/fusion/tools.go internal/fusion/tools_test.), Task 5: Add Optional Web Search / Fetch Tool Loop

### Community 535 - "Community 535"
Cohesion: 0.25
Nodes (7): 3.1 Business Context, 3.2 Technical Context, 3. Context and Scope, code:block1 (Hermes Agent ──(MCP tools via ─┐             ┌─(HTTP :9800)─), Communication Partners, External Interfaces, System Boundary

### Community 536 - "Community 536"
Cohesion: 0.25
Nodes (8): code:go (package domains), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/domains/ -ru), code:go (package domains), code:go ("arc42:docsync":             Arc42DocsyncTree(),), code:go ("arc42:docsync":             "Per-section arc42 + README doc), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/domains/ -co), code:bash (PATH=/usr/local/go/bin:$PATH make check-quick), Task 19: `arc42:docsync` tree + Commit 2

### Community 537 - "Community 537"
Cohesion: 0.25
Nodes (7): code:go (func (tt *TranspositionTable) Save() error {), code:go (func exportSLOMetrics(path string, sloData map[string]float6), code:go (sloData := CollectAgentSLOs(g.cfg.ValidationGate.EvidencePat), code:go (// KanbanAndHermesDomainTrees returns the kanban workflow an), Task 1: Fix `TranspositionTable.Save` to delegate to `util.SaveJSONAtomic`, Task 2: Extract and fix the gardener's SLO-metrics export to use `util.SaveJSONAtomic`, Task 3: Give the kanban and hermes-evolve trees the same smoke-test coverage as `AllDomainTrees()`

### Community 538 - "Community 538"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 540 - "Community 540"
Cohesion: 0.36
Nodes (5): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent ./internal/knowledg), GREEN phase, RED phase, RED Phase Result, Summary

### Community 542 - "Community 542"
Cohesion: 0.43
Nodes (4): code:block1 (--- FAIL: TestRunTask_TerminalBackstopFlipsOutcome (0.00s)), GREEN phase, RED phase, Summary

### Community 543 - "Community 543"
Cohesion: 0.39
Nodes (5): code:block1 (--- FAIL: TestEvolveTreeV2_WritesPreMutationSnapshot (0.00s)), code:block2 (gofmt -l internal/gardener/evolve_v2.go), GREEN phase, RED phase, Summary

### Community 544 - "Community 544"
Cohesion: 0.25
Nodes (7): code:block1 (internal/domains/domains_test.go:2425:26: undefined: Resolve), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 546 - "Community 546"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 547 - "Community 547"
Cohesion: 0.25
Nodes (7): FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT, Summary

### Community 548 - "Community 548"
Cohesion: 0.32
Nodes (5): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-ci-doctor	0.009s), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-ci-doctor	0.009s), GREEN phase, RED phase, Summary

### Community 549 - "Community 549"
Cohesion: 0.32
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block2 (# github.com/nico/go-bt-evolve/internal/evolution [build fai), GREEN phase, RED phase, Summary

### Community 550 - "Community 550"
Cohesion: 0.25
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH go build ./...), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 551 - "Community 551"
Cohesion: 0.25
Nodes (7): code:block1 (internal/evolution/learning_test.go:139:33: NewExpertKnowled), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 552 - "Community 552"
Cohesion: 0.25
Nodes (6): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.091s), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.085s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 553 - "Community 553"
Cohesion: 0.25
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./internal/evo), code:block2 (niche "n50|d50|" stores the live tree pointer itself instead), code:block3 (expected CycleMetrics.EliteReseed == true — an earlier cycle), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 554 - "Community 554"
Cohesion: 0.25
Nodes (7): code:block1 (/usr/local/go/bin/go test ./cmd/benchcmp ./cmd/bt-dashboard ), code:block2 (--- FAIL: TestNoCommittedCommandBinaries (0.00s)), code:block3 (/usr/local/go/bin/go test ./cmd/benchcmp ./cmd/bt-dashboard ), code:block4 (ok  github.com/nico/go-bt-evolve/cmd/benchcmp        0.014s), GREEN phase, RED phase, Summary

### Community 555 - "Community 555"
Cohesion: 0.25
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -run 'Te), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 557 - "Community 557"
Cohesion: 0.25
Nodes (7): FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT, Summary

### Community 558 - "Community 558"
Cohesion: 0.25
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/reliability ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.73), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.80), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 559 - "Community 559"
Cohesion: 0.29
Nodes (6): code:block1 (internal/evolution/island_test.go:146:6: im.Expert undefined), code:block2 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block3 (ok  github.com/nico/go-bt-evolve/internal/evolution  3.6-3.7), GREEN phase, RED phase, Summary

### Community 560 - "Community 560"
Cohesion: 0.25
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH gofmt -l internal/knowledge/fee), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 561 - "Community 561"
Cohesion: 0.25
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block2 (internal/evolution/learning_test.go:206:3: unknown field Spe), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 562 - "Community 562"
Cohesion: 0.25
Nodes (7): code:block1 (/usr/local/go/bin/go test -race ./internal/util -short -coun), code:block2 (# github.com/nico/go-bt-evolve/internal/util [github.com/nic), code:block3 (save error = commit .../fixture.json: rename .../fixture.jso), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 563 - "Community 563"
Cohesion: 0.25
Nodes (7): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/domains -sho), FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 564 - "Community 564"
Cohesion: 0.39
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.143s), GREEN phase, RED phase, Summary

### Community 565 - "Community 565"
Cohesion: 0.25
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestBuildCompositeChildren_GuardEvaluatedAtRunTime), code:go (if cond := guardConditionForChild(node.Edges, i); cond != ""), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 566 - "Community 566"
Cohesion: 0.25
Nodes (7): code:block1 (internal/agent/webhook_publisher_test.go:650:16: pub.breaker), FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 567 - "Community 567"
Cohesion: 0.25
Nodes (7): code:block1 (--- PASS: TestExecutionRouterDegradesToScheduledAnalysisOnRe), FILES_CHANGED, NOTES, RED_COMMAND, RED Phase Report, RED_RESULT, What I verified

### Community 572 - "Community 572"
Cohesion: 0.48
Nodes (5): TestIdleGate_FirstCycleAlwaysRuns(), TestIdleGate_ForcesACycleAfterTheIdleCeiling(), TestIdleGate_RunsAgainWhenNewReflectionDataArrives(), TestIdleGate_SkipsWhenNoNewReflectionData(), writeReflection()

### Community 574 - "Community 574"
Cohesion: 0.48
Nodes (6): act(), ApplyReliability(), cond(), seqFallback(), WrapReliable(), ReliabilitySpec

### Community 575 - "Community 575"
Cohesion: 0.33
Nodes (5): containsLeafChildrenMsg(), TestRunTask_BackstopsEmptyResultOnNonSuccess(), TestRunTask_PreservesPendingApprovalOutcome(), TestRunTask_PreservesRateLimitCarryoverOutcome(), TestValidateTree_LeafTypesRejectChildren()

### Community 577 - "Community 577"
Cohesion: 0.57
Nodes (5): act(), cond(), seq(), TestValidateErrorHandlerProposal_RefusesUnrecoverableCategories(), TestValidateErrorHandlerProposal_RejectsPurelyDeclarativeRecovery()

### Community 579 - "Community 579"
Cohesion: 0.38
Nodes (4): docChangeContextFromBlackboard(), init(), registerArc42SyncNode(), TestDocChangeContextFromBlackboardFallsBackToGit()

### Community 583 - "Community 583"
Cohesion: 0.52
Nodes (6): assertRouteConds(), evalCond(), TestRouteConditions_ExactFallback(), TestRouteConditions_FellBackToDefault(), TestRouteConditions_ModelResolved(), TestRouteConditions_NoRouting()

### Community 584 - "Community 584"
Cohesion: 0.57
Nodes (5): degeneratePopulation(), runWithoutPanic(), TestEvolve_DegeneratePopulations(), TestEvolveWithExperience_DegeneratePopulations(), TestMemeticEvolve_DegeneratePopulations()

### Community 586 - "Community 586"
Cohesion: 0.38
Nodes (3): ctxRecordingExecutor, TestAgentRouterExecute_PropagatesCallerContext(), TestAgentRouterExecute_StopsFailoverWhenContextDone()

### Community 589 - "Community 589"
Cohesion: 0.29
Nodes (7): 4.1 Start the server, 4.2 Verify it's running, 4.3 Explore the API, 4. Start the Dashboard (3 min), code:bash (./bin/bt-dashboard &), code:bash (curl -s http://localhost:9800/api/health | head -c 200), code:bash (# Platform summary)

### Community 590 - "Community 590"
Cohesion: 0.29
Nodes (7): 1.1 Clone and enter the repo, 1.2 Verify Go version, 1.3 Download dependencies, 1. Setup (2 min), code:bash (git clone https://github.com/nico/go-bt-evolve.git), code:bash (go version), code:bash (go mod download)

### Community 591 - "Community 591"
Cohesion: 0.29
Nodes (7): 5.1 Explore available trees, 5.2 Run a task through the GoDev tree, 5.3 Try different task types, 5. Execute Tasks Through Behavior Trees (5 min), code:bash (# List all registered trees), code:bash (# Via mcporter (if installed):), code:bash (# Research task → deep_research tree)

### Community 592 - "Community 592"
Cohesion: 0.29
Nodes (7): code:bash (# Reset tree to default), code:go (ss := d.Stats[selectorName]), Decision tree optimizer crashes with SIGSEGV, Evolution & Gardener Issues, Gardener applies 0 mutations per cycle, Mutation death spiral (97.3% regression rate), Transposition Table always returns 0 entries

### Community 593 - "Community 593"
Cohesion: 0.29
Nodes (7): Background test output flooded by zsh init, code:bash (go test -short -count=1 -timeout 60s ./...), code:bash (go test -count=1 -timeout 1200s ./...), code:bash (exec bash -c 'cd ~/go-bt-evolve && go test -v -run "TestName), Empty tool output = success, not error, Ollama-dependent tests timeout, Test & Benchmark Issues

### Community 594 - "Community 594"
Cohesion: 0.29
Nodes (7): CLI `bt-agent-cli schedule`, code:json ({ "agent": "system-monitor", "schedule": "*/5 * * * *", "tim), code:bash (go run ./cmd/bt-agent-cli/ schedule system-monitor --every "), MCP `bt_agent_schedule`, Schedule syntax, Scheduler behavior, Scheduling

### Community 595 - "Community 595"
Cohesion: 0.29
Nodes (7): code:bash (AGENT_HOME="${BT_AGENT_HOME:-$HOME/.go-bt-evolve}"), code:powershell ($AgentHome = if ($env:BT_AGENT_HOME) { $env:BT_AGENT_HOME } ), code:bash (# Terminal 1 — MCP server + scheduler + A2A (port 8686)), First-time setup, Linux / macOS, Start the runtime, Windows (PowerShell)

### Community 596 - "Community 596"
Cohesion: 0.29
Nodes (7): code:go (func TestConfig_DefaultsQualityPreset(t *testing.T)), code:bash (cd /home/nico/go-bt-evolve), code:go (func DefaultConfig() Config), code:go (var QualityPreset = []string{), code:bash (go test ./internal/fusion -run TestConfig -count=1), code:bash (git add internal/fusion/config.go internal/fusion/config_tes), Task 1: Add Fusion Config Validation

### Community 597 - "Community 597"
Cohesion: 0.29
Nodes (7): code:go (type OpenAICompatConfig struct {), code:go (OpenRouterHost  string `json:"openrouter_host" env:"OPENROUT), code:bash (go test ./internal/llm -run 'TestOpenAICompat|TestNewProvide), code:bash (git add internal/llm/openai_compat.go internal/llm/openai_co), code:go (func TestOpenAICompat_Generate_SendsChatCompletion(t *testin), code:json ({), Task 2: Add OpenRouter/OpenAI-Compatible Model Client

### Community 598 - "Community 598"
Cohesion: 0.29
Nodes (7): code:go (func TestJudge_ParsesStructuredJSON(t *testing.T)), code:text (You are the Fusion judge. Compare panel responses; do not me), code:go (func Judge(ctx context.Context, caller ModelCaller, cfg Conf), code:go (func parseAnalysisJSON(raw string) (Analysis, error)), code:bash (go test ./internal/fusion -run 'TestJudge|TestParseAnalysis'), code:bash (git add internal/fusion/judge.go internal/fusion/judge_test.), Task 4: Implement Judge Structured Analysis

### Community 599 - "Community 599"
Cohesion: 0.29
Nodes (6): Anti-patterns / don'ts, arc42 Conformance Checklists (companion to 2026-07-16-arc42-docs-consolidation-design.md), arc42 Section 3 (Context and Scope) — Conformance Checklist, Do's, Does NOT belong here (lives elsewhere in arc42), Required content & structure

### Community 600 - "Community 600"
Cohesion: 0.29
Nodes (6): GOAP Runner: honest outcome signal, commit auto-fix, and build stamping, Item 1 — Honest outcome labels + authoritative quality, Item 2 — VCS build stamping (fix inert drift detection), Item 3 — Commit auto-fix loop (up to 10 Claude attempts), Out of scope, Verification

### Community 601 - "Community 601"
Cohesion: 0.29
Nodes (7): code:bash (PATH=/usr/local/go/bin:$PATH go get \), code:go (package engine), code:go (package engine), code:go (var extraHandlers []slog.Handler), code:go (logShutdown := engine.InitLogExport("bt-agent")), code:bash (git add internal/engine/log_correlation.go internal/engine/l), Task 4: Log correlation — trace-aware slog handler + OTLP log bridge

### Community 602 - "Community 602"
Cohesion: 0.38
Nodes (7): code:block1 (/usr/local/go/bin/go test ./internal/engine -count=1 -run Te), code:block2 (--- FAIL: TestSuperpowersRuntime_ActionsRegistered_Scheduled), RED_COMMAND, RED Phase Complete, RED Phase Report, RED Phase Result, RED_RESULT

### Community 604 - "Community 604"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.143s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 605 - "Community 605"
Cohesion: 0.38
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.445s), GREEN phase, RED phase, Summary

### Community 607 - "Community 607"
Cohesion: 0.43
Nodes (3): GREEN phase, RED phase, Summary

### Community 608 - "Community 608"
Cohesion: 0.43
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/a2a -short -), GREEN phase, RED phase, Summary

### Community 609 - "Community 609"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/a2a ./cmd/bt-agent -sho), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.264s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 610 - "Community 610"
Cohesion: 0.57
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 611 - "Community 611"
Cohesion: 0.38
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.094s), code:block2 (--- PASS: TestRegistry_RollbackTree (0.01s)), GREEN phase, RED phase

### Community 612 - "Community 612"
Cohesion: 0.43
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-gardener	0.068s), code:block2 (# github.com/nico/go-bt-evolve/cmd/bt-gardener [github.com/n), GREEN phase, RED phase

### Community 613 - "Community 613"
Cohesion: 0.38
Nodes (5): code:block1 (cmd/bt-agent/feedback_tools_test.go:547:10: undefined: perso), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.356s), GREEN phase, RED phase, Summary

### Community 614 - "Community 614"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent ./intern), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.442s), GREEN phase, RED phase, Summary

### Community 615 - "Community 615"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestMockLLMResponseLengths (0.00s)), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 616 - "Community 616"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (=== RUN 1 ===  ok  github.com/nico/go-bt-evolve/internal/dom), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 617 - "Community 617"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 618 - "Community 618"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (structure cycle must leave last-reviewed SHA unchanged: got ), GREEN phase, RED phase, RED phase result, Summary

### Community 620 - "Community 620"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/evolution ./cmd/bt-agen), code:block2 (ok  github.com/nico/go-bt-evolve/internal/evolution  4.429s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 621 - "Community 621"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.083s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 622 - "Community 622"
Cohesion: 0.38
Nodes (5): code:block1 (internal/agent/webhook_publisher_test.go:363:13: pub.DLQ und), code:block2 (ok  github.com/nico/go-bt-evolve/internal/agent   3.557s), GREEN phase, RED phase, Summary

### Community 623 - "Community 623"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 624 - "Community 624"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/knowledge ./cmd/bt-agen), code:block2 (internal/knowledge/analytics_test.go:179:5: kg.ExpectedDomai), GREEN phase, RED phase, Summary

### Community 625 - "Community 625"
Cohesion: 0.43
Nodes (3): code:block1 (panic: onRotate exploded), GREEN phase, RED phase

### Community 626 - "Community 626"
Cohesion: 0.33
Nodes (5): code:go (for _, dep := range sm.DependsOn {), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-evaluator ./in), code:block3 (ok  	github.com/nico/go-bt-evolve/cmd/bt-evaluator	0.026s), GREEN phase, RED phase

### Community 627 - "Community 627"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-dashb), code:block2 (--- FAIL: TestRegistry_DomainDescriptionsResolveThroughDescr), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 628 - "Community 628"
Cohesion: 0.38
Nodes (5): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.841s), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	2.636s), GREEN phase, RED phase, Summary

### Community 629 - "Community 629"
Cohesion: 0.43
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), GREEN phase, RED phase

### Community 630 - "Community 630"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestMetricsTracker_SaveAggregatesDeepSearchCoverag), code:block2 (/usr/local/go/bin/go test ./internal/gardener -short -count=), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.114s), GREEN phase, RED phase

### Community 631 - "Community 631"
Cohesion: 0.38
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestBTEvolveIslandDomainsModeAttributesFitnessPerD), GREEN phase, RED phase

### Community 632 - "Community 632"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine -shor), code:block2 (internal/engine/actions_goap_fusion_test.go:420:2: cannot as), GREEN phase, RED phase, Summary

### Community 633 - "Community 633"
Cohesion: 0.43
Nodes (4): code:block1 (--- FAIL: TestRunTask_NilChainStateDoesNotPanic (0.00s)), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.278s), GREEN phase, RED phase

### Community 634 - "Community 634"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/gardener -short -count=), code:block2 (internal/gardener/evolve_v2_test.go:813:4: unknown field Sel), GREEN phase, RED phase, Summary

### Community 635 - "Community 635"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 636 - "Community 636"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestDetectPath_DataPipelineKeywordFallback), code:block2 (--- FAIL: TestDetectPath_SecurityKeywordFallback/sql_injecti), GREEN phase, RED phase, Summary

### Community 637 - "Community 637"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN_COMMANDS, GREEN phase, GREEN_RESULTS, NOTES, RED phase

### Community 638 - "Community 638"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/security -short -count=), code:block2 (--- FAIL: TestRotateKey_ReplacementDoesNotSilentlyExpire (0.), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 639 - "Community 639"
Cohesion: 0.29
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 640 - "Community 640"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block2 (ok  github.com/nico/go-bt-evolve/internal/gardener  0.191s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 641 - "Community 641"
Cohesion: 0.29
Nodes (6): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agentexec ./), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agentexec	0.018s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 642 - "Community 642"
Cohesion: 0.29
Nodes (6): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/fusion -shor), code:block2 (--- FAIL: TestRun_JudgeAndSynthesizeGetOwnTimeoutBudget (0.4), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 643 - "Community 643"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/engine -run 'TestStatus), code:block2 (--- FAIL: TestReactiveParallelFoldsAbortedAsFailure/success_), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 644 - "Community 644"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/engine -run 'TestBuildR), code:block2 (--- FAIL: TestBuildRateLimit_IntervalResolution), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 645 - "Community 645"
Cohesion: 0.29
Nodes (6): FILES_CHANGED, GREEN phase, NOTES, RED_COMMAND, RED phase, RED_RESULT

### Community 646 - "Community 646"
Cohesion: 0.29
Nodes (6): code:block1 (/usr/local/go/bin/go test ./internal/agent ./internal/a2a -s), code:block2 (--- FAIL: TestAgentCircuitBreakerStore_SaveLocksReadModifyWr), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 647 - "Community 647"
Cohesion: 0.29
Nodes (6): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./inte), code:block2 (internal/agent/runner_test.go:133:10: undefined: AuctionWinn), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 648 - "Community 648"
Cohesion: 0.47
Nodes (4): cmdStatus(), programStatusLines(), TestProgramStatusLines_AllDone(), TestProgramStatusLines_MidProgress()

### Community 649 - "Community 649"
Cohesion: 0.47
Nodes (3): TestServer_Run_MixedToolConcurrentCallsDoNotRaceOnSharedBlackboard(), TestServer_ToolPanicRecovery(), mixedToolSharedLeaf

### Community 650 - "Community 650"
Cohesion: 0.47
Nodes (5): evaluateEventPredicate(), evaluatePredicate(), eventMatches(), toFloat(), EventSource

### Community 651 - "Community 651"
Cohesion: 0.6
Nodes (5): addProgram(), TestActive_PrefersRealWorkOverFiller(), TestActive_SelfFixStillOutranksEverything(), TestActiveExcludingFiller_CoverageAloneDoesNotBlockSeeding(), TestActiveExcludingFiller_RealProgramStillBlocksSeeding()

### Community 652 - "Community 652"
Cohesion: 0.47
Nodes (4): TestVerifyNotebookLMEvidenceAcceptsRealNotebookPayload(), TestVerifyNotebookLMEvidenceRejectsEmptyOrUngroundedSuccess(), TestVerifyNotebookLMEvidenceRejectsFabricatedOutput(), verifyNotebookLMEvidenceAction()

### Community 654 - "Community 654"
Cohesion: 0.53
Nodes (5): init(), registerSuperpowersDebugActions(), superpowersDebugPhase(), superpowersDebugPhaseAction(), debugPhaseSpec

### Community 655 - "Community 655"
Cohesion: 0.33
Nodes (3): PriorityTaskQueue, Queue, QueueError

### Community 657 - "Community 657"
Cohesion: 0.33
Nodes (4): CompanyState, Decision, QuarterResult, SprintResult

### Community 658 - "Community 658"
Cohesion: 0.47
Nodes (4): noRunnerHome(), repoRoot(), TestRun_JSONOutput(), TestRun_TextOutput()

### Community 659 - "Community 659"
Cohesion: 0.53
Nodes (4): handleBlackboard(), handleBlackboardScopes(), parseBlackboardScope(), scopeError

### Community 660 - "Community 660"
Cohesion: 0.4
Nodes (5): loadWorkflowBlackboard(), loadWorkflows(), pollWorkflowStatus(), renderWorkflows(), runWorkflow()

### Community 661 - "Community 661"
Cohesion: 0.53
Nodes (5): pollLiveData(), renderCategories(), renderOverview(), renderSystemStatus(), updateOverviewStats()

### Community 662 - "Community 662"
Cohesion: 0.33
Nodes (6): code:bash (# Audit all ChainAction nodes for low max_tokens), Empty outcome with no error, OOM kill (exit 137), Runtime Issues, Tree reports "success" but produces garbage output, Tree times out at 600s without completing

### Community 663 - "Community 663"
Cohesion: 0.33
Nodes (5): Agent names and files, code:sh (# Loopback listeners), Compatibility impact, HTTP authentication and agent storage migration, Launch settings

### Community 664 - "Community 664"
Cohesion: 0.33
Nodes (6): Blackboard (context offloading), Chain templates, code:bash (go run ./cmd/bt-agent-cli/ bb list --scope agent --id code-r), Dashboard API, MCP tools, ReAct tools (in `agent:` chains)

### Community 665 - "Community 665"
Cohesion: 0.33
Nodes (6): code:go (const ChainFusion ChainKind = "fusion"), code:go (case ChainFusion:), code:go (func TestExecFusion_StoresAnalysisAndResponses(t *testing.T)), code:bash (go test ./internal/fusion -count=1), code:bash (git add internal/engine/chains.go internal/engine/chains_fus), Task 8: Add ChainKind `fusion` to BT Engine

### Community 666 - "Community 666"
Cohesion: 0.33
Nodes (6): code:go (type ModelCaller interface {), code:go (func TestRunPanel_RunsModelsConcurrently(t *testing.T)), code:go (func RunPanel(ctx context.Context, caller ModelCaller, cfg C), code:bash (go test ./internal/fusion -run TestRunPanel -count=1), code:bash (git add internal/fusion/panel.go internal/fusion/panel_test.), Task 3: Implement Parallel Panel Runner

### Community 667 - "Community 667"
Cohesion: 0.33
Nodes (6): code:go (func shouldUseFusionCond(b *Blackboard) bool), code:go (RegisterCondition("ShouldUseFusion", shouldUseFusionCond)), code:go (func TestShouldUseFusion_ResearchPrompt(t *testing.T)), code:bash (go test ./internal/engine -run 'TestShouldUseFusion|TestForc), code:bash (git add internal/engine/fusion_conditions.go internal/engine), Task 9: Add Fusion Routing Conditions

### Community 668 - "Community 668"
Cohesion: 0.33
Nodes (6): code:bash (graphify query "where are built-in agent trees registered an), code:text (search_files("allPlatformTrees|ResolveAgentName|AgentDefinit), code:go (func TestFusionAgent_Listed(t *testing.T)), code:bash (go test ./... -run 'TestFusionAgent|TestFusionDeliberationTr), code:bash (git add <registration files>), Task 11: Add Agent Template / CLI Registration

### Community 669 - "Community 669"
Cohesion: 0.33
Nodes (5): 1.1 Requirements Overview, 1.1a Target Vision — Personalized Self-Evolving Agents (Roadmap), 1.2 Quality Goals, 1.3 Stakeholders, 1. Introduction and Goals

### Community 670 - "Community 670"
Cohesion: 0.33
Nodes (6): code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 18: The 13 registered nodes — TDD

### Community 671 - "Community 671"
Cohesion: 0.33
Nodes (6): code:bash (grep -rn "bt-otlp-collector\|otlp-collector\|BT_OTLP_COLLECT), code:bash (git rm -r cmd/bt-otlp-collector), code:markdown (### Added), code:bash (make observability-up && sleep 20), code:bash (make check-full), Task 10: Remove bt-otlp-collector, changelog, end-to-end verification

### Community 672 - "Community 672"
Cohesion: 0.33
Nodes (6): code:go (package agent), code:go (runCtx := ctx), code:go (_, whSpan := tracing.StartSpan(runCtx.Context, "agent.webhoo), code:go (if err != nil {), code:bash (git add internal/agent/ cmd/bt-agent/main.go), Task 7: Run root span + scheduler webhook span

### Community 673 - "Community 673"
Cohesion: 0.33
Nodes (6): code:go (package engine), code:go (Logger *slog.Logger `json:"-"` // run-scoped logger (run_id/), code:go (// Log returns the run-scoped logger when bound, else the gl), code:go (bb.Logger = engine.L().With("run_id", runID, "agent", agentN), code:bash (git add internal/engine/tree.go internal/engine/blackboard_l), Task 5: Run-scoped logger on the Blackboard

### Community 674 - "Community 674"
Cohesion: 0.33
Nodes (6): code:go (func TestPersistentMemSequence_ResumesFromPersistedCursor(t ), code:go (// internal/engine/persistent_mem_sequence.go), code:go (case "PersistentMemSequence":), code:go (case "MemSelector", "PersistentMemSequence", "ForEachTask":), code:bash (git add internal/engine/persistent_mem_sequence.go internal/), Task 3: `PersistentMemSequence` composite + unique-name validation

### Community 675 - "Community 675"
Cohesion: 0.33
Nodes (6): code:go (// internal/engine/foreach_task_test.go), code:go (// internal/engine/foreach_task.go), code:go (func superpowersTaskRed(ctx context.Context, run *Superpower), code:go (RegisterCondition("PlanHasIndependentTasks", func(bb *Blackb), code:bash (git add internal/engine/foreach_task.go internal/engine/fore), Task 6: `ForEachTask` composite + TDD phase-split actions + `PlanHasIndependentTasks`

### Community 676 - "Community 676"
Cohesion: 0.33
Nodes (6): code:go (// internal/engine/chain_state_util_test.go), code:go (// internal/engine/mem_nodes_test.go), code:go (// internal/engine/chain_state_util.go), code:go (case "MemSequence":), code:bash (git add internal/engine/chain_state_util.go internal/engine/), Task 1: `chainStateInt` helper + expose `MemSequence` node type

### Community 677 - "Community 677"
Cohesion: 0.47
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	4.403s), GREEN phase, RED phase

### Community 678 - "Community 678"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent-cli ./cmd/bt-agent ), code:block2 (--- FAIL: TestParseBBScopeFlagTrimsID (0.00s)), code:block3 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent-cli	0.013s), GREEN phase, RED phase

### Community 680 - "Community 680"
Cohesion: 0.33
Nodes (5): code:block1 (tasks_test.go:99: Approved()[0].ID = "low-early", want "crit), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./internal/dashboard ./), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.641s), GREEN phase, RED phase

### Community 681 - "Community 681"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/gardener -short -count=), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 682 - "Community 682"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH gofmt -l internal/agentexec/wir), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 683 - "Community 683"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestBTEvolveBottlenecksBiasesRetrievalOnLastFailur), GREEN phase, RED phase, Summary

### Community 685 - "Community 685"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), GREEN phase, Outcome, RED phase

### Community 686 - "Community 686"
Cohesion: 0.53
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 687 - "Community 687"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-dashboard -sho), GREEN phase, RED phase, Summary

### Community 689 - "Community 689"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/evolution -s), code:block2 (# github.com/nico/go-bt-evolve/internal/evolution [github.co), GREEN phase, RED phase

### Community 690 - "Community 690"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 691 - "Community 691"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestRunTask_PreservesRateLimitSentinelOnFailureCod), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 692 - "Community 692"
Cohesion: 0.4
Nodes (4): code:block1 (tools_test.go:3074: bt_evolve_qlearning response must surfac), GREEN phase, RED phase, Summary

### Community 693 - "Community 693"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./internal/evo), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 694 - "Community 694"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go build ./...), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 695 - "Community 695"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/blocks ./int), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/blocks	0.042s), GREEN phase, RED phase, Summary

### Community 696 - "Community 696"
Cohesion: 0.4
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.530s), GREEN phase, RED phase, Summary

### Community 697 - "Community 697"
Cohesion: 0.33
Nodes (4): code:block1 (--- FAIL: TestMAPElitesPopulation_EvolveMAPElitesCrisisInter), code:block2 (internal/evolution/learning_test.go:90:10: pop.LastMutationR), RED phase complete, RED phase result

### Community 698 - "Community 698"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/security ./c), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/security	4.185s), GREEN phase, RED phase

### Community 699 - "Community 699"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), code:block2 (internal/evolution/learning_test.go:286:14: pop.HealthSnapsh), GREEN phase, RED phase

### Community 700 - "Community 700"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestRunTaskResult_RateLimitCarryoverOutcome_DoesNo), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./internal/dashboard ./), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.882s), GREEN phase, RED phase

### Community 701 - "Community 701"
Cohesion: 0.4
Nodes (4): code:block1 (# github.com/nico/go-bt-evolve/internal/agent [github.com/ni), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	2.567s), GREEN phase, RED phase

### Community 702 - "Community 702"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 703 - "Community 703"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -run 'Te), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 704 - "Community 704"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestStringMatch_FitnessBreaksKeywordTie), GREEN phase, RED phase, Summary

### Community 705 - "Community 705"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./cmd/bt-gardener ./internal/agent), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-gardener	0.520s), GREEN phase, RED phase, Summary

### Community 706 - "Community 706"
Cohesion: 0.4
Nodes (4): code:block1 (cmd/bt-dashboard/main_test.go:19:15: undefined: dashTaskQueu), GREEN phase, RED phase, Summary

### Community 707 - "Community 707"
Cohesion: 0.4
Nodes (4): GREEN phase, RED phase, Summary, Summary

### Community 708 - "Community 708"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestPrioritizeGoapGoals_StampsResearchGoalDurably ), GREEN phase, RED phase, Summary

### Community 709 - "Community 709"
Cohesion: 0.47
Nodes (4): code:block1 (--- FAIL: TestBuildGardenerConfig_TranspositionTableWired (0), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-gardener	0.208s), GREEN phase, RED phase

### Community 710 - "Community 710"
Cohesion: 0.4
Nodes (4): code:block1 (circuit breaker did not short-circuit: subprocess was launch), GREEN phase, RED phase, Summary

### Community 711 - "Community 711"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go fmt ./internal/knowledge/... ./cmd/bt-a), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.126s), GREEN phase, RED phase

### Community 712 - "Community 712"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener -sh), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.108s), GREEN phase, RED phase, Summary

### Community 713 - "Community 713"
Cohesion: 0.47
Nodes (4): code:block1 (--- FAIL: TestBTDelegateToTreeHoldsBBMu (0.00s)), GREEN phase, RED phase, Summary

### Community 714 - "Community 714"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestBTUseGoTreeHoldsBBMu (0.00s)), GREEN phase, RED phase, Summary

### Community 716 - "Community 716"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener ./c), GREEN phase, RED phase, Summary, Summary

### Community 717 - "Community 717"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/agentexec ./cmd/bt-agen), code:block2 (internal/agentexec/dynamic_resolver_test.go:131:12: undefine), GREEN phase, RED phase, Summary

### Community 718 - "Community 718"
Cohesion: 0.33
Nodes (5): code:block1 (result = -1, want SUCCESS(1): a trailing no-plan failure mus), GREEN phase, RED phase, RED phase complete, Summary

### Community 719 - "Community 719"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH gofmt -l internal/knowledge/fee), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 720 - "Community 720"
Cohesion: 0.33
Nodes (3): code:block1 (go test ./internal/domains ./internal/benchmark -short -coun), code:block2 (ok  github.com/nico/go-bt-evolve/internal/domains    5.123s), Summary

### Community 721 - "Community 721"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/evolution ./), code:block2 (ok    github.com/nico/go-bt-evolve/internal/evolution      3), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.895s), GREEN phase, RED phase

### Community 722 - "Community 722"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test -race ./internal/util -short -coun), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 723 - "Community 723"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test -race ./internal/evolution ./inter), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 724 - "Community 724"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test -race ./internal/evolution ./inter), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 725 - "Community 725"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/domains -sho), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 726 - "Community 726"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestBTEvolveSelectorsRegisteredAndReordersFromDura), GREEN phase, RED phase, Summary

### Community 727 - "Community 727"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/dashboard ./), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.837s), GREEN phase, RED phase, Summary

### Community 728 - "Community 728"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/domains ./internal/engi), code:block2 (--- FAIL: TestAuctionDemoTreeHasNoSilentNoOps), GREEN phase, RED phase, Summary

### Community 729 - "Community 729"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/a2a ./internal/agentexe), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 730 - "Community 730"
Cohesion: 0.33
Nodes (5): code:block1 (learned Selector ordering not applied from the real producti), code:block2 (PATH=/usr/local/go/bin:$PATH go build ./...), GREEN phase, RED phase, Summary

### Community 731 - "Community 731"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/a2a -short -count=1 -ti), GREEN phase, RED phase, Summary, Summary

### Community 732 - "Community 732"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/benchmark -run TestScor), code:block2 (--- PASS: TestScoreMutation_PruneRoutingCondition_BreaksPath), GREEN phase, RED phase, Summary

### Community 733 - "Community 733"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/engine	45.347s), GREEN phase, RED phase, Summary

### Community 734 - "Community 734"
Cohesion: 0.33
Nodes (3): code:go (func fitnessScoreFromBB(outcome string, qualityScore float64), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/blocks	0.071s), Investigation Result: Milestone already satisfied

### Community 735 - "Community 735"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), code:block2 (traces_test.go:298: unknown field ParentName in struct liter), GREEN phase, RED phase, Summary

### Community 736 - "Community 736"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/security -short -count=), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 737 - "Community 737"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 738 - "Community 738"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 739 - "Community 739"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/knowledge ./cmd/bt-agen), code:block2 (analytics_test.go:328:11: entry.LastFailureTask undefined (t), GREEN phase, RED phase, Summary

### Community 740 - "Community 740"
Cohesion: 0.33
Nodes (5): code:block1 (GetTask on a follow-up request to the same agent endpoint fa), GREEN phase, RED phase, Summary, Summary

### Community 741 - "Community 741"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (VerifyRed rejected valid scoped RED evidence: RED command un), GREEN phase, RED phase, Summary

### Community 742 - "Community 742"
Cohesion: 0.33
Nodes (5): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./inte), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.785s), GREEN phase, RED phase, Summary

### Community 743 - "Community 743"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 744 - "Community 744"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestA2AHandoffBlock_BuildAndValidate (0.00s)), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./internal/blocks ./int), code:block3 (ok  	github.com/nico/go-bt-evolve/internal/blocks	0.032s), GREEN phase, RED phase

### Community 745 - "Community 745"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestActionsGoapFusionTestFile_ChargeRaceCommentNot), code:block2 (/usr/local/go/bin/go test -race ./internal/engine -short -co), GREEN phase, RED phase, Summary

### Community 746 - "Community 746"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/engine -run 'TestBuildR), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 747 - "Community 747"
Cohesion: 0.33
Nodes (5): code:block1 (expected 1 for StandardActions + build-a-widget goal, got -1), GREEN phase, RED phase, RED phase result, Summary

### Community 748 - "Community 748"
Cohesion: 0.33
Nodes (5): code:block1 (--- FAIL: TestHandleAnalyze_TaskIDsUniqueAcrossAnalyses (0.0), code:block2 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-dashboard ./in), GREEN phase, RED phase, Summary

### Community 749 - "Community 749"
Cohesion: 0.33
Nodes (5): code:block1 (ollama_test.go:41: expected Generate to succeed after one ji), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 750 - "Community 750"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestEvolveToolsSurfacePopulationHealthSnapshot (0.), GREEN phase, RED phase, RED phase result

### Community 751 - "Community 751"
Cohesion: 0.33
Nodes (5): code:block1 (internal/evolution/pareto_test.go:341:15: pf.Load undefined ), FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 752 - "Community 752"
Cohesion: 0.47
Nodes (4): code:block1 (internal/knowledge/embeddings_test.go:175:10: undefined: emb), GREEN phase, RED phase, Summary

### Community 753 - "Community 753"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/agent ./internal/a2a -s), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 754 - "Community 754"
Cohesion: 0.33
Nodes (5): code:block1 (/usr/local/go/bin/go test ./internal/evolution -short -count), FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 755 - "Community 755"
Cohesion: 0.6
Nodes (3): parseDocGo(), TestDocGoPackageClause(), TestDocGoPackageCommentMentions()

### Community 758 - "Community 758"
Cohesion: 0.7
Nodes (4): runHealthAction(), TestAssessNotebookLMPipelineHealth(), withSynthesesDir(), writeSynthesis()

### Community 760 - "Community 760"
Cohesion: 0.6
Nodes (3): findChildTick(), registerTickTestAction(), TestRunRecordsTerminalChildTicksWithParents()

### Community 761 - "Community 761"
Cohesion: 0.4
Nodes (3): Bidder, RoutingStrategy, TestRoutingStrategy_String()

### Community 766 - "Community 766"
Cohesion: 0.5
Nodes (3): addChatMsg(), agentNames, sendChat()

### Community 767 - "Community 767"
Cohesion: 0.4
Nodes (5): Build & Compilation, code:bash (export PATH=$PATH:/usr/local/go/bin), Cross-file type collisions, Dashboard raw string literal issues, "package ... is not in GOROOT"

### Community 768 - "Community 768"
Cohesion: 0.4
Nodes (5): code:bash (ollama pull qwen3.6:35b-a3b), Model not found, OLLAMA_HOST missing http:// scheme, Ollama-Specific Issues, Slow inference on Jetson

### Community 769 - "Community 769"
Cohesion: 0.4
Nodes (5): code:bash (# Check if dashboard is alive), code:bash (# Step 1: Build), Dashboard Issues, Empty API responses, pkill kills the build process

### Community 770 - "Community 770"
Cohesion: 0.4
Nodes (5): code:go (type Config struct {), code:go (func Load() (*Config, error)                // priority: def), Functions, Package: config, Types

### Community 771 - "Community 771"
Cohesion: 0.4
Nodes (5): code:go (type Server struct { ... }), code:go (func NewServer(name, version string) *Server), Functions, Package: mcp, Types

### Community 772 - "Community 772"
Cohesion: 0.4
Nodes (5): code:go (if os.Getenv("OPENROUTER_API_KEY") == "" || os.Getenv("BT_TE), code:text (~google/gemini-flash-latest), code:bash (BT_TEST_REAL_FUSION=1 OPENROUTER_API_KEY=$OPENROUTER_API_KEY), code:bash (git add internal/fusion/openrouter_integration_test.go), Task 14: Add Real-API Smoke Test Behind Env Gate

### Community 773 - "Community 773"
Cohesion: 0.4
Nodes (5): code:go (func TestSynthesize_UsesAnalysisAndResponses(t *testing.T)), code:go (func Synthesize(ctx context.Context, caller ModelCaller, cfg), code:bash (go test ./internal/fusion -run TestSynthesize -count=1), code:bash (git add internal/fusion/synthesize.go internal/fusion/synthe), Task 6: Implement Final Synthesis

### Community 774 - "Community 774"
Cohesion: 0.4
Nodes (5): code:text (Compare the strongest arguments for and against carbon taxes), code:go (cmd := engine.BuildTree(evolution.FusionDeliberationTree(), ), code:bash (go test ./internal/evolution -run TestFusionDeliberation_End), code:bash (git add internal/evolution/fusion_integration_test.go), Task 13: Add End-to-End Fake-LLM BT Test

### Community 775 - "Community 775"
Cohesion: 0.4
Nodes (5): code:go (bb.ChainState["fusion_status"] = result.Status), code:go (func TestExecFusion_WritesChainState(t *testing.T)), code:bash (go test ./internal/fusion -count=1), code:bash (git add internal/engine/chains.go internal/engine/fusion_sta), Task 12: Add Metrics and Blackboard Persistence

### Community 776 - "Community 776"
Cohesion: 0.4
Nodes (5): code:go (func Run(ctx context.Context, caller ModelCaller, cfg Config), code:go (func TestRun_EndToEnd(t *testing.T)), code:bash (go test ./internal/fusion -count=1), code:bash (git add internal/fusion/run.go internal/fusion/run_test.go), Task 7: Add End-to-End Fusion Runner

### Community 777 - "Community 777"
Cohesion: 0.4
Nodes (5): code:go (func FusionDeliberationTree() *SerializableNode {), code:go (func TestFusionDeliberationTree_Validates(t *testing.T)), code:bash (go test ./internal/evolution -run TestFusionDeliberationTree), code:bash (git add internal/evolution/fusion_trees.go internal/evolutio), Task 10: Add Fusion BT Tree

### Community 778 - "Community 778"
Cohesion: 0.4
Nodes (5): code:bash (timeout 180 /tmp/bt-agent-cli run superpowers-prod-runner \), code:bash (BT_HITL_AUTO_APPROVE=false timeout 180 /tmp/bt-agent-cli run), code:bash (timeout 120 /tmp/bt-agent-cli run goap-fusion-runner \), code:bash (BT_HITL_AUTO_APPROVE=false timeout 180 /tmp/bt-agent-cli run), Task 17: Live verification matrix

### Community 779 - "Community 779"
Cohesion: 0.4
Nodes (4): 2. Architecture Constraints, Conventions, Organizational Constraints, Technical Constraints

### Community 780 - "Community 780"
Cohesion: 0.4
Nodes (4): 10.1 Quality Tree, 10.2 Quality Scenarios, 10. Quality Requirements, code:block1 (go-bt-evolve)

### Community 781 - "Community 781"
Cohesion: 0.4
Nodes (5): Anti-patterns to reject, Do's (from official tips), Does NOT belong here (goes elsewhere in arc42), Required content & structure, Section 6 — Runtime View

### Community 782 - "Community 782"
Cohesion: 0.4
Nodes (5): Do's (from official tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns (tips warn against), Required content & structure, Section 12 — Glossary

### Community 783 - "Community 783"
Cohesion: 0.4
Nodes (5): Anti-patterns to reject, Do's, Does NOT belong here (lives elsewhere in arc42), Required content & structure, Section 8 — Crosscutting Concepts

### Community 784 - "Community 784"
Cohesion: 0.4
Nodes (5): Do's (from the official tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns, Required content & structure, Section 2 — Architecture Constraints

### Community 785 - "Community 785"
Cohesion: 0.4
Nodes (5): Anti-patterns (don'ts), Does NOT belong here (lives elsewhere in arc42), Required content (per arc42 template), Section 4 — Solution Strategy, Structure & style (do's)

### Community 786 - "Community 786"
Cohesion: 0.4
Nodes (5): Belongs elsewhere in arc42, Do's, Don'ts / anti-patterns, Required content & structure, Section 5 — Building Block View

### Community 787 - "Community 787"
Cohesion: 0.4
Nodes (5): Anti-patterns / don'ts, Do's (from the official tips), Does NOT belong here (lives elsewhere in arc42), Required content & structure, Section 11 — Risks and Technical Debt

### Community 788 - "Community 788"
Cohesion: 0.4
Nodes (5): Do's (from the arc42 tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns, Required content & structure, Section 7 — Deployment View

### Community 789 - "Community 789"
Cohesion: 0.4
Nodes (5): Belongs elsewhere in arc42, Do's (from the official tips), Don'ts / anti-patterns, Required content & structure, Section 10 — Quality Requirements

### Community 790 - "Community 790"
Cohesion: 0.4
Nodes (5): Belongs elsewhere in arc42 (move it out), Do's, Don'ts / anti-patterns, Required structure (arc42 template), Section 1 — Introduction and Goals

### Community 791 - "Community 791"
Cohesion: 0.4
Nodes (5): Anti-patterns / don'ts, Do's, Does NOT belong here (lives elsewhere in arc42), Required content & structure, Section 3 — Context and Scope

### Community 792 - "Community 792"
Cohesion: 0.4
Nodes (5): Anti-patterns to reject, Do's, Does NOT belong in section 9 (lives elsewhere in arc42), Required content & structure, Section 9 — Architecture Decisions

### Community 793 - "Community 793"
Cohesion: 0.4
Nodes (5): arc42 Section 12 (Glossary) — Conformance Checklist, Do's (from official tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns (tips warn against), Required content & structure

### Community 794 - "Community 794"
Cohesion: 0.4
Nodes (5): arc42 Section 5 (Building Block View) — Conformance Checklist, Belongs elsewhere in arc42, Do's, Don'ts / anti-patterns, Required content & structure

### Community 795 - "Community 795"
Cohesion: 0.4
Nodes (5): Anti-patterns / don'ts, arc42 Section 11 (Risks & Technical Debt) — Conformance Checklist, Do's (from the official tips), Does NOT belong here (lives elsewhere in arc42), Required content & structure

### Community 796 - "Community 796"
Cohesion: 0.4
Nodes (5): Anti-patterns to reject, arc42 Section 9 (Architecture Decisions) — Conformance Checklist, Do's, Does NOT belong in section 9 (lives elsewhere in arc42), Required content & structure

### Community 797 - "Community 797"
Cohesion: 0.4
Nodes (5): arc42 Section 7 (Deployment View) — Conformance Checklist, Do's (from the arc42 tips), Does NOT belong here (lives elsewhere in arc42), Don'ts / anti-patterns, Required content & structure

### Community 798 - "Community 798"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 799 - "Community 799"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 2: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 3: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…

### Community 800 - "Community 800"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 801 - "Community 801"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: `go test -short -race ./internal/dashboard/... ./internal/agent/` —…, Task 2: NotebookLM research: `go test -short -race ./internal/engine/ ./internal/goap/` — add a …, Task 3: NotebookLM research: `go test -short -race ./internal/evolution/ ./cmd/bt-agent/` — asse…

### Community 802 - "Community 802"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 803 - "Community 803"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make the GOAP-fusion state-hash history durable across cron ticks by …, Task 2: NotebookLM research: Add the missing producer for `goap_fusion_noop_patch_streak`, persist…, Task 3: Unblock engine tests — fix import cycle or test blockers preventing test execution (file…

### Community 804 - "Community 804"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 805 - "Community 805"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…, Task 2: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…, Task 3: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…

### Community 806 - "Community 806"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-ci-doctor/main.…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-ci-doctor/main.…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-ci-doctor/main.…

### Community 807 - "Community 807"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the LLM subprocess and fallback-chain execution path ag…, Task 2: NotebookLM research: In `internal/engine/tree.go`'s `RunTask`, after the terminal `switch`…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 808 - "Community 808"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…, Task 2: NotebookLM research: Fix `precheckGoapStaleMilestones`'s claim release so it actually clea…, Task 3: NotebookLM research: Add locking around `engine.SaveSLOMetrics`, now reachable concurrentl…

### Community 809 - "Community 809"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: NotebookLM research: Wire `IslandModel.Bank = deps.expBank` into the `bt_evolve_island` ha…

### Community 810 - "Community 810"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…

### Community 811 - "Community 811"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 812 - "Community 812"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Record SLO evidence in the interactive/MCP execution path (`agent.Run…, Task 2: NotebookLM research: Add a regression test asserting `precheckGoapStaleMilestones`'s red-e…, Task 3: NotebookLM research: Wire `a2a.AuctionCardsFn` into `cmd/bt-dashboard`'s in-process execut…

### Community 813 - "Community 813"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 814 - "Community 814"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 2: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 815 - "Community 815"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 816 - "Community 816"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 2: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 3: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…

### Community 817 - "Community 817"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Acquire `w.mu` around every `w.Status` write in `Workflow.RunFullPipe…

### Community 818 - "Community 818"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…

### Community 819 - "Community 819"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 820 - "Community 820"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 821 - "Community 821"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 822 - "Community 822"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 823 - "Community 823"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Partial-apply landings classify as landed: committed* apply st…, Task 2: Program "Q3 Reliability — Partial-apply landings classify as landed: committed* apply st…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 824 - "Community 824"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 825 - "Community 825"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 826 - "Community 826"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…

### Community 827 - "Community 827"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 828 - "Community 828"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 829 - "Community 829"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 830 - "Community 830"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 831 - "Community 831"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make `ExecutionRouter`'s deterministic `ScheduledAnalysisPath` a genu…, Task 2: NotebookLM research: Persist the Superpowers plan path + active plan to the durable agent-…, Task 3: NotebookLM research: Stop the brittle heuristic that fabricates a bogus "engine tests exec…

### Community 832 - "Community 832"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 833 - "Community 833"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 2: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 3: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…

### Community 834 - "Community 834"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 2: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 3: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…

### Community 835 - "Community 835"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…

### Community 836 - "Community 836"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…

### Community 837 - "Community 837"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 838 - "Community 838"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 839 - "Community 839"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 840 - "Community 840"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: NotebookLM research: Wire `Population.MemeticEvolve` (internal/evolution/local_search.go:2…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 841 - "Community 841"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 2: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 3: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…

### Community 842 - "Community 842"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 843 - "Community 843"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Implement TDAD logic in the evolution pipeline to maintain an in-memo…

### Community 844 - "Community 844"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 845 - "Community 845"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…

### Community 846 - "Community 846"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 847 - "Community 847"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 848 - "Community 848"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Panic-proof the evolution execution path and close the gardener experience loop (…, Task 2: Program "Panic-proof the evolution execution path and close the gardener experience loop (…, Task 3: Program "Panic-proof the evolution execution path and close the gardener experience loop (…

### Community 849 - "Community 849"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 3: NotebookLM research: Add path-parameter matching to `RouteIndex.Lookup` in the API respons…

### Community 850 - "Community 850"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 851 - "Community 851"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: NotebookLM research: Implement a `FormalVerifier` decorator that integrates with BehaVerif…

### Community 852 - "Community 852"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 853 - "Community 853"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 854 - "Community 854"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 855 - "Community 855"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 856 - "Community 856"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 857 - "Community 857"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 858 - "Community 858"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 859 - "Community 859"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 860 - "Community 860"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 861 - "Community 861"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…

### Community 862 - "Community 862"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 2: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 3: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…

### Community 863 - "Community 863"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 5/5: Ad…, Task 2: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 3: NotebookLM research: Implement the `RuleGate` decorator node and `MonotonicityAuditor` in …

### Community 864 - "Community 864"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 865 - "Community 865"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…

### Community 866 - "Community 866"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: NotebookLM research: `internal/engine/actions_goap_fusion_prod_additions.go` (both the `Ve…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 867 - "Community 867"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 868 - "Community 868"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: In `cmd/bt-agent/tools.go`'s `bt_evolve_island` handler, set the cons…, Task 2: NotebookLM research: Thread the already-constructed `agentRouter` (`cmd/bt-agent/main.go:6…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 869 - "Community 869"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 870 - "Community 870"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Finish propagating the rate-limit-carryover h…, Task 2: Program "Q1 Correctness / Q3 Reliability — Finish propagating the rate-limit-carryover h…, Task 3: NotebookLM research: Wire `internal/knowledge`'s `ComputeAnalytics()` (Bottlenecks/Selecti…

### Community 871 - "Community 871"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Adopt the dashboard's ThinkTank→Workflow task-derivation eng…, Task 2: Program "Q1 Correctness — Adopt the dashboard's ThinkTank→Workflow task-derivation eng…, Task 3: Program "Q1 Correctness — Adopt the dashboard's ThinkTank→Workflow task-derivation eng…

### Community 872 - "Community 872"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…

### Community 873 - "Community 873"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: NotebookLM research: Make the deterministic `Discover` path fitness-aware in `internal/kno…

### Community 874 - "Community 874"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: NotebookLM research: Fix the unlocked concurrent map iteration/write in `KnowledgeGraph.Bu…

### Community 875 - "Community 875"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 2: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 3: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…

### Community 876 - "Community 876"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: NotebookLM research: Make the NotebookLM pipeline monitor able to see an auth-dead pipelin…

### Community 877 - "Community 877"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 878 - "Community 878"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 879 - "Community 879"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 880 - "Community 880"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the selection-pressure gap — make tree breeding and deterministic discove…, Task 2: Program "Close the selection-pressure gap — make tree breeding and deterministic discove…, Task 3: Program "Close the selection-pressure gap — make tree breeding and deterministic discove…

### Community 881 - "Community 881"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 882 - "Community 882"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 883 - "Community 883"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 2: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…, Task 3: Program "Make dead-letter replay fire in the production multi-process topology (Q3 Reliabi…

### Community 884 - "Community 884"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 885 - "Community 885"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Close the leaky `impl_degraded` fallback so every failing exit of `Cl…, Task 2: NotebookLM research: Make the CIRCUITPOLICY state durable across cron ticks so the breaker…, Task 3: NotebookLM research: Add the missing production producer for `goap_fusion_noop_patch_strea…

### Community 886 - "Community 886"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire `NotebookLMFitness` into the knowledge graph's per-tree fitness …, Task 2: NotebookLM research: Expose `Registry.DeactivateAll` as a gardener MCP tool so operators h…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 887 - "Community 887"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…

### Community 888 - "Community 888"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 889 - "Community 889"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire the Stockfish transposition-table search into the garden…, Task 2: Program "Q2 Evolvability — Wire the Stockfish transposition-table search into the garden…, Task 3: Program "Q2 Evolvability — Wire the Stockfish transposition-table search into the garden…

### Community 890 - "Community 890"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Replace `internal/dashboard/workflow_orchestrator.go`'s hand-rolled c…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 891 - "Community 891"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Fix `MetaValidator.MinComposite`'s default scale bug in `internal/evo…

### Community 892 - "Community 892"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Harden `SanitizeUserID` so a user id cannot escape its workspace — …, Task 2: NotebookLM research: Make generated-tree resolution user-scoped. `ResolveGeneratedTree`/`r…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 893 - "Community 893"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 894 - "Community 894"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 895 - "Community 895"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 896 - "Community 896"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Give Expert Knowledge a durable, learning archive instead of …, Task 2: Program "Q2 Evolvability — Give Expert Knowledge a durable, learning archive instead of …, Task 3: NotebookLM research: Give ExpertKnowledge (internal/evolution) durable, learning persisten…

### Community 897 - "Community 897"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 898 - "Community 898"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 899 - "Community 899"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 2: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 3: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…

### Community 900 - "Community 900"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 901 - "Community 901"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 902 - "Community 902"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…

### Community 903 - "Community 903"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…

### Community 904 - "Community 904"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Fix `Gardener.applyLearnedSelectorOrdering` (internal/gardener/evolve…

### Community 905 - "Community 905"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 906 - "Community 906"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 907 - "Community 907"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 908 - "Community 908"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the gardener experience-grounded — record, retrieve, and share mutation ex…, Task 2: Program "Make the gardener experience-grounded — record, retrieve, and share mutation ex…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 909 - "Community 909"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/compos…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/compos…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/compos…

### Community 910 - "Community 910"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 911 - "Community 911"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 2: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 3: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…

### Community 912 - "Community 912"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Add a source-scan guard test that fails if `"goap_fusion_rate_limited…

### Community 913 - "Community 913"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "QualityGate.Probe is unreachable-by-construction in RefineGated, so local-search …, Task 2: NotebookLM research: Give circuit-breaker persistence exactly one owner: replace `internal…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 914 - "Community 914"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: NotebookLM research: Make `KnowledgeGraph.Query` (`graph.go:256-271`) return trees sorted …

### Community 915 - "Community 915"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Add `"telegram_clarify"` to the hand-maintained `resolverSpecialCaseT…, Task 3: NotebookLM research: In `internal/llm/fallback.go`, replace `FallbackLLM`'s hand-rolled `b…

### Community 916 - "Community 916"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 917 - "Community 917"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 918 - "Community 918"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 919 - "Community 919"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 2: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 3: NotebookLM research: Add source-level regression tests pinning `AutoRestart: agent.AutoRes…

### Community 920 - "Community 920"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 921 - "Community 921"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 2: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 3: NotebookLM research: Extend the benchmark-suite gate (`benchmarkGateEvolvedWinner`/`benchm…

### Community 922 - "Community 922"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make the CIRCUITPOLICY state-hash history durable across cron ticks. …, Task 2: NotebookLM research: Add the missing production producer for the consecutive no-op-patch s…, Task 3: NotebookLM research: Fix the misleading contract in `PublishGoapFusionStateHash`'s own doc…

### Community 923 - "Community 923"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 924 - "Community 924"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…

### Community 925 - "Community 925"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: NotebookLM research: Fix cross-user tree-ID collision in personalized-agent resolution —…

### Community 926 - "Community 926"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 2: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 3: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…

### Community 927 - "Community 927"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-evaluator/main.…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-evaluator/main.…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-evaluator/main.…

### Community 928 - "Community 928"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…

### Community 929 - "Community 929"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 930 - "Community 930"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the self-healing wiring drift across every production Evolve variant so see…, Task 2: Program "Close the self-healing wiring drift across every production Evolve variant so see…, Task 3: Program "Close the self-healing wiring drift across every production Evolve variant so see…

### Community 931 - "Community 931"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …

### Community 932 - "Community 932"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…

### Community 933 - "Community 933"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: NotebookLM research: In `RetryPolicy.ExecuteContext` (internal/reliability/errors.go:673-7…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 934 - "Community 934"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 935 - "Community 935"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Sanitize the `name` field in `dashboard.CreateAgent`/`DeleteAgent` (`…

### Community 936 - "Community 936"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 937 - "Community 937"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 3/5: Im…, Task 2: NotebookLM research: Review complete. Summary of what I found across the commit range `e2a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 938 - "Community 938"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: NotebookLM research: cmd/bt-agent/tools.go — five MCP tool handlers still bypass the new…, Task 3: NotebookLM research: internal/dashboard/executor.go — `AgentExecutor.RunTaskResult` neve…

### Community 939 - "Community 939"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix `hashTree` in `internal/evolution/learning.go` to hash the full s…, Task 2: NotebookLM research: Make `KnowledgeGraph.RecordRun` mark its own feedback dirty (or add t…, Task 3: NotebookLM research: Give `startup.CompanyState` its own mutex (or serialize all `Company`…

### Community 940 - "Community 940"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 941 - "Community 941"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…

### Community 942 - "Community 942"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…

### Community 943 - "Community 943"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: NotebookLM research: Stop `CleanupGraphifyOut` from deleting the incremental graphify extr…

### Community 944 - "Community 944"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 945 - "Community 945"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 946 - "Community 946"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: NotebookLM research: Wire the new honest-outcome-signal (`isHealthyOutcome`/authoritative …, Task 3: NotebookLM research: Fix `reapOrphanedSuperpowersBranches` to actually cover the "/tmp wip…

### Community 947 - "Community 947"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 2: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 3: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…

### Community 948 - "Community 948"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 3: NotebookLM research: Replace `internal/a2a/signing.go`'s fake "signature" (a bare unkeyed …

### Community 949 - "Community 949"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the HITL-adoption and live-rescan gaps…, Task 2: NotebookLM research: Fix `handleAgentEndpoint` (internal/a2a/server.go:431-456) so it reus…, Task 3: NotebookLM research: Widen `AuctionDelegate`'s fallback condition (internal/a2a/auction.go…

### Community 950 - "Community 950"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/evolution/finance_trees.go`, replace all 9 inline `{Type…

### Community 951 - "Community 951"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 952 - "Community 952"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: NotebookLM research: Persist `TreeMeta.RecentRuns` through the knowledge-graph feedback sn…

### Community 953 - "Community 953"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 954 - "Community 954"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 2: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 3: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…

### Community 955 - "Community 955"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — extend SafeGo panic recovery to the remaining unguarded daemon…, Task 2: Program "Q3 Reliability — extend SafeGo panic recovery to the remaining unguarded daemon…, Task 3: Program "Q3 Reliability — extend SafeGo panic recovery to the remaining unguarded daemon…

### Community 956 - "Community 956"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 957 - "Community 957"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix silent write-failure→corrupt-rename in gardener persistence, an…, Task 2: NotebookLM research: Add a mutex to `mcpDeps` guarding shared-blackboard mutation in the M…, Task 3: NotebookLM research: Check and log `Blackboards.Set` errors when promoting workflow step o…

### Community 958 - "Community 958"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire `evolution.MetaValidator` into the gardener's live mutation-acce…, Task 2: NotebookLM research: Fix the dual task-state representation in the dashboard sprint-execut…, Task 3: NotebookLM research: Wire `internal/a2a/signing.go`'s `SignAgentCard`/`VerifyAgentCard` in…

### Community 959 - "Community 959"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: On a non-rate-limit failure of `runSuperpowersRuntimeFromExistingPlan…, Task 2: NotebookLM research: Make the "ANY ClaudeSuperpowersPath failure degrades instead of abort…, Task 3: Unblock engine tests — fix import cycle or test blockers preventing test execution (file…

### Community 960 - "Community 960"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 961 - "Community 961"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 962 - "Community 962"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 963 - "Community 963"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Adopt the dashboard's ThinkTank→Workflow task-derivation eng…, Task 2: NotebookLM research: Mark `bt_fusion.go`'s `ApproveFusionReportWrite` HumanApprovalGate as…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 964 - "Community 964"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 965 - "Community 965"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…

### Community 966 - "Community 966"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 2: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 3: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…

### Community 967 - "Community 967"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…

### Community 968 - "Community 968"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 969 - "Community 969"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 970 - "Community 970"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 971 - "Community 971"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 972 - "Community 972"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Bound the durable island-model archive against runaway growth (Q2 Evolvability, Q…, Task 2: Program "Bound the durable island-model archive against runaway growth (Q2 Evolvability, Q…, Task 3: Program "Bound the durable island-model archive against runaway growth (Q2 Evolvability, Q…

### Community 973 - "Community 973"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 2: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 974 - "Community 974"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…

### Community 975 - "Community 975"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 2: NotebookLM research: Add a concurrency guard preventing two `CompanyOrchestrator.RunSprint…, Task 3: NotebookLM research: Fix the stale/inaccurate comment in `TestStoreInTranspositionTable`'s…

### Community 976 - "Community 976"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 977 - "Community 977"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Scope the RED-phase failure assertion to the tests the RED phase actu…, Task 2: NotebookLM research: Fix `buildDeterministicImplementationPlan` so its RED/GREEN command n…, Task 3: NotebookLM research: Ensure a "RED unexpectedly passed" task failure degrades the cycle to…

### Community 978 - "Community 978"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 2: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 3: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…

### Community 979 - "Community 979"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 980 - "Community 980"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Harden the brainstorm plan-expansion acceptance gate in `brainstormEx…, Task 2: NotebookLM research: Bound the winning-agent dispatch in `RunAuction` with a deadline (reu…, Task 3: NotebookLM research: Give `cmdStatus` a unit test and fix the "all milestones done but pro…

### Community 981 - "Community 981"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the experience-grounded evolution feedback loop (Q2 Evolvability)" mileston…, Task 2: Program "Close the experience-grounded evolution feedback loop (Q2 Evolvability)" mileston…, Task 3: Program "Close the experience-grounded evolution feedback loop (Q2 Evolvability)" mileston…

### Community 982 - "Community 982"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 983 - "Community 983"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 2: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 984 - "Community 984"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 985 - "Community 985"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Island-model adoption persists trees bypassing every gardener safety gate" milest…, Task 2: NotebookLM research: Set `run.Phase` on the plan-resume path. `runSuperpowersRuntimeFromEx…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 986 - "Community 986"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "GOAP fusion evidence gate doesn't recognize the committed_pr_opened ApplyStatus" …, Task 2: NotebookLM research: Fix two misleading doc comments in shipped tests that describe an alr…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 987 - "Community 987"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 988 - "Community 988"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 989 - "Community 989"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Consolidate the duplicate 3-state circuit breaker implementation in `…, Task 2: NotebookLM research: Wire `evolution.TelegramClarifyTree()` (`internal/evolution/telegram_…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 990 - "Community 990"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the dashboard API response-validator's …, Task 2: NotebookLM research: In `internal/knowledge/feedback_persist.go`, add `RecentRuns []RunSum…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 991 - "Community 991"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 2: NotebookLM research: Make the local-search refinement pass respect the validation gate —…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 992 - "Community 992"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…

### Community 993 - "Community 993"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…

### Community 994 - "Community 994"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Export a single canonical `evolution.CloneTree` and delete the four d…

### Community 995 - "Community 995"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 996 - "Community 996"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the feedback-escalation resume loop so…, Task 2: Program "Q4 Personalization & Self-Growth — Close the feedback-escalation resume loop so…, Task 3: NotebookLM research: Implement `GenerateWithMaxTokens` on the real production LLM stack (`…

### Community 997 - "Community 997"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 3: NotebookLM research: Fix `MetaValidator.checkFitness` in `internal/evolution/meta_validato…

### Community 998 - "Community 998"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 3: NotebookLM research: Aggregate `CrisisReasons` into `IslandStats` (mirroring the existing …

### Community 999 - "Community 999"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the dashboard API response-validator's …, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the dashboard API response-validator's …, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the dashboard API response-validator's …

### Community 1000 - "Community 1000"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 5/5: Ad…, Task 2: NotebookLM research: Make `isToolMatch` symmetric in its empty-string guard. The new guard…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1001 - "Community 1001"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1002 - "Community 1002"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Consolidate the duplicated Shannon-entropy / information-gain / Gini-…, Task 3: NotebookLM research: Fix the silently-ignored JSON decode error on the HITL approve/reject…

### Community 1003 - "Community 1003"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…

### Community 1004 - "Community 1004"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…

### Community 1005 - "Community 1005"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Make `reorder_children` mutations deterministic per-op so the gardene…

### Community 1006 - "Community 1006"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…

### Community 1007 - "Community 1007"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1008 - "Community 1008"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1009 - "Community 1009"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…

### Community 1010 - "Community 1010"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1011 - "Community 1011"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 4/5: Wi…, Task 2: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 3: NotebookLM research: Implement the `KernelNode` composite and `VALIDPATCH` kernel function…

### Community 1012 - "Community 1012"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 1013 - "Community 1013"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1014 - "Community 1014"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — extend SafeGo panic recovery to the remaining unguarded daemon…, Task 2: Program "Q3 Reliability — extend SafeGo panic recovery to the remaining unguarded daemon…, Task 3: NotebookLM research: Wire `RebuildBackoff` and `Scheduler.AnyInFlight()` into the producti…

### Community 1015 - "Community 1015"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1016 - "Community 1016"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix the unsynchronized read of `KnowledgeGraph.Trees` in `bt_kg_summa…

### Community 1017 - "Community 1017"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…

### Community 1018 - "Community 1018"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1019 - "Community 1019"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1020 - "Community 1020"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the HITL-adoption and live-rescan gaps…, Task 2: NotebookLM research: In the gardener's deep-search mutation path, apply `deep.BestMutation…, Task 3: NotebookLM research: Wire `TaskStateBridge.A2AToBT`/`IsTerminal` into the A2A client deleg…

### Community 1021 - "Community 1021"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1022 - "Community 1022"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: NotebookLM research: Make `gardener_rollback` (and the automatic fail-closed rollback path…, Task 3: NotebookLM research: Delete the two orphaned duplicate subsystems in internal/evolution th…

### Community 1023 - "Community 1023"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…

### Community 1024 - "Community 1024"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…

### Community 1025 - "Community 1025"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1026 - "Community 1026"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth / Q2 Evolvability — Wire the dashboard's live …, Task 2: Program "Q4 Personalization & Self-Growth / Q2 Evolvability — Wire the dashboard's live …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1027 - "Community 1027"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…

### Community 1028 - "Community 1028"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Close the ChainAction pre-execution validation gap so a mistyp…, Task 2: Program "Q1 Correctness — Close the ChainAction pre-execution validation gap so a mistyp…, Task 3: Program "Q1 Correctness — Close the ChainAction pre-execution validation gap so a mistyp…

### Community 1029 - "Community 1029"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…, Task 3: NotebookLM research: Wire `evolution.MetaPromptEvolver`/`MutationContext` into the gardene…

### Community 1030 - "Community 1030"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: NotebookLM research: Harden `SanitizeUserID` to reject path-traversal identifiers — afte…

### Community 1031 - "Community 1031"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1032 - "Community 1032"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1033 - "Community 1033"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1034 - "Community 1034"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…

### Community 1035 - "Community 1035"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Add a consecutive-`escalate_failed` retry cap to the ClaudeErrorHandl…

### Community 1036 - "Community 1036"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 1037 - "Community 1037"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1038 - "Community 1038"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: NotebookLM research: Make `IdleGate` count only genuine reflection records — filter `ref…

### Community 1039 - "Community 1039"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1040 - "Community 1040"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1041 - "Community 1041"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/knowledge/graph.go`, make `RegisterEvolved` (lines 267-2…

### Community 1042 - "Community 1042"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1043 - "Community 1043"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1044 - "Community 1044"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 2: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 3: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…

### Community 1045 - "Community 1045"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Fix `TaskAgentMap`'s case mismatch in `internal/dashboard/executor.go…, Task 3: NotebookLM research: In `internal/a2a/auction.go`, let the real winner dispatch in `RunAuc…

### Community 1046 - "Community 1046"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1047 - "Community 1047"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: NotebookLM research: Restore the degrade-not-dead-letter contract in the NotebookLM auth a…, Task 3: NotebookLM research: Bound the two unbounded resources that commit 72ce4fa introduced into…

### Community 1048 - "Community 1048"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 2: NotebookLM research: Add a non-retryable classification path for deterministic build/lint/…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1049 - "Community 1049"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1050 - "Community 1050"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1051 - "Community 1051"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…

### Community 1052 - "Community 1052"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 1053 - "Community 1053"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…

### Community 1054 - "Community 1054"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1055 - "Community 1055"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: In `gatherReviewContext`, guard the structure-review branch (`round%3…, Task 2: NotebookLM research: Make the review-mode rotation counter outcome-aware. Move/guard `save…, Task 3: NotebookLM research: Harden the `firstNonEmptyGoapLine` fallback in `runClaudeCodeReviewRe…

### Community 1056 - "Community 1056"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…

### Community 1057 - "Community 1057"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Replace the hand-rolled `replaceAll` template substitution in `intern…

### Community 1058 - "Community 1058"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 2: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 3: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…

### Community 1059 - "Community 1059"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1060 - "Community 1060"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1061 - "Community 1061"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: NotebookLM research: Teach the Superpowers task executor a characterization/coverage task …, Task 3: NotebookLM research: Stop the red-pass loop-breaker from closing coverage milestones that …

### Community 1062 - "Community 1062"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 1063 - "Community 1063"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: NotebookLM research: Wire `Population.EvolveQLearning` (and reapply the already-implemente…, Task 3: NotebookLM research: Cap island-model population size so the durable archive can't grow un…

### Community 1064 - "Community 1064"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1065 - "Community 1065"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1066 - "Community 1066"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1067 - "Community 1067"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/domains/alert…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1068 - "Community 1068"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1069 - "Community 1069"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 1070 - "Community 1070"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…

### Community 1071 - "Community 1071"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/dlq.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/dlq.go…, Task 3: NotebookLM research: Wire `RunGoapFusionNotebookLMResearch` (internal/engine/actions_goap_…

### Community 1072 - "Community 1072"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the fusion multi-model panel (`internal/fusion`) agains…, Task 2: Program "Q3 Reliability — Harden the fusion multi-model panel (`internal/fusion`) agains…, Task 3: Program "Q3 Reliability — Harden the fusion multi-model panel (`internal/fusion`) agains…

### Community 1073 - "Community 1073"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1074 - "Community 1074"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: In `internal/evolution/learning.go:972-997`, make `cloneTree`'s `Meta…, Task 3: NotebookLM research: In `internal/dashboard/workflow_orchestrator.go:418-433`, replace the…

### Community 1075 - "Community 1075"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Self-fix: deploy-drift churn test leaks adoption-dedup state into the sibling-rol…, Task 2: NotebookLM research: Thread `ReinforcementLearner`'s epsilon-decay schedule (`DecayEpsilon…, Task 3: NotebookLM research: Call `a2a.Server.Stop()` on SIGINT/SIGTERM in `cmd/bt-agent/main.go`'…

### Community 1076 - "Community 1076"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the experience-grounded evolution feedback loop (Q2 Evolvability)" mileston…, Task 2: NotebookLM research: Persist the scheduler's per-agent circuit breaker state to `circuit_b…, Task 3: NotebookLM research: Make A2A auction dispatch honest and resilient — `SendTask` must re…

### Community 1077 - "Community 1077"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 1078 - "Community 1078"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 1079 - "Community 1079"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1080 - "Community 1080"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Finish propagating the rate-limit-carryover h…, Task 2: Program "Q1 Correctness / Q3 Reliability — Finish propagating the rate-limit-carryover h…, Task 3: Program "Q1 Correctness / Q3 Reliability — Finish propagating the rate-limit-carryover h…

### Community 1081 - "Community 1081"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…, Task 2: NotebookLM research: Add runner-up fallback to `Auctioneer.RunAuction` — when the top-sc…, Task 3: NotebookLM research: Consolidate `internal/agent`'s duplicated circuit-breaker state machi…

### Community 1082 - "Community 1082"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…

### Community 1083 - "Community 1083"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: NotebookLM research: Close the ADR-010 user-workspace containment break in `SanitizeUserID…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1084 - "Community 1084"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: NotebookLM research: Implement the "churn-gate" ADR-196 flagged as unimplemented: skip re-…

### Community 1085 - "Community 1085"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: NotebookLM research: Wire the DTAnalyzer/BTOptimizer entropy-based Selector reordering pas…, Task 3: NotebookLM research: Consolidate the hand-rolled `path+".tmp"` + `os.Rename` atomic-write …

### Community 1086 - "Community 1086"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1087 - "Community 1087"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 1088 - "Community 1088"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Add `so.mu.RLock()`/`RUnlock()` to `SelectorOptimizer.OrderChildren` …

### Community 1089 - "Community 1089"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…

### Community 1090 - "Community 1090"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Add a collision guard to `KnowledgeGraph.Register`/`Factory.CreateTre…

### Community 1091 - "Community 1091"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1092 - "Community 1092"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1093 - "Community 1093"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …

### Community 1094 - "Community 1094"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the NSGA-II self-healing wiring gap, the last production Evolve variant wit…, Task 2: Program "Close the NSGA-II self-healing wiring gap, the last production Evolve variant wit…, Task 3: NotebookLM research: `internal/knowledge/impact.go`'s `BuildImpactGraph`/`ImpactGraph.Test…

### Community 1095 - "Community 1095"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Close the dashboard's dead task-metrics and circuit-breaker re…, Task 2: Program "Q3 Reliability — Close the dashboard's dead task-metrics and circuit-breaker re…, Task 3: Program "Q3 Reliability — Close the dashboard's dead task-metrics and circuit-breaker re…

### Community 1096 - "Community 1096"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1097 - "Community 1097"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — wire the dashboard's tested Workflow/Approval…, Task 2: Program "Q1 Correctness / Q3 Reliability — wire the dashboard's tested Workflow/Approval…, Task 3: Program "Q1 Correctness / Q3 Reliability — wire the dashboard's tested Workflow/Approval…

### Community 1098 - "Community 1098"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1099 - "Community 1099"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make `BuildCardRegistry` (`internal/a2a/card.go:110`) surface per-age…, Task 2: NotebookLM research: Consolidate `writePrometheusMetrics`'s and `MetricsJSON`'s inline `gl…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1100 - "Community 1100"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…

### Community 1101 - "Community 1101"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 2: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1102 - "Community 1102"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…

### Community 1103 - "Community 1103"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Adopt the new `reliability.CircuitBreakerStore` in at least one of th…, Task 2: NotebookLM research: (none — the alias-based circuit-breaker consolidation itself (`inte…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1104 - "Community 1104"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1105 - "Community 1105"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1106 - "Community 1106"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Route A2A task execution through the platform…, Task 2: Program "Q1 Correctness / Q3 Reliability — Route A2A task execution through the platform…, Task 3: NotebookLM research: Wire `agent.History` recording into `BTAgentExecutor.Execute` (`inter…

### Community 1107 - "Community 1107"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…

### Community 1108 - "Community 1108"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 2: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1109 - "Community 1109"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…, Task 2: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…, Task 3: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…

### Community 1110 - "Community 1110"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1111 - "Community 1111"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Wire the dashboard's dead task-metrics and block-fitness recor…, Task 2: Program "Q3 Reliability — Wire the dashboard's dead task-metrics and block-fitness recor…, Task 3: Program "Q3 Reliability — Wire the dashboard's dead task-metrics and block-fitness recor…

### Community 1112 - "Community 1112"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix `internal/a2a.BuildCardRegistry` (card.go:145-155) so a failed pe…

### Community 1113 - "Community 1113"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Stop `ShepherdFleetPR`'s idle branch from asserting `bb.QualityAuthor…

### Community 1114 - "Community 1114"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Adopt SafeGo panic recovery across unguarded production fan-out goroutines (Q3 Re…, Task 2: Program "Adopt SafeGo panic recovery across unguarded production fan-out goroutines (Q3 Re…, Task 3: Program "Adopt SafeGo panic recovery across unguarded production fan-out goroutines (Q3 Re…

### Community 1115 - "Community 1115"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…

### Community 1116 - "Community 1116"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1117 - "Community 1117"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 1118 - "Community 1118"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1119 - "Community 1119"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1120 - "Community 1120"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1121 - "Community 1121"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1122 - "Community 1122"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 2: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 3: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…

### Community 1123 - "Community 1123"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Route `bt_evolve_qd` (MAP-Elites), `bt_evolve_multiobjective` (NSGA-I…, Task 3: NotebookLM research: Make the dashboard workflow orchestrator's HITL approval step fail cl…

### Community 1124 - "Community 1124"
Cohesion: 0.4
Nodes (5): code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 14: Section manifest (`arc42_sections.go`) — TDD

### Community 1125 - "Community 1125"
Cohesion: 0.4
Nodes (5): code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (package engine), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 15: Guideline slicer — TDD

### Community 1126 - "Community 1126"
Cohesion: 0.4
Nodes (5): code:go (func TestClassifyAffectedArc42Sections(t *testing.T) {), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (// arc42ClassifierTimeout bounds the cheap which-sections-ch), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 20: Classifier prefilter — TDD

### Community 1127 - "Community 1127"
Cohesion: 0.4
Nodes (5): code:go (func TestSyncReadmeUpdates(t *testing.T) {), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (// syncReadme keeps the hand-written README.md honest after ), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 17: `syncReadme` — TDD

### Community 1128 - "Community 1128"
Cohesion: 0.4
Nodes (5): code:go (type arc42SyncFakeClaude struct {), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (// docChangeContext describes the change a documentation pas), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), Task 16: `docChangeContext` + `syncArc42Section` — TDD

### Community 1129 - "Community 1129"
Cohesion: 0.4
Nodes (5): code:go (var arc42GoalsDocPaths = []string{), code:bash (PATH=/usr/local/go/bin:$PATH go test ./internal/engine/ -run), code:go (RegisterAction("ReadADRs", func(ctx *btcore.BTContext[Blackb), code:bash (PATH=/usr/local/go/bin:$PATH go build ./...), Task 11: Code repoints (goals reader, ReadADRs, tree prompts, bt-docgen)

### Community 1130 - "Community 1130"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1131 - "Community 1131"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1132 - "Community 1132"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — stop HITL escalation from silently auto-appro…, Task 2: Program "Q1 Correctness / Q3 Reliability — stop HITL escalation from silently auto-appro…, Task 3: Program "Q1 Correctness / Q3 Reliability — stop HITL escalation from silently auto-appro…

### Community 1133 - "Community 1133"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give Q-learning evolution durable cross-run memory (Q2 Evolvability)" milestone 4…, Task 2: NotebookLM research: Persist the evolved winner tree from the three production genetic-evo…, Task 3: NotebookLM research: Wire the already-implemented MAP-Elites durable archive (`Save`/`Load…

### Community 1134 - "Community 1134"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Consolidate the platform's three independent "PreGate→StrategyRoute…

### Community 1135 - "Community 1135"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1136 - "Community 1136"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Close the deploy-drift restart-handoff gap: adopted binaries m…, Task 2: Program "Q3 Reliability — Close the deploy-drift restart-handoff gap: adopted binaries m…, Task 3: Program "Q3 Reliability — Close the deploy-drift restart-handoff gap: adopted binaries m…

### Community 1137 - "Community 1137"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…

### Community 1138 - "Community 1138"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1139 - "Community 1139"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1140 - "Community 1140"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1141 - "Community 1141"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1142 - "Community 1142"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: NotebookLM research: Preserve the `goap_fusion_rate_limited` outcome sentinel end-to-end i…, Task 3: NotebookLM research: Reclassify `bt_fusion.go`'s `ApproveFusionReportWrite` HITL gate from…

### Community 1143 - "Community 1143"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…

### Community 1144 - "Community 1144"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1145 - "Community 1145"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: NotebookLM research: In `DeadLetterQueue.Requeue`, track replay attempts in a separate fie…

### Community 1146 - "Community 1146"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1147 - "Community 1147"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 3: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…

### Community 1148 - "Community 1148"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1149 - "Community 1149"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: FILES1:** `internal/gardener/evolve_v2.go`, `internal/gardener/garden…

### Community 1150 - "Community 1150"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…

### Community 1151 - "Community 1151"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: NotebookLM research: Make `cmd/bt-dashboard` itself periodically call `knowledge.ComputeAn…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1152 - "Community 1152"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1153 - "Community 1153"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: NotebookLM research: GOAL2 (Q3 Reliability/containment):** `SanitizeUserID` in `internal/p…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1154 - "Community 1154"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 2: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 3: NotebookLM research: In `internal/reliability/errors.go`'s `ClassifyError`, hoist the type…

### Community 1155 - "Community 1155"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…

### Community 1156 - "Community 1156"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1157 - "Community 1157"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…

### Community 1158 - "Community 1158"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 1159 - "Community 1159"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1160 - "Community 1160"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q2 Evolvability — Surface knowledge-graph fitness and evolutio…, Task 2: NotebookLM research: GOAL1: Fix the `internal/llm/acp.go` stderr data race in `ACPClient.G…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1161 - "Community 1161"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1162 - "Community 1162"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1163 - "Community 1163"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1164 - "Community 1164"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Self-fix (review 2026-07-22): reflection LoadAll counts non-reflection JSON as ph…, Task 2: NotebookLM research: In `internal/engine/chains.go`'s `execFusion` (around line 915), repl…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1165 - "Community 1165"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1166 - "Community 1166"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 2: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 3: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…

### Community 1167 - "Community 1167"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 2: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 3: NotebookLM research: Wire `MetaPromptEvolver`/`MutationContext` (internal/evolution/diff.g…

### Community 1168 - "Community 1168"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — DT-ordering pass reads the per-tree DT stats the daemon actua…, Task 2: Program "Q2 Evolvability — DT-ordering pass reads the per-tree DT stats the daemon actua…, Task 3: NotebookLM research: Fix quality-gate rollback to walk back past a multi-cycle regression …

### Community 1169 - "Community 1169"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Add source-audit pinning tests for the new `InFlightFn` wiring in `cm…, Task 3: NotebookLM research: Fix the `meeting_notes` domain tree's benchmark task wording so its `…

### Community 1170 - "Community 1170"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1171 - "Community 1171"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…

### Community 1172 - "Community 1172"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 2: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 3: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…

### Community 1173 - "Community 1173"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1174 - "Community 1174"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: In `runClaudeCodeReviewResearch`, only advance the last-reviewed SHA …, Task 2: NotebookLM research: Make `completeGoapProgramMilestone` verify the applied run actually e…, Task 3: NotebookLM research: Store each research goal with its gap as a paired unit (or make `Prio…

### Community 1175 - "Community 1175"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Replace the 9 inline `WasSuccessful`-based `OutcomeSelector` blocks i…

### Community 1176 - "Community 1176"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 2: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…, Task 3: Program "Give every registered evolution algorithm a production entry point (Q2 Evolvabili…

### Community 1177 - "Community 1177"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1178 - "Community 1178"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1179 - "Community 1179"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1180 - "Community 1180"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make platform health measurable and deployment drift self-evident (Q3 Reliability…, Task 2: Program "Make platform health measurable and deployment drift self-evident (Q3 Reliability…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1181 - "Community 1181"
Cohesion: 0.4
Nodes (5): code:bash (PATH=/usr/local/go/bin:$PATH go get \), code:go (package tracing), code:go (package tracing), code:bash (git add go.mod go.sum internal/tracing/otel.go internal/trac), Task 2: OTel SDK trace facade in internal/tracing

### Community 1182 - "Community 1182"
Cohesion: 0.4
Nodes (5): code:go (package llm), code:go (package llm), code:go (llmForRun = llm.NewTracedLLM(llmRecorder, "agent-llm")), code:bash (git add internal/llm/traced.go internal/llm/traced_test.go i), Task 8: LLM call spans

### Community 1183 - "Community 1183"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1184 - "Community 1184"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/compos…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/compos…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1185 - "Community 1185"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: NotebookLM research: Fix the trust/scoping gap in the new live-mutation MCP tools (`bt_liv…

### Community 1186 - "Community 1186"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: NotebookLM research: In `internal/a2a/server.go`'s `Execute`, after `engine.RunTask` retur…, Task 3: NotebookLM research: `cmd/bt-gardener/main.go:312`'s initial cycle (`results, _ := g.RunCy…

### Community 1187 - "Community 1187"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1188 - "Community 1188"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…

### Community 1189 - "Community 1189"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1190 - "Community 1190"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…

### Community 1191 - "Community 1191"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 2: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 3: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…

### Community 1192 - "Community 1192"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — make `bt_impact_tests` honestly normalize and reject source pa…, Task 2: NotebookLM research: Fix `bt_impact_tests` (MCP tool) to normalize the `source` path befor…, Task 3: NotebookLM research: Route `handleAnalyze`'s LLM-derived task creation through the existin…

### Community 1193 - "Community 1193"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…, Task 2: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…, Task 3: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…

### Community 1194 - "Community 1194"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: NotebookLM research: In `internal/engine/goap_claude_backoff.go`, make `parseClaudeWeeklyR…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1195 - "Community 1195"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1196 - "Community 1196"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …

### Community 1197 - "Community 1197"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…

### Community 1198 - "Community 1198"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 2: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 3: NotebookLM research: Add a bounded age-based sweep to `reliability.DeadLetterQueue` (e.g. …

### Community 1199 - "Community 1199"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 2: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 3: NotebookLM research: Wire `dashboard.RecordTask` into `AgentExecutor.RunTaskResult` (singl…

### Community 1200 - "Community 1200"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1201 - "Community 1201"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 3: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…

### Community 1202 - "Community 1202"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1203 - "Community 1203"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/gardener/evolve_v2.go`, make the reorder-only save path …

### Community 1204 - "Community 1204"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire `dashboard.PickTreeForTask` (`internal/dashboard/executor.go:153…, Task 2: NotebookLM research: Make gardener v2's crisis intervention apply the detector's calibrate…, Task 3: NotebookLM research: Make `dashboard.TaskStore` load/save atomic and fail loudly on corrup…

### Community 1205 - "Community 1205"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire the platform's existing `reliability.SafeGo` panic-recovery help…, Task 2: NotebookLM research: Extend `Population.selfHealGeneration` (crisis detection, emergency m…, Task 3: NotebookLM research: Persist the crisis detector's `reason` string (`"diversity_collapse"`…

### Community 1206 - "Community 1206"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Give the five benchmark-gated evolution algorithms adaptive, …, Task 2: Program "Q2 Evolvability — Give the five benchmark-gated evolution algorithms adaptive, …, Task 3: Program "Q2 Evolvability — Give the five benchmark-gated evolution algorithms adaptive, …

### Community 1207 - "Community 1207"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 2: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 3: NotebookLM research: Q2 Evolvability and Q1 Correctness. (files: internal/evolution/impact…

### Community 1208 - "Community 1208"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make the runaway-loop backstop half-open (self-clearing on trip) and …, Task 2: NotebookLM research: Stop `PublishGoapFusionStateHash` from accumulating identical idle ha…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1209 - "Community 1209"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1210 - "Community 1210"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1211 - "Community 1211"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 2/5: Im…, Task 2: NotebookLM research: In `VerifyGoapFusionEvidence`, accept the bare-repo build/test delega…, Task 3: NotebookLM research: Add test coverage for the evidence gate consuming a delegated verify …

### Community 1212 - "Community 1212"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Stop silent write-failure and breaker-bypass gaps in gardener …, Task 2: NotebookLM research: Implement Task 2 of the runtime-tree-mutation plan — a live-run reg…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1213 - "Community 1213"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1214 - "Community 1214"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 1215 - "Community 1215"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1216 - "Community 1216"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Make evolved-tree knowledge-graph metadata survive a daemon r…, Task 2: NotebookLM research: Fix `dlq.SetReplayExecutor`'s outcome classification in `cmd/bt-agent…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1217 - "Community 1217"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `cmd/bt-dashboard/main.go`'s `buildDashboardKnowledgeGraph` and `c…

### Community 1218 - "Community 1218"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 1/4: I…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…

### Community 1219 - "Community 1219"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1220 - "Community 1220"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…

### Community 1221 - "Community 1221"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1222 - "Community 1222"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 2: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 3: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…

### Community 1223 - "Community 1223"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wrap the 18 unrecovered `go func(...)` goroutine spawns in `internal/…, Task 2: NotebookLM research: In `internal/reliability/reliability.go`, default `DeadLetterEntry.ID…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1224 - "Community 1224"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…

### Community 1225 - "Community 1225"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1226 - "Community 1226"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give the deterministic Selector-ordering optimizers a production entry point — …, Task 2: Program "Give the deterministic Selector-ordering optimizers a production entry point — …, Task 3: NotebookLM research: Apply the accumulated telemetry — actually reorder Selector childre…

### Community 1227 - "Community 1227"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 2: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 3: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…

### Community 1228 - "Community 1228"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: NotebookLM research: In `internal/gardener/evolve_v2.go`'s `evolveTreeV2`, load the durabl…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1229 - "Community 1229"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q2 Evolvability — Surface knowledge-graph fitness and evolutio…, Task 2: Program "Q1 Correctness / Q2 Evolvability — Surface knowledge-graph fitness and evolutio…, Task 3: Program "Q1 Correctness / Q2 Evolvability — Surface knowledge-graph fitness and evolutio…

### Community 1230 - "Community 1230"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: NotebookLM research: Change `OutcomeErrorDetail` to preserve the head of the output, not o…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1231 - "Community 1231"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: NotebookLM research: Write the still-missing `TestScheduledTrees_NoUnattendedHITLDeadlock`…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1232 - "Community 1232"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: NotebookLM research: Make `OutcomeErrorDetail` preserve the head of the run output, not on…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1233 - "Community 1233"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: NotebookLM research: Reconcile the two disconnected approval surfaces `handleAnalyze` crea…, Task 3: NotebookLM research: `handleAgentExecute` (dashboard's `/api/agents/execute`, the HTTP cou…

### Community 1234 - "Community 1234"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 1235 - "Community 1235"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…

### Community 1236 - "Community 1236"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: NotebookLM research: Wire `bt_evolve_qd`/`bt_evolve_island`'s benchmark-gate-accepted winn…, Task 3: NotebookLM research: Wire `handleHealth` (`cmd/bt-dashboard/main.go:1161`) to call `dashbo…

### Community 1237 - "Community 1237"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 1238 - "Community 1238"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1239 - "Community 1239"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1240 - "Community 1240"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: NotebookLM research: Consolidate `internal/domains/trees.go`'s eight near-identical domain…, Task 3: NotebookLM research: Wrap each per-tree evolution call in `Gardener.RunCycleV2` with the e…

### Community 1241 - "Community 1241"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: NotebookLM research: Give `Judge` and `Synthesize` their own timeout budgets in `internal/…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1242 - "Community 1242"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1243 - "Community 1243"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1244 - "Community 1244"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1245 - "Community 1245"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: In `executeSuperpowersTaskBatch`, key the final snapshot-unwrap on wh…, Task 2: NotebookLM research: Make the partial-landing cleanup fail safe. The three recovery comman…, Task 3: NotebookLM research: Add regression tests for the snapshot-commit-failure degrade path. `p…

### Community 1246 - "Community 1246"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1247 - "Community 1247"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…, Task 2: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…, Task 3: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…

### Community 1248 - "Community 1248"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1249 - "Community 1249"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1250 - "Community 1250"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Give `bt-dashboard` and `bt-gardener` an `InFlightFn` guard on their …, Task 2: NotebookLM research: Log a warning when `internal/a2a/signing.go`'s signing-key persistenc…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1251 - "Community 1251"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1252 - "Community 1252"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …

### Community 1253 - "Community 1253"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…

### Community 1254 - "Community 1254"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…

### Community 1255 - "Community 1255"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1256 - "Community 1256"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1257 - "Community 1257"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1258 - "Community 1258"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: NotebookLM research: Implement TDAD logic in the evolution pipeline to maintain an in-memo…

### Community 1259 - "Community 1259"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 2: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…, Task 3: Program "Consolidate the two competing Selector-reordering subsystems in internal/evolutio…

### Community 1260 - "Community 1260"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: NotebookLM research: Give the graceful carryover an honest SLO/history state instead of ov…

### Community 1261 - "Community 1261"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1262 - "Community 1262"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/dlq.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/dlq.go…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/dlq.go…

### Community 1263 - "Community 1263"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction subsystem production hardening (review follow-up 2026-07-04)" milestone 1…, Task 2: Program "Auction subsystem production hardening (review follow-up 2026-07-04)" milestone 2…, Task 3: Program "Auction subsystem production hardening (review follow-up 2026-07-04)" milestone 3…

### Community 1264 - "Community 1264"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 3: NotebookLM research: In `internal/gardener/evolve_v2.go:196-202`, replace the self-referen…

### Community 1265 - "Community 1265"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1266 - "Community 1266"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1267 - "Community 1267"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1268 - "Community 1268"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1269 - "Community 1269"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1270 - "Community 1270"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1271 - "Community 1271"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…

### Community 1272 - "Community 1272"
Cohesion: 0.4
Nodes (4): code:go (grid.Insert(desc, &evolution.Individual{Tree: cloneTreeForGa), Task 1: Snapshot the live tree when the gardener records a diversity observation, Task 2: Give `MAPElitesGrid.Insert` snapshot ownership so the GA path stops aliasing too, Task 3: Make resolver-reachable domain trees first-class members of the coverage union

### Community 1273 - "Community 1273"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…

### Community 1274 - "Community 1274"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 3/5: Im…, Task 2: NotebookLM research: Add an auction control-plane orchestrator to `internal/a2a` — an `A…, Task 3: NotebookLM research: Make the A2A responder bid-aware. In `BTAgentExecutor.Execute` (`inte…

### Community 1275 - "Community 1275"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1276 - "Community 1276"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 2: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…, Task 3: Program "Q2 Evolvability — Feed Expert Knowledge's learned-pattern archive back into mut…

### Community 1277 - "Community 1277"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1278 - "Community 1278"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 2: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 3: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…

### Community 1279 - "Community 1279"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "MAP-Elites diversity archive stores live tree pointers, so the new elite-reseed c…, Task 2: NotebookLM research: Persist the verification phase and its evidence on the failure paths …, Task 3: NotebookLM research: Migrate the three production description consumers from direct map in…

### Community 1280 - "Community 1280"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Make evolved-tree knowledge-graph metadata survive a daemon r…, Task 2: Program "Q2 Evolvability — Make evolved-tree knowledge-graph metadata survive a daemon r…, Task 3: Program "Q2 Evolvability — Make evolved-tree knowledge-graph metadata survive a daemon r…

### Community 1281 - "Community 1281"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1282 - "Community 1282"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Stop `RecordRun("evolved")` from incrementing `RunCount`; keep `RunCo…, Task 2: NotebookLM research: Split evolved *structural* fitness out of the single `TreeMeta.Fitnes…, Task 3: NotebookLM research: In `Factory.selectParents`, exclude category-alias keys from the cand…

### Community 1283 - "Community 1283"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Export `evolution.cloneTree`'s metadata copy with a recursive deep-co…, Task 2: NotebookLM research: Route `internal/a2a`'s auction/bidding capability matching through `i…, Task 3: NotebookLM research: Consolidate `internal/dashboard`'s two parallel Task/Approval models …

### Community 1284 - "Community 1284"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q1 Correctness — Wire the real GOAP A* planner into productio…, Task 2: Program "Q2 Evolvability / Q1 Correctness — Wire the real GOAP A* planner into productio…, Task 3: Program "Q2 Evolvability / Q1 Correctness — Wire the real GOAP A* planner into productio…

### Community 1285 - "Community 1285"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1286 - "Community 1286"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…

### Community 1287 - "Community 1287"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1288 - "Community 1288"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1289 - "Community 1289"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…, Task 2: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…, Task 3: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…

### Community 1290 - "Community 1290"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1291 - "Community 1291"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1292 - "Community 1292"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 2: Program "Wire proactive crisis intervention into the GA evolution loops so population deat…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1293 - "Community 1293"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1294 - "Community 1294"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1295 - "Community 1295"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 2: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 3: NotebookLM research: Add best-effort re-landing of orphaned `superpowers/*` branches befor…

### Community 1296 - "Community 1296"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the self-healing wiring drift across every production Evolve variant so see…, Task 2: Program "Close the self-healing wiring drift across every production Evolve variant so see…, Task 3: NotebookLM research: Harden `CheckCodebaseFit` so a diagnostic-only shell probe cannot har…

### Community 1297 - "Community 1297"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1298 - "Community 1298"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: NotebookLM research: Register the orphaned Kanban tree family (6 trees in `internal/domain…, Task 3: NotebookLM research: Wire `cmd/bt-dashboard/main.go`'s `handleHealth` to call the already-…

### Community 1299 - "Community 1299"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…, Task 2: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…, Task 3: Program "Q2 Evolvability — Make gardener mutation rollback automatic, multi-revision, an…

### Community 1300 - "Community 1300"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…

### Community 1301 - "Community 1301"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Remove the bare `"resets"` substring check from `isClaudeRateLimit` (…

### Community 1302 - "Community 1302"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix `evaluateCondition`'s prefix-match bug in `internal/dashboard/wor…, Task 2: NotebookLM research: Wire `domains.DTStatsPath` in production, mirroring the already-fixed…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1303 - "Community 1303"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 2: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…, Task 3: Program "Make the MAP-Elites quality-diversity archive durable and accumulating so illumin…

### Community 1304 - "Community 1304"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 2: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…, Task 3: Program "Close the engine-wide `research.ProgramStore` concurrent-writer lost-update gap (…

### Community 1305 - "Community 1305"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…

### Community 1306 - "Community 1306"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/types.…, Task 3: NotebookLM research: Register `evolution.TelegramClarifyTree()` under `"telegram_clarify"`…

### Community 1307 - "Community 1307"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1308 - "Community 1308"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 2: NotebookLM research: Wire `IslandModel.Bank = deps.expBank` into the `bt_evolve_island` to…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1309 - "Community 1309"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1310 - "Community 1310"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the gardener experience-grounded — record, retrieve, and share mutation ex…, Task 2: Program "Make the gardener experience-grounded — record, retrieve, and share mutation ex…, Task 3: Program "Make the gardener experience-grounded — record, retrieve, and share mutation ex…

### Community 1311 - "Community 1311"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…, Task 2: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…, Task 3: Program "Adopt the horizontal-scaling substrate (`RemoteExecutor` + `AgentRouter`) end-to-…

### Community 1312 - "Community 1312"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1313 - "Community 1313"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1314 - "Community 1314"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 2: NotebookLM research: Add `TestScheduledTrees_NoUnattendedHITLDeadlock` to `internal/domain…, Task 3: NotebookLM research: Harden `handleGoapRedPassCycleFailure`/`recordGoapResearchGoalRedPass…

### Community 1315 - "Community 1315"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 2: NotebookLM research: Retire `internal/dashboard/workflow_engine.go`'s `Workflow`/`Workflow…, Task 3: NotebookLM research: Wire `ExperienceBank` into `bt_evolve_island`'s `IslandModel` (files:…

### Community 1316 - "Community 1316"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: NotebookLM research: Guard `IslandModel.EvolveAll`'s migration-cadence check against `Migr…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1317 - "Community 1317"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make platform health measurable and deployment drift self-evident (Q3 Reliability…, Task 2: Program "Make platform health measurable and deployment drift self-evident (Q3 Reliability…, Task 3: Program "Make platform health measurable and deployment drift self-evident (Q3 Reliability…

### Community 1318 - "Community 1318"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1319 - "Community 1319"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…, Task 3: NotebookLM research: Wire the interactive/dashboard-triggered execution path into `knowled…

### Community 1320 - "Community 1320"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…, Task 2: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…, Task 3: Program "Q4 Personalization & Self-Growth — Close the dashboard HITL approval-finalizati…

### Community 1321 - "Community 1321"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1322 - "Community 1322"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…

### Community 1323 - "Community 1323"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1324 - "Community 1324"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 2: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 3: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…

### Community 1325 - "Community 1325"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: NotebookLM research: Harden `githubTokenFromCredentialStore`'s github.com host match to ch…

### Community 1326 - "Community 1326"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: Add `Warn`/`Info` logging when a parked superpowers run is abandoned …

### Community 1327 - "Community 1327"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1328 - "Community 1328"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1329 - "Community 1329"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make `Store.WaitForRequest` (internal/hitl/store_extensions.go) termi…, Task 2: NotebookLM research: Give `ParetoPopulation.EvolvePareto` (internal/evolution/pareto.go) a…, Task 3: NotebookLM research: Populate `knowledge.DecisionTrace.Steps` at both production trace sit…

### Community 1330 - "Community 1330"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Register the 13 resolver-special-case, bare (non-`domain:`-prefixed) …, Task 2: NotebookLM research: Extend `internal/knowledge/analytics.go`'s `ExpectedDomains`/`Coverag…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1331 - "Community 1331"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1332 - "Community 1332"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 2: NotebookLM research: Make the A2A auction winner circuit breaker survive across engine tic…, Task 3: NotebookLM research: Persist the aggregate fields (`total_cycles`, `active_trees`, `best_f…

### Community 1333 - "Community 1333"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Program claim/lease: a sibling agent must not plan a program a…, Task 2: Program "Q3 Reliability — Program claim/lease: a sibling agent must not plan a program a…, Task 3: Program "Q3 Reliability — Program claim/lease: a sibling agent must not plan a program a…

### Community 1334 - "Community 1334"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 2: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 3: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…

### Community 1335 - "Community 1335"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 2: NotebookLM research: Fix `ComposeTaskTree` (`internal/blocks/compose.go:68-82`) so it stop…, Task 3: NotebookLM research: Consolidate `composeWithMiddle` (`internal/blocks/compose.go:84-121`)…

### Community 1336 - "Community 1336"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/knowledge/registry.go`, add `"telegram_clarify"` to `res…

### Community 1337 - "Community 1337"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Stop silent write-failure and breaker-bypass gaps in gardener …, Task 2: Program "Q3 Reliability — Stop silent write-failure and breaker-bypass gaps in gardener …, Task 3: Program "Q3 Reliability — Stop silent write-failure and breaker-bypass gaps in gardener …

### Community 1338 - "Community 1338"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1339 - "Community 1339"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1340 - "Community 1340"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1341 - "Community 1341"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1342 - "Community 1342"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1343 - "Community 1343"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1344 - "Community 1344"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Close the ChainAction pre-execution validation gap so a mistyp…, Task 2: NotebookLM research: Implement TDAD logic in the evolution pipeline to maintain an in-memo…, Task 3: NotebookLM research: Wire the existing `lineageSkipsReEvolution` guard into `bt_evolve_sel…

### Community 1345 - "Community 1345"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1346 - "Community 1346"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1347 - "Community 1347"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Make `internal/benchmark/benchmark.go`'s local `cloneTree` (and `inte…

### Community 1348 - "Community 1348"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: NotebookLM research: Implement the `Adaptor` composite node to enable local plan revision …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1349 - "Community 1349"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Close the fitness-feedback and lineage-skip consistency gaps …, Task 2: Program "Q2 Evolvability — Close the fitness-feedback and lineage-skip consistency gaps …, Task 3: NotebookLM research: Implement TDAD logic in the evolution pipeline to maintain an in-memo…

### Community 1350 - "Community 1350"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1351 - "Community 1351"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1352 - "Community 1352"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1353 - "Community 1353"
Cohesion: 0.4
Nodes (5): code:go (// internal/engine/mem_selector.go), code:go (case "MemSelector":), code:bash (git add internal/engine/mem_selector.go internal/engine/mem_), code:go (func TestMemSelector_ResumesAtRunningChildAndSkipsFailed(t *), Task 2: `MemSelector` composite (blackboard-keyed)

### Community 1354 - "Community 1354"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…

### Community 1355 - "Community 1355"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 2: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…, Task 3: Program "Q2 Evolvability — Wire durable pre-mutation snapshots, tree rollback, and bench…

### Community 1356 - "Community 1356"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 1357 - "Community 1357"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…

### Community 1358 - "Community 1358"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: NotebookLM research: Wrap the unguarded raw goroutine fan-outs with `reliability.SafeGo` p…, Task 3: NotebookLM research: Fix the DLQ's cross-process membership loss: make `mergeFromDisk` ado…

### Community 1359 - "Community 1359"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 2/4: B…, Task 2: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 3/4: I…, Task 3: Program "Test-Driven Agentic Development and Graph-Based Impact Analysis" milestone 4/4: B…

### Community 1360 - "Community 1360"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…

### Community 1361 - "Community 1361"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: NotebookLM research: In `cmd/bt-dashboard/main.go`'s `handleDLQ`, add a `"categories": dlq…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1362 - "Community 1362"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1363 - "Community 1363"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: NotebookLM research: Add retention/pruning to the GOAP-fusion materializer-snapshot writer…

### Community 1364 - "Community 1364"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1365 - "Community 1365"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Gate the standalone evolution-algorithm tools' durable-archiv…, Task 2: Program "Q2 Evolvability — Gate the standalone evolution-algorithm tools' durable-archiv…, Task 3: Program "Q2 Evolvability — Gate the standalone evolution-algorithm tools' durable-archiv…

### Community 1366 - "Community 1366"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1367 - "Community 1367"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…

### Community 1368 - "Community 1368"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/reliab…, Task 3: NotebookLM research: Extend `RouteIndex.Lookup` in `internal/api/response_validator.go` to…

### Community 1369 - "Community 1369"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire bt_evolve_qd's MAP-Elites illumination into the shared E…, Task 2: Program "Q2 Evolvability — Wire bt_evolve_qd's MAP-Elites illumination into the shared E…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1370 - "Community 1370"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 3: NotebookLM research: Surface `Population.HealthSnapshot()` in the JSON responses of the pr…

### Community 1371 - "Community 1371"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Experience-Grounded Safety Monotonicity and Certified Evolution" milestone 2/4: B…, Task 2: Program "Experience-Grounded Safety Monotonicity and Certified Evolution" milestone 3/4: I…, Task 3: Program "Experience-Grounded Safety Monotonicity and Certified Evolution" milestone 4/4: B…

### Community 1372 - "Community 1372"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the LLM subprocess and fallback-chain execution path ag…, Task 2: Program "Q3 Reliability — harden the LLM subprocess and fallback-chain execution path ag…, Task 3: Program "Q3 Reliability — harden the LLM subprocess and fallback-chain execution path ag…

### Community 1373 - "Community 1373"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Fix and wire the dashboard's dead-code `RunFullPipeline` auton…, Task 2: Program "Q1 Correctness — Fix and wire the dashboard's dead-code `RunFullPipeline` auton…, Task 3: Program "Q1 Correctness — Fix and wire the dashboard's dead-code `RunFullPipeline` auton…

### Community 1374 - "Community 1374"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1375 - "Community 1375"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1376 - "Community 1376"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…

### Community 1377 - "Community 1377"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix the a2a card-trust bypass and `Server.CardCache` data race. (file…

### Community 1378 - "Community 1378"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…

### Community 1379 - "Community 1379"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…

### Community 1380 - "Community 1380"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 2: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 3: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…

### Community 1381 - "Community 1381"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Self-fix (review 2026-07-22): bt_kg_coverage_gaps structurally pinned at 0 in bt-…, Task 2: NotebookLM research: Add a `wireDTOrdering` function to `cmd/bt-gardener/config.go` (mirro…, Task 3: NotebookLM research: Add an MCP tool in `cmd/bt-agent/tools.go` (e.g. `bt_gardener_dt_diag…

### Community 1382 - "Community 1382"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: NotebookLM research: Wire `IslandModel.Bank = deps.expBank` into the `bt_evolve_island` ha…

### Community 1383 - "Community 1383"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix the `ValidationGate` bypass in `internal/gardener/evolve_v2.go`'s…

### Community 1384 - "Community 1384"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `cmd/bt-agent/tools.go`, add the `benchmarkGateEvolvedWinner` regr…

### Community 1385 - "Community 1385"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 2: Program "Q3 Reliability — Persist GOAP fusion research-goal/milestone charge stamps dura…, Task 3: NotebookLM research: Implement TDAD logic in the evolution pipeline to maintain an in-memo…

### Community 1386 - "Community 1386"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…

### Community 1387 - "Community 1387"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…

### Community 1388 - "Community 1388"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1389 - "Community 1389"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate hand-duplicated atomic JSON persistence in…, Task 2: Program "Q5 Consistency & Reuse — Consolidate hand-duplicated atomic JSON persistence in…, Task 3: Program "Q5 Consistency & Reuse — Consolidate hand-duplicated atomic JSON persistence in…

### Community 1390 - "Community 1390"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1391 - "Community 1391"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1392 - "Community 1392"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix `goapGoalFailureNote`'s double truncation, which discards the act…, Task 2: NotebookLM research: Make bt-ci-doctor's advisory-check footnote actually fire instead of …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1393 - "Community 1393"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/fanout…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1394 - "Community 1394"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Guard the MCP server's shared per-daemon state (`mcpDeps`) aga…, Task 2: NotebookLM research: Add a mutex to `mcpDeps` guarding every read-modify-write of `deps.bb…, Task 3: NotebookLM research: Harden `handleGoapRedPassCycleFailure`/`recordGoapResearchGoalRedPass…

### Community 1395 - "Community 1395"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1396 - "Community 1396"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 1397 - "Community 1397"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…

### Community 1398 - "Community 1398"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: NotebookLM research: Wire `wrapWithErrorHandler` into every domain-tree resolution path, n…, Task 3: NotebookLM research: Thread the failing task's context into `ExperienceBank.AddFromMutatio…

### Community 1399 - "Community 1399"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1400 - "Community 1400"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1401 - "Community 1401"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 2: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…, Task 3: Program "Q2 Evolvability — Condition bottleneck mutation search on the failing task's se…

### Community 1402 - "Community 1402"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: In `internal/agent/runner.go`, propagate `bb.ChainState["auction_awar…

### Community 1403 - "Community 1403"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire the knowledge graph into the gardener daemon's live prio…, Task 2: Program "Q2 Evolvability — Wire the knowledge graph into the gardener daemon's live prio…, Task 3: Program "Q2 Evolvability — Wire the knowledge graph into the gardener daemon's live prio…

### Community 1404 - "Community 1404"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Route auction/delegation-shaped tasks to the `auction_demo` tree in `…, Task 2: NotebookLM research: Delete the dead `RecordSelectorOutcomes` bridge (and its tests) from …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1405 - "Community 1405"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1406 - "Community 1406"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — make `bt_impact_tests` honestly normalize and reject source pa…, Task 2: Program "Q1 Correctness — make `bt_impact_tests` honestly normalize and reject source pa…, Task 3: Program "Q1 Correctness — make `bt_impact_tests` honestly normalize and reject source pa…

### Community 1407 - "Community 1407"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 2: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…, Task 3: Program "Q3 Reliability — close the second-wave SafeGo panic-recovery gap in production-…

### Community 1408 - "Community 1408"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Panic-proof the evolution execution path and close the gardener experience loop (…, Task 2: NotebookLM research: Make the A2A auction's per-winner circuit breaker actually persist ac…, Task 3: NotebookLM research: Make `MetricsTracker.Save()` actually write the aggregate fields the …

### Community 1409 - "Community 1409"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Guard `internal/evolution/island.go`'s `EvolveAll` (line 204) — `if…

### Community 1410 - "Community 1410"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/evolution/pareto.go`, fix `ParetoPopulation.SelectPareto…

### Community 1411 - "Community 1411"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1412 - "Community 1412"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix the swallowed errors in the SLO-metrics export block of the garde…

### Community 1413 - "Community 1413"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…

### Community 1414 - "Community 1414"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: In `internal/engine/goap_claude_backoff.go`, make `writeSharedClaudeB…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1415 - "Community 1415"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1416 - "Community 1416"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1417 - "Community 1417"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1418 - "Community 1418"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1419 - "Community 1419"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1420 - "Community 1420"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1421 - "Community 1421"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1422 - "Community 1422"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1423 - "Community 1423"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1424 - "Community 1424"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make the A2A card registry (`Server.CardCache`) refreshable at runtim…, Task 2: NotebookLM research: Stop discarding the evolved winner tree in `bt_evolve_memetic` and in…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1425 - "Community 1425"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1426 - "Community 1426"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Make `writeSharedClaudeBackoff` (internal/engine/goap_claude_backoff.…

### Community 1427 - "Community 1427"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give the deterministic Selector-ordering optimizers a production entry point — …, Task 2: Program "Give the deterministic Selector-ordering optimizers a production entry point — …, Task 3: Program "Give the deterministic Selector-ordering optimizers a production entry point — …

### Community 1428 - "Community 1428"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 2: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…, Task 3: Program "Fix the `SuiteForTree` benchmark-gating blind spot — ~9 registered domain trees…

### Community 1429 - "Community 1429"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1430 - "Community 1430"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1431 - "Community 1431"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the ChainState nil-map panic across eng…

### Community 1432 - "Community 1432"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1433 - "Community 1433"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…

### Community 1434 - "Community 1434"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1435 - "Community 1435"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1436 - "Community 1436"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 2/5: Im…, Task 2: NotebookLM research: In `VerifyGoapFusionEvidence`, make the deterministic `## GOAP Fusion…, Task 3: NotebookLM research: Have `ReportFusionCycle` carry an explicit, gate-parseable delegation…

### Community 1437 - "Community 1437"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1438 - "Community 1438"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1439 - "Community 1439"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1440 - "Community 1440"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire the entropy/Gini-based BTOptimizer/DTAnalyzer decision-t…, Task 2: Program "Q2 Evolvability — Wire the entropy/Gini-based BTOptimizer/DTAnalyzer decision-t…, Task 3: Program "Q2 Evolvability — Wire the entropy/Gini-based BTOptimizer/DTAnalyzer decision-t…

### Community 1441 - "Community 1441"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1442 - "Community 1442"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Make the daemon's SchedulerConfig construction testable so the Feedba…, Task 2: NotebookLM research: Correct the stale MCP-tool-count comment. (files: cmd/bt-agent/tools.…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1443 - "Community 1443"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1444 - "Community 1444"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 2: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…, Task 3: Program "Q2 Evolvability — Finish gating every evolution algorithm's durable-archive win…

### Community 1445 - "Community 1445"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: NotebookLM research: Wire `Analytics.Centrality` and `Analytics.ToolContention` (`internal…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1446 - "Community 1446"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1447 - "Community 1447"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1448 - "Community 1448"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…

### Community 1449 - "Community 1449"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…

### Community 1450 - "Community 1450"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1451 - "Community 1451"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1452 - "Community 1452"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1453 - "Community 1453"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Clear the durable Superpowers plan state on every non-rate-limit `Cla…, Task 2: NotebookLM research: Stop conflating a rate-limit carryover with a real degradation in the…, Task 3: Unblock engine tests — fix import cycle or test blockers preventing test execution (file…

### Community 1454 - "Community 1454"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the knowledge-graph analytics→action loop — make ComputeAnalytics signa…, Task 2: Program "Close the knowledge-graph analytics→action loop — make ComputeAnalytics signa…, Task 3: Program "Close the knowledge-graph analytics→action loop — make ComputeAnalytics signa…

### Community 1455 - "Community 1455"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the fusion multi-model panel (`internal/fusion`) agains…, Task 2: NotebookLM research: Q1 Correctness, Q2 Evolvability, and Q4 Personalization. (files: inte…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1456 - "Community 1456"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1457 - "Community 1457"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…

### Community 1458 - "Community 1458"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1459 - "Community 1459"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix `resolveTreeIDWithResolver` in `internal/domains/tree_resolver.go…

### Community 1460 - "Community 1460"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…

### Community 1461 - "Community 1461"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 2: NotebookLM research: In `internal/gardener/evolve_v2.go`, extend the deep-search mutation-…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1462 - "Community 1462"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1463 - "Community 1463"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 3: NotebookLM research: In `internal/engine/actions_superpowers_prod.go`, replace `ExecuteSup…

### Community 1464 - "Community 1464"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…, Task 2: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…, Task 3: Program "Q2 Evolvability — Harden and activate the gardener's Stockfish-style deep-searc…

### Community 1465 - "Community 1465"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Make the OutcomeSelector's self-correction an…, Task 2: NotebookLM research: Close the TOCTOU gap in `persistEvolvedWinner` between the `EvolvedFi…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1466 - "Community 1466"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1467 - "Community 1467"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Fix the tree-ID naming mismatch between `gardener.Registry` and `know…

### Community 1468 - "Community 1468"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…

### Community 1469 - "Community 1469"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1470 - "Community 1470"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: NotebookLM research: In `reliability.RetryPolicy.ExecuteContext`, when the context is canc…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1471 - "Community 1471"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 2: Program "Q2 Evolvability — Give Pareto-front and NSGA-II evolution the same durable cros…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1472 - "Community 1472"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…, Task 2: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…, Task 3: Program "Q3 Reliability — Harden Hermes webhook delivery with retry, circuit breaker, an…

### Community 1473 - "Community 1473"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 3: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…

### Community 1474 - "Community 1474"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "GOAP program claim never released after milestone success — blocks program up t…, Task 2: NotebookLM research: Release the program-store claim on the success path — `completeGoap…, Task 3: NotebookLM research: Fix `runPRShepherd`'s post-merge local-master sync so a batch that la…

### Community 1475 - "Community 1475"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness — Adopt the dashboard's ThinkTank→Workflow task-derivation eng…, Task 2: NotebookLM research: Fix the unsynchronized read of `*pipelineRunRecord` fields in `handle…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1476 - "Community 1476"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — stop HITL escalation from silently auto-appro…, Task 2: NotebookLM research: Thread per-user tree resolution through `agent.RunDeps.ResolveTree`/`…, Task 3: NotebookLM research: Isolate `internal/a2a/auction_test.go` from the real `~/.go-bt-evolve…

### Community 1477 - "Community 1477"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "engine.ValidateTreeFull cycle detector false-positives on same-name leaf idiom" m…, Task 2: NotebookLM research: Extend `parseClaudeRateLimitReset` in `internal/engine/goap_claude_ba…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1478 - "Community 1478"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/a2a.go…

### Community 1479 - "Community 1479"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire `internal/gardener`'s autonomous mutation loop to `internal/know…, Task 2: NotebookLM research: Fix the gardener's learned Selector-ordering pass (milestone 4) — i…, Task 3: NotebookLM research: Wire `dashboard.RecordTask` into the real task-execution path — it …

### Community 1480 - "Community 1480"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1481 - "Community 1481"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 3: NotebookLM research: Add a `wireDTOrdering` function to `cmd/bt-gardener/config.go` (mirro…

### Community 1482 - "Community 1482"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/dep…, Task 3: NotebookLM research: Rename `TestNewRunDeps_ConfigLoadFailure_FallsBackToZeroConfig` (inte…

### Community 1483 - "Community 1483"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 2: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…, Task 3: Program "Make the island-model durable archive production-safe — per-tree scoped, bounde…

### Community 1484 - "Community 1484"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1485 - "Community 1485"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1486 - "Community 1486"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/mem_se…

### Community 1487 - "Community 1487"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/evolution/learning.go:972` (`cloneTree`) and `internal/g…

### Community 1488 - "Community 1488"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Bound the durable island-model archive against runaway growth (Q2 Evolvability, Q…, Task 2: NotebookLM research: Add Claude usage/credit-limit string patterns (`"reached your "`, `"/…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1489 - "Community 1489"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 2: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 3: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…

### Community 1490 - "Community 1490"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Fix parallel workflow steps so a failed/errored child branch propagat…, Task 3: NotebookLM research: Route the gardener's existing per-tree MAP-Elites diversity archive i…

### Community 1491 - "Community 1491"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1492 - "Community 1492"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…

### Community 1493 - "Community 1493"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 3: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…

### Community 1494 - "Community 1494"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1495 - "Community 1495"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Unify the two divergent Claude-backoff fallback-window constants into…

### Community 1496 - "Community 1496"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Fix `MeetingNotesSuite`'s `"document the decision log from the quarte…, Task 3: NotebookLM research: Replace the plain `*gardener.Gardener`/`*agent.Scheduler` variables c…

### Community 1497 - "Community 1497"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1498 - "Community 1498"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 3: NotebookLM research: In `internal/knowledge/graph.go`, make `Register` and `RegisterEvolve…

### Community 1499 - "Community 1499"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1500 - "Community 1500"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make evolution self-healing observable end-to-end (Q3 Reliability) — surface ga…, Task 2: NotebookLM research: In `internal/agent/scheduler.go` (~line 735-740), `reportAgentOutcome…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1501 - "Community 1501"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1502 - "Community 1502"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Give Q-learning evolution durable cross-run memory (Q2 Evolvability)" milestone 1…, Task 2: Program "Give Q-learning evolution durable cross-run memory (Q2 Evolvability)" milestone 2…, Task 3: Program "Give Q-learning evolution durable cross-run memory (Q2 Evolvability)" milestone 3…

### Community 1503 - "Community 1503"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1504 - "Community 1504"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…

### Community 1505 - "Community 1505"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1506 - "Community 1506"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 2: Program "Make the dead-letter queue drop-safe and truly replayable (Q3 Reliability)" miles…, Task 3: NotebookLM research: GOAL1 (top, Q2 Evolvability)** — The deterministic `Discover` fallb…

### Community 1507 - "Community 1507"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the automated deploy-drift loop so committed fixes reach the running daemon…, Task 2: NotebookLM research: Fix `cmd/bt-dashboard/main.go`'s deploy-drift watcher so it can actua…, Task 3: NotebookLM research: Stamp `BuildRevision` on the second `DeadLetterEntry` push site, `int…

### Community 1508 - "Community 1508"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…

### Community 1509 - "Community 1509"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — ShepherdFleetPR merges a pinned batch head: pushing every new …, Task 2: Program "Q3 Reliability — ShepherdFleetPR merges a pinned batch head: pushing every new …, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1510 - "Community 1510"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1511 - "Community 1511"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1512 - "Community 1512"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix guaranteed task-ID collisions in `handleAnalyze`'s new Workflow-d…, Task 2: NotebookLM research: Guard the new package-level `currentWorkflow` and `Workflow.Tasks` ag…, Task 3: NotebookLM research: Make `persistEvolvedWinner`'s knowledge-graph bookkeeping and disk wr…

### Community 1513 - "Community 1513"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hooks.…, Task 2: NotebookLM research: In `internal/blocks/compose.go`, `ComposeTaskTree` must stop overwrit…, Task 3: NotebookLM research: Consolidate `composeWithMiddle` (`internal/blocks/compose.go:84-121`)…

### Community 1514 - "Community 1514"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Thread failing-task context through the ExperienceBank write …, Task 2: Program "Q2 Evolvability — Thread failing-task context through the ExperienceBank write …, Task 3: Program "Q2 Evolvability — Thread failing-task context through the ExperienceBank write …

### Community 1515 - "Community 1515"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 1/5: De…, Task 2: NotebookLM research: Make `KnowledgeGraph` tree discovery deterministic and rank-based. In…, Task 3: NotebookLM research: Propagate A2A `input-required` back into the BT outcome vocabulary. `…

### Community 1516 - "Community 1516"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 2: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…, Task 3: Program "Q1 Correctness / Q3 Reliability — Close the remaining unguarded `deps.bb`/`deps…

### Community 1517 - "Community 1517"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1518 - "Community 1518"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…

### Community 1519 - "Community 1519"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1520 - "Community 1520"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 2: Program "Q2 Evolvability — Close the self-healing envelope gap across the remaining GA v…, Task 3: NotebookLM research: Q2 Evolvability and Q1 Correctness. (files: internal/evolution/impact…

### Community 1521 - "Community 1521"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1522 - "Community 1522"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1523 - "Community 1523"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…

### Community 1524 - "Community 1524"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1525 - "Community 1525"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire the just-added `ExpertKnowledge` learned-pattern feedback into t…, Task 2: NotebookLM research: Feed `evaluator.IterativeDeepening`'s `BestMutation`/`BestFitness` re…, Task 3: NotebookLM research: Make `BTAgentExecutor.Execute`'s auction-bid branch call the tested `…

### Community 1526 - "Community 1526"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1527 - "Community 1527"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement the `SpineMemory` service in `internal/agent/spine.go` to r…, Task 2: NotebookLM research: Implement the `Adaptor` local repair service in `internal/engine/kern…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1528 - "Community 1528"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Persist the GOAP-fusion CIRCUITPOLICY state-hash history across cron …, Task 2: NotebookLM research: Add the missing `goap_fusion_noop_patch_streak` producer and persist …, Task 3: Unblock engine tests — fix import cycle or test blockers preventing test execution (file…

### Community 1529 - "Community 1529"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Register `telegram_clarify` in `knowledge.BuildKnowledgeGraph()` and …

### Community 1530 - "Community 1530"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1531 - "Community 1531"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1532 - "Community 1532"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1533 - "Community 1533"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1534 - "Community 1534"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/blocks/qualit…

### Community 1535 - "Community 1535"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: NotebookLM research: Add a scheduler liveness/heartbeat watchdog so a wedged (not crashed)…

### Community 1536 - "Community 1536"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 2: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…, Task 3: Program "Q3 Reliability & Q5 Consistency — Non-destructive goap-fusion materializer with…

### Community 1537 - "Community 1537"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 3: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…

### Community 1538 - "Community 1538"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 2: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…, Task 3: Program "Q2 Evolvability / Q4 Personalization & Self-Growth — Reconnect the standalone `…

### Community 1539 - "Community 1539"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 2: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…, Task 3: Program "Q3 Reliability — Harden the direct A2A delegation path (`DelegateToA2A`) with c…

### Community 1540 - "Community 1540"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: In `internal/engine/goap_claude_backoff.go`, make `saveClaudeBackoffS…

### Community 1541 - "Community 1541"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1542 - "Community 1542"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Remove the committed 8.5 MB compiled binary `bt-scalability-probe` fr…, Task 2: NotebookLM research: Route the dashboard's production agent-execution handlers through the…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1543 - "Community 1543"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Fleet-PR branch/state writes race outside prShepherdMu" milestone 1/1: internal/e…, Task 2: NotebookLM research: Fix `cmd/bt-dashboard/main.go`'s `/api/health` handler (`handleHealth…, Task 3: NotebookLM research: In `cmd/bt-agent/tools.go`'s `bt_evolve_selectors` handler (`tools.go…

### Community 1544 - "Community 1544"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Close the selection-pressure gap — make tree breeding and deterministic discove…, Task 2: Program "Close the selection-pressure gap — make tree breeding and deterministic discove…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1545 - "Community 1545"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1546 - "Community 1546"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/condit…, Task 2: NotebookLM research: Make all keyword-matching conditions in `internal/engine/conditions_d…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1547 - "Community 1547"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1548 - "Community 1548"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 2: Program "Make `evolveTreeV2` actually use the evolution package's five non-heuristic algor…, Task 3: NotebookLM research: Make `ValidationGate` + a pre-change `SnapshotTreeWithFitness` a prec…

### Community 1549 - "Community 1549"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q4 Personalization & Self-Growth — Close the HITL-adoption and live-rescan gaps…, Task 2: Program "Q4 Personalization & Self-Growth — Close the HITL-adoption and live-rescan gaps…, Task 3: Program "Q4 Personalization & Self-Growth — Close the HITL-adoption and live-rescan gaps…

### Community 1550 - "Community 1550"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: NotebookLM research: Preserve `pending_approval` as a real, non-terminal outcome through `…, Task 2: NotebookLM research: Gate `persistEvolvedWinner` (and its `RegisterEvolved` fitness bookke…, Task 3: NotebookLM research: Wire `internal/gardener/evolve_v2.go`'s tested "learned selector orde…

### Community 1551 - "Community 1551"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Add gosec (or an equivalent local static-analysis pass) to the Superp…, Task 3: NotebookLM research: Widen `verifySuperpowersRuntimeInDir`'s `main-build` check (`internal…

### Community 1552 - "Community 1552"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1553 - "Community 1553"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 3: NotebookLM research: Extend the `#nosec`-justification convention established after the PR…

### Community 1554 - "Community 1554"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 3: NotebookLM research: Give scheduled long-running agents (goap-fusion-loop-runner/-runner) …

### Community 1555 - "Community 1555"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1556 - "Community 1556"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: NotebookLM research: Add missing test coverage for the `InFlightFn` guards that `bt-dashbo…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1557 - "Community 1557"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 2: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 1558 - "Community 1558"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…

### Community 1559 - "Community 1559"
Cohesion: 0.4
Nodes (4): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 2: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…, Task 3: Program "Deterministic coverage backlog: characterization tests for internal/engine/delega…

### Community 1560 - "Community 1560"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1561 - "Community 1561"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1562 - "Community 1562"
Cohesion: 0.5
Nodes (3): code:block1 (tools_test.go:1267: first run on a cold home must report "wa), GREEN phase, RED phase

### Community 1563 - "Community 1563"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1564 - "Community 1564"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1565 - "Community 1565"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1566 - "Community 1566"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1568 - "Community 1568"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1569 - "Community 1569"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1571 - "Community 1571"
Cohesion: 0.4
Nodes (4): code:block1 (scheduler_test.go:810: failure_reason = "agent outcome: fail), GREEN phase, RED phase, Summary

### Community 1572 - "Community 1572"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1573 - "Community 1573"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1574 - "Community 1574"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1575 - "Community 1575"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1576 - "Community 1576"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1577 - "Community 1577"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1578 - "Community 1578"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1579 - "Community 1579"
Cohesion: 0.4
Nodes (4): code:block1 (internal/knowledge/graph_test.go:47:10: tree.StructuralFitne), GREEN phase, RED phase, Summary

### Community 1580 - "Community 1580"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), code:block2 (internal/knowledge/feedback_test.go:250:10: tree.EvolvedCoun), GREEN phase, RED phase

### Community 1581 - "Community 1581"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), code:block2 (--- FAIL: TestFactory_SelectParentsExcludesCategoryAliasKeys), GREEN phase, RED phase

### Community 1582 - "Community 1582"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1583 - "Community 1583"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1584 - "Community 1584"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1585 - "Community 1585"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1586 - "Community 1586"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1587 - "Community 1587"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1588 - "Community 1588"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1589 - "Community 1589"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	5.718s), GREEN phase, RED phase

### Community 1590 - "Community 1590"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1591 - "Community 1591"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1592 - "Community 1592"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1593 - "Community 1593"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1595 - "Community 1595"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1596 - "Community 1596"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1597 - "Community 1597"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.637s), GREEN phase, RED phase

### Community 1598 - "Community 1598"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1599 - "Community 1599"
Cohesion: 0.4
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine ./int), code:block2 (--- FAIL: TestChainAction_SingleShotExecutors_RetryOnTransie), Summary

### Community 1600 - "Community 1600"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1601 - "Community 1601"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), GREEN phase, RED phase, Summary

### Community 1602 - "Community 1602"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1603 - "Community 1603"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1604 - "Community 1604"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestDiscover_RegisterNotStarvedBySlowEmbeddingCall), GREEN phase, RED phase

### Community 1605 - "Community 1605"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1606 - "Community 1606"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1607 - "Community 1607"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1608 - "Community 1608"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1609 - "Community 1609"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1610 - "Community 1610"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1611 - "Community 1611"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1612 - "Community 1612"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.210s), GREEN phase, RED phase

### Community 1613 - "Community 1613"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1614 - "Community 1614"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1615 - "Community 1615"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1616 - "Community 1616"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1617 - "Community 1617"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1618 - "Community 1618"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1619 - "Community 1619"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1620 - "Community 1620"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent ./intern), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.565s), GREEN phase, RED phase

### Community 1621 - "Community 1621"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1622 - "Community 1622"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1623 - "Community 1623"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/a2a ./intern), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.484-0.545s), GREEN phase, RED phase

### Community 1624 - "Community 1624"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1625 - "Community 1625"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1627 - "Community 1627"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1628 - "Community 1628"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1630 - "Community 1630"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1631 - "Community 1631"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1632 - "Community 1632"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1633 - "Community 1633"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1634 - "Community 1634"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1635 - "Community 1635"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1636 - "Community 1636"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1637 - "Community 1637"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1638 - "Community 1638"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1639 - "Community 1639"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.229s), GREEN phase, RED phase

### Community 1640 - "Community 1640"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1641 - "Community 1641"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1642 - "Community 1642"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1643 - "Community 1643"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1644 - "Community 1644"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1645 - "Community 1645"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1646 - "Community 1646"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1647 - "Community 1647"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1648 - "Community 1648"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1649 - "Community 1649"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1650 - "Community 1650"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -short -), code:block2 (cmd/bt-agent/wiring_test.go:195:9: undefined: experienceBank), GREEN phase, RED phase

### Community 1651 - "Community 1651"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1652 - "Community 1652"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent-cli -sho), code:block2 (# github.com/nico/go-bt-evolve/cmd/bt-agent-cli [github.com/), GREEN phase, RED phase

### Community 1653 - "Community 1653"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestPersistEvolvedWinner_ConcurrentCallsStayConsis), GREEN phase, RED phase, Summary

### Community 1654 - "Community 1654"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1655 - "Community 1655"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1656 - "Community 1656"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1657 - "Community 1657"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1658 - "Community 1658"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1659 - "Community 1659"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1661 - "Community 1661"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1662 - "Community 1662"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1663 - "Community 1663"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1664 - "Community 1664"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1665 - "Community 1665"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.110s), GREEN phase, RED phase

### Community 1666 - "Community 1666"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 1667 - "Community 1667"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1668 - "Community 1668"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-garde), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.098s), GREEN phase, RED phase

### Community 1669 - "Community 1669"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1670 - "Community 1670"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1671 - "Community 1671"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1672 - "Community 1672"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.219s), GREEN phase, RED phase

### Community 1673 - "Community 1673"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1674 - "Community 1674"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1675 - "Community 1675"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1676 - "Community 1676"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1677 - "Community 1677"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1678 - "Community 1678"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1679 - "Community 1679"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), GREEN phase, RED phase, RED phase result

### Community 1680 - "Community 1680"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1681 - "Community 1681"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1682 - "Community 1682"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1683 - "Community 1683"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestPublishGoapFusionStateHash_ResultReflectsDurab), GREEN phase, RED phase

### Community 1684 - "Community 1684"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1685 - "Community 1685"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1686 - "Community 1686"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestAppendGoapResearchGoalsKeepsGoalGapPairsAligne), GREEN phase, RED phase

### Community 1687 - "Community 1687"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestCompleteGoapProgramMilestoneSkipsRunThatMissed), GREEN phase, RED phase, Summary

### Community 1688 - "Community 1688"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1689 - "Community 1689"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1690 - "Community 1690"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -short -), code:block2 (--- FAIL: TestDLQReplayWaitReportsFailureOutcome (0.01s)), GREEN phase, RED phase

### Community 1691 - "Community 1691"
Cohesion: 0.4
Nodes (4): code:block1 (internal/reliability/reliability_test.go:236:16: entries[0].), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.02), GREEN phase, RED phase

### Community 1692 - "Community 1692"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1693 - "Community 1693"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 1694 - "Community 1694"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1695 - "Community 1695"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1696 - "Community 1696"
Cohesion: 0.6
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.492s), GREEN phase, RED phase

### Community 1697 - "Community 1697"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1698 - "Community 1698"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1699 - "Community 1699"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1700 - "Community 1700"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1701 - "Community 1701"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1702 - "Community 1702"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1703 - "Community 1703"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1704 - "Community 1704"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1705 - "Community 1705"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1706 - "Community 1706"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1707 - "Community 1707"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1708 - "Community 1708"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1709 - "Community 1709"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1710 - "Community 1710"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1712 - "Community 1712"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1713 - "Community 1713"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1714 - "Community 1714"
Cohesion: 0.6
Nodes (3): GREEN phase, RED phase, Summary

### Community 1715 - "Community 1715"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1716 - "Community 1716"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1717 - "Community 1717"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1718 - "Community 1718"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1719 - "Community 1719"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1720 - "Community 1720"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent ./intern), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.206s), GREEN phase, RED phase

### Community 1721 - "Community 1721"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1722 - "Community 1722"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1723 - "Community 1723"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1724 - "Community 1724"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1725 - "Community 1725"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1726 - "Community 1726"
Cohesion: 0.5
Nodes (3): code:block1 (tools_test.go:704: bt_evolve_qd must reject population=1 wit), GREEN phase, RED phase

### Community 1727 - "Community 1727"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1728 - "Community 1728"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1729 - "Community 1729"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1730 - "Community 1730"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1731 - "Community 1731"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1732 - "Community 1732"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1733 - "Community 1733"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1734 - "Community 1734"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1735 - "Community 1735"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1736 - "Community 1736"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1737 - "Community 1737"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./cmd/), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.254s), GREEN phase, RED phase

### Community 1738 - "Community 1738"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1739 - "Community 1739"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1740 - "Community 1740"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1741 - "Community 1741"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1742 - "Community 1742"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-dashboard ./in), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.250s), GREEN phase, RED phase

### Community 1743 - "Community 1743"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1744 - "Community 1744"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1745 - "Community 1745"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1746 - "Community 1746"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1747 - "Community 1747"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1749 - "Community 1749"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1750 - "Community 1750"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1751 - "Community 1751"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1752 - "Community 1752"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1753 - "Community 1753"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1754 - "Community 1754"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1755 - "Community 1755"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1756 - "Community 1756"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestVerifySuperpowersRunRuntime_PersistsPhaseAndEv), GREEN phase, RED phase

### Community 1757 - "Community 1757"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1758 - "Community 1758"
Cohesion: 0.4
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.841s), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	2.636s), Summary

### Community 1759 - "Community 1759"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1760 - "Community 1760"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1761 - "Community 1761"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1762 - "Community 1762"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1763 - "Community 1763"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestBrainstormRejectedWhenRunOmitsShort (0.00s)), GREEN phase, RED phase, Summary

### Community 1764 - "Community 1764"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1765 - "Community 1765"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1766 - "Community 1766"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1767 - "Community 1767"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1768 - "Community 1768"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1769 - "Community 1769"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1770 - "Community 1770"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-ci-doctor -sho), code:block2 (# github.com/nico/go-bt-evolve/cmd/bt-ci-doctor [github.com/), RED phase complete

### Community 1771 - "Community 1771"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1772 - "Community 1772"
Cohesion: 0.4
Nodes (4): code:block1 (internal/knowledge/feedback_test.go:247:5: kg.RecordEvolvedF), GREEN phase, RED phase, Summary

### Community 1773 - "Community 1773"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1774 - "Community 1774"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestHandleAgentExecute_RoutesThroughAgentRouter (0), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.021s), GREEN phase, RED phase

### Community 1775 - "Community 1775"
Cohesion: 0.4
Nodes (4): GREEN phase, RED phase, Verification, What was implemented

### Community 1776 - "Community 1776"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestGoapFusionStateHashes_PersistAcrossRuns (0.00s), GREEN phase, RED phase, Summary

### Community 1777 - "Community 1777"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1778 - "Community 1778"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1779 - "Community 1779"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/fusion	1.130s), GREEN phase, RED phase

### Community 1780 - "Community 1780"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1781 - "Community 1781"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/reliability ), code:block2 (ok  github.com/nico/go-bt-evolve/internal/reliability   10.7), GREEN phase, RED phase

### Community 1782 - "Community 1782"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1783 - "Community 1783"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), GREEN phase, RED phase

### Community 1784 - "Community 1784"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/benchcmp ./cmd/bt-agent -sho), code:block2 (--- FAIL: TestNoCommittedRootBinaries (0.02s)), GREEN phase, RED phase

### Community 1785 - "Community 1785"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1786 - "Community 1786"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1787 - "Community 1787"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1788 - "Community 1788"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1789 - "Community 1789"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1790 - "Community 1790"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1792 - "Community 1792"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1793 - "Community 1793"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1794 - "Community 1794"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1795 - "Community 1795"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1796 - "Community 1796"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1797 - "Community 1797"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1798 - "Community 1798"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1799 - "Community 1799"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1800 - "Community 1800"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1801 - "Community 1801"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1802 - "Community 1802"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1803 - "Community 1803"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1804 - "Community 1804"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1805 - "Community 1805"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1806 - "Community 1806"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1807 - "Community 1807"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1808 - "Community 1808"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1809 - "Community 1809"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1810 - "Community 1810"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1811 - "Community 1811"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1812 - "Community 1812"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 1813 - "Community 1813"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1814 - "Community 1814"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1815 - "Community 1815"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1816 - "Community 1816"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1817 - "Community 1817"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1818 - "Community 1818"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1819 - "Community 1819"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1820 - "Community 1820"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1821 - "Community 1821"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 1822 - "Community 1822"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1823 - "Community 1823"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1824 - "Community 1824"
Cohesion: 0.4
Nodes (4): code:block1 (internal/agentexec/wiring_test.go:50:12: undefined: ResolveG), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agentexec	0.018s), GREEN phase, RED phase

### Community 1825 - "Community 1825"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1826 - "Community 1826"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent-cli ./cmd/bt-agent ), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent-cli	0.015s), GREEN phase, RED phase

### Community 1827 - "Community 1827"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1828 - "Community 1828"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1829 - "Community 1829"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1830 - "Community 1830"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1831 - "Community 1831"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1832 - "Community 1832"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1833 - "Community 1833"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1834 - "Community 1834"
Cohesion: 0.5
Nodes (3): code:block1 (internal/evolution/island_model_test.go:348:5: im.Cap undefi), GREEN phase, RED phase

### Community 1835 - "Community 1835"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1836 - "Community 1836"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1837 - "Community 1837"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1838 - "Community 1838"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestEvolveTreeV2_DeepSearchResultAppliedWhenGreedy), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.110s), GREEN phase, RED phase

### Community 1839 - "Community 1839"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1840 - "Community 1840"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1841 - "Community 1841"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1842 - "Community 1842"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/dashboard -s), GREEN phase, RED phase, RED phase complete

### Community 1843 - "Community 1843"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1844 - "Community 1844"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1845 - "Community 1845"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1846 - "Community 1846"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1847 - "Community 1847"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1848 - "Community 1848"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1849 - "Community 1849"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1850 - "Community 1850"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1851 - "Community 1851"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1852 - "Community 1852"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestBTUseResearchTreeHoldsBBMu (0.00s)), GREEN phase, RED phase

### Community 1853 - "Community 1853"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1854 - "Community 1854"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1855 - "Community 1855"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1856 - "Community 1856"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1857 - "Community 1857"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1858 - "Community 1858"
Cohesion: 0.4
Nodes (4): code:block1 (fatal error: concurrent map iteration and map write), GREEN phase, RED phase, Summary

### Community 1859 - "Community 1859"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1860 - "Community 1860"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1861 - "Community 1861"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1862 - "Community 1862"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -run TestAttemptOut), GREEN phase, RED phase, RED Phase Complete

### Community 1863 - "Community 1863"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1864 - "Community 1864"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestAllDomainTreesHaveDescriptionsAndLeafCoverage), GREEN phase, RED phase, RED phase complete

### Community 1865 - "Community 1865"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1866 - "Community 1866"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1867 - "Community 1867"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1868 - "Community 1868"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1869 - "Community 1869"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1870 - "Community 1870"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1871 - "Community 1871"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1872 - "Community 1872"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1873 - "Community 1873"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1874 - "Community 1874"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestGardenerRunCycleTool_CallAppliesLearnedSelecto), GREEN phase, RED phase, Summary

### Community 1875 - "Community 1875"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1876 - "Community 1876"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1877 - "Community 1877"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1878 - "Community 1878"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agentexec ./), GREEN phase, RED phase, Summary

### Community 1879 - "Community 1879"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1880 - "Community 1880"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1881 - "Community 1881"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1882 - "Community 1882"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1883 - "Community 1883"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1884 - "Community 1884"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1885 - "Community 1885"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1886 - "Community 1886"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1887 - "Community 1887"
Cohesion: 0.4
Nodes (4): code:block1 (internal/engine/actions_superpowers_prod_test.go:564:2: unde), Final report, GREEN phase, RED phase

### Community 1888 - "Community 1888"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1889 - "Community 1889"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1890 - "Community 1890"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestAnalyzeImprovementGaps_NoFabricatedEngineTestB), GREEN phase, RED phase

### Community 1891 - "Community 1891"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1892 - "Community 1892"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1893 - "Community 1893"
Cohesion: 0.4
Nodes (4): GREEN phase, RED phase, Summary, Summary

### Community 1894 - "Community 1894"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1895 - "Community 1895"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1896 - "Community 1896"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1897 - "Community 1897"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1898 - "Community 1898"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1899 - "Community 1899"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1900 - "Community 1900"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1901 - "Community 1901"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1902 - "Community 1902"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1903 - "Community 1903"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1904 - "Community 1904"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/a2a -short -count=1 -ti), code:block2 (--- FAIL: TestTaskStateBridge_A2AToBT (0.00s)), GREEN phase, RED phase

### Community 1905 - "Community 1905"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1906 - "Community 1906"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1907 - "Community 1907"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1908 - "Community 1908"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1909 - "Community 1909"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1910 - "Community 1910"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1911 - "Community 1911"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1912 - "Community 1912"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/knowledge ./internal/do), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	18.133s), GREEN phase, RED phase

### Community 1913 - "Community 1913"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1914 - "Community 1914"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1915 - "Community 1915"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1916 - "Community 1916"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1917 - "Community 1917"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1918 - "Community 1918"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1919 - "Community 1919"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1920 - "Community 1920"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine -run ), code:block2 (--- PASS: TestScheduledRuntime_ResumedTickChargesGoalOnGenui), GREEN phase, RED phase

### Community 1921 - "Community 1921"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1922 - "Community 1922"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1923 - "Community 1923"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1924 - "Community 1924"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1925 - "Community 1925"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1926 - "Community 1926"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1927 - "Community 1927"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1928 - "Community 1928"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine ./cmd), GREEN phase, RED phase, Summary

### Community 1929 - "Community 1929"
Cohesion: 0.4
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.104s), code:block2 (--- PASS: TestGoapPlanningRunsRealGOAPPlannerFirst (0.00s)), GREEN phase, RED phase

### Community 1930 - "Community 1930"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1931 - "Community 1931"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1932 - "Community 1932"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1933 - "Community 1933"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1934 - "Community 1934"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1935 - "Community 1935"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1936 - "Community 1936"
Cohesion: 0.4
Nodes (4): GREEN phase, RED phase, RED phase complete, Summary

### Community 1937 - "Community 1937"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1938 - "Community 1938"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1939 - "Community 1939"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1940 - "Community 1940"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1941 - "Community 1941"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1942 - "Community 1942"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1943 - "Community 1943"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1944 - "Community 1944"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1945 - "Community 1945"
Cohesion: 0.4
Nodes (4): code:block1 (internal/engine/mcp_server_test.go:18:10: srv.HasTool undefi), GREEN phase, RED phase, RED phase complete

### Community 1946 - "Community 1946"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1947 - "Community 1947"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1948 - "Community 1948"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1949 - "Community 1949"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/gardener ./cmd/bt-agent), GREEN phase, RED phase, Summary

### Community 1950 - "Community 1950"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1951 - "Community 1951"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1952 - "Community 1952"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1953 - "Community 1953"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1954 - "Community 1954"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1955 - "Community 1955"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1956 - "Community 1956"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1957 - "Community 1957"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1959 - "Community 1959"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1960 - "Community 1960"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1961 - "Community 1961"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1962 - "Community 1962"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/benchmark -r), GREEN phase, RED phase, Summary

### Community 1963 - "Community 1963"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1964 - "Community 1964"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1965 - "Community 1965"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 1966 - "Community 1966"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1967 - "Community 1967"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-dashboard -sho), code:block2 (--- FAIL: TestHandleAnalyze_UsesWorkflowForTaskDerivation (0), GREEN phase, RED phase

### Community 1968 - "Community 1968"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard ./cmd/bt-agent ), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.070s), GREEN phase, RED phase

### Community 1969 - "Community 1969"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1970 - "Community 1970"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1971 - "Community 1971"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1972 - "Community 1972"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1973 - "Community 1973"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1974 - "Community 1974"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1975 - "Community 1975"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1976 - "Community 1976"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 1977 - "Community 1977"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1978 - "Community 1978"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine ./internal/resea), code:block2 (--- FAIL: TestRefundGoapMilestoneAttempt_ReleasesProgramClai), GREEN phase, RED phase

### Community 1979 - "Community 1979"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1980 - "Community 1980"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1981 - "Community 1981"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1982 - "Community 1982"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1983 - "Community 1983"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1984 - "Community 1984"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1985 - "Community 1985"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1986 - "Community 1986"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1987 - "Community 1987"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1988 - "Community 1988"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1989 - "Community 1989"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1990 - "Community 1990"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1991 - "Community 1991"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-dashboard ./in), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.205s), GREEN phase, RED phase

### Community 1992 - "Community 1992"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1993 - "Community 1993"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agentexec ./), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agentexec	0.018s), GREEN phase, RED phase

### Community 1994 - "Community 1994"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1995 - "Community 1995"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1996 - "Community 1996"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1997 - "Community 1997"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1998 - "Community 1998"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 1999 - "Community 1999"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2000 - "Community 2000"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2001 - "Community 2001"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2002 - "Community 2002"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2003 - "Community 2003"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2004 - "Community 2004"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -run 'Tes), GREEN phase, RED phase, Summary

### Community 2005 - "Community 2005"
Cohesion: 0.4
Nodes (4): code:block1 (internal/engine/chains_test.go:3326:61: too many arguments i), code:block2 (/usr/local/go/bin/go test -race ./internal/engine -short -ru), GREEN phase, RED phase

### Community 2006 - "Community 2006"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2007 - "Community 2007"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (--- FAIL: TestRunSuperpowersRuntime_PlanResumeSetsImplementa), GREEN phase, RED phase

### Community 2008 - "Community 2008"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2009 - "Community 2009"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2010 - "Community 2010"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/agent ./internal/reliab), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	4.109s), GREEN phase, RED phase

### Community 2011 - "Community 2011"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 2012 - "Community 2012"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2013 - "Community 2013"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2014 - "Community 2014"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), GREEN phase, RED phase, Summary

### Community 2015 - "Community 2015"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2016 - "Community 2016"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 2017 - "Community 2017"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2018 - "Community 2018"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2019 - "Community 2019"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2020 - "Community 2020"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2021 - "Community 2021"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2022 - "Community 2022"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2023 - "Community 2023"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2024 - "Community 2024"
Cohesion: 0.4
Nodes (4): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.427s), GREEN phase, RED phase, Summary

### Community 2025 - "Community 2025"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2026 - "Community 2026"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2027 - "Community 2027"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2028 - "Community 2028"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2029 - "Community 2029"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2030 - "Community 2030"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestDeterministicPlanDoesNotTargetAlwaysGreenTest ), GREEN phase, RED phase, RED Phase Complete

### Community 2031 - "Community 2031"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2032 - "Community 2032"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2033 - "Community 2033"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener ./i), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.086s-0), GREEN phase, RED phase

### Community 2034 - "Community 2034"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2035 - "Community 2035"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2036 - "Community 2036"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2037 - "Community 2037"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/domains ./in), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.165s), GREEN phase, RED phase

### Community 2038 - "Community 2038"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2039 - "Community 2039"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2040 - "Community 2040"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 2041 - "Community 2041"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2042 - "Community 2042"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2043 - "Community 2043"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2044 - "Community 2044"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2045 - "Community 2045"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2046 - "Community 2046"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2047 - "Community 2047"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test -race ./internal/evolution -short ), code:block2 (# github.com/nico/go-bt-evolve/internal/evolution [.../inter), GREEN phase, RED phase

### Community 2048 - "Community 2048"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 2049 - "Community 2049"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2050 - "Community 2050"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2051 - "Community 2051"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2052 - "Community 2052"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2053 - "Community 2053"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2054 - "Community 2054"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (--- FAIL: TestSuperpowersPipelineIsGuarded (0.00s)), GREEN phase, RED phase

### Community 2055 - "Community 2055"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (--- FAIL: TestResolveTreeID_SuperpowersPipeline (0.00s)), GREEN phase, RED phase

### Community 2056 - "Community 2056"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2057 - "Community 2057"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2058 - "Community 2058"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2059 - "Community 2059"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestSchedulerAndDLQReplayDispatchThroughAgentRoute), GREEN phase, RED phase, Summary

### Community 2060 - "Community 2060"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2061 - "Community 2061"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2062 - "Community 2062"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2063 - "Community 2063"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2064 - "Community 2064"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2065 - "Community 2065"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2066 - "Community 2066"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2067 - "Community 2067"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2068 - "Community 2068"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2069 - "Community 2069"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2070 - "Community 2070"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2071 - "Community 2071"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, GREEN_COMMANDS, GREEN_RESULTS, NOTES

### Community 2072 - "Community 2072"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2073 - "Community 2073"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2074 - "Community 2074"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2075 - "Community 2075"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2076 - "Community 2076"
Cohesion: 0.4
Nodes (4): code:block1 (--- FAIL: TestRunTaskRecordsAgentMetrics (0.00s)), GREEN phase, RED phase, RED Phase Summary

### Community 2077 - "Community 2077"
Cohesion: 0.4
Nodes (4): GREEN phase, RED phase, RED phase complete, Summary

### Community 2078 - "Community 2078"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2079 - "Community 2079"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2080 - "Community 2080"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2081 - "Community 2081"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2082 - "Community 2082"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2083 - "Community 2083"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/engine ./internal/domai), GREEN phase, RED phase, Summary

### Community 2084 - "Community 2084"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2085 - "Community 2085"
Cohesion: 0.4
Nodes (4): code:block1 (ClaimedBy = "prior-cycle-run-id", want cleared — stale-miles), GREEN phase, RED phase, Summary

### Community 2086 - "Community 2086"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2087 - "Community 2087"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2088 - "Community 2088"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2089 - "Community 2089"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2090 - "Community 2090"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2091 - "Community 2091"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2092 - "Community 2092"
Cohesion: 0.4
Nodes (4): FILES_CHANGED, NOTES, RED_COMMAND, RED_RESULT

### Community 2093 - "Community 2093"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2094 - "Community 2094"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2095 - "Community 2095"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2096 - "Community 2096"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2097 - "Community 2097"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2098 - "Community 2098"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2099 - "Community 2099"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2100 - "Community 2100"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2101 - "Community 2101"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2102 - "Community 2102"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2103 - "Community 2103"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.454s), GREEN phase, RED phase

### Community 2104 - "Community 2104"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2105 - "Community 2105"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2106 - "Community 2106"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2107 - "Community 2107"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2108 - "Community 2108"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2109 - "Community 2109"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2110 - "Community 2110"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2111 - "Community 2111"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2112 - "Community 2112"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2113 - "Community 2113"
Cohesion: 0.4
Nodes (4): code:block1 (/usr/local/go/bin/go test ./internal/domains -short -count=1), code:block2 (internal/domains/domains_test.go:757:20: undefined: NonRegis), GREEN phase, RED phase

### Community 2114 - "Community 2114"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2115 - "Community 2115"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2116 - "Community 2116"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2117 - "Community 2117"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2118 - "Community 2118"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2119 - "Community 2119"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2120 - "Community 2120"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2121 - "Community 2121"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2122 - "Community 2122"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2123 - "Community 2123"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2124 - "Community 2124"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2125 - "Community 2125"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2126 - "Community 2126"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2127 - "Community 2127"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2128 - "Community 2128"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2129 - "Community 2129"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2130 - "Community 2130"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2131 - "Community 2131"
Cohesion: 0.4
Nodes (4): code:block1 (PATH=/usr/local/go/bin:$PATH go test -race ./internal/dashbo), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	3.409s), GREEN phase, RED phase

### Community 2132 - "Community 2132"
Cohesion: 0.4
Nodes (4): Changed Files, Superpowers Finish Report, Tasks, Verification

### Community 2133 - "Community 2133"
Cohesion: 0.4
Nodes (4): 1.1 Requirements Overview, 1.2 Quality Goals, 1.3 Stakeholders, 1. Introduction and Goals

### Community 2135 - "Community 2135"
Cohesion: 0.5
Nodes (3): feedbackPersistState, feedbackSnapshot, treeFeedback

### Community 2136 - "Community 2136"
Cohesion: 0.67
Nodes (3): LoadPipeline(), RunPipeline(), RunPipelineWithID()

### Community 2143 - "Community 2143"
Cohesion: 0.83
Nodes (3): envDefault(), main(), run()

### Community 2146 - "Community 2146"
Cohesion: 0.5
Nodes (4): Boolean `false` in config file is ignored, code:bash (# Check the file exists), Config file not found, Configuration Issues

### Community 2147 - "Community 2147"
Cohesion: 0.5
Nodes (4): API key not working on dashboard, code:bash (# Check if BT_API_KEY is set), MCP rate limit exceeded, Security Issues

### Community 2148 - "Community 2148"
Cohesion: 0.5
Nodes (4): code:bash (bt-agent run fusion-deliberation "Survey the strongest argum), code:bash (go build ./...), code:bash (git add docs/fusion-bt-agent.md README.md), Task 15: Documentation and Operator Notes

### Community 2149 - "Community 2149"
Cohesion: 0.5
Nodes (4): code:bash (/usr/local/go/bin/gofmt -w \), code:bash (systemctl --user stop bt-agent), code:bash (graphify update .), Task 16: Verified-patching deployment

### Community 2150 - "Community 2150"
Cohesion: 0.5
Nodes (4): code:go (package domains), code:go (package engine), code:bash (/usr/local/go/bin/go test ./internal/domains ./internal/engi), Task 1: Lock current stub behavior with failing tests

### Community 2151 - "Community 2151"
Cohesion: 0.5
Nodes (4): code:go (type SuperpowersTaskExecutor struct {), code:text (FILES_CHANGED:), code:bash (git status --short --untracked-files=all), Task 9: Production task execution with Claude Code + TDD

### Community 2152 - "Community 2152"
Cohesion: 0.5
Nodes (3): 4. Solution Strategy, Key Technology Decisions, Quality Goals → Solution Approaches

### Community 2153 - "Community 2153"
Cohesion: 0.5
Nodes (3): 11. Risks and Technical Debt, Known Technical Debt, Prioritized Risk Table

### Community 2154 - "Community 2154"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2155 - "Community 2155"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Wire the dashboard's dead task-metrics and block-fitness recor…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2156 - "Community 2156"
Cohesion: 0.5
Nodes (3): Task 1: Give non-registry domain trees first-class descriptions via a unified description lookup, Task 2: Make smoke-test coverage of the domain-tree registry self-enforcing, Task 3: Extend condition coverage to benchmark suite reachability for every domain tree

### Community 2157 - "Community 2157"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2158 - "Community 2158"
Cohesion: 0.5
Nodes (3): Task 1: Add category-aware retry helper for chain LLM calls, Task 2: Wire the retry helper into the six single-shot chain executors, Task 3: Replace ad hoc immediate retries in execMapReduce/execRefine with the shared policy

### Community 2159 - "Community 2159"
Cohesion: 0.5
Nodes (3): Task 1: Add an injectable RNG seam to the evolution mutation primitives, Task 2: Route the four evolution algorithms through the seam and de-flake the Observe tests, Task 3: Close the domain-tree smoke, description, and condition-coverage gap

### Community 2160 - "Community 2160"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Re-land the lost `SanitizeUserID` traversal guard: prefix all-dot ide…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2161 - "Community 2161"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 5/5: Ad…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2162 - "Community 2162"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2163 - "Community 2163"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Fetch the Create-Agent tree dropdown from the existing `/api/trees` e…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2164 - "Community 2164"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests pinning `internal/config/defaults.go`'s exported defaults, Task 2: Behavioral routing coverage for RefactoringTree, SecurityAuditTree, DataPipelineTree, Task 3: Behavioral routing coverage for MeetingNotesTree, TradingSignalTree, GameAITree

### Community 2165 - "Community 2165"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2166 - "Community 2166"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2167 - "Community 2167"
Cohesion: 0.5
Nodes (3): Task 1: bt_blocks_compose rejects unknown strategy tree IDs instead of silently dropping the router, Task 2: Validate composed trees and surface Save errors before activating via bt_blocks_compose save=true, Task 3: Guard bt-agent-cli positional-arg subcommands against index panic and trim feedback identifiers

### Community 2168 - "Community 2168"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2169 - "Community 2169"
Cohesion: 0.5
Nodes (3): Task 1: Extract tunable parameters from struct fields, not just Metadata keys, Task 2: Tree-level CMA-ES tuning helper wrapping Extract→Optimize→Apply, Task 3: Algorithm selection and per-tree reporting in bt_evolve_bottlenecks

### Community 2170 - "Community 2170"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests for eval_suites.go (Q1 backlog m1/3): pin AllSuites() structure and task-builder helper semantics, Task 2: Characterization tests for swebench_verified.go (Q1 backlog m3/3): pin resolution boundary, failure/aggregation semantics, and JSON wire format, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: internal/domains/trees.go, internal/domains/domains_test.go)

### Community 2171 - "Community 2171"
Cohesion: 0.5
Nodes (3): Task 1: RunSuite path-match types — Result.PathMatched + RunMetrics.PathMatchRate, Task 2: ScoreMutation/RunABTest penalize routing regressions even when Outcome still "succeeds", Task 3: Cross-tree integration guard — PathMatchRate consulted for every registered domain suite

### Community 2172 - "Community 2172"
Cohesion: 0.5
Nodes (3): Task 1: Track the real tree ID separately from the collision-disambiguated registry name, Task 2: Filter gardener reflection scoring by real tree ID and `Record.User`, Task 3: Stop `bankFor` from permanently caching the shared-bank fallback on a transient open error

### Community 2173 - "Community 2173"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Remove the committed 8.5MB `bt-scalability-probe` binary from version…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2174 - "Community 2174"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2175 - "Community 2175"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2176 - "Community 2176"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests for `actions_bt_manager.go` pure helper functions, Task 2: Characterization tests for the registered BT Manager actions, Task 3: Per-tree smoke assertions for the domain trees currently covered only by the generic loop

### Community 2177 - "Community 2177"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2178 - "Community 2178"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests pinning MockLLM's exported behavior, Task 2: Derive the resolver-reachable domain-tree work list from production instead of a test literal, Task 3: Rewire every resolver-reachable guard onto the production registry and close the missing coverage legs

### Community 2179 - "Community 2179"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2180 - "Community 2180"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2181 - "Community 2181"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Remove the two committed root-level build artifacts and prevent recur…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2182 - "Community 2182"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2183 - "Community 2183"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2184 - "Community 2184"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2185 - "Community 2185"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Review complete. The report above is my full output in the required G…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2186 - "Community 2186"
Cohesion: 0.5
Nodes (3): Task 1: Add migration accounting and stable generation cadence to IslandModel, Task 2: Register deterministic bt_evolve_island MCP tool in bt-agent, Task 3: Domain-seeded islands — per-domain trees as island populations

### Community 2187 - "Community 2187"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2188 - "Community 2188"
Cohesion: 0.5
Nodes (3): Task 1: Per-tree behavioral-diversity archive accessor on Gardener, Task 2: Wire the diversity archive into `CrisisDetector.Detect`'s call site, Task 3: Structural completeness guard for domain-tree coverage registries

### Community 2189 - "Community 2189"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2190 - "Community 2190"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2191 - "Community 2191"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2192 - "Community 2192"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2193 - "Community 2193"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2194 - "Community 2194"
Cohesion: 0.5
Nodes (3): Task 1: Derive the rotation replacement TTL from the rotated key instead of a hardcoded 24h, Task 2: Give callers an explicit replacement-TTL opt-out and correct the RotateKey contract docs, Task 3: Close the registry hole that lets a domain tree escape smoke, description, and condition coverage

### Community 2195 - "Community 2195"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/quality…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2196 - "Community 2196"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2197 - "Community 2197"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire the knowledge graph into the gardener daemon's live prio…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2198 - "Community 2198"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2199 - "Community 2199"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-dashboard/agent…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2200 - "Community 2200"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2201 - "Community 2201"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "CompanyState mutex held across multi-minute LLM tree executions in RunSprint" mil…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2202 - "Community 2202"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests for actions_godev.go's fixed-output actions (SuggestImprovements, CompileGoCode, RunGoTests, AnalyzeTestResults), Task 2: Characterization tests for actions_godev.go's LLM-plan-backed actions (ReviewGoCode, FixBuildErrors, ExplainGoConcept), Task 3: Guard AllDomainTrees() registry purity and confirm smoke/description/condition coverage holds

### Community 2203 - "Community 2203"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/hitl_e…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2204 - "Community 2204"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2205 - "Community 2205"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Harden the ADR-010 personalization layer — user isolation, crash safety, and re…, Task 2: NotebookLM research: Defense-in-depth in the retry policy: when a rate-limit error's known…

### Community 2206 - "Community 2206"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2207 - "Community 2207"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2208 - "Community 2208"
Cohesion: 0.5
Nodes (3): Task 1: Make the replacement key's lifetime a derived policy instead of a hardcoded constant, Task 2: RotateKey inherits the rotated key's lifetime and gains an explicit TTL opt-out, Task 3: Describe the resolver-reachable trees that the description maps currently cannot answer for

### Community 2209 - "Community 2209"
Cohesion: 0.5
Nodes (3): Task 1: Characterize NotebookLMState in-memory semantics (hash, cooldowns, counters, summary), Task 2: Make NotebookLM state persistence testable and fix the nil-ChainState panic in the load action, Task 3: Fail-closed guard — every exported domain tree constructor must land in a coverage registry

### Community 2210 - "Community 2210"
Cohesion: 0.5
Nodes (3): Task 1: Restore + configure feedback persistence at scheduler startup, Task 2: Accumulate feedback on every run and flush on shutdown, Task 3: Close remaining domain-tree smoke/description/condition coverage gaps

### Community 2211 - "Community 2211"
Cohesion: 0.5
Nodes (3): Task 1: Trim `--id` when building the CLI blackboard scope in `parseBBScopeFlag`, Task 2: Pin the cross-tool round-trip: CLI scope reads entries the daemon wrote, Task 3: Verify domain-tree smoke/description/condition coverage holds fleet-wide (no fabrication)

### Community 2212 - "Community 2212"
Cohesion: 0.5
Nodes (3): Task 1: Characterization tests for BT Manager's reflection-driven report actions, Task 2: Characterization tests for BT Manager bootstrap/intervention actions and pure helpers, Task 3: Close domain-tree smoke/description/condition coverage gaps

### Community 2213 - "Community 2213"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2214 - "Community 2214"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Panic-proof the evolution execution path and close the gardener experience loop (…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2215 - "Community 2215"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2216 - "Community 2216"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Bound the ExperienceBank: add a capacity cap (~200-500 entries) with …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2217 - "Community 2217"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2218 - "Community 2218"
Cohesion: 0.5
Nodes (3): Task 1: Add raise-only evolved-fitness write-back to the knowledge graph, Task 2: Wire the write-back into `bt_evolve_bottlenecks` with scale normalization, Task 3: Confirm/consolidate domain-tree coverage guard

### Community 2219 - "Community 2219"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/goap_r…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2220 - "Community 2220"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Consolidate the triplicated block-fitness outcome-scor…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2221 - "Community 2221"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Auction subsystem production hardening (review follow-up 2026-07-04)" milestone 4…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2222 - "Community 2222"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Give Q-learning evolution durable cross-run memory (Q2 Evolvability)" milestone 4…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2223 - "Community 2223"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Blackboard holds a global mutex while blocking forever on a cross-process flock" …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2224 - "Community 2224"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2225 - "Community 2225"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2226 - "Community 2226"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2227 - "Community 2227"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Close the fitness feedback loop into tree discovery and re-evolution targeting (Q…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2228 - "Community 2228"
Cohesion: 0.5
Nodes (3): Task 1: Wire the benchmark-suite gate into `bt_evolve_island`, Task 2: Harden the remaining `bt_evolve_island` archive tests against gate-induced flakiness, Task 3: Domain trees smoke/description/condition coverage — re-verify only

### Community 2229 - "Community 2229"
Cohesion: 0.5
Nodes (4): code:bash (grep -E '^### ADR-[0-9]+' docs/arc42/09-decisions.md | sed '), code:bash (H=$(grep -cE '^### ADR-[0-9]+' docs/arc42/09-decisions.md)), code:bash (for n in 001 002 003 004 005 006 007; do), Task 6: Merge §9 (canonical ADR log + docs/adr fold-in)

### Community 2230 - "Community 2230"
Cohesion: 0.5
Nodes (4): code:bash (grep -rn 'assemble' internal/goap/), code:bash (grep -rn 'SaveDocument\|CollectAllSections\|GenerateTOC\|All), code:bash (PATH=/usr/local/go/bin:$PATH make test), Task 22: Assembly retirement + Commit 4

### Community 2231 - "Community 2231"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2232 - "Community 2232"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/benchmark/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2233 - "Community 2233"
Cohesion: 0.5
Nodes (3): Task 1: Behavior tests for handleAgentCreate/handleAgentDelete lifecycle, Task 2: Behavior tests for handleSecurityAudit/handleAlertRules, Task 3: Descriptions-map coverage for resolver-reachable trees outside AllDomainTrees

### Community 2234 - "Community 2234"
Cohesion: 0.5
Nodes (3): Task 1: Make WorkerPool Submit/Shutdown fail-safe against send-on-closed panic, Task 2: Wire graceful WorkerPool shutdown into the dashboard lifecycle and surface pool state, Task 3: Guarantee smoke tests, descriptions, and condition coverage for all domain trees

### Community 2235 - "Community 2235"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blocks/observ…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2236 - "Community 2236"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Auction-based A2A task allocation for multi-agent coordination" milestone 5/5: Ad…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2237 - "Community 2237"
Cohesion: 0.5
Nodes (3): Task 1: Sidecar flock primitive for the experience-bank snapshot, Task 2: Hold the sidecar lock across addEntry's merge-from-disk → atomic-rename window, Task 3: Make MarkReused/public Persist merge-under-lock so full-file rewrites cannot drop the other writer's entries

### Community 2238 - "Community 2238"
Cohesion: 0.5
Nodes (3): Task 1: Add durable load for GOAP fusion charge/refund stamps, Task 2: Wire durable load into resumed-tick research-goal charging, Task 3: Regression-cover the milestone refund path through the full scheduled-resume entry point

### Community 2239 - "Community 2239"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2240 - "Community 2240"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2241 - "Community 2241"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Wire `a2a.AuctionCardsFn` (and the underlying A2A card source) into `…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2242 - "Community 2242"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/registr…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2243 - "Community 2243"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2244 - "Community 2244"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2245 - "Community 2245"
Cohesion: 0.5
Nodes (3): Task 1: Characterize `StatusAborted` and the abort semantics its consumers depend on, Task 2: Characterization tests for `BuildRateLimit` throttling and metadata parsing, Task 3: Close the two remaining domain-tree coverage holes — unregistered constructors and unregistered tree *variants*

### Community 2246 - "Community 2246"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — harden the knowledge-graph embedding client against an unrespo…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2247 - "Community 2247"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/monito…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2248 - "Community 2248"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2249 - "Community 2249"
Cohesion: 0.5
Nodes (3): Task 1: Branch age and abandonment gate helpers for superpowers branch reaping, Task 2: Wire age/abandonment gates into reapOrphanedSuperpowersBranches with archive-before-delete, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage

### Community 2250 - "Community 2250"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2251 - "Community 2251"
Cohesion: 0.5
Nodes (3): Task 1: Add normalized fitness write-back method to the knowledge graph, Task 2: Wire the write-back into `bt_evolve_bottlenecks` and prove bottleneck-clearing, Task 3: Assert domain-tree smoke/description/condition coverage stays complete

### Community 2252 - "Community 2252"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2253 - "Community 2253"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-docgen/main.go …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2254 - "Community 2254"
Cohesion: 0.5
Nodes (3): Task 1: Add `FeedbackFile()` path helper for the feedback snapshot, Task 2: Wire `FeedbackPath` into the production daemon scheduler, Task 3: Enforce descriptions + leaf-condition coverage across all domain trees

### Community 2255 - "Community 2255"
Cohesion: 0.5
Nodes (3): Task 1: Add leaf-with-children validation rule for AlwaysSucceed in ValidateTreeFull, Task 2: Generalize the rule to all childless leaf types and surface it through the flat ValidateTree, Task 3: Fleet-wide domain-tree guard — no leaf carries children, and condition/description coverage holds

### Community 2256 - "Community 2256"
Cohesion: 0.5
Nodes (3): Task 1: Durable Claude rate-limit backoff state (helpers + persistence), Task 2: Claude review fallback consumes and records the backoff, Task 3: Superpowers runtime honors and records the backoff

### Community 2257 - "Community 2257"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Rewrite `gardener.CollectAgentSLOs()` (`internal/gardener/gardener.go…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2258 - "Community 2258"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2259 - "Community 2259"
Cohesion: 0.5
Nodes (3): Task 1: util/persist.go — perm-aware atomic save with tmp cleanup, Task 2: evolution — delete file_lock.go, migrate 19 lock sites and 15 atomic-write blocks onto the canonical owners, Task 3: domains — extend resolver coverage to the prefix-namespaced switch branches

### Community 2260 - "Community 2260"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/a2a/doc.go an…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2261 - "Community 2261"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/benchcmp/main.go a…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2262 - "Community 2262"
Cohesion: 0.5
Nodes (3): Task 1: Persist replay outcomes on DeadLetterEntry — LastReplayAt/LastReplayError stamped in Replay()'s failure path, Task 2: bt_dlq_replay wait=true reports the actual replay error instead of a canned reason, Task 3: Dashboard /api/dlq listing surfaces replay outcomes and a failed_replays aggregate

### Community 2263 - "Community 2263"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Fix the tree-ID mismatch in `RegisterNotebookLMFitness` so the anti-f…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2264 - "Community 2264"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evaluator/eva…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2265 - "Community 2265"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Stop `internal/engine` unit tests from exec-ing the live NotebookLM C…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2266 - "Community 2266"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q1 Correctness / Q3 Reliability — Close the mcpDeps shared-blackboard data race…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2267 - "Community 2267"
Cohesion: 0.5
Nodes (3): Task 1: Add an exported tool-invocation seam to engine.Server, Task 2: Register bt_evolve_qd (MAP-Elites quality-diversity) MCP tool, Task 3: Register bt_evolve_multiobjective (NSGA-II) MCP tool

### Community 2268 - "Community 2268"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Fix the bt-fusion HITL gate misclassification that deadlocks e…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2269 - "Community 2269"
Cohesion: 0.5
Nodes (3): Task 1: IslandModel per-island population cap with fitness-ordered eviction, Task 2: Island-count cap, eviction counters, and Stats/Summary surfacing, Task 3: Wire caps into the bt_evolve_island tool and surface eviction in its JSON result

### Community 2270 - "Community 2270"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2271 - "Community 2271"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2272 - "Community 2272"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2273 - "Community 2273"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Implement autonomous stall recovery using Dijkstra’s algorithm over…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2274 - "Community 2274"
Cohesion: 0.5
Nodes (4): code:go (// internal/engine/semaphore_guard_test.go), code:go (// internal/engine/semaphore_guard.go), code:bash (git add internal/engine/semaphore_guard.go internal/engine/s), Task 5: `SemaphoreGuard` decorator

### Community 2275 - "Community 2275"
Cohesion: 0.5
Nodes (4): code:go (// internal/engine/classify_task_kind_test.go), code:go (RegisterAction("ClassifyTaskKind", func(ctx *btcore.BTContex), code:bash (git add internal/engine/actions_superpowers_prod.go internal), Task 8: `ClassifyTaskKind` action (skill-dispatch router input)

### Community 2276 - "Community 2276"
Cohesion: 0.5
Nodes (4): code:go (// internal/engine/cached_condition_test.go), code:go (// internal/engine/cached_condition.go), code:bash (git add internal/engine/cached_condition.go internal/engine/), Task 4: `CachedCondition` decorator

### Community 2277 - "Community 2277"
Cohesion: 0.5
Nodes (4): code:go (// internal/engine/review_cycle_test.go), code:go (// internal/engine/review_cycle.go), code:bash (git add internal/engine/review_cycle.go internal/engine/revi), Task 7: `ReviewCycle` decorator

### Community 2278 - "Community 2278"
Cohesion: 0.5
Nodes (4): code:go (// internal/engine/superpowers_grill_test.go), code:go (// internal/engine/superpowers_grill.go), code:bash (git add internal/engine/superpowers_grill.go internal/engine), Task 9: `GrillDesignArtifact` action (NotebookLM-answered design interrogation with graded fallback)

### Community 2279 - "Community 2279"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2280 - "Community 2280"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Register the ~24 domain trees missing from `internal/knowledge/regist…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2281 - "Community 2281"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Add characterization tests for `internal/blocks/a2a.go`'s `A2AHandoff…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2282 - "Community 2282"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: NotebookLM research: Mirror the loop tree's terminal `AlwaysSucceed` "ResearchOptional" sk…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2283 - "Community 2283"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/blackboard/ha…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2284 - "Community 2284"
Cohesion: 0.5
Nodes (3): Task 1: Export `shiftEdgeIndices` in `internal/evolution/mutate.go` for cross-package reuse, Task 2: Wire `ShiftEdgeIndices` into `applyMutationOp`'s add/remove cases, Task 3: Re-verify domain-tree smoke/description/condition coverage is still complete

### Community 2285 - "Community 2285"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/domains/noteb…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2286 - "Community 2286"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/action…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2287 - "Community 2287"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Self-fix (review 2026-07-22): ScoreOutcome success-string fallback contradicts su…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2288 - "Community 2288"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q2 Evolvability — Wire the entropy/Gini-based BTOptimizer/DTAnalyzer decision-t…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2289 - "Community 2289"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Blackboard holds a global mutex while blocking forever on a cross-process flock" …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2290 - "Community 2290"
Cohesion: 0.5
Nodes (3): Task 1: Legacy island-archive adoption helpers: detection + merge via im.Load, Task 2: Wire one-time legacy adoption into the bt_evolve_island handler, Task 3: Fail-safe retire ordering: never strand adopted genomes

### Community 2291 - "Community 2291"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q5 Consistency & Reuse — Graph fidelity and pipeline robustness: untrack heavy …, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2292 - "Community 2292"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/def…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2293 - "Community 2293"
Cohesion: 0.5
Nodes (3): Task 1: Fix alert_router's CriticalAlert/HealthAlert keyword coverage gaps, Task 2: Fix trading_signal's IsTAPath false-positive on "reversion", Task 3: Make branch-reachability coverage check actual path match, not just success

### Community 2294 - "Community 2294"
Cohesion: 0.5
Nodes (3): Task 1: NotebookLM research: `treeFeedback` gains `RecentRuns` and `SaveFeedback` serializes it, Task 2: `LoadFeedback` restores `RecentRuns` so domain fitness scores real history after restart, Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: internal/domains/trees.go, internal/domains/domains_test.go)

### Community 2295 - "Community 2295"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/rate_l…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2296 - "Community 2296"
Cohesion: 0.5
Nodes (3): Task 1: Dashboard route-coverage — agent-lifecycle Route definitions + coverage guard, Task 2: Dashboard route-coverage — task/workflow/blackboard Route definitions, Task 3: Dashboard route-coverage — pipeline Route definitions + consolidated zero-uncovered guard

### Community 2297 - "Community 2297"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Close the knowledge-graph analytics→action loop — make ComputeAnalytics signa…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2298 - "Community 2298"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for cmd/bt-agent/hitl_tool…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2299 - "Community 2299"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2300 - "Community 2300"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/parall…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2301 - "Community 2301"
Cohesion: 0.5
Nodes (3): Task 1: Coverage backlog milestone 1/3 — characterize internal/blocks/dlq.go, Task 2: Coverage backlog milestone 2/3 — characterize internal/blocks/evolve.go, Task 3: Coverage backlog milestone 3/3 — characterize internal/blocks/expand.go

### Community 2302 - "Community 2302"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/engine/superp…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2303 - "Community 2303"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/evolution/met…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2304 - "Community 2304"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2305 - "Community 2305"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "RotateKey silently caps the replacement API key at 24 hours" milestone 1/1: In in…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2306 - "Community 2306"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2307 - "Community 2307"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agentexec/pip…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2308 - "Community 2308"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Q3 Reliability — Harden the LLM text-generation call path with retry-with-full-…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2309 - "Community 2309"
Cohesion: 0.5
Nodes (3): Task 1: Extract a pure quality-blended signal helper, Task 2: Wire the blended signal into the RecordRun fitness EMA, Task 3: Guard clamping and zero-quality invariants end-to-end through RecordRun

### Community 2310 - "Community 2310"
Cohesion: 0.5
Nodes (3): Task 1: Add cancellable, bounded file-lock acquisition in reliability, Task 2: Decouple blackboard in-memory serialization from cross-process flock wait, Task 3: Make domain-tree smoke-test, description, and condition coverage contract explicit and registry-driven

### Community 2311 - "Community 2311"
Cohesion: 0.5
Nodes (3): Task 1: Add durable feedback persistence substrate (SaveFeedback/LoadFeedback), Task 2: Debounce the flush (dirty flag + throttled write + Flush/shutdown), Task 3: Ensure all domain trees have smoke tests, descriptions, and condition coverage

### Community 2312 - "Community 2312"
Cohesion: 0.5
Nodes (3): Superpowers Implementation Plan, Task 1: Program "Deterministic coverage backlog: characterization tests for internal/agent/langage…, Task 2: Ensure all domain trees have smoke tests, descriptions, and condition coverage (files: int…

### Community 2313 - "Community 2313"
Cohesion: 0.5
Nodes (3): Task 1: Export agent.OutcomeErrorDetail with unit coverage for tail-distillation behavior, Task 2: Route the scheduler retry closure's no-runErr failure through OutcomeErrorDetail, Task 3: Consolidated registry-coverage guard for domain-tree smoke, description, and condition invariants

### Community 2314 - "Community 2314"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2315 - "Community 2315"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2316 - "Community 2316"
Cohesion: 0.5
Nodes (3): code:block1 (# github.com/nico/go-bt-evolve/internal/a2a [github.com/nico), GREEN phase, RED phase

### Community 2318 - "Community 2318"
Cohesion: 0.5
Nodes (3): code:block1 (tools_test.go:3151: bt_evolve_expert must warm-start from th), GREEN phase, RED phase

### Community 2319 - "Community 2319"
Cohesion: 0.5
Nodes (3): code:block1 (internal/knowledge/feedback_test.go:289:11: undefined: blend), GREEN phase, RED phase

### Community 2322 - "Community 2322"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2323 - "Community 2323"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestRegisterMCPToolsCommentMatchesActualToolCount ), GREEN phase, RED phase

### Community 2324 - "Community 2324"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.069s), GREEN phase, RED phase

### Community 2327 - "Community 2327"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.099s), GREEN phase, RED phase

### Community 2328 - "Community 2328"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestRecoverGoapFusionPendingPatches_WarnsWhenAband), GREEN phase, RED phase

### Community 2329 - "Community 2329"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.795s), GREEN phase, RED phase

### Community 2330 - "Community 2330"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/engine	61.971s), GREEN phase, RED phase

### Community 2331 - "Community 2331"
Cohesion: 0.5
Nodes (3): code:block1 (error_handler_node_test.go:565: consecutive escalate_failed ), GREEN phase, RED phase

### Community 2333 - "Community 2333"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestDeadLetterQueue_SaveMergesSiblingRequeueStamps), GREEN phase, RED phase

### Community 2334 - "Community 2334"
Cohesion: 0.5
Nodes (3): code:block1 (# github.com/nico/go-bt-evolve/internal/dashboard [github.co), GREEN phase, RED phase

### Community 2335 - "Community 2335"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestEvolveTreeV2_CrisisIntervention_UsesCalibrated), GREEN phase, RED phase

### Community 2336 - "Community 2336"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.029s), GREEN phase, RED phase

### Community 2341 - "Community 2341"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.205s), GREEN phase, RED phase

### Community 2342 - "Community 2342"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestConvertToAgentCard_AttachesSignature), GREEN phase, RED phase

### Community 2343 - "Community 2343"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2345 - "Community 2345"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestSanitizeUserID_AdversarialIDs (0.00s)), GREEN phase, RED phase

### Community 2346 - "Community 2346"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine ./cmd/bt-agent -), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/engine	53.965s), Summary

### Community 2347 - "Community 2347"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (structure cycle must leave last-reviewed SHA unchanged: got ), RED phase result

### Community 2349 - "Community 2349"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.260s), GREEN phase, RED phase

### Community 2350 - "Community 2350"
Cohesion: 0.5
Nodes (3): code:block1 (internal/dashboard/metrics_test.go:112:13: undefined: TreeSn), GREEN phase, RED phase

### Community 2351 - "Community 2351"
Cohesion: 0.5
Nodes (3): code:block1 (internal/gardener/evolve_v2_test.go:2846:3: unknown field Is), GREEN phase, RED phase

### Community 2352 - "Community 2352"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2354 - "Community 2354"
Cohesion: 0.5
Nodes (3): code:block1 (internal/gardener/evolve_v2_test.go:1110:4: unknown field DT), GREEN phase, RED phase

### Community 2355 - "Community 2355"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/blocks ./int), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/blocks	0.042s), Summary

### Community 2356 - "Community 2356"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener ./c), code:block2 (cmd/bt-gardener/main_test.go:81:16: undefined: wireSelectorO), Summary

### Community 2358 - "Community 2358"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./cmd/), code:block2 (ok  github.com/nico/go-bt-evolve/internal/agent   3.557s), Summary

### Community 2360 - "Community 2360"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2362 - "Community 2362"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2363 - "Community 2363"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/evolution ./), GREEN phase, RED phase

### Community 2366 - "Community 2366"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2367 - "Community 2367"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	0.233s), GREEN phase, RED phase

### Community 2368 - "Community 2368"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2369 - "Community 2369"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestPersistGoapProgram_ConcurrentCallersAllSurvive), GREEN phase, RED phase

### Community 2370 - "Community 2370"
Cohesion: 0.5
Nodes (3): code:block1 (mid-batch degrade must still unwrap committed snapshots to b), GREEN phase, RED phase

### Community 2371 - "Community 2371"
Cohesion: 0.5
Nodes (3): code:block1 (learning_test.go:569: CrisisReasons = [], want to contain di), GREEN phase, RED phase

### Community 2372 - "Community 2372"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./cmd/benchcmp ./cmd/bt-dashboard ), code:block2 (ok  github.com/nico/go-bt-evolve/cmd/benchcmp        0.014s), Summary

### Community 2373 - "Community 2373"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2374 - "Community 2374"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./cmd/bt-gardener ./internal/agent), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-gardener	0.520s), Summary

### Community 2375 - "Community 2375"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.125s), GREEN phase, RED phase

### Community 2378 - "Community 2378"
Cohesion: 0.5
Nodes (3): code:block1 (total_cycles = 0, want 3), GREEN phase, RED phase

### Community 2379 - "Community 2379"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine -run "TestValida), GREEN phase, RED phase

### Community 2380 - "Community 2380"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.025s), GREEN phase, RED phase

### Community 2382 - "Community 2382"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/reliability	10.99), GREEN phase, RED phase

### Community 2384 - "Community 2384"
Cohesion: 0.5
Nodes (3): code:block1 (internal/gardener/user_trees_test.go:93:9: e.TreeID undefine), GREEN phase, RED phase

### Community 2385 - "Community 2385"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent -short -), GREEN phase, RED phase

### Community 2386 - "Community 2386"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2388 - "Community 2388"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), GREEN phase, RED phase

### Community 2389 - "Community 2389"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2390 - "Community 2390"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.132s), GREEN phase, RED phase

### Community 2391 - "Community 2391"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine ./internal/a2a .), GREEN phase, RED phase

### Community 2392 - "Community 2392"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2393 - "Community 2393"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./cmd/bt-agent ./intern), code:block2 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.442s), Summary

### Community 2394 - "Community 2394"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2395 - "Community 2395"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/gardener -sh), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.108s), Summary

### Community 2397 - "Community 2397"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestIsToolMatch_EmptyPath), GREEN phase, RED phase

### Community 2398 - "Community 2398"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/evolution -s), GREEN phase, RED phase

### Community 2400 - "Community 2400"
Cohesion: 0.5
Nodes (3): code:block1 (cmd/bt-agent/wiring_test.go:49:9: undefined: feedbackSnapsho), GREEN phase, RED phase

### Community 2401 - "Community 2401"
Cohesion: 0.5
Nodes (3): code:block1 (FILES_CHANGED:), GREEN phase, RED phase

### Community 2403 - "Community 2403"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/a2a	0.466s), GREEN phase, RED phase

### Community 2406 - "Community 2406"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/fusion	0.159s), GREEN phase, RED phase

### Community 2407 - "Community 2407"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/util	0.015s), GREEN phase, RED phase

### Community 2408 - "Community 2408"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.223s), GREEN phase, RED phase

### Community 2409 - "Community 2409"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2410 - "Community 2410"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/evolution -s), GREEN phase, RED phase

### Community 2411 - "Community 2411"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.134s), GREEN phase, RED phase

### Community 2413 - "Community 2413"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	4.357s), GREEN phase, RED phase

### Community 2414 - "Community 2414"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestScheduler_PersistsFeedbackOnRunAndStop (0.01s)), GREEN phase, RED phase

### Community 2415 - "Community 2415"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH gofmt -l internal/engine/action), GREEN phase, RED phase

### Community 2416 - "Community 2416"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.384s), GREEN phase, RED phase

### Community 2417 - "Community 2417"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/domains	5.093s), GREEN phase, RED phase

### Community 2418 - "Community 2418"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2419 - "Community 2419"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/a2a ./internal/agentexe), code:block2 (--- FAIL: TestDaemonConfiguresAuctionDelegateHook (0.00s)), RED phase complete

### Community 2420 - "Community 2420"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestGenerateWithRetry_IncrementsBlackboardTokensUs), GREEN phase, RED phase

### Community 2421 - "Community 2421"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestBTEvolveIslandRegisteredAndReturnsIslandMetric), GREEN phase, RED phase

### Community 2422 - "Community 2422"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2423 - "Community 2423"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/benchcmp	0.014s), GREEN phase, RED phase

### Community 2424 - "Community 2424"
Cohesion: 0.5
Nodes (3): code:block1 (internal/a2a/auction_test.go:264:18: auc.RunAuction undefine), GREEN phase, RED phase

### Community 2425 - "Community 2425"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/benchmark -run TestRunS), GREEN phase, RED phase

### Community 2426 - "Community 2426"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/benchmark -run TestScor), code:block2 (--- PASS: TestScoreMutation_PruneRoutingCondition_BreaksPath), Summary

### Community 2427 - "Community 2427"
Cohesion: 0.5
Nodes (3): code:block1 (expected status 'failed' after synthesis phase error, got "c), GREEN phase, RED phase

### Community 2428 - "Community 2428"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./cmd/bt-dashboard -short -count=1), GREEN phase, RED phase

### Community 2429 - "Community 2429"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/research	0.052s), GREEN phase, RED phase

### Community 2430 - "Community 2430"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2431 - "Community 2431"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/engine -short -count=1 ), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/engine	45.347s), Summary

### Community 2432 - "Community 2432"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2433 - "Community 2433"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2435 - "Community 2435"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/gardener	0.126s), GREEN phase, RED phase

### Community 2437 - "Community 2437"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2438 - "Community 2438"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/engine -shor), GREEN phase, RED phase

### Community 2439 - "Community 2439"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestRunSprint_DoesNotHoldStateLockAcrossTreeExecut), GREEN phase, RED phase

### Community 2441 - "Community 2441"
Cohesion: 0.5
Nodes (3): code:block1 (bt_evolve_multiobjective must recommend more than the hardco), GREEN phase, RED phase

### Community 2442 - "Community 2442"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2443 - "Community 2443"
Cohesion: 0.5
Nodes (3): code:block1 (PATH=/usr/local/go/bin:$PATH go test ./internal/agent ./inte), code:block2 (ok  	github.com/nico/go-bt-evolve/internal/agent	3.785s), Summary

### Community 2444 - "Community 2444"
Cohesion: 0.5
Nodes (3): code:block1 (internal/engine/superpowers_worktree_sweep_test.go:186:87: t), GREEN phase, RED phase

### Community 2445 - "Community 2445"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/engine	66.987s), GREEN phase, RED phase

### Community 2447 - "Community 2447"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/evolution	3.909s), GREEN phase, RED phase

### Community 2448 - "Community 2448"
Cohesion: 0.5
Nodes (3): code:block1 (internal/gardener/evolve_v2_test.go:811:67: too many argumen), GREEN phase, RED phase

### Community 2449 - "Community 2449"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestWithFusionKnowledgeStubsLiveNotebookLMResearch), GREEN phase, RED phase

### Community 2450 - "Community 2450"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	0.135s), GREEN phase, RED phase

### Community 2451 - "Community 2451"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/dashboard	0.882s), GREEN phase, RED phase

### Community 2452 - "Community 2452"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.211s), GREEN phase, RED phase

### Community 2453 - "Community 2453"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2454 - "Community 2454"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2455 - "Community 2455"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestQuery_SortedByFitnessDescThenIDAsc (0.00s)), GREEN phase, RED phase

### Community 2458 - "Community 2458"
Cohesion: 0.5
Nodes (3): code:block1 (cmd/bt-dashboard/main_test.go:559:2: undefined: currentWorkf), GREEN phase, RED phase

### Community 2459 - "Community 2459"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Result

### Community 2460 - "Community 2460"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestSaveSLOMetrics_ConcurrentCallsDoNotRaceOrCorru), GREEN phase, RED phase

### Community 2461 - "Community 2461"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./internal/knowledge -short -count), GREEN phase, RED phase

### Community 2462 - "Community 2462"
Cohesion: 0.5
Nodes (3): code:block1 (internal/knowledge/feedback_persist_test.go:42:16: src.SaveF), GREEN phase, RED phase

### Community 2463 - "Community 2463"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestKnowledgeGraphRegistersResolverSpecialCaseTree), GREEN phase, RED phase

### Community 2464 - "Community 2464"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestPrometheusHandler_DoesNotHoldAgentLockAcrossFu), GREEN phase, RED phase

### Community 2465 - "Community 2465"
Cohesion: 0.5
Nodes (3): code:block1 (/usr/local/go/bin/go test ./cmd/bt-agent -short -count=1 -ti), code:block2 (--- FAIL: TestEvolveToolsSurfacePopulationHealthSnapshot (0.), RED phase result

### Community 2466 - "Community 2466"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.209s), GREEN phase, RED phase

### Community 2467 - "Community 2467"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestHandleAgentExecute_SetsQualityScoreFromRunResu), GREEN phase, RED phase

### Community 2468 - "Community 2468"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-dashboard	0.185s), GREEN phase, RED phase

### Community 2469 - "Community 2469"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/cicd	0.024s), GREEN phase, RED phase

### Community 2470 - "Community 2470"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestGoapGoalFailureNote_PreservesActionableEnding ), GREEN phase, RED phase

### Community 2471 - "Community 2471"
Cohesion: 0.5
Nodes (3): code:block1 (internal/domains/trees_test.go:109:31: undefined: benchmark.), GREEN phase, RED phase

### Community 2472 - "Community 2472"
Cohesion: 0.5
Nodes (3): code:block1 (internal/evolution/learning_test.go:775:81: not enough argum), GREEN phase, RED phase

### Community 2473 - "Community 2473"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/cmd/bt-agent	1.279s), GREEN phase, RED phase

### Community 2474 - "Community 2474"
Cohesion: 0.5
Nodes (3): GREEN phase, RED phase, Summary

### Community 2475 - "Community 2475"
Cohesion: 0.5
Nodes (3): code:block1 (--- FAIL: TestBuildDashboardKnowledgeGraph_SetsExpectedDomai), GREEN phase, RED phase

### Community 2476 - "Community 2476"
Cohesion: 0.5
Nodes (3): code:block1 (ok  	github.com/nico/go-bt-evolve/internal/knowledge	19.624s), GREEN phase, RED phase

### Community 2492 - "Community 2492"
Cohesion: 0.67
Nodes (3): History (`${AGENT_HOME}/history/<agent>.jsonl`), Memory (`${AGENT_HOME}/memory/<agent>/memory.json`), Memory and history

### Community 2493 - "Community 2493"
Cohesion: 0.67
Nodes (3): Agent YAML schema, code:yaml (name: code-reviewer), Fields that matter today

### Community 2494 - "Community 2494"
Cohesion: 0.67
Nodes (3): Agent home directory, code:block2 (${AGENT_HOME}/), Layout

### Community 2495 - "Community 2495"
Cohesion: 0.67
Nodes (3): code:go (package engine), code:bash (/usr/local/go/bin/go test ./internal/engine -count=1 -run 'T), Task 2: Add typed production runtime model

### Community 2496 - "Community 2496"
Cohesion: 0.67
Nodes (3): code:bash (/usr/local/go/bin/gofmt -w <changed-go-files>), code:bash (strings bin/bt-agent-cli | grep 'SuperpowersPipeline_Main'), Task 10: Production verification runner

### Community 2497 - "Community 2497"
Cohesion: 0.67
Nodes (3): code:go (type SuperpowersMode string), code:bash (/tmp/bt-agent-cli run superpowers-prod-smoke --input "dry_ru), Task 14: End-to-end dry-run mode

### Community 2498 - "Community 2498"
Cohesion: 0.67
Nodes (3): code:bash (/home/nico/.local/bin/claude --print --dangerously-skip-perm), code:go (type CommandRunner interface {), Task 4: Extract a reusable command runner and Claude runner

### Community 2499 - "Community 2499"
Cohesion: 0.67
Nodes (3): code:go (const superpowersRunsDir = "/home/nico/go-bt-evolve/docs/sup), code:go (func TestWriteArtifactOnce_Idempotent(t *testing.T) {), Task 3: Add artifact store and idempotent phase writes

### Community 2500 - "Community 2500"
Cohesion: 0.67
Nodes (3): code:bash (systemctl --user stop bt-agent), code:bash (python3 - <<'PY'), Rollback Plan

### Community 2501 - "Community 2501"
Cohesion: 0.67
Nodes (3): code:go (func planSuperpowersWorktree(run *SuperpowersRun) (path stri), code:bash (/usr/local/go/bin/go test ./internal/domains ./internal/engi), Task 5: Production worktree manager

### Community 2502 - "Community 2502"
Cohesion: 0.67
Nodes (3): code:text (ReadVaultResearch → ReadGraphifyReport → AnalyzeImprovementG), code:text (WriteSuperpowersImplementationPlanFromGoapContext), Task 13: Integrate GOAP fusion with production Superpowers runtime

### Community 2722 - "Community 2722"
Cohesion: 0.67
Nodes (3): code:bash (# ----- 5. arc42 section files validation -----), code:bash (bash scripts/check-doc-drift.sh; echo "exit=$?"), Task 12: Drift script — replace check #5 with the arc42 block

### Community 2723 - "Community 2723"
Cohesion: 0.67
Nodes (3): code:bash (cd <repo-root>), code:bash (git status --short          # expect: empty), Task 1: Branch setup

### Community 2724 - "Community 2724"
Cohesion: 0.67
Nodes (3): code:markdown (# arc42 Section Guidelines), code:bash (grep -c '^## Section [0-9]' docs/arc42/GUIDELINES.md   # exp), Task 8: Create `docs/arc42/GUIDELINES.md`

### Community 2725 - "Community 2725"
Cohesion: 0.67
Nodes (3): code:bash (PATH=/usr/local/go/bin:$PATH make check-quick), code:bash (git add -A), Task 13: Gate + Commit 1

### Community 2726 - "Community 2726"
Cohesion: 0.67
Nodes (3): code:bash (grep -c 'RegisterAction(' internal/engine/*.go | awk -F: '{s), code:bash (grep -c '^| Q[0-9] | \*\*' docs/arc42/01-introduction-goals.), Task 2: Merge §1 (Introduction and Goals)

### Community 2727 - "Community 2727"
Cohesion: 0.67
Nodes (3): code:bash (git rm docs/arc42/go-bt-evolve-arc42.md docs/arc42/09-test-s), code:bash (grep -rn 'docs/adr' README.md docs/*.md | grep -v 'docs/plan), Task 9: Deletions + cross-doc reference updates

### Community 2728 - "Community 2728"
Cohesion: 0.67
Nodes (3): code:bash (for d in $(find internal -maxdepth 1 -type d ! -name interna), code:bash (grep -F '## 5.0 Composable Blocks' docs/arc42/05-building-bl), Task 4: Merge §5 (Building Block View) — biggest restructure

### Community 2871 - "Community 2871"
Cohesion: 0.67
Nodes (3): code:go (// internal/domains/superpowers_workflow_test.go), code:bash (git add internal/domains/superpowers_workflow.go internal/do), Task 10: `domain:superpowers_workflow` tree

## Knowledge Gaps
- **8500 isolated node(s):** `{ defineConfig }`, `{ chromium }`, `maxTokensCapable`, `fallbackSpec`, `DeepSeekConfig` (+8495 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **1223 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `main()` connect `Community 28` to `Community 1`, `Community 132`, `Community 260`, `Community 7`, `Community 8`, `Community 392`, `Community 521`, `Community 264`, `Community 10`, `Community 523`, `Community 21`, `Community 31`, `Community 36`, `Community 37`, `Community 39`, `Community 40`, `Community 171`, `Community 173`, `Community 174`, `Community 50`, `Community 53`, `Community 182`, `Community 58`, `Community 64`, `Community 198`, `Community 331`, `Community 80`, `Community 482`, `Community 226`, `Community 104`, `Community 110`, `Community 114`, `Community 119`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `registerMCPTools()` connect `Community 8` to `Community 1`, `Community 6`, `Community 144`, `Community 24`, `Community 26`, `Community 28`, `Community 289`, `Community 34`, `Community 163`, `Community 36`, `Community 165`, `Community 37`, `Community 35`, `Community 40`, `Community 43`, `Community 50`, `Community 189`, `Community 61`, `Community 64`, `Community 193`, `Community 71`, `Community 331`, `Community 78`, `Community 207`, `Community 80`, `Community 211`, `Community 84`, `Community 2136`, `Community 93`, `Community 99`, `Community 106`, `Community 116`, `Community 122`, `Community 125`, `Community 126`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `DefaultTree()` connect `Community 1` to `Community 6`, `Community 136`, `Community 8`, `Community 12`, `Community 145`, `Community 26`, `Community 28`, `Community 415`, `Community 160`, `Community 287`, `Community 34`, `Community 53`, `Community 61`, `Community 66`, `Community 67`, `Community 68`, `Community 71`, `Community 584`, `Community 2142`, `Community 95`, `Community 229`, `Community 108`, `Community 116`, `Community 244`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Are the 156 inferred relationships involving `NewKnowledgeGraph()` (e.g. with `TestDiscoverRelated_EmptyGraph()` and `TestDiscoverRelated_ConnectedTo()`) actually correct?**
  _`NewKnowledgeGraph()` has 156 INFERRED edges - model-reasoned connections that need verification._
- **Are the 152 inferred relationships involving `DefaultTree()` (e.g. with `.loadAll()` and `TestRegistry_PersistedTreeOverridesBuiltin()`) actually correct?**
  _`DefaultTree()` has 152 INFERRED edges - model-reasoned connections that need verification._
- **Are the 151 inferred relationships involving `NewRequest()` (e.g. with `TestWaitForRequest_Approve()` and `TestWaitForRequest_Escalate()`) actually correct?**
  _`NewRequest()` has 151 INFERRED edges - model-reasoned connections that need verification._
- **Are the 113 inferred relationships involving `registerMCPTools()` (e.g. with `TestBTEvolveGeneticRoutesThroughExperienceBank()` and `TestBTEvolveQDRegisteredAndReturnsQDMetrics()`) actually correct?**
  _`registerMCPTools()` has 113 INFERRED edges - model-reasoned connections that need verification._