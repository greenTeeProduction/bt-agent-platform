...
- **Are the 79 inferred relationships involving `newDefaultConfig()` (e.g. with `TestSaveFile_RoundTrip()` and `TestValidate_NegativeRateLimit()`) actually correct?**
  _`newDefaultConfig()` has 79 INFERRED edges - model-reasoned connections that need verification._