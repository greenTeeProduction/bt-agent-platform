...
func saveRecordWithDelay(t *testing.T, store *evolution.Store, r *evolution.Record) {
	t.Helper()
	saveRecordCounter++
	if r.TaskID == "" {
...
func TestReflectTool_Call_WithLLM(t *testing.T) {
	bb := &engine.Blackboard{
		LLM:     &engine.MockLLM{WentWellResp: "good planning", ToImproveResp: "better execution"},
		Task:    "test task",
		Outcome: "success",
		Plan:    "step by step",
	}
...
func TestFitnessTool_Call(t *testing.T) {
	...
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "task1", Outcome: evolution.Success})
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "task2", Outcome: evolution.Failure})
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "task3", Outcome: evolution.Failure})
...
func TestEvolveTool_Call_NotEnoughFailures(t *testing.T) {
	...
	// Only 1 failure
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "t", Outcome: evolution.Failure})
...
func TestEvolveTool_Call_Success(t *testing.T) {
	...
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "t1", Outcome: evolution.Failure})
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "t2", Outcome: evolution.Failure})
	saveRecordWithDelay(t, refStore, &evolution.Record{Task: "t3", Outcome: evolution.Failure})
...
func TestToolNames_And_Descriptions(t *testing.T) {
	bb := &engine.Blackboard{LLM: engine.NewMockLLM()}
	tools := []tools.Tool{
...
func TestMockLLM_SatisfiesInterface(t *testing.T) {
	var m llm.LLM = &engine.MockLLM{GenerateResp: "mock response", ComplexityResp: "medium", PlanResp: "1. Analyze\n2. Execute\n3. Verify", WentWellResp: "good planning", ToImproveResp: "better execution"}
