...
func FellowResearchTree(fellow Fellow, topic string) *evolution.SerializableNode {
	...
	return &evolution.SerializableNode{
		...
		Children: []evolution.SerializableNode{
			...
			{
				Type: "ChainAction",
				Name: fmt.Sprintf("agent:You are %s (%s analyst). Research the topic '%s' from your unique perspective: %s. Use your expertise in %s to identify key insights, supporting data, and critical factors. Produce a structured analysis with clearly labeled sections: KEY INSIGHTS, EVIDENCE, ASSUMPTIONS, and RECOMMENDATION.",
			...
			{
				Type: "ChainAction",
				Name: fmt.Sprintf("agent:You are %s. Review your own analysis on '%s'. Identify your cognitive biases, unchallenged assumptions, and analytical blind spots. Be brutally honest about where your perspective might be distorting your conclusions. List specific biases and how they may have influenced your findings.", fellow.Name, topic),
				Metadata: map[string]any{
					"system_msg": sysMsg,
...
func DebateTree(fellows []Fellow, topic string) *evolution.SerializableNode {
	...
	for i, f := range fellows {
		...
		rebuttalChildren[i] = evolution.SerializableNode{
			Type: "ChainAction",
			Name: fmt.Sprintf("agent:You are %s. You have been challenged on your position regarding '%s'. Defend your thesis. Address the criticisms directly: which points do you concede, and which do you refute with stronger evidence? Refine your argument based on what you've learned. Acknowledge valid counterpoints while strengthening your core thesis.", f.Name, topic),
			Metadata: map[string]any{
				"system_msg": sys,
				"max_tokens": float64(2048),
			},
	...
	children = append(children, evolution.SerializableNode{
		Type: "ChainAction",
		Name: fmt.Sprintf("agent:You are the debate moderator. After hearing opening statements, cross-examination, and rebuttals from %d fellows on '%s', identify areas of common ground. What do all (or most) perspectives agree on? Where are the irreducible disagreements? Map the landscape of consensus and conflict. List: POINTS OF AGREEMENT, POINTS OF DISAGREEMENT, UNRESOLVED TENSIONS.", len(fellows), topic),
		Metadata: map[string]any{
...
func SynthesisTree() *evolution.SerializableNode {
	return &evolution.SerializableNode{
		...
		Children: []evolution.SerializableNode{
			...
			{
				Type: "ChainAction",
				Name: "agent:Based on the synthesis, produce a clear, actionable RECOMMENDATION. This should be a specific course of action (or set of options) that follows from the analysis. Include: (1) Primary recommendation with rationale, (2) Alternative options with trade-offs, (3) Confidence level (low/medium/high) with justification, (4) Key conditions or triggers that would change the recommendation. Output as: RECOMMENDATION: ... RATIONALE: ... ALTERNATIVES: ... CONFIDENCE: ... CONDITIONAL TRIGGERS: ...",
				Metadata: map[string]any{
...
func PeerReviewTree() *evolution.SerializableNode {
	...
	for i, r := range reviewRoles {
		reviewChildren[i] = evolution.SerializableNode{
			Type: "ChainAction",
			Name: fmt.Sprintf("agent:%s: %s", r.Name, r.Focus),
			Metadata: map[string]any{
				"system_msg": r.System,
				"max_tokens": float64(2048),
			},
...
func ReportGenerationTree() *evolution.SerializableNode {
	return &evolution.SerializableNode{
		...
		Children: []evolution.SerializableNode{
			...
			{
				Type: "ChainAction",
				Name: "agent:Write the BACKGROUND section of the report. Explain the context: why this topic matters, what prompted this analysis, the scope of investigation, the analytical approach used (multi-perspective think tank with dialectical debate), and any key definitions or frameworks the reader needs to understand the analysis. Be thorough but not tedious.",
				Metadata: map[string]any{
