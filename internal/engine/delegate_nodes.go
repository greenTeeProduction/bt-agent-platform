...
func registerDelegateNodes() {
	RegisterCondition("HasDelegateTarget", func(b *Blackboard) bool {
	...
	RegisterAction("DelegateToTree", func(ctx *btcore.BTContext[Blackboard]) int {
		b := ctx.Blackboard
		if b.ChainState == nil {
		...
		out, err := DelegateToTreeFn(treeID, b)
		if err != nil {
	...
	RegisterAction("LoadAgentMemory", func(ctx *btcore.BTContext[Blackboard]) int {
		b := ctx.Blackboard
		if b.ChainState == nil {
			b.ChainState = make(map[string]any)
		}
		agentName := agentNameFromBB(b)
		if agentName == "" || AgentMemoryBaseDir == "" {
			b.ChainState["agent_memory"] = ""
			return 1
		}
		ms, err := agent.NewMemoryStore(AgentMemoryBaseDir+"/.go-bt-evolve/memory", agentName, 200)
		if err != nil {
	...
	RegisterAction("WriteAgentMemory", func(ctx *btcore.BTContext[Blackboard]) int {
		b := ctx.Blackboard
		agentName := agentNameFromBB(b)
		if agentName == "" || AgentMemoryBaseDir == "" {
			return 1
	...
	RegisterAction("MergeResults", func(ctx *btcore.BTContext[Blackboard]) int {
		b := ctx.Blackboard
		if len(b.Results) == 0 {
			if b.Result != "" {
				return 1
	...
	RegisterAction("PrepareA2AHandoff", func(ctx *btcore.BTContext[Blackboard]) int {
		b := ctx.Blackboard
		if b.ChainState == nil {
			b.ChainState = make(map[string]any)
		}
...
func agentNameFromBB(b *Blackboard) string {
	if b == nil || b.ChainState == nil {
		return ""
	}
