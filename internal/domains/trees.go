...
func AgentMonitorTree() *evolution.SerializableNode {
	return &evolution.SerializableNode{Type: "Sequence", Name: "AgentMonitor_Main", Children: []evolution.SerializableNode{
		...
		sel("StrategyRouter",
			...
			seq("RestartPath",
				cond("HasDeadAgents", "Dead agents detected"),
				act("RestartDeadAgents", "Kill PID, restart binary"),
				act("VerifyRestart", "Re-check health after restart"),
			),
			seq("AlertPath",
				cond("PersistentFailures", "Agent failed restart 3+ times"),
				act("SendAlert", "Log alert to stderr, metrics, notification"),
				act("EscalateToOperator", "Flag for human intervention"),
			),
		...
		act("ReflectOnOutcome", "Reflect on monitoring quality"),
		outcome(),
		act("UpdateBehaviorTree", "Evolve"),
	}}
}
