		...
		sel("StrategyRouter",
			...
			seq("ExecutionPath",
				chainAgent("RefactoringAgent",
					"You are a refactoring agent. {{.Task}} Use file_read to inspect code, shell_exec to run analysis tools, code_exec to test changes. Report refactoring suggestions with specific code changes.",
					[]string{"file_read", "shell_exec", "code_exec"}),
			),
		),
		...
		sel("StrategyRouter",
			...
			seq("ExecutionPath",
				chainAgent("DataPipelineAgent",
					"You are a data pipeline agent. {{.Task}} Process data, run ETL tasks, handle queue operations, index sessions, extract memories. Use file_ops for reading/writing, code_exec for running scripts, web_search for documentation.",
					[]string{"file_ops", "code_exec", "web_search"}),
			),
		),
		...
		sel("StrategyRouter",
			...
			seq("ExecutionPath",
				chainAgent("MeetingNotesAgent",
					"You are a meeting notes agent. {{.Task}} Transcribe meetings, extract action items, summarize discussions. Use file_ops to read transcripts, code_exec for processing, web_search for context.",
					[]string{"file_ops", "code_exec", "web_search"}),
			),
		),
		...
		sel("StrategyRouter",
			...
			seq("ExecutionPath",
				chainAgent("GameAIAgent",
					"You are a game AI agent. {{.Task}} Implement game AI behaviors, patrol/combat/flee logic. Use code_exec for testing behaviors, web_search for AI patterns.",
					[]string{"code_exec", "web_search"}),
			),
		),
		...
		sel("StrategyRouter",
			...
			seq("ExecutionPath",
				chainAgent("TradingSignalAgent",
					"You are a trading signal agent. {{.Task}} Analyze market data, generate signals, assess risk. Use web_search for market data, calculator for analysis, file_ops for reports.",
					[]string{"web_search", "calculator", "file_ops"}),
			),
		),
...
func GoapPlanningTree() *evolution.SerializableNode {
	return &evolution.SerializableNode{Type: "Sequence", Name: "GoapPlanning_Main", Children: []evolution.SerializableNode{
		act("SetupUniversalTools", "Give chain agents access to web_search, file_ops, code_exec"),
		...
		sel("StrategyRouter",
			seq("AssessPath",
				...
				chainAgent("PlanningAssessAgent",
					"You are a planning assessment agent. Assess the current state: scan files, check configurations, review logs. Use web_search for external research, file_ops to read local files, code_exec to run diagnostic commands. Produce a structured assessment report with findings and recommendations.",
					[]string{"web_search", "file_ops", "code_exec"}),
			),
			seq("SyncPath",
				cond("IsSyncRequest", "Detect sync/pollinate/cross/align keywords"),
				chainAgent("PlanningSyncAgent",
					"You are a synchronization agent. Compare two systems (skills vs trees, configs vs reality, vault vs platform). Use file_ops to read files, web_search for reference, code_exec to run diff/comparison commands. Report mismatches with specific file paths and suggested fixes.",
					[]string{"web_search", "file_ops", "code_exec"}),
			),
			seq("ExecutionPath",
				chainAgent("PlanningAgent",
					"You are a planning agent. {{.Task}} Think step by step. Use web_search for research, file_ops to read/write files, code_exec to run commands. Produce a complete, actionable output.",
					[]string{"web_search", "file_ops", "code_exec"}),
			),
		),
		...
		sel("StrategyRouter",
			...
			seq("GraphifyPath",
				...
				chainAgent("GraphifyAgent",
					"You are a codebase analysis agent. Run graphify commands to analyze code structure: graphify update . to refresh, graphify query for insights, graphify path A B for relationships. Use file_ops to read GRAPH_REPORT.md and source files. Produce a structural analysis with findings.",
					[]string{"code_exec", "file_ops"}),
			),
			seq("ExecutionPath",
				chainAgent("ResearchAgent",
		...
		sel("StrategyRouter",
			...
			seq("ImplementPath",
				...
				chainAgent("DevopsImplementAgent",
					"You are an implementation agent. Read implementation plans from the vault, use go_build to compile changes, go_test to verify, go_vet to check quality. Use file_ops to read/write code and web_search for reference. Complete the implementation task.",
					[]string{"go_build", "go_test", "go_vet", "file_ops", "web_search"}),
			),
			seq("ExecutionPath",
				chainAgent("DevopsAgent",
					"You are a DevOps agent. {{.Task}} Use go_build, go_test, go_vet for Go development, file_ops for reading/writing, web_search for reference. Complete the task step by step.",
					[]string{"go_build", "go_test", "go_vet", "file_ops", "web_search"}),
			),
		),
