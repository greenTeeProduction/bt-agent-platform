...
func NotebookLMConsumerTree() *evolution.SerializableNode {
	return &evolution.SerializableNode{
		...
		Children: []evolution.SerializableNode{
			...
			{
				Type:        "ChainAction",
				Name:        "agent:Task: {{.Task}}\n\nYou MUST actually execute these commands via shell_exec and file_read. Do NOT describe what you would do — DO it:\n\nSTEP 1 — Run: shell_exec \"ls -lt /mnt/ssd/clawd/wiki/bt-research/syntheses/nlm-research-*.md 2>/dev/null | head -5\"\nSTEP 2 — Run: shell_exec \"ls -lt /mnt/ssd/clawd/wiki/bt-research/plans/ 2>/dev/null | head -5\"\nSTEP 3 — Use file_read to read the newest nlm-research file from step 1 (never this agent's own summary/health reports — only nlm-research-*.md producer output counts as research).\nSTEP 4 — Use shell_exec to count sources: shell_exec \"grep -c 'source_count' /mnt/ssd/clawd/wiki/bt-research/syntheses/nlm-research-*.md 2>/dev/null | tail -5 || echo 0\"\nSTEP 5 — Write a summary via file_write to /mnt/ssd/clawd/wiki/bt-research/nlm-consumer-summary.md with: today's date, newest research file name and its age (research is STALE only if the newest nlm-research file is older than 48 hours), source trends, and whether new plans need creation.\nSTEP 6 — Final answer: a concise health report built ONLY from the real outputs of steps 1-5: newest research file + age, staleness verdict, source count, plan status. No scratch commentary. NEVER fabricate.",
				Description: "Consumer chain agent: read the newest nlm-research synthesis, compute source trends, and write a real summary back to the vault",
