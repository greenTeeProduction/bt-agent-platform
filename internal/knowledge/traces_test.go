...
// =============================================================================
// Dead code removal — RecordSelectorOutcomes bridge (NotebookLM research)
// =============================================================================

// TestRecordSelectorOutcomesBridgeRemoved pins the deletion of the dead
// RecordSelectorOutcomes bridge: arc42 (§Deploy Drift / Selector-ordering)
// notes no scheduler or production caller ever wires a trace-processing loop
// to it, so it — and the two tests above that exist only to exercise it — are
// being deleted rather than left as unreachable code. Scanning the source
// directly (instead of calling the function) keeps this test failing for as
// long as the bridge exists, independent of any caller.
func TestRecordSelectorOutcomesBridgeRemoved(t *testing.T) {
	src, err := os.ReadFile("traces.go")
	if err != nil {
		t.Fatalf("reading traces.go: %v", err)
	}
	if strings.Contains(string(src), "func RecordSelectorOutcomes") {
		t.Fatal("RecordSelectorOutcomes is dead code (no production caller) and must be deleted from traces.go, along with its dedicated tests")
	}
}
