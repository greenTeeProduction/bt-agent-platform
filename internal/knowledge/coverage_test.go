...
func TestListByCategory_All(t *testing.T) {
	...
	counts := map[string]int{
		"core": 5, "finance": 10, "research": 5, "domain": 47,
...
func TestSummary_GlobalGraph(t *testing.T) {
	...
	if !strings.Contains(s, "81 trees") {
		t.Errorf("global summary should show 81 trees, got: %s", s)
	}
...
func TestGlobalGraph(t *testing.T) {
	...
	if len(g.Trees) != 81 {
		t.Errorf("expected 81 trees in GlobalGraph, got %d", len(g.Trees))
	}
}
