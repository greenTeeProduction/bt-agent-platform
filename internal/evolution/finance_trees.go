...
func financeFallbackChainAction() SerializableNode {
	return SerializableNode{
		Type:        "ChainAction",
		Name:        "agent:{{.Task}}",
		Description: "Execute finance-agent fallback path with real tool-capable ChainAction instead of AnalyzeTask/ExecutePlan stubs",
		Metadata: map[string]any{
			"system_msg": "You are a finance-domain BT agent fallback executor. Analyze the requested financial task, use available tools when needed, and return concrete analysis or execution results. Do not return placeholder text.",
			"tools":      []any{"web_search", "file_ops", "code_exec", "calculator"},
			...
			},
			{
				...
				Children: []SerializableNode{
					...
					// Fallback: generic execution
					{
						...
						Children: []SerializableNode{financeFallbackChainAction()},
					},
				},
			},
			{Type: "Action", Name: "ReflectOnOutcome", Description: "Reflect on analysis quality"},
			{
				...
				Children: []SerializableNode{
					{Type: "Condition", Name: "WasSuccessful", Description: "Exit if success"},
					{
						...
						Children: []SerializableNode{{Type: "Action", Name: "SelfCorrect", Description: "Fix and retry"}},
					},
	...
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{Type: "Action", Name: "ReflectOnOutcome", Description: "Reflect"},
			{
				...
				Children: []SerializableNode{
					{Type: "Condition", Name: "WasSuccessful", Description: "Exit if success"},
					{Type: "Retry", Name: "RetrySelfCorrect", MaxRetries: 3, Children: []SerializableNode{{Type: "Action", Name: "SelfCorrect", Description: "Fix"}}},
...
func AllFinanceTrees() map[string]*SerializableNode {
