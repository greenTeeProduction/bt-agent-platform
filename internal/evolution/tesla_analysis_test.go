...
func requireOllamaFinanceTest(t *testing.T) {
	...
	if os.Getenv("RUN_OLLAMA_FINANCE_TESTS") != "1" {
		t.Skip("set RUN_OLLAMA_FINANCE_TESTS=1 to run live Ollama finance integration tests")
	}
}

func TestTeslaFullAnalysis(t *testing.T) {
	requireOllamaFinanceTest(t)
	...
	for _, r := range runs {
		tree, ok := evolution.AllFinanceTrees()[r.Name]
		if !ok {
			continue
		}
		fmt.Fprintf(outFile, "\n═══════════════════════════════════════\n")
...
func TestTeslaEarnings(t *testing.T) {
	requireOllamaFinanceTest(t)
	runTeslaTree(t, "earnings_reviewer", evolution.EarningsReviewerTree())
}
func TestTeslaMarket(t *testing.T) {
	requireOllamaFinanceTest(t)
	runTeslaTree(t, "market_researcher", evolution.MarketResearcherTree())
}
func TestTeslaModel(t *testing.T) {
	requireOllamaFinanceTest(t)
	runTeslaTree(t, "model_builder", evolution.ModelBuilderTree())
}
func TestTeslaValuation(t *testing.T) {
	requireOllamaFinanceTest(t)
	runTeslaTree(t, "valuation_reviewer", evolution.ValuationReviewerTree())
}
