...
func KanbanTaskCreatorTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{
				Type: "ChainAction",
				Name: "agent:Create a new Focalboard task card: 1) Analyze the task description for completeness. 2) Generate an actionable title. 3) Write clear acceptance criteria as checkboxes (- [ ]). 4) Set priority (critical/high/medium/low). 5) Determine the AQAL quadrant (q-i/q-it/q-we/q-its/q-all). 6) Create the card in the BACKLOG column. 7) Report: card ID, title, priority, quadrant.",
			...
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				...
				{Type: "ChainAction", Name: "agent:Card creation failed. Check: is the board accessible? Is the column name correct? Is the card format valid?", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
...
func KanbanRefinerTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				{Type: "Condition", Name: "WasSuccessful"},
				{Type: "ChainAction", Name: "agent:Refinement failed. Possible issues: card not in TODO column, board permissions, invalid card format. Diagnose and retry.", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
...
func KanbanQATree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				{Type: "Condition", Name: "WasSuccessful"},
				{Type: "ChainAction", Name: "agent:QA check failed to execute. Verify board access and card state. Retry with corrected approach.", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
...
func KanbanBoardMonitorTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{
				Type: "Selector", Name: "MonitorRouter", Children: []SerializableNode{
					...
					// Dispatch next ready card
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Find the next dispatchable card: 1) Scan TODO column → dispatch refiner. 2) Scan APPROVED column → dispatch developer. 3) Scan QA column → dispatch QA agent. 4) For each: verify the card meets the column gate before moving. 5) Report: cards dispatched, agent assignments.",
								Metadata: map[string]any{
									"max_tokens": float64(8),
					...
					// Daily standup
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Generate a daily standup summary: 1) Cards completed (moved to DONE). 2) Cards in progress with status. 3) Blocked cards and blockers. 4) Upcoming cards ready for review. 5) Velocity: cards completed this week vs last week. Format as concise standup report.",
								Metadata: map[string]any{
									"max_tokens": float64(8),
...
func KanbanWorkflowTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{
				Type: "Selector", Name: "WorkflowRouter", Children: []SerializableNode{
					// Create new card
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Create a new task card following DoR standards: actionable title, description, acceptance criteria (- [ ] items), priority (critical/high/medium/low), AQAL quadrant (q-i/q-it/q-we/q-its/q-all). Place in BACKLOG column.",
					...
					// Refine card
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Refine a TODO card: expand description, add implementation notes, ensure AC are testable, add architecture constraints. Move TODO→PLANNING→REFINED. Verify DoR gate passes.",
								Metadata: map[string]any{"max_tokens": float64(8)},
							},
						},
					},
					// QA check
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Run QA on a card: check all AC are [x], verify implementation, check for regressions, generate PASS/FAIL report. PASS → move QA→REVIEW. FAIL → move back to IN PROGRESS with issues.",
								Metadata: map[string]any{"max_tokens": float64(8)},
							},
						},
					},
					// Board scan (default)
					{
						...
						Children: []SerializableNode{
							{
								Type: "ChainAction",
								Name: "agent:Scan the full Kanban board: count cards per column, detect stale items, identify bottlenecks, check for cards ready for next phase. Produce a board health report with recommendations.",
								Metadata: map[string]any{"max_tokens": float64(8)},
							},
						},
					},
			...
			{
				Type: "ChainAction",
				Name: "agent:Verify Definition of Done: all checkboxes [x], QA report PASS, description reflects implementation. Flag any DoD violations.",
				Metadata: map[string]any{"max_tokens": float64(5)},
			},
			{Type: "Action", Name: "ReflectOnOutcome"},
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				{Type: "Condition", Name: "WasSuccessful"},
				{Type: "ChainAction", Name: "agent:Kanban operation failed. Verify board is accessible, check column names, validate card format.", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
...
func KanbanAutoPilotTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{
				Type: "ChainAction",
				Name: "agent:Run the Kanban autopilot: 1) Scan TODO → dispatch refiner for unrefined cards. 2) Scan APPROVED → dispatch developer. 3) Scan QA → dispatch QA agent. 4) Scan IN PROGRESS → check for stale cards (>2 days idle). 5) For each card processed, validate the column gate before moving. 6) Report: cards processed, movements, issues found.",
				Metadata: map[string]any{
			...
			{
				Type: "ChainAction",
				Name: "agent:Audit card transitions: verify no card skipped a column, no unauthorized transitions (BACKLOG→TODO, REFINED→APPROVED, REVIEW→DONE require human approval), all cards have proper gates met.",
				Metadata: map[string]any{"max_tokens": float64(6)},
			},
			{Type: "Action", Name: "ReflectOnOutcome"},
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				{Type: "Condition", Name: "WasSuccessful"},
				{Type: "ChainAction", Name: "agent:Autopilot issue. Check board connectivity, agent availability, card state.", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
