		...
		Children: []SerializableNode{
			...
			// Phase 2: Tab Navigation Test
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Navigate the dashboard at http://localhost:9800. Click each sidebar tab: Overview, ThinkTank, Company, Tasks, Trees, MindMap, Evolution. Verify each tab loads content (not empty or error). Check that the active tab is highlighted. Report any tabs that fail to load.",
						Metadata: map[string]any{"max_tokens": float64(8)},
					},
				},
			},
			...
			// Phase 4: Chat Flow Test
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Test the chat feature: 1) Click the 💬 chat button (bottom-right), 2) Verify chat panel opens, 3) Verify agent name matches current tab, 4) Type a message and send, 5) Verify 'Agent is thinking...' appears, 6) Wait up to 60s for response. Report if chat opens, sends messages, and receives responses.",
						Metadata: map[string]any{"max_tokens": float64(8)},
					},
				},
			},
			// Phase 5: MindMap Test
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Test the mind map: 1) Navigate to MindMap tab, 2) Verify SVG renders (check for <path> and <rect> elements), 3) Switch tree selector to 'Stockfish Evolution', 4) Verify tree re-renders, 5) Click zoom buttons, 6) Hover a node and verify detail popup appears. Report any rendering issues.",
						Metadata: map[string]any{"max_tokens": float64(8)},
					},
				},
			},
			// Phase 6: Mobile Layout
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Test mobile responsive layout: 1) Resize browser to 375x812 (iPhone), 2) Verify sidebar is hidden, 3) Verify main content fills screen, 4) Verify chat panel uses full width, 5) Verify grid switches to 2-column, 6) Click tasks tab, verify tasks are readable. Report layout issues on mobile.",
						Metadata: map[string]any{"max_tokens": float64(8)},
					},
				},
			},
				...
				Children: []SerializableNode{
					...
					// Fix layout issues
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Review layout issues found. Fix CSS problems: missing responsive breakpoints, overflow issues, alignment problems, color contrast. Suggest specific CSS changes for the dashboard HTML template.",
					...
					// Fix API issues
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Fix API issues: missing endpoints, incorrect JSON responses, slow responses that need async handling, error handling gaps. Suggest specific Go code changes.",
								Metadata: map[string]any{"max_tokens": float64(10)},
							},
						},
					},
			...
			// Phase 8: Re-verify fixes
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Re-build the dashboard binary and restart the server. Then re-run ALL tests from phases 1-6. Confirm that previously failing tests now pass. Report final pass/fail status for each test.",
						Metadata: map[string]any{"max_tokens": float64(10)},
					},
				},
			},
			// Phase 9: Report
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Generate a final UI Review Report. Include: test summary (total passed/failed), issues found and fixed, remaining issues, overall UI quality score (1-10), and recommendations for future improvements.",
						Metadata: map[string]any{"max_tokens": float64(8)},
					},
			...
			// Outcome
			{
				...
				Children: []SerializableNode{
					...
					{
						Type: "ChainAction",
						Name: "agent:UI review found issues. Self-correct: re-analyze the failing tests and suggest alternative fixes.",
