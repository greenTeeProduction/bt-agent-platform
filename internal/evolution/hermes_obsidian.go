...
func HermesObsidianOptimizerTree() *SerializableNode {
	return &SerializableNode{
		...
		Children: []SerializableNode{
			...
			{Type: "Selector", Name: "PipelineRouter", Children: []SerializableNode{
				...
				// Path 4: Knowledge audit
				{
					...
					Children: []SerializableNode{
						...
						{
							Type: "ChainAction",
							Name: "agent:Conduct knowledge audit: 1) Identify knowledge gaps — what should be documented but isn't? 2) Check for broken wikilinks. 3) Verify source attributions exist. 4) Assess vault coverage by topic area. 5) Check for orphan pages (no incoming links). 6) Rate vault health: completeness, accuracy, freshness. 7) Report: gaps found, links broken, overall health score.",
							Metadata: map[string]any{"max_tokens": float64(10)},
						},
					},
				},
				...
				// Path 5: Publish output
				{
					...
					Children: []SerializableNode{
						...
						{
							Type: "ChainAction",
							Name: "agent:Generate output from wiki knowledge: 1) Identify the target audience and format. 2) Extract relevant wiki notes with source attributions. 3) Generate the deliverable: report as markdown, presentation as slides, briefing as documents. 4) Save to output/ with appropriate naming. 5) Maintain source traceability: every claim links back to a wiki note. 6) Human-in-loop: mark confidence levels, flag items needing review.",
							Metadata: map[string]any{"max_tokens": float64(10)},
						},
					},
				},
			...
			{
				Type: "ChainAction",
				Name: "agent:Verify quality gates: 1) All wiki changes have direct source quotes. 2) No raw/ files were modified. 3) Cross-links are valid. 4) Human-in-loop items are flagged. 5) The vault is cleaner than when we started.",
			...
			{
				Type: "ChainAction",
				Name: "agent:Run AQAL check: I (subjective) — are notes clear and useful? It (objective) — are formatting, links, metadata correct? We (cultural) — will future sessions understand this? Its (systemic) — does it maintain vault conventions? Rate each 1-5.",
				Metadata: map[string]any{"max_tokens": float64(5)},
			},
			...
			{Type: "Selector", Name: "OutcomeSelector", Children: []SerializableNode{
				...
				{Type: "ChainAction", Name: "agent:Vault operation failed. Diagnose: file permissions, vault path, missing source material. Retry with corrected approach.", Metadata: map[string]any{"max_tokens": float64(4)}},
			}},
		},
	}
}
