		...
		Children: []SerializableNode{
				...
				Children: []SerializableNode{
					...
					// Path 1: Session Start
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:At vault root, read AGENTS.md to understand agent identity. Read today's memory/YYYY-MM-DD.md for recent context. Read SCHEMA.md for vault conventions. Read HEARTBEAT.md if present. Summarize: what's active, what needs attention, any open tasks.",
					...
					// Path 2: Ingest new material
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Process new source material: 1) Save raw content to raw/ folder (appropriate subfolder based on type: article, transcript, paper). 2) Never edit raw files — append-only. 3) Extract key insights. 4) If insights are durable, proceed to synthesize into wiki/.",
								Metadata: map[string]any{
					...
					// Path 4: Cross-link audit
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Audit wiki pages for cross-linking: 1) Find pages with fewer than 2 outgoing [[wikilinks]]. 2) Search for related concepts that should be linked. 3) Add missing links with brief context. 4) Report: pages fixed, orphan count remaining.",
								Metadata: map[string]any{
									"max_tokens": float64(8),
					...
					// Path 5: Update index
					{
						...
						Children: []SerializableNode{
							...
							{
								Type: "ChainAction",
								Name: "agent:Update wiki/_index.md: 1) Count total notes by category. 2) Add any new pages to the appropriate section. 3) Update quadrant balance analysis. 4) Update the 'last updated' date. 5) Flag any categories that are underrepresented.",
								Metadata: map[string]any{
									"max_tokens": float64(8),
			...
			// Quality gate
			{
				...
				Children: []SerializableNode{
					{
						Type: "ChainAction",
						Name: "agent:Verify vault integrity: 1) Check no duplicate pages were created. 2) Verify all wiki pages have YAML frontmatter with required fields. 3) Check no raw/ files were modified. 4) Confirm _index.md reflects current state. Report any issues found.",
			...
			{
				Type: "ChainAction",
				Name: "agent:Run AQAL four-quadrant audit on the vault work just completed. I (subjective): Was the work clear and useful? It (objective): Are formatting, links, metadata correct? We (cultural): Will future sessions understand this? Its (systemic): Does it maintain vault conventions? Rate each quadrant 1-5.",
				Metadata: map[string]any{
			...
			// Outcome
			{
				...
				Children: []SerializableNode{
					...
					{
						Type: "ChainAction",
						Name: "agent:Vault operation issue detected. Self-correct: check file permissions, verify vault path, retry with corrected approach.",
