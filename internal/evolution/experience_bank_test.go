...
// TestEvolveWithExperience_ResurrectsExtinctSpecialist verifies milestone 2/5
// of the self-healing-wiring program: EvolveWithExperience must wrap its
// experience-guided breeding in the same selfHealGeneration envelope Evolve
// already uses (learning.go:179), so the seeded pop.Specialists registry
// (newProductionPopulation, cmd/bt-agent/tools.go:244) is actually consulted
// on the production experience-bank path — bt_evolve_genetic,
// bt_evolve_bottlenecks, bt_evolve_selection_pressure — instead of sitting
// inert. Today EvolveWithExperience never reads p.Specialists or p.Crisis at
// all.
//
...
// extinct. After one EvolveWithExperience generation (with a non-nil bank so
// the experience-guided path runs instead of degrading to plain Evolve), the
// population must contain a resurrected:true individual, CrisisReasons must
// record diversity_collapse, and Resurrections must be positive.
func TestEvolveWithExperience_ResurrectsExtinctSpecialist(t *testing.T) {
	dir := t.TempDir()
	eb, err := NewExperienceBank(dir)
	if err != nil {
		t.Fatalf("NewExperienceBank: %v", err)
	...
	pop.EvolveWithExperience(1, func(*SerializableNode) float64 { return 1.0 }, eb)
	...
	if !resurrected {
		t.Fatal("expected EvolveWithExperience to resurrect the extinct goap specialist and inject a resurrected:true-tagged individual into the population")
	}
