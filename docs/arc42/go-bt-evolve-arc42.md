...
├── workflow_engine.go   — Workflow orchestration (Workflow/WorkflowTask/Approval, WorkflowPriority ordinal, RecommendationsToTasks/PendingApprovals/ApproveTask/RejectTask/ExecuteSprint/RunFullPipeline); since 2026-07-13 (ADR-080) `handleAnalyze` (cmd/bt-dashboard/main.go) is the first production caller of `NewWorkflow`/`RecommendationsToTasks`/`Prioritize`, and since 2026-07-13 (ADR-081) three new handlers — `handleWorkflowPending`/`handleWorkflowApprove`/`handleWorkflowReject`, held on the package-level `currentWorkflow` var — give `PendingApprovals`/`ApproveTask`/`RejectTask` their own production callers too; `ExecuteSprint`/`RunFullPipeline` remain test-only, and `RunFullPipeline` no longer auto-approves `Priority <= PriorityHigh` tasks as `"system"` — every task now needs an explicit `ApproveTask`/`RejectTask` call — and still guards its `w.Tasks[0].Title` read against a zero-task pipeline instead of indexing unconditionally
...
- ⚠️ `currentWorkflow` is a single unscoped package var overwritten by every `handleAnalyze` call. A second analysis run silently discards any pending approvals left over from a prior `Workflow`, and concurrent dashboard sessions share one workflow rather than one each.
