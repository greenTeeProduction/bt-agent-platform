...
**Status:** Accepted (2026-07-12) — milestones 1–2 of 5; `EvolveQLearning` and `Population.MemeticEvolve` remain outside the shared envelope pending later milestones of this program, and `EvolveMAPElites` (`map_elites.go`) still carries its own independent copy of the same logic rather than calling `selfHealGeneration`.
