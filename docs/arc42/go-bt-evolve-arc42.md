...
├── experience_bank.go   — ExperienceBank: persisted successful-mutation entries (EvoRepair-style), Jaccard retrieval by tree type, bounded at 500 entries with quality-aware eviction; merge-on-Add reconciliation keeps the file safe for the two concurrent writers (daemon + gardener)
...
- ⚠️ The merge shrinks the loss window to a single Add (read → rewrite) but is not a lock: two Adds racing inside that window can still drop one writer's newest entry
