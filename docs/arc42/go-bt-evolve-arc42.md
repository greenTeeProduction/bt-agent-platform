...
- ⚠️ This closes only the scheduler-side gap. The separate per-winner breakers ADR-064 adds inside the A2A auction path are a distinct `reliability.CircuitBreaker` map scoped to the `Auctioneer`'s own lifetime, held in memory only, and do not write to `circuit_breakers.json` — a unified view across both breaker populations remains open (tracked as milestone 5 of the A2A reliability program, ADR-064)
...
**Status:** Accepted (2026-07-13) — closes milestones 1 (honest `SendTask` errors) and 3 (per-candidate breaker + retry) of the 5-milestone "A2A reliability adoption" program; milestones 2 and 4 (SafeGo across the A2A/dashboard fan-out and daemon background loops) were already closed by ADR-046/ADR-047 and the SafeGo second-wave passes. Milestone 5 (persisting these per-winner breakers into the shared `circuit_breakers.json` file ADR-063 now writes) remains open — the `Auctioneer.breakers` map is in-memory only and does not survive a restart.
...
- ⚠️ Per-winner breaker state does not persist across restarts (see ADR-063's Consequences) and is scoped to a single `Auctioneer` instance, so a redeployed daemon starts every winner's circuit closed again
