...
├── specialist_registry.go — SpecialistRegistry: keeps the best validated archetype per "specialist:<type>" so crisis recovery can Resurrect an extinct niche (Observe/ExtinctSpecialists/Resurrect; wired end-to-end only from tests so far)
├── mutations.go         — 10 mutation operators (add_before, add_after, wrap_retry, prune, swap_children, etc.)
├── learning.go          — cloneTree (sole deep-copy implementation), Population.Evolve (now runs proactive crisis intervention each generation — DetectPopulation → emergency mutation rate + ResetPopulation, ADR-031) + EvolveWithExperience (bank-warm-started variant) + EvolveQLearning (QTable-guided mutation-category selection), RetrieveExperienceHints (exported top-K bank retrieval reused by the gardener's candidate biasing)
...
Reachable in production through `bt_evolve_qd` (`cmd/bt-agent/tools.go`), the one deterministic tool that drives plain `Population.Evolve`. Pinned by `internal/evolution/learning_test.go` (`Evolve` records `diversity_collapse`/`regression_spiral`; an emergency generation uses a rate ≥ `EmergencyRate` and clears the streaks while a healthy generation is unaffected; seeded specialists observe into a `SpecialistRegistry`).

**Status:** Accepted (2026-07-09) — milestones 1–3 of 5 complete.
...
- ⚠️ Only plain `Population.Evolve` (reached via `bt_evolve_qd`) runs the intervention; the experience-warm-started `EvolveWithExperience` (the `bt_evolve_genetic`/`bt_evolve_bottlenecks` path, ADR-017) and `EvolveQLearning` do not yet, so crisis intervention is not universal across the evolve tools
- ⚠️ Milestones 4–5 remain: no production binary yet seeds a population from `SeedSpecialists`, drives `Observe` during a live `Evolve`, or resurrects an extinct specialist on crisis — the seed→observe→resurrect loop is exercised only from tests
