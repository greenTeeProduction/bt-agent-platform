...
| Knowledge | `internal/knowledge`, `internal/research`, `internal/factory` | Knowledge graph (capabilities, embeddings, discovery, auto-creation); research memory: content-hash dedup knowledge store + multi-cycle program store (ADR-003 JSON); factory breeding (crossover + archetypes) |
