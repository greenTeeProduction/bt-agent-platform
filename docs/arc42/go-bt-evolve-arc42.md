...
├── workflow_engine.go   — Workflow orchestration (Workflow/WorkflowTask/Approval, WorkflowPriority ordinal, RecommendationsToTasks/ApproveTask/ExecuteSprint/RunFullPipeline); since 2026-07-13 (ADR-080) `handleAnalyze` (cmd/bt-dashboard/main.go) is the first production caller of `NewWorkflow`/`RecommendationsToTasks`/`Prioritize` — `ApproveTask`/`ExecuteSprint`/`RunFullPipeline` remain test-only, and `RunFullPipeline` now guards its `w.Tasks[0].Title` read against a zero-task pipeline instead of indexing unconditionally
...
**Status:** Accepted (2026-07-13) — milestones 1–3/4. Milestone 4/4 of the stated program is not part of this change.
