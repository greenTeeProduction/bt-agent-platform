...
├── workflow_engine.go   — Workflow orchestration (Workflow/WorkflowTask/Approval, WorkflowPriority ordinal, RecommendationsToTasks/ApproveTask/ExecuteSprint/RunFullPipeline); still has no production caller outside its own tests
...
| **Task Approval (dashboard)** | The audit trail on `dashboard.Task` (`internal/dashboard/tasks.go`) for approve/reject decisions on thinktank-derived tasks, added by mirroring the `Approval` struct `workflow_engine.go` already declared for its own (still production-callerless) `WorkflowTask`. `TaskStore.Approve`/`Reject` set `ApprovedBy`/`ApprovedAt` or `RejectedAt`/`Reason`/`IsApproved` and persist through the normal save path; `Approved()` — unchanged in that it still returns only tasks a human has explicitly approved — now sorts them by `workflow_engine.go`'s `WorkflowPriority` ordinal (critical first) then `Sprint`, so `handleSprintExecute` dispatches the most urgent approved work first. ADR-072. |
