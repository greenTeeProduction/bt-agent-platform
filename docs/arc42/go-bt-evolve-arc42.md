...
## ADR-074: Bounded HTTP Timeout and Circuit Breaker Harden `EmbeddingClient.GetEmbedding` Against an Unresponsive Ollama Backend
...
**Decision:** Two package-level guards now wrap every Ollama embedding call. `embeddingHTTPClient := &http.Client{Timeout: 2 * time.Second}` replaces the bare `http.Post`, bounding every request. `embeddingBreaker := reliability.NewCircuitBreaker("ollama-embeddings", 3, 60*time.Second)` gates `GetEmbedding` — `Allow()` is checked before dialing and short-circuits with an error while the circuit is open, and the round trip calls `RecordFailure`/`RecordSuccess` — mirroring the existing 3-state `reliability.CircuitBreaker` pattern (ADR-007) and the per-client breaker `ACPClient` already carries for its own subprocess calls (ADR-054). Both `BuildIndex` and `discoverWithEmbeddings` are unchanged callers of `GetEmbedding`, so the guard is transparent to them: an open circuit surfaces as the same `error` return `discoverWithEmbeddings` already treats as a clean fallback (empty id, zero score).

**Status:** Accepted (2026-07-13). Milestones 1–2/5 of the "Q3 Reliability — harden the knowledge-graph embedding client against an unresponsive Ollama backend" program; milestones 3–5 are not yet implemented.
