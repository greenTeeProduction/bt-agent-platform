...
- ⚠️ All three tools drive `EvolveWithExperience` (ADR-017), whose live-bank path does not run the ADR-031 crisis intervention; with a wired experience bank the `health` fields therefore report the null baseline (empty `crisis_reasons`, `resurrections` 0, `last_mutation_rate` 0) and carry real intervention data only on the nil-bank fallback to plain `Evolve` — the same coverage gap ADR-031 flags for these tools, now merely observable rather than closed
