...
- ⚠️ This is milestones 1–3 of a 5-milestone program; three more bare `go` spawns remain, but all three are currently dormant with no production caller — `internal/security/apikey.go`'s `KeyRotationScheduler.loop`, `internal/config/watcher.go`'s `ConfigWatcher.loop`, and `internal/reliability/heartbeat.go`'s `NodeHeartbeat.cleanupLoop` (both constructors) — lower urgency than the three closed here, which were all reachable from a running daemon.
- Pinned by the tests named above.
