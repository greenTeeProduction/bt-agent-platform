...
| **Circuit Breaker** | 3-state pattern (closed/open/half-open) that prevents cascading failures. Per-agent isolation via AgentCircuitBreakerStore. Since 2026-07-13 (ADR-054), `ACPClient` also carries its own `reliability.CircuitBreaker` keyed by ACP command, tripped by repeated subprocess-start/session-setup failures so a crash-looping ACP subprocess is not respawned on every `GenerateCtx` call. |
