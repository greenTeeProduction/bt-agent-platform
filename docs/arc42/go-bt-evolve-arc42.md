...
- ⚠️ `bt_evolve_island` (`cmd/bt-agent/tools.go`), the sole production caller of `IslandModel.Migrate`, still builds its model via the bare `NewIslandModel(interval, rate)` constructor and never sets `.Bank` — the transfer is implemented and covered by `internal/evolution/island_model_test.go`, but not yet wired into the one production entry point that would exercise it live
- Pinned by tests in `internal/evolution/island_model_test.go`
