...
├── selector_optimizer.go — SelectorOptimizer: per-Selector child success/failure/running telemetry, OrderChildren priority recommendation, and durable SaveSelectorStats/LoadSelectorStats (ADR-024 flock + atomic tmp+rename, merge-sums counts on load)
...
**Status:** Accepted (2026-07-09) — partial program (milestones 1–3 of 5).
...
- ⚠️ **Not yet wired end to end (milestones 4–5 open):** no production scheduler calls `RecordSelectorOutcomes`, and no production path loads the store to reorder a live tree's Selector children (`LoadSelectorStats`/`OptimizeSelectors` have test callers only) — the persistence primitives and the bridge exist, but the learn→persist→apply loop is not yet closed in a running binary
