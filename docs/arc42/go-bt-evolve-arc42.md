...
**User-scoped runtime-generated tree resolution (2026-07-13, ADR-067, "Harden the ADR-010 personalization layer" program milestone 3/5):** The runtime-generated-tree fallback `agentexec.resolveUserTree` resolves a deterministic slug ID like `goal:automate_<slug>` by scanning every `users/<id>/trees` workspace in *sorted* order and returning the first hit — so two different users who each independently compile a personal tree under the same slug-derived ID collide, and the alphabetically-earlier user's tree silently satisfies the other's request (bob asking for his automation could receive alice's). `agentexec.ResolveGeneratedTreeForUser(user, id)` and its `domains.ResolveTreeIDForUser(user, id)` counterpart are the scoped fix: the shared reflections dir is still consulted first, but the per-user fallback loads only `users/<user>/trees` — never the cross-user scan. `domains.DynamicResolveForUserFn` is the new injection seam `agentexec.init` wires to `ResolveGeneratedTreeForUser` alongside the existing unscoped `DynamicResolveFn`, and a new `resolveTreeIDWithResolver` helper factors `resolveTreeID`'s ID-prefix branching so the scoped and unscoped entry points share one implementation instead of drifting. Pinned by `TestResolveGeneratedTreeForUser_TwoUserCollision` and `TestResolveTreeIDForUser_ScopesToRequestingUser` (`internal/agentexec/wiring_test.go`), which save colliding same-ID trees for two users and assert each resolves their own. ⚠️ Not yet reachable from production: `agentexec.NewRunDeps()` and `cmd/bt-agent/main.go`'s `resolveTree` (feeding both `agent.RunDeps.ResolveTree` and `a2a_mod.SetTreeResolver`) still call the unscoped `domains.ResolveTreeID`, and `internal/agent/runner.go`'s single-argument `TreeResolver` type has no requesting-user parameter to thread through — wiring a real user identity into that call chain so the new scoping actually protects the live `goal:automate_*` request path is the program's next milestone.
...
- ⚠️ This closes the specific gap ADR-056 flagged in its own consequences (see that entry, now marked resolved) but is scoped to milestones 1–3 of a 4-milestone program; the remaining milestone is not addressed here.
- Pinned by the tests named above.
