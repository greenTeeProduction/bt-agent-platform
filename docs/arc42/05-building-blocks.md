...
A few `internal/domains` trees belong to none of the three registries and are
reachable only through `ResolveTreeID`'s bare-ID special cases
(`tree_resolver.go`): notably `superpowers_pipeline`
(`superpowers_pipeline.go` → ADR-013), which is therefore outside
`SmokeTestableDomainTrees()` and described via `ResolverReachableDescriptions`
rather than either registry map.
