...
| Service | `internal/a2a` | Agent-to-Agent (A2A) protocol integration, incl. auction-based task allocation ([§8](08-crosscutting-concepts.md) A2A Auction Task Allocation); the auction's per-winner circuit breakers persist through `internal/agent`'s store (`NewWinnerCircuitBreakerStore`) rather than this package's own load/save, so the block owns the auction policy and none of the shared file's format or merge rules (→ ADR-065); `treeTags` derives a card's skill/bid tags from `knowledge.GlobalGraph`'s fitness-weighted `Capability` list via the new `KnowledgeGraph.TreeCapabilities` accessor — the same canonical "what can this tree do" model `internal/knowledge` uses for `Discover` — falling back to legacy ad hoc tree-ID splitting only for trees not yet registered in the graph |
| Service | `internal/agent` | Agent lifecycle: registry, scheduler, memory, pub/sub AgentBus; `AgentCircuitBreakerStore` is the platform's sole implementation of `circuit_breakers.json` persistence ([§8](08-crosscutting-concepts.md) File-Based Persistence) — a per-store `keyPrefix` decides which of that shared file's entries the store owns (`Load` restores only owned keys, `Save` merges rather than rewrites), so the default scheduler/dashboard store and the `NewWinnerCircuitBreakerStore` `internal/a2a` uses are mirror halves of one key space instead of the two hand-rolled load/save pairs that had drifted apart (→ ADR-063, ADR-065) |
...
| Service | `internal/domains` | Domain-specific behavior trees (catalog below); `ExpectedDomainIDs` converts a tree registry to the `domain:<name>` ID form `knowledge.KnowledgeGraph.ExpectedDomains` expects, giving `cmd/bt-agent` and `cmd/bt-dashboard` one canonical conversion instead of each duplicating it (→ ADR-182); `SmokeTestableDomainTrees()` is the canonical union of `AllDomainTrees()` and `KanbanAndHermesDomainTrees()` from which every coverage guard derives its work list, replacing the hand-copied tree literals that previously let a newly registered tree go silently unexercised; descriptions follow the same registry split, now three-way — `Descriptions` covers the curated registry, `NonRegistryDescriptions` the kanban/hermes trees, and `ResolverReachableDescriptions` the `ResolverReachableDomainTrees()` set — the trees on neither registry surface that are nonetheless selectable in production through `ResolveTreeID` (`tree_resolver.go`, consumed by `switch_tree`/A2A/template validation; currently `superpowers_pipeline` → ADR-013), a production registry rather than the test-file enumeration ADR-255 left it as, so all three maps are guarded against a registry that ships, with `DescriptionFor(name)` resolving across all three in that precedence order (whitespace-only entries report as a miss) and then retrying a miss once through `ResolverIDAliases`, so the bare IDs `ResolveTreeID` accepts under a spelling its registry does not use (`kanban:refiner` → `kanban_refiner`, `notebooklm-consumer` → `notebooklm_consumer`) describe as the one tree they resolve to rather than rendering as bare identifiers — an alias rather than a fourth description map, since each map is orphan-guarded against its own tree set and a duplicated sentence would be an orphan in all three; the hop is last and single, so a name any map answers for never reaches it and no alias can chain, so callers need not know which side a tree lives on — it is the sole supported name→description lookup and every production consumer now routes through it (`gardener.Registry.loadAll`'s builtin registration, `cmd/bt-dashboard`'s `/api/trees` catalog (5.4), `cmd/bt-agent`'s `bt_use_domain_tree` switch confirmation), each having previously indexed `Descriptions` directly and so silently rendered a bare identifier for any registry tree whose description lives in one of the other two maps; the three maps are kept disjoint and each orphan-guarded against its own tree set, since a name in two of them would make the precedence silently choose between divergent descriptions |
| Service | `internal/hitl` | Human-in-the-loop approval for behavior tree execution |
| Service | `internal/startup` | Startup-company simulation; `CompanyState` owns its own `sync.Mutex` (`Lock`/`Unlock`) because multiple `*Workflow` (`internal/dashboard`) and `*CompanyOrchestrator` wrappers routinely share one underlying `*CompanyState` pointer (`cmd/bt-dashboard`'s package-level `companyState`), and each wrapper's own private mutex only ever serialized that wrapper's own fields, not the shared state underneath it; `CompanyOrchestrator.RunSprint` snapshots the state fields its three sequential `runTree` calls need (each up to a 120s LLM timeout) under a short `Lock`, runs them unlocked, then re-acquires the lock only to apply the resulting mutations, so a concurrent reader (`Summary()`, `cmd/bt-dashboard`'s `handleDefaultCompany`) is never blocked for the full multi-minute sprint instead of just the field-update window — mirroring the unlock-around-`RunSprint` pattern `dashboard/workflow_engine.go`'s `ExecuteSprint` already used one level up (5.4) |
...
| Infrastructure | `internal/benchmark` | A/B testing and statistical mutation-quality benchmark suites; `RunSuite` now cross-checks each task's detected `Path` against its declared `TaskCase.ExpectedPath`/`PossiblePaths` (`pathMatches`, `Result.PathMatched`, `RunMetrics.PathMatchRate`), and `RunABTest`/`ScoreMutation` weight the `PathMatchRate` delta so a mutation that reroutes tasks onto the wrong `StrategyRouter` branch scores non-positive even when mocked actions on the wrong path still report `bb.Outcome == "success"`; `SuiteForTreeNamed` is the fallback-aware counterpart to `SuiteForTree` — its second return value is false only on the default branch, so a tree silently inheriting an unrelated suite is distinguishable from a deliberate match, which is what lets `internal/domains` assert registry-wide that every resolved suite's `ExpectedPath` values name real nodes in their tree; the eight kanban/hermes trees get bespoke suites (`KanbanTaskCreatorSuite`, `KanbanRefinerSuite`, `KanbanQASuite`, `KanbanBoardMonitorSuite`, `KanbanWorkflowSuite`, `KanbanAutoPilotSuite`, `HermesEvolveSuite`, `HermesObsidianSuite`) for the same reason the 10 finance agents do — the generic fallback gave them `ExpectedPath` values naming nodes their trees do not contain, making the suite's condition coverage unsatisfiable by real execution |
...
| Infrastructure | `internal/gardener` | 24/7 tree-evolution daemon; `RunCycleV2` ranks trees via `Config.KnowledgeGraph`'s `ComputeAnalytics()` (Bottlenecks first, then SelectionPressure), spending the per-cycle mutation budget on trees that need it before falling back to alphabetical order when unset (→ ADR-146); `cmd/bt-gardener/config.go`'s `buildGardenerConfig` now wires a live `*knowledge.KnowledgeGraph` (with persisted feedback loaded) so this ranking runs in production, and `evolveTreeV2` writes an `"evolved"` `RunRecord` back to it whenever a cycle accepts a mutation, mirroring `recordEvolvedFitness` (`cmd/bt-agent/tools.go`) and closing the daemon's side of the learn→evolve loop; `Registry.Rescan()` re-runs the user-tree scan and is called once per `cmd/bt-gardener` ticker cycle, so autopilot-compiled/HITL-approved personal trees become visible without a daemon restart (→ ADR-133); `CollectAgentSLOs` now reads persisted cross-process evidence via `engine.LoadSLOEvidence(EvidencePath)` instead of the in-process-only `engine.AllSLOMetrics()` sync.Map — always empty here since the gardener process never executes trees itself — mirroring `ValidationGate`'s file-fallback (`validation_gate.go`), so `RunCycleV2`'s per-cycle `slo-metrics.json` export and the dashboard's `GardenerMetrics.SLOs` (5.4) are no longer permanently empty in production; beyond the Stockfish mutation ordering the daemon now actively drives three more evolution algorithms (5.3), each re-scoring its candidate against the target tree's own reflection records before adopting it and each reported per cycle on `CycleMetrics`: a periodic per-domain `IslandModel.EvolveAll` population-exploration pass (`runIslandExploration`, enabled by `Config.IslandModel`/`IslandInterval` and wired in `cmd/bt-gardener/config.go`, running before the per-tree pipeline so a migrated winner becomes that cycle's baseline → `IslandAdopted`; because it runs first, `adoptIslandWinner` is a persist path with no downstream gate left to catch it, so it clears the same fail-closed `Gate.IsDisabledFor` check, reflection-evidence gate and `ValidationGate` `evolveTreeV2` clears before its own save — not merely the fitness and bloat-cap checks — an island breeding by random mutation being precisely what those gates exist to keep off disk), MAP-Elites active elitism (`reseedFromDiversityArchive` — a `evolution.CrisisDiversityCollapse` verdict reseeds the cycle from an archived elite in a *different* niche instead of mutating current-best again, promoting `treeDiversityGrid` from a write-only diversity signal to an elitism source → `EliteReseed`; `recordDiversityObservation` therefore `cloneTreeForGardener`s before archiving, since `MAPElitesGrid.Insert` stores the `*Individual` it is handed without copying (5.3) while `Registry.List` returns the same live `*SerializableNode` the pipeline mutates in place every cycle — archiving that pointer verbatim left each cell aliasing current-best under a stale per-cell fitness, so the reseed re-scored the live tree against itself and `EliteReseed` could never fire in production), and a post-structural-mutation `LocalSearcher.RefineGated` parameter-tuning pass (`refineTreeParameters`, `EvolveV2Config.DisableLocalSearch`/`LocalSearchStrategy` — the one improvement no `MutationOp` can express → `LocalSearchDelta`), which runs *above* the `ValidationGate` block and is itself a reason to enter it (alongside an applied mutation or an elite reseed), since it mutates the live tree and forces the save on its own — after the gate, a tuned tree the gate had just rejected would reach disk unvalidated, and a rejection now also zeroes `LocalSearchDelta` because the rollback reverted the tuning; the gate itself (`validation_gate.go`) accepts by multi-objective non-domination rather than scalar per-threshold tests — the SLO snapshot and the configured floors become two points over `evolution.DimSuccessRate`/`DimRecoveryRate` (the latter scored only once `FailedCalls > 0`, since a flawless tree's `RecoveryRate` is 0 by definition) and `evolution.ParetoAccepts` (5.3) rejects only evidence the thresholds dominate outright, with `ValidationGateConfig.MaxObjectiveRegression` (default 0.10, 0 reproducing the old scalar gate) capping how far any one axis may fall so an unbounded success-rate collapse cannot be bought with a high recovery rate; structural-candidate *generation* is itself two-source since `augmentWithMCTSCandidates` — `MCTSMutator.Candidates` proposals are folded into `evaluator.OrderMutations`' list by `evolution.MergeScoredMutations` (5.3) so both generators face one descending-score competition, and each merged candidate still clears the same benchmark, pre-score, quality gate and meta-validation before it is applied; whether the search runs is a per-tree call by `evolution.SelectStructuralStrategy` over the specialist registry and the (now shared, via `seedSelectorOptimizer`) Selector telemetry, so a proven archetype with fully telemetered Selectors keeps the heuristic-only ordering (`EvolveV2Config.MCTSStructuralSearch`/`MCTSIterations`/`Specialists`, on by default in `DefaultEvolveV2Config` and hence in `cmd/bt-gardener`) |
...
four registries — three enumerating `internal/domains` trees, one
`internal/evolution`:
...
| `domains.KanbanAndHermesDomainTrees()` (`internal/domains/trees.go`) | Kanban workflow (`kanban.go`): `kanban_task_creator`, `kanban_refiner`, `kanban_qa`, `kanban_monitor`, `kanban_workflow`, `kanban_autopilot` · Hermes self-evolution (`hermes_evolve.go`, `hermes_obsidian.go`): `hermes_evolve`, `hermes_obsidian`; deliberately excluded from `AllDomainTrees()` — not part of the gardener/dashboard registry surface — but reachable from `SmokeTestableDomainTrees()` and so held to the same `BuildTree` smoke, condition-description, per-tree description (`NonRegistryDescriptions`) and benchmark-suite `ExpectedPath` guarantees as registry trees |
| `domains.ResolverReachableDomainTrees()` (`internal/domains/trees.go`) | `superpowers_pipeline` (`superpowers_pipeline.go` → ADR-013, ADR-255) — the domains-package trees operators can select through `ResolveTreeID` (`tree_resolver.go`, reached by `switch_tree`/A2A/template validation) but which sit on neither registry surface above, so `SmokeTestableDomainTrees()` cannot see them; this registry is their coverage work list and the reference set `ResolverReachableDescriptions` is orphan-guarded against. Held disjoint from that union, so a tree that later joins a smoke registry moves out rather than being listed twice — `hermes_obsidian` is resolver-reachable but lives in `KanbanAndHermesDomainTrees()`, which already covers it, and is deliberately absent here |
...
Each of the three `internal/domains` registries is the orphan-guard reference
set for exactly one description map (`internal/domains` row above), and
`ResolverReachableDomainTrees()` is held disjoint from the
`SmokeTestableDomainTrees()` union, so no tree has two description homes.
Registry membership — not a hand-copied test literal — is what subjects a tree
to the smoke-structure, condition-description, description and edge-metadata
guards, and exhaustiveness over `ResolveTreeID`'s bare-ID branches is derived
from `tree_resolver.go` itself: a branch resolving to a domains-package tree on
no registry fails the build instead of shipping operator-selectable and
uncovered, while branches resolving into other packages are declared as such.
Condition nodes across the package come from `trees.go`'s shared `cond()`
constructor, which attaches the machine-readable `EdgeGuard` typed edge the
engine evaluates alongside the prose `Description` (→ ADR-250).
...
Level 2 whitebox of `internal/evolution`. Four shared mechanisms, stated
...
(`island.go`), and deterministic tie-breaking (`pareto.go`) — and the NSGA-II
front machinery exists exactly once, as the unexported
`nonDominatedSort`/`crowdingDistances` pair in `multi_objective.go`, which
`NSGAIIPopulation`'s slice-valued methods, `NSGAIISorter`'s map-valued ones,
and `pareto.go`'s `ParetoAccepts` all delegate to, in place of the two
byte-for-byte copies that previously let `Evaluate`'s fronts and `Evolve`'s
offspring-replacement fronts disagree about the same population.
...
| `crisis_detector.go` | Proactive diversity-collapse/regression-spiral/quality-crash detection with a calibrated emergency mutation rate feeding both the GA envelope and the gardener's mutation budget (→ ADR-031, ADR-102); the reason strings are exported constants because consumers branch on *which* crisis fired — the gardener answers a diversity collapse with a MAP-Elites reseed, the wrong response to stagnation (→ 5.1 gardener) | `DetectPopulation`, `Detect`/`Intervene`, `GetEmergencyMutationRate`, `CrisisDiversityCollapse`/`CrisisStagnation`/`CrisisRegressionSpiral`/`CrisisQualityCrash` |
| `decision_tree.go` | C4.5/CART information-gain metrics over Selector paths with durable count-summing archive; degrades to a no-op on empty telemetry; `Entropy`/`GiniImpurity` delegate their math to `selector_optimizer.go`'s canonical `Entropy`/`GiniImpurityFromProbs` instead of duplicating the -Σp·log2(p) / 1-Σp² arithmetic (→ ADR-029) | `DTAnalyzer`, `BTOptimizer.OptimizeSelectors` |
...
| `island.go` | Island model with periodic migration and per-domain durable merge archive bounded by `Cap`/`IslandCap` with observable eviction (→ ADR-033, ADR-034, ADR-040); cross-domain experience transfer riding migration via optional `Bank` (→ ADR-062, ADR-076); cross-island winner benchmark-gated (→ ADR-096); real within-island breeding + optional `ExpertKnowledge` (→ ADR-104, ADR-106); `SeedIsland`/`BestIndividualFor` are the warm-start and migrate-out halves a long-lived caller needs to drive the model from outside the package without racing on the live populations — first consumer is the gardener's periodic exploration pass (5.1), which keeps the subpopulation it has been evolving across cycles and scores the champion with its own fitness function rather than the island's recorded `Fitness` | `IslandModel.EvolveAll`/`Migrate`/`Save`/`Load`, `SeedIsland`, `BestIndividualFor`, `TotalMigrations`; driven by `bt_evolve_island` |
...
| `local_search.go` | Memetic refinement (hill-climb / simulated annealing / tabu) with the breeding step inside the shared envelope (→ ADR-121); `extractMutableParams` tunes `SerializableNode`'s own `MaxRetries`/`TimeoutMs` fields alongside the metadata knobs, since production trees set the struct fields and leave `Metadata` nil; `hillClimb` is steepest-ascent over a deterministic coarse-to-fine neighbor ladder (`paramStepFactors`, plus ±1 for integer-backed params) rather than a random ±5% nudge, which rounded flat on those integer fields; `RefineGated` is the gated entry point for callers outside a GA population (the gardener's post-mutation pass, 5.1) — it keeps the tuned tree only when it strictly beats the baseline *and* the quality gate accepts, consulting the gate through the non-recording `Probe` so a discarded speculative tuning never burns the tree's fail-closed streak; that AND is non-vacuous only because `Probe`'s composite floor is absolute (below) — the strict-improvement check guarantees the gate only ever sees post > pre, so what it still refuses is a tuning that improved yet left the tree under the health floor, and without it every refinement would reach the live tree (and, via the gardener's forced save, disk) ungated | `LocalSearcher`, `Search`, `RefineGated`/`RefineResult`, `Population.MemeticEvolve`; driven by `bt_evolve_memetic` |
| `map_elites.go` | MAP-Elites quality diversity over behavioral descriptors; durable capped merge-safe grid (→ ADR-033, ADR-043); shared envelope replaced its hand-inlined crisis loop, fixing a single-niche-collapse blind spot in `DiversityScore()` (→ ADR-121); optional `ExpertKnowledge` (→ ADR-104); `EliteSeed` exposes the archive as an active elitism source — the fittest elite in a niche *other* than the caller's current one, above a fitness floor, so a collapsed niche can be escaped by reseeding rather than by mutating current-best again (first consumer: the gardener's diversity-crisis reseed, 5.1) — which makes `Insert`'s ownership contract load-bearing, and hence now explicit: the grid stores the `*Individual` and its `Tree` as given rather than copying, so a caller archiving a tree it goes on to evolve in place must clone first (5.1 gardener) or every cell aliases that one live object against a stale per-cell `Fitness` and `EliteSeed` hands the caller back its own current tree | `BehavioralDescriptor`, `Descriptor`, `MAPElitesGrid.Insert` (stores, does not copy), `MAPElitesGrid.Save/Load` (`Cap`), `EliteSeed`, `MAPElitesPopulation.EvolveMAPElites`; driven by `bt_evolve_qd` (→ ADR-110, gate → ADR-111) |
| `mcts_mutate.go`, `llm_supervisor.go` | MCTS-guided mutation search; deterministic, `-short`-safe heuristic supervisor mirroring the LLM policy contract (→ ADR-110); `Candidates` is the non-all-or-nothing counterpart to `Mutate` — instead of collapsing a search into one winning tree it emits each *root-level* mutation that beat the parent (deeper nodes were mutated on top of an already-mutated tree, so their ops would not replay against the untouched parent) as a standalone `MutationOp` scored into the same `(0.5, 1.0]` band `evaluator.OrderMutations` uses, damped by how marginal the best gain actually was, so a caller can benchmark and gate them one at a time; `MergeScoredMutations` folds the two generators into one descending-score list, collapsing a duplicate op/target to the higher-scoring side and keeping that side's `Reason`; `SelectStructuralStrategy` averages the two "how little do we already know about this tree" affinities below and augments at ≥ 0.5. Sole consumer: the gardener's structural-mutation stage (5.1) | `MCTSMutator.Mutate`/`Candidates`, `ScoredMutation`, `MergeScoredMutations`, `SelectStructuralStrategy`/`StructuralStrategy`, `LLMSupervisor.Guide` |
...
| `multi_objective.go` | NSGA-II: fast non-dominated sort and crowding distance (the package-canonical implementations — see the shared mechanisms above), seeded specialists (→ ADR-051); durable final-front archive delegating to `ParetoFront` (→ ADR-091); persistence benchmark-gated (→ ADR-096); optional `ExpertKnowledge` observation (→ ADR-104, ADR-106) | `NSGAIIPopulation.Evolve`/`Save`/`Load` (`Cap`/`Archive`); driven by `bt_evolve_multiobjective` |
...
| `pareto.go` | Pareto-front evolution with front-elitism crossover (→ ADR-038); durable dominance-merged, cap-evicting archive (→ ADR-091); benchmark-gated (→ ADR-113); optional `ExpertKnowledge` (→ ADR-104, ADR-106); `ParetoAccepts` exports the front-0 membership test as a reusable acceptance rule for callers outside a GA population — accept a candidate unless some baseline strictly dominates it, so trade-offs pass and only regressions are refused — with `DimRecoveryRate` as the SLO-sourced dimension `StructuralMultiFitness` cannot derive from tree structure; first consumer is the gardener's validation gate (5.1) | `MultiFitness`, `ParetoFront.Save/Load`, `ParetoPopulation.EvolvePareto`, `StructuralMultiFitness`, `ParetoAccepts`, `DimRecoveryRate`; driven by `bt_evolve_pareto` (→ ADR-057) |
| `quality_gate.go` | Reactive regression/floor gate with per-tree consecutive-failure disable; multi-revision pre-mutation snapshot history, each optionally tagged with its composite fitness, so fail-closed rollback walks back past a multi-cycle regression streak to the last known-good revision rather than only the immediately-preceding one (→ ADR-093, ADR-115); `Probe` is the non-recording counterpart to `Validate`/`ValidateFor` — no streak increment, reset, or disabling — for speculative changes the caller may discard (`LocalSearcher.RefineGated`), so a tuning attempt that does not pan out cannot push a healthy tree toward fail-closed; its *verdict* deliberately differs, in that its `MinComposite` floor is absolute where `Validate`'s applies only to a score that also declined — `Validate` gates the structural loop, whose alternative to a sub-floor improvement is keeping an even weaker tree, whereas `Probe`'s caller has already refused everything that is not an improvement, so a direction-conditional floor would be unreachable there and the whole gate check would degenerate to an unconditional accept | `Validate`/`ValidateFor`/`Probe`, `SnapshotTree`/`SnapshotTreeWithFitness`, `RestoreTree`/`RestoreTreeRevision`/`RestoreTreeBeforeRegressionStreak`/`ListRevisions`; `gardener_rollback` tool |
| `reflection_store.go` | Per-tree fitness evidence from run reflections | `FilterByTreeName`(`Strict`) |
| `selector_optimizer.go` | Per-Selector child success/failure/running telemetry, learned priority ordering, durable merge-summing stats; `Entropy`/`GiniImpurityFromProbs` are the package's canonical entropy/Gini-impurity formulas over an arbitrary probability distribution, now also consumed by `decision_tree.go`'s `DTAnalyzer` in place of its own duplicated math (→ ADR-029, ADR-079); `MCTSAffinity` is the learned-ordering half of `SelectStructuralStrategy`'s per-tree decision — the fraction of a tree's named Selectors with fewer than `MinSamples` recorded outcomes (the same threshold `OrderChildren` gates on), i.e. how much of the tree the cheap evidence-backed reordering *cannot* already improve; a nil optimizer or a Selector-free tree yields 1.0 | `SelectorOptimizer`, `OrderChildren`, `ApplyLearnedOrdering`, `SaveSelectorStats`/`LoadSelectorStats`, `ParseSelectorOrderingStrategy` (validates an ordering-strategy string, defaulting to `OrderBySuccessRate`; shared by every production wiring site), `Entropy`, `GiniImpurityFromProbs`, `MCTSAffinity`; driven by `bt_evolve_selectors` |
| `specialist_registry.go` | Best validated archetype per `specialist:<type>` so crisis recovery can resurrect an extinct niche; consulted by every production evolve variant through the shared envelope (→ ADR-031, ADR-038, ADR-051, ADR-121); `MCTSAffinity` is the archetype half of `SelectStructuralStrategy`'s per-tree decision — 0.0 when the tree's structural skeleton (node types and nesting only, so the match survives the node renaming ordinary evolution performs) matches a preserved archetype, since gambling speculative structural search on the very shape crisis recovery would resurrect works against the registry's purpose; 1.0 for a nil or empty registry | `Observe`/`ExtinctSpecialists`/`Resurrect`/`MCTSAffinity`; seeded via `SeedSpecialistRegistry` |
