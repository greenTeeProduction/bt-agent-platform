...
| `quality_gate.go` | Reactive regression/floor gate with per-tree consecutive-failure disable; multi-revision pre-mutation snapshot history, each optionally tagged with its composite fitness, so fail-closed rollback walks back past a multi-cycle regression streak to the last known-good revision rather than only the immediately-preceding one (→ ADR-093, ADR-115); `Probe` is the read-only counterpart to `Validate`/`ValidateFor` — same verdict, no streak increment, reset, or disabling — for speculative changes the caller may discard (`LocalSearcher.RefineGated`), so a tuning attempt that does not pan out cannot push a healthy tree toward fail-closed | `Validate`/`ValidateFor`/`Probe`, `SnapshotTree`/`SnapshotTreeWithFitness`, `RestoreTree`/`RestoreTreeRevision`/`RestoreTreeBeforeRegressionStreak`/`ListRevisions`; `gardener_rollback` tool |
