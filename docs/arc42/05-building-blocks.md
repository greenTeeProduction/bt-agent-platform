...
| `selector_optimizer.go` | Per-Selector child success/failure/running telemetry, learned priority ordering, durable merge-summing stats; `Entropy`/`GiniImpurityFromProbs` are the package's canonical entropy/Gini-impurity formulas over an arbitrary probability distribution, now also consumed by `decision_tree.go`'s `DTAnalyzer` in place of its own duplicated math (→ ADR-029, ADR-079) | `SelectorOptimizer`, `OrderChildren`, `ApplyLearnedOrdering`, `SaveSelectorStats`/`LoadSelectorStats`, `ParseSelectorOrderingStrategy` (validates an ordering-strategy string, defaulting to `OrderBySuccessRate`; shared by every production wiring site), `Entropy`, `GiniImpurityFromProbs`; driven by `bt_evolve_selectors` |
