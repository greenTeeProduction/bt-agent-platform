...
| Infrastructure | `internal/reliability` | Circuit breakers, retry, DLQ, error categorization; canonical block-fitness outcome scoring (`ScoreOutcome`), reused by `internal/blocks`' `ScoreFromBlackboard`, `internal/engine`'s `fitnessScoreFromBB`, and `internal/dashboard`'s `recordBlockFitnessMetric` (5.4) in place of the formula previously triplicated across all three |
