...
- **Wiring:** in `BuildClaudeErrorHandler`'s tick, when `requestErrorHandlerProposal` returns `!Resolvable` AND a valid `CodeFix`, call `seedCodeFixProgram(sig, codeFix.Title, codeFix.Milestone, "self-fix:error-handler:"+sig)` before passing the failure through. The tree failure still returns `-1` immediately (async fix). Ledger stamps happen **after** seed (not inside the Claude proposal call): `escalated` on success (cool out), `escalate_deferred` on policy blocks like `BT_SELF_FIX=off` / cap / title collision (cool out so Claude is not re-called every tick), `escalate_failed` on transient store/IO misses (does **not** cool out — next firing retries). Invalid/absent `code_fix` still stamps `unresolvable` at proposal time.
