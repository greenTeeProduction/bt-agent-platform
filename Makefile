...
# Run the scalability probe against a live BT dashboard node.
# Supports both single-node (default) and multi-node probes.
# Override target with TARGET=http://host:9800 and API key with BT_API_KEY.
scalability-probe:
	@mkdir -p $(BIN_DIR)
	@$(GO) build -o $(BIN_DIR)/bt-scalability-probe ./cmd/bt-scalability-probe/
	@$(BIN_DIR)/bt-scalability-probe --target "$${TARGET:-http://localhost:9800}" --json
