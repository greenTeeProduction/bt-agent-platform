...
- **(engine):** Runtime tree mutation — behavior trees can now be mutated (nodes added or removed) while they are being ticked. Mutations queue per live run (from action nodes via `bb.EnqueueMutation`, or externally via the new `bt_live_runs` / `bt_live_mutate` / `bt_live_mutations` MCP tools) and apply copy-on-write at tick boundaries with validation, llm-origin allowlisting, and pointer-state migration (an unrelated graft never restarts an in-progress MemSequence). `persist: true` snapshots the mutated tree to `~/.go-bt-evolve/mutated_trees/`; **unscoped** `resolveTree` consults it override-first. User-owned `RunOnce` paths (`resolveTreeForUser`) intentionally skip overrides — snapshots are keyed by tree ID only, so loading them on personal automations would cross-shadow same-slug trees across users. Spec: `docs/superpowers/specs/2026-07-17-runtime-tree-mutation-design.md`.
- **(engine):** Self-fixing fleet — when the ClaudeErrorHandler judges a tree failure unresolvable *and* a genuine source bug (`code_fix`), or the proactive `domain:self_review` agent finds defects in recent autonomous commits, `seedCodeFixProgram` queues a `self-fix:*` program for the goap-fusion loop (TDD → verify → auto-apply). Guards: `BT_SELF_FIX=off` kill switch, per-signature cooldown ledger, open-program cap (`BT_SELF_FIX_MAX_OPEN`, default 3), and denial of patches that name self-fix guard files. EH ledger verdicts: `escalated` (seed succeeded), `escalate_deferred` (policy block — cools out), `escalate_failed` (transient store/IO — retries). Spec: `docs/superpowers/specs/2026-07-17-self-fixing-fleet-design.md`.
